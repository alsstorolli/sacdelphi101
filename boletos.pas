...
// 05.08.2022 - homologação CEF 'exige pdf original'
...
// 16.01.2020
        ...
        if FGeral.GetConfig1AsFloat('JUROSMORA')>0 then
          B.Instrucoes.Add('Após o vencimento cobrar mora de R$ '+formatfloat(f_cr,roundvalor( b.Valor*(FGeral.GetConfig1AsFloat('JUROSMORA')/3000)))+' ao dia');
//        B.Instrucoes.Add('Após o vencimento cobrar multa de R$ '+formatfloat(f_cr,Q.fieldbyname('pend_valor').ascurrency*0.02));
// 19.06.10
        if FGeral.GetConfig1AsFloat('MULTABOLETO')>0 then
          B.Instrucoes.Add('Após o vencimento cobrar multa de R$ '+formatfloat(f_cr,b.Valor*(FGeral.GetConfig1AsFloat('MULTABOLETO')/100)));
...
//        B.Instrucoes.Add('Sujeito a protesto se não for pago no vencimento');
        if FGeral.GetConfig1AsInteger('DIASVENBOLETO')>0 then
           B.Instrucoes.Add('Sujeito a protesto após '+inttostr(FGeral.GetConfig1AsInteger('DIASVENBOLETO'))+'o. dia útil após vencimento')
        else
           B.Instrucoes.Add('Protestar 5o. dia útil após o vencimento');
// 02.07.13 - Simar
...
//        B.Instrucoes.Add('Sujeito a protesto se não for pago no vencimento');
        if FGeral.GetConfig1AsInteger('DIASVENBOLETO')>0 then begin
           ...
           Titulo.Mensagem.Add('Sujeito a protesto após '+inttostr(FGeral.GetConfig1AsInteger('DIASVENBOLETO'))+'o. dia útil após vencimento');
           ...
           Titulo.Mensagem.Add('Protestar 5o. dia útil após o vencimento');
...
//        B.Instrucoes.Add('ficando vedado qualquer outra forma de pagamento que não seja através do presente boleto');
        B.Instrucoes.Add('ficando vedado qualquer outra forma de pagamento que não seja através deste');

        B.LocalPagamento:='Pagável em qualquer Banco do Sistema de Compensação';
        B.Aceite:='N';
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if (FGeral.GetConfig1AsFloat('JUROSMORA')>0 ) and
           ...
           then begin
            B.Instrucoes.Add('Após o vencimento cobrar multa de '+formatfloat(f_cr,(FGeral.GetConfig1AsFloat('MULTABOLETO')))+' %'+
...
// 09.05.19 - dani vidanova colocr em %. a mora e a multa
                ...
                (FGeral.GetConfig1AsFloat('MULTABOLETO')=0)
             then
               B.Instrucoes.Add('Após o vencimento cobrar mmora de '+formatfloat(f_cr,roundvalor((FGeral.GetConfig1AsFloat('JUROSMORA')/30)))+' % ao dia');
...
//          B.Instrucoes.Add('Após 5 dias de atraso cobrar multa de R$ '+formatfloat(f_cr,Q.fieldbyname('pend_valor').ascurrency*(FGeral.GetConfig1AsFloat('MULTABOLETO')/100)));
            B.Instrucoes.Add('Após vencimento cobrar multa de '+formatfloat(f_cr,(FGeral.GetConfig1AsFloat('MULTABOLETO')))+' %' );
// 02.04.18 - Simar  - Irma
//        B.Instrucoes.Add('Após 5 dias de atraso sujeito a PROTESTO' );
// 23.04.19
        B.Instrucoes.Add('PROTESTO Automático após 5 dias do vencimento' );
// 16.04.18
        if ( Global.Topicos[1296] ) and (TemDesconto='S') then begin
           ...
           if (Q.FieldByName('clie_descontovenda').AsCurrency > 0)
              ...
              then begin
//              B.Instrucoes.Add('Desconto de '+currtostr(Q.FieldByName('clie_descontovenda').AsCurrency)+' % até o vencimento' );
...
// 21.09.17
  if ( EdTodos.text='B' ) and ( EdInstrucaocob.text='01' ) then begin
    Avisoerro('Não permitido gerar remessa de titulos baixados com instrução de cobrança 01');
    exit;
  end;
...
//      Sistema.Endprocess('Arquivo '+Salvar.FileName+' gerado');
      ...
      Sistema.Endprocess('Checar Cedente.  Não foi possível gravar o arquivo '+ACBrBoleto1.NomearqRemessa+' em '+ACBrBoleto1.DirArqRemessa+
  ...
  for p:=1 to GridPedidos.RowCount do begin
    ...
    if (trim(op)<>'') and (trim(GridPedidos.Cells[GridPedidos.getcolumn('marcado'),p])<>'') then begin
      ...
      if not Q.eof then begin
          ...
          CriaBoleto(Q,'','','N');
// cria boleto só pra ter o objeto com os dados do boleto carregado na memoria...
...
begin
  ...
  if (EdBanco.text='748') then begin
    ...
    xEd.Items.Add('06 - Alteração de Vencimento');
    xEd.Items.Add('07 - Concessão de Desconto');
    xEd.Items.Add('08 - Cancelamento de Desconto');
    xEd.Items.Add('09 - Protestar(transferir de devolução para protesto');
    xEd.Items.Add('10 - Sustar protesto e baixa título');
    xEd.Items.Add('11 - Sustar protesto e manter em carteira');
    xEd.Items.Add('12 - Alteração de juros de mora');
    xEd.Items.Add('13 - Dispensar cobrança de juros de mora');
...
//     AcbrBoleto1.EnviarEmail(xemail,'Boletos de cobrança',CorpoEmail,true);
...
//        FGerapdf.RLReport1.NewPage;
  ...
  end else Aviso('Não encontrado boletos marcados para envio de email');
            ...
            ACBrBoleto1.Enviar;
{
            for x := 0 to  ACBrBoleto1.ListaRetornoWeb.Count do begin
               ...
               if ACBrBoleto1.ListaRetornoWeb[x].ListaRejeicao.Count > 0 then
                  ...
                  Avisoerro('Operação:'+op+' Mensagem '+ACBrBoleto1.ListaRetornoWeb[x].ListaRejeicao[y].Mensagem+' '+
