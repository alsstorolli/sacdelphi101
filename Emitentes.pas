...
// 15.07.2021 - gs cred
   Edit.Items.add('739 - BGN');
   Edit.Items.add('318 - BMG');
   Edit.Items.add('655 - BV');
   Edit.Items.add('655 - BV');
   Edit.Items.add('623 - PAN');
   Edit.Items.add('707 - DAYCOVAL');
   Edit.Items.add('212 - ORIGINAL');
   Edit.Items.add('189 - BONSUCESSO');
   Edit.Items.add('254 - PARANÁ');
   Edit.Items.add('250 - BCV');
   Edit.Items.add('422 - SAFRA');
   Edit.Items.add('321 - CREFAZ');
   Edit.Items.add('108 - PORTOCRED');
   Edit.Items.add('029 - ITAU CONSIGNADO');
   Edit.Items.add('149 - FACTA');
   Edit.Items.add('069 - CREFISA');
   Edit.Items.add('336 - CREFISA');
...
begin
   ...
   if not Q.eof then begin
     if OP='I' then
       EdEmit_conta.invalid('Banco, agencia e conta já cadastrados no emitente '+Q.fieldbyname('emit_descricao').asstring)
     else
       EdEmit_descricao.text:=Q.fieldbyname('emit_descricao').asstring;
   end else begin
     if OP='A' then
       EdEmit_conta.invalid('Banco, agencia e conta não encontrados');
   end;
end;
...
begin
   ...
   if not Q.eof then begin
     Avisoerro('Conta usada no cheque '+Q.fieldbyname('cheq_cheque').asstring+' emissão '+
...
begin
   FRel.ReportFromSQL('select * from emitentes order by emit_descricao','RelEmitentesCheque','Relação de Emitentes');
end;
