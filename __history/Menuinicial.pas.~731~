...
// 04.04.16
      ...
      if not Q.eof then Lista.Add('Notas a autorizar OU números a inutilizar');
...
//          SistemaZeos.Connect('S','127.0.0.1');
          ...
          try
             ...
             Avisoerro('Não foi possível conectar o base de dados '+SistemaZeos.UserDataBase);
          end;
        except
          Avisoerro('Não foi possível conectar o banco de dados '+ConexaoZeos.Database+' em '+ConexaoZeos.HostName);
        end;
}
...
// 05.03.20
     if Global.Usuario.OutrosAcessos[0519] then begin
       ...
       if Global.Usuario.codigo <> 100 then begin
          ...
          if confirma('Usar balança ?') then begin
...
// 09.06.20 - Fama - preço dif. entre 002 e 003...
...
// 20.06.16
    if  (Global.topicos[1043]) or (Global.topicos[1045]) then
      if not FGeral.ConectaContax then begin
        Avisoerro('Não foi possível conectar banco de dados do Contax');
        exit;
      end;
...
// 08.01.12- Benatto - Reducao Z
     if ( FGeral.ConfiguradoECF ) then begin
       if ( not FezReducaoZ ) then begin
         Avisoerro('Atenção ! Redução Z não encontrada no banco de dados');
         CanClose:=Confirma('Sair do sistema mesmo assim ?');
       end else
         CanClose:=Confirma('Confirma a finalização do sistema');
     end else
       CanClose:=Confirma('Confirma a finalização do sistema');
...
begin
//  Avisoerro('Opção em desenvolvimento');
...
begin
   Avisoerro('Opção não disponível para Pronta Entrega');
...
begin
   ...
   PMsg.Caption := 'Verificando pedidos móvel';
...
begin
  if not Global.Topicos[1021] then
    Avisoerro('Sistema não habilitado para imprimir cupom fiscal')
...
begin
    ...
    FConsulta.execute( 'clientes',campos,
                      ...
                      'order by clie_nome','Litros para Entrega do Leite da Criança',
                      ' select '+campos+' from'+
