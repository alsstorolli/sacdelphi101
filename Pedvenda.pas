...
// 14.06.13
{
  if Global.Usuario.OutrosAcessos[0504] then begin
    ...
    FPedVenda.EdProduto.Title:='Referência';
  end else begin
    FPedVenda.EdProduto.FindField:='esto_codigo';
    FPedVenda.EdProduto.Title:='Código';
  end;
...
//    FPedVenda.EdPort_codigo.enabled:=false;
// 19.05.14 - vivan cobrança - angela
    FPedVenda.EdPort_codigo.enabled:=true;
...
//                     'Obs:'+obs+#13+
         ...
         if somestre<>'S' then
           Campostogrid(Qbusca);
         EdNUmerodoc.INvalid('Pedido já baixado');
         desativabotoes;
         EdCliente.Valid;
         CalculaTotal;
...
// 07.07.14 - Novicarnes - Elize
         ...
         EdCliente.Invalid('Ainda não definido a condição de pagamento neste cliente');
         exit
// 03.09.14 - Benato
    ...
    if Arq.TTabelaPreco.Locate('tabp_codigo',EdMoes_tabp_codigo.AsInteger,[]) then begin
      ...
      if campounidadesmvto.Tipo<>'' then begin
        if ( pos( Global.CodigoUnidade,Arq.TTabelaPreco.FieldByName('tabp_unidadesmvto').AsString ) = 0 )
          ...
          then begin
          EdMoes_tabp_codigo.Invalid('Tabela não permitida nesta unidade');
        end;
      end;
    end else begin
...
//    EdProduto.Invalid('Atenção.   Preço de Venda ( ou custo ) zerado no cadastro do produto');
//    exit;
//  end;
...
//                ' and '+FGeral.GetSqlDataNula('moes_datacont')+
  ...
  if not Q.eof then
    TLista.Add('Codigo     | Data        | Quantidade|     Unitário | Descrição                   ' );
...
// 24.11.13
   ...
   if Q2.Eof then begin
      Avisoerro('Não encontrado este pedido para incluir este item');
      exit;
...
// 16.08.06
      ...
      if (Op='A') and (OPbotao='A') then
        Sistema.Setfield('Ocor_Descricao',EdProduto.Text+' Tam '+FTamanhos.Getdescricao(EdCodtamanho.asinteger)+
                        ...
                        desccopa+
                        ' ALTERADO para '+EdQtde.Text+' peças' )
      else
        Sistema.Setfield('Ocor_Descricao',EdProduto.Text+' Tam '+FTamanhos.Getdescricao(EdCodtamanho.asinteger)+
                        ' Cor '+Fcores.Getdescricao(Edcodcor.asinteger)+
                        desccopa+' somado mais '+EdQtde.Text+' peças' );
...
// 16.04.19 - devido a ajustes do cadastro de usuarios por isonel começou a checar aqui o limite
...
{ - retirado em 24.04.06 - pois pedira usuario pra liberar venda mesmo com restrição de crédito
  if ( FCondPagto.GetAvPz(EdFpgt_codigo.text)='P') and (OP='I') then begin
...
// 11.11.14 - Vivan
  if trim( EdCliente.resultfind.fieldbyname('clie_situacao').asstring ) = 'B' then begin
         EdCliente.Invalid('Verificar situação do cliente : '+EdCliente.resultfind.fieldbyname('clie_situacao').asstring );
         exit;
  end;
  if Global.Topicos[1413] then begin
// 11.11.14 - vivan cobrança angela
      if (trim (EdCliente.ResultFind.fieldbyname('Clie_portadores').asstring)<>'') then begin
...
// 28.01.15
      if Global.topicos[1418] then begin
        if confirma('Confirma impressão') then
...
//    EdCliente.Clearall(FPedVenda,99);
// 09.07.2021 - senão apaga o 'PV' e data...
    EdCliente.Clear;
...
// 10.01.06 - entrou em alteração mas usou botão incluir e tentou alterar dai duplica o item - tania jamanta...
...
begin
   ...
   if (not Q.eof)  then begin
     if Q.fieldbyname('mped_situacao').asstring=sitbaixado then begin
       Avisoerro('Pedido já baixado.   Exclusão não permitida');
       Q.close;
       Freeandnil(Q);
       exit;
     end;
...
////////////////////////
   ...
   if (not Q.eof)  then begin
     if Q.fieldbyname('mped_situacao').asstring=sitbaixado then begin
       Avisoerro('Pedido já baixado.   Cancelamento não permitido');
       Q.close;
       Freeandnil(Q);
       exit;
     end;
...
// 03.04.06
  if EdCliente.isempty then exit;
  if not Confirma('Confirma recálculo') then exit;
...
// 10.01.06 - entrou em alteração mas usou botão incluir e tentou alterar dai duplica o item - tania jamanta...
  if (Edproduto.enabled) and (OP='A') then begin
    if FGeral.ProcuraGrid(Grid.getcolumn('move_esto_codigo'),Edproduto.text,Grid,Grid.getcolumn('codtamanho'),
      ...
      EdCodcopa.Invalid('Produto já existente.  Usar a opção Alterar');
    end;
  end;
...
// 08.09.15 - Iso...dianteiro ' + caro'  - retirado em 21.09.18 deixado somente na balança
{
...
/////////////////////////////////////////////////////////////////
  ...
  if not Q.eof then begin
    Avisoerro('Requisição '+EdNumerodoc.text+' já encontrada em '+FGeral.FormataData(Q.fieldbyname('moes_datamvto').asdatetime));
    FGeral.FechaQuery(Q);
    exit;
  end;
  FGeral.FechaQuery(Q);
  if not confirma('Gerar Requisição no almoxarifado ?') then exit;
  Sistema.BeginProcess('Gerando requisição');
  try
    Transacao:=FGeral.GetTransacao;
    GeraDetalhe;
    GeraMestre;
    Sistema.Commit;
    Sistema.endprocess('Requisição '+EdNumerodoc.text+' Gerada.');
  except
    Sistema.endprocess('Requisição não gerada.  Tente mais tarde');
  end;
...
// 26.11.13 - Metalforte - calcula preço de venda usando peso e depois aplicando a margem do grupo
...
begin
   ...
   if (not QBusca.eof)  then begin
     if Qbusca.fieldbyname('mped_situacao').asstring=sitbaixado then begin
       Avisoerro('Pedido já baixado.   Alteração não permitida');
       exit;
     end;
...
//    Avisoerro('Cliente com restrição de crédito.  Verificar.');
//    exit;
//  end;
...
begin
  ...
  if not FileExists( opendialog1.filename ) then begin
    Avisoerro('Não encontrado arquivo '+opendialog1.filename );
    exit;
  end;
      ...
      for i:=0 to ListaProdutos.count-1 do begin
        ...
        if not QP.eof then begin
          ...
          if not Qt.eof then begin
            ...
            Grid.Cells[grid.getcolumn('tamanho'),Abs(x)]:='NÃO ENC.';
          end;
          if not Qc.Eof then begin
            ...
            Grid.Cells[grid.getcolumn('cor'),Abs(x)]:='NÃO ENC.';
          end;
          Qt.close;Qc.close;
          ...
          Grid.Cells[grid.getcolumn('move_esto_codigo'),Abs(x)]:='NÃO ENCONTRADO';
...

{
  ...
  if not confirma('Confirma importação da obra '+PObra.codigo+' Cliente '+PObra.nomecliente+' ?') then exit;
...
//}
  ...
  if length(trim(PObra.cnpj_cpf))>=11 then begin
    ...
    if not Q.Eof then begin
      ...
      Aviso('Cnpj/cpf '+Stringtosql(PObra.cnpj_cpf)+' não encontrado');
    end;
