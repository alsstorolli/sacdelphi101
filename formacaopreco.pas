...
//   FFormacaoPreco.Caption:='Formação do Preço de Venda - Orçamento Numero '+inttostr(numero)+' Nome : '+NomeOrcam+' Obra : '+Numeroobra;
   FFormacaoPreco.Caption:='Formação do Preço de Venda - Orçamento Numero '+inttostr(numero)+' Opção : '+NomeOrcam+' Obra : '+Numeroobra;
   Show;
...
// 29.09.10 - pode ver orçammentos(valores) das unidades q pode acessar
//               ' and '+FGeral.GetIN('orcc_unid_codigo',Global.Usuario.UnidadesMvto,'C') );
// 04.04.11 - para mostrar cfe a unidade onde foi gravado o orçamento
...
begin
   if (EdOrca_margem.ascurrency>0) and (EdOrca_margem.ascurrency<FGeral.GetConfig1AsFloat('MARGEMMIN')) then
     EdOrca_margem.Invalid('Margem não permitida')
   else
     CalculaPreco;
end;
...
var Q:TSqlquery;
    ...
    begin
    // nao esquecer q terá q ser um post para cada linha do grid 'somando' o codigo do produto
    // na condicaopost;;
...
/////////////////////////////////////////////////////////////////////////////////////
  ...
  if Global.CodigoUnidade<>xUnidadeObra then begin
    Avisoerro('Não permitido salvar orçamento fora da unidade onde foi incluido');
    exit;
  end;
...
//////////////
// 08.07.10 - Atualiza o 'orçamento base' quando salva - Adriano+Marcelo
...
/////
  try
    FFormacaoPreco.Caption:='Formação do Preço de Venda - Orçamento Numero '+inttostr(numero)+' Nome : '+GNomeOrcam;
    Sistema.commit;
// 25.08.09
    FGeral.GravaLog(22,'Opção '+GNomeOrcam+' Orçamento '+inttostr(numero)+' Obra '+NumeroObra,true );
    Aviso('Informações gravadas');
  except
    Avisoerro('Não foi possível gravar as informações');
  end;
  FGeral.FechaQuery(Q);
end;
...
begin
  if Eddesconto01.ascurrency>3 then
    EdDesconto01.Invalid('Desconto máximo ultrapassado')
  else
    CalculaPreco;
end;
...
begin
   if baprova.Tag=1 then begin
     Avisoerro('Opção de orçamento aprovada não pode ser copiada');
   end else begin
...
//   if EdNomeorcam.text=GNomeOrcam then
   if EdNomeorcam.text=NomeOrcam then
     EdNomeOrcam.invalid('Nome não pode ser o mesmo do orçamento atual');
end;
...
begin
  if OPOrcam='M' then begin
    ...
    try
      ...
      Sistema.endprocess('Exclusão não efetuada.  Problemas no banco de dados');
    end;
    EdOrcam.enabled:=false;
    EdOrcam.Visible:=false;
    Close;
...
// 18.12.08
      ...
      Sistema.beginprocess('Pesquisando acessórios obra '+NumeroObra);
//      obra:=NrObra+Numeroobra;
      obra:=Numeroobra;
    ...
    begin
      ...
      for x:=0 to ListaCustoGrupo.Count-1 do begin
         ...
         if Pcustogrupo.codgrupo=999 then
           ReMargensporgrupo.Lines.Add('  '+currtostr(FGeral.GetConfig1AsFloat('MARGEMMOORCA'))+'%     '+inttostr(Pcustogrupo.codgrupo)+' - Mão de Obra')
//  ver como identificar a obra como revenda ou 'obra'
         else if FOrcamentos.Edorca_tipovenda.text='Revenda' then
...
begin
   Ed.Items.Clear;
   Ed.Items.add('PINTURA ELETROSTÁTICA');
   Ed.Items.add('ANODIZAÇÃO COLORIDA');
   Ed.Items.add('ANODIZAÇÃO FOSCO');
   Ed.Items.add('VIDRO');
   Ed.Items.add('LAMINAÇÃO');
...
///////////////////////////////////////////////////////////////
   ...
   if Q.eof then begin
     Avisoerro('Orcamento numero '+inttostr(numero)+' nome '+nomeorcam+' não encontrado');
     Q.Close;
     exit;
   end;
   QIns:=sqltoquery('select * from orcainsumos '+
...
//   FRel.AddTit(FGeral.TituloRelUnidade(Global.CodigoUnidade));
   FRel.AddTit('Planilha de Orçamento de Obra '+Numeroobra+' do Orçamento '+nomeorcam+'  - Cliente '+FOrcamentos.EdOrca_cliente1.text+'  - Representante '+FOrcamentos.EdRepr_codigo.text+' - '+FRepresentantes.GetDescricao( FOrcamentos.EdRepr_codigo.asinteger ) );
   if xTipoVenda=TipoVendaServicos then
     FRel.AddTit('Fone '+FOrcamentos.EdOrca_fone.text+' - Celular '+FOrcamentos.EdOrca_celular.text+'  - Linha '+FOrcamentos.EdOrca_linha.text+'  - Area '+FOrcamentos.EdOrca_area.text+' - VENDA DE SERVIÇOS' )
   else
   ...
   FRel.AddTit('Tamanho m2 '+FOrcamentos.EdOrca_area.text+' -  Peso Bruto '+EdBrutototal.text+' - Peso Real '+EdRealtotal.text+' - Peso Líquido '+EdPesoliquido.text  );
   FRel.Addtit('Custo por KG real '+EdCustokgreal.text+'  - Custo por KG Líquido '+Edcustopesoliquido.text+' - Custo por m2 '+EdCustom2.text  );
...
//   FRel.AddCol( 060,3,'N','' ,'##0.00'        ,'Sobra Bruta'         ,''         ,'',false);
   ...
   FRel.AddCol( 070,3,'N','' ,''              ,'Preço Unitário'         ,''         ,'',false);
   FRel.AddCol( 080,3,'N','' ,''              ,'Custo/Peça'         ,''         ,'',false);
...
//   FRel.AddCol( 080,3,'N','' ,''              ,''         ,''         ,'',false);

  {
   ...
   FRel.AddCel('Peso Bruto        :'+Transform(totalpesobruto,f_cr)+space(10)+
               ...
               'Peso Líquido      :'+Transform(Q.FieldByName('orcc_pesoliquido').AsFloat,f_cr)+
               'Média Perda       :'+Transform(EdMediaperda.ascurrency,f_cr)
                 );
   FRel.AddCel('Custo por KG Real :'+Transform(EdCustokgreal.asfloat,f_cr)+space(10)+
               'Custo por KG Líquido :'+Transform(EdCustopesoliquido.asfloat,f_cr)+
...
//   end;
   ...
   FRel.AddCel('Salários de Produção');
   FRel.AddCel('');
   FRel.AddCel('');
   FRel.AddCel('');
   ...
   FGEral.ImprimelinhaRel(colunas,replicate('-',70));
//   FGEral.PulalinhaRel(colunas);
   FRel.AddCel('TOTAL DA MÃO DE OBRA');
   FRel.AddCel('');
   FRel.AddCel('');
   FRel.AddCel('');
   FRel.AddCel('');
...
//   FGEral.ImprimelinhaRel(colunas,replicate('-',70));
//   FGEral.PulalinhaRel(colunas);
   FRel.AddCel('GASTOS PRODUÇÃO S/ MATERIAIS');
   FRel.AddCel('');
   FRel.AddCel('');
   ...
   FRel.AddCel('Despesas Extraordinárias');
   FRel.AddCel('');
   FRel.AddCel('');
   FRel.AddCel('');
   FRel.AddCel('');
   ...
   TituloTipoVenda:='';
   if xTipoVenda=TipoVendaServicos then
     TituloTipoVenda:='SERVIÇOS';
   FRel.AddCel('FORMAÇÃO DO PREÇO DE VENDA');
   if xTipoVenda=TipoVendaServicos then
...
//   FGEral.PulalinhaRel(colunas);
   ...
   if xTipoVenda=TipoVendaServicos then
     FRel.AddCel('Preço de Venda dos Serviços')
   else
     FRel.AddCel('Preço de Venda');
   FRel.AddCel('');
   FRel.AddCel('');
   FRel.AddCel('');
   FRel.AddCel('');
...
//   FGEral.PulalinhaRel(colunas);
   ...
   FRel.AddCel('Preço de Venda com Juros Financeiros');
   FRel.AddCel('');
   FRel.AddCel('');
   FRel.AddCel('');
   FRel.AddCel('');
   ...
   FRel.AddCel('DESPESAS VARIÁVEIS DE VENDA');
   if xTipoVenda=TipoVendaServicos then
     FRel.AddCel(titulotipovenda)
   else
     FRel.AddCel('');
...
//
   ...
   FRel.AddCel('');
//
   FRel.AddCel('Contribuição Social');
   FRel.AddCel('');
   FRel.AddCel('');
   ...
   totalvalorvendacli:=totalvalorvendacli+valorunitario;
//
   FRel.AddCel('Comissões e Encargos');
   FRel.AddCel('');
   FRel.AddCel('');
...
//
//
   FRel.AddCel('Reflexo Sobre Comissões');
   FRel.AddCel('');
   FRel.AddCel('');
...
//
   ...
   FGEral.ImprimelinhaRel(colunas,replicate('-',70));
   FGEral.PulalinhaRel(colunas);
   FRel.AddCel('REF. PARA TOMADA DE DECISÃO');
   FRel.AddCel('');
   FRel.AddCel('');
   FRel.AddCel('');
   FRel.AddCel('');
   ...
   FRel.AddCel('');
//
   FRel.AddCel('Preço de Venda por metro quadrado');
   FRel.AddCel('');
   FRel.AddCel('');
   FRel.AddCel('');
   if FOrcamentos.EdOrca_area.asfloat>0 then
   ...
   FRel.AddCel('');
//
   FRel.AddCel('Preço de Venda por KG líquido');
   FRel.AddCel('');
   FRel.AddCel('');
   FRel.AddCel('');
   if EdPesoliquido.asfloat>0 then
   ...
   FRel.AddCel('');
//
   FRel.AddCel('Preço de Venda por KG Real');
   FRel.AddCel('');
   FRel.AddCel('');
   FRel.AddCel('');
   if EdRealtotal.asfloat>0 then
...
begin
  ...
  if (not Q.Eof) and (baprova.Tag=0) then
    Avisoerro('Opção '+Q.fieldbyname('orcc_nome').asstring+' deste orçamento já aprovada')
...
// 22.05.13
    if ( trim(FGeral.GetConfig1AsString('usuariosfaturamento'))<>'' )  and (Confirma('Gerar aviso pra faturamento ?')) then begin
       FGeral.PlanoAcaoSistema(numero,FGeral.GetConfig1AsString('usuariosfaturamento')
                ...
                'FATURAMENTO CONTRATO','Orçamento '+inttostr(numero)+' Aprovado ',
...
begin
    ...
    if not QBusca.Eof then
      ...
      EdCodigoperfissac.Invalid('Codigo não encontrado');
      exit;
    end;
    ...
    if QEstoque.eof then begin
       EdCodigoperfissac.INvalid('Codigo não encontrado no estoque da unidade '+Global.CodigoUnidade);
