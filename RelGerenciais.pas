...
//    FGeral.SetaItemsMovimento(EdTipomov);
    ...
    if tp=1 then
      Caption:='Relatório detalhado da transação'
    else if tp=2 then
      Caption:='Relatório de Auditoria Fiscal'
    else if tp=3 then
      Caption:='Relatório de Cheques Recebidos/Emitidos'
    else if tp=4 then
      Caption:='Relatório de Auditoria de Custos'
    else if tp=5 then
      Caption:='Relatório de Vendas / Comissão'
    else if tp=6 then
      Caption:='Relatório de Vendas / Quantidade'
    else if tp=7 then begin
      Caption:='Relatório de Consignações em Aberto'  ;
      EdTipocad.enabled:=true;
      EdCodTipo.enabled:=true;
    end else if tp=8 then
      Caption:='Relatório de Posição de Cliente'
    else if tp=9 then
      Caption:='Relatório de Pronta Entrega em Aberto'
    else if tp=10 then
      ...
      Caption:='Relatório de Conferência de Descontos'
    else if tp=13 then
      Caption:='Relatório de Média de Vendas'
    else if tp=14 then
      Caption:='Relatório de Detalhamento de Venda Consignada'
    else if tp=15 then
      Caption:='Relatório de Resumido de Consig. em Aberto'
    else if tp=16 then
      Caption:='Relatório de Conferência de Consignação/Pronta Entrega'
    else if tp=17 then
      Caption:='Relatório de Transferências Corrigidas'
    else if tp=18 then
      Caption:='Relatório de Atendimentos de Representantes'
    else if tp=19 then
      Caption:='Relatório Reg. Especial Aberto Resumido'
    else if tp=20 then
      Caption:='Relatório de Custo da Mercadoria Vendida'
    else if tp=21 then
      Caption:='Relatório de Inadimplência de Cheques Recebidos'
    else if tp=22 then begin
      Caption:='Relatório de Vendas por Produto';
      EdTipocad.enabled:=true;
      EdCodTipo.enabled:=true;
    end else if tp=24 then
      Caption:='Relatório de Atendimentos dos Pedidos de Venda por Cliente'
    else if tp=25 then
      Caption:='Relatório de Peças Pendentes dos Pedidos de Venda'
    else if tp=26 then
      Caption:='Relação dos Pedidos de Venda'
    else if tp=27 then
      Caption:='Relação dos Produtos mais Pedidos'
    else if tp=28 then
      Caption:='Relação Atendimentos dos Pedidos de Venda por Produto'
    else if tp=29 then
      Caption:='Relação(Impressão) de Pedidos digitados'
    else if tp=30 then begin

      Caption:='Relação das Entradas/Saidas de Abate/Fazenda';
      EdBrinco.Enabled:=true;

    end else if tp=31 then
      Caption:='Relação de Comissões por Grupo de Produto'
    else if tp=32 then
      Caption:='Auditoria Fiscal nos itens das notas'
    else if tp=33 then begin

      Caption:='Relatório de Despesas dos Veículos da empresa';
// 17.10.19
      ...
      Caption:='Pedido de Venda Bovinos/Suínos'
    else if tp=36 then
      Caption:='Comissão sobre Entrada de Abate'
...
// 13.06.19
      ...
      Caption:='Conferência de Acréscimos';
      ...
      if (QMovesto.fieldbyname('moes_comv_codigo').asinteger>0) and ( pos(tipomov,Global.TiposEntrada)>0 )  and ( not Global.topicos[1043] ) then begin
        if Global.Topicos[1002] then begin
          if QPendencias.eof then
            s:=strspace('Contabilização   ',25)+sep+''+sep+''+sep+''+sep+''+sep+'Débito :'+QMovesto.FieldByName('comv_debito').Asstring+
                    ' '+FPlano.GetDescricao(QMovesto.FieldByName('comv_debito').AsInteger)+
                    'Crédito : '+QMovesto.FieldByName('comv_credito').Asstring+
                    FPlano.GetDescricao(QMovesto.FieldByName('comv_credito').AsInteger)

          else
            s:=strspace('Contabilização   ',25)+sep+''+sep+''+sep+''+sep+''+sep+'Débito :'+QPendencias.FieldByName('pend_plan_conta').Asstring+
...
// 08.12.20 - Apropriações
   ...
   if campo.Tipo<>'' then begin
      ...
      if not QApropria.eof then  begin
        ...
        s:=strspace('Movimento Apropriações',25)+sep+TipoMovimento+sep+strspace(QApropria.fieldbyname('apro_numerodoc').Asstring,10)+
...
// 29.04.08
    if pos(Global.CodContrato,tiposdemovimento)>0 then begin
      ...
      if not QProducao.eof then begin
        TipoMovimento:=strspace('Ordem de Produção',33);
        Unidade:=QProducao.fieldbyname('movp_unid_codigo').asstring;
        s:=strspace('Movimento Produção',25)+sep+TipoMovimento+sep+strspace(QProducao.fieldbyname('movp_numerodoc').Asstring,10)+
...
////////////////////////      inner join planocon on (pcon_conta=mcon_pcon_conta)'+
      ...
      while not Qcontabil.eof do begin
        ...
        if Qcontabil.fieldbyname('mcon_dc').asstring='D' then
          s:=strspace('Contabilização   ',25)+sep+QPlano.fieldbyname('pcon_descricao').asstring+sep+transacaocontax+sep+''+sep+FGeral.Formatavalor(QContabil.fieldbyname('mcon_valor').ascurrency,f_cr)+sep+'Débito :'+QContabil.FieldByName('mcon_pcon_conta').Asstring
        else
          s:=strspace('Contabilização   ',25)+sep+QPlano.fieldbyname('pcon_descricao').asstring+sep+transacaocontax+sep+''+sep+FGeral.Formatavalor(QContabil.fieldbyname('mcon_valor').ascurrency,f_cr)+sep+'Crédito :'+QContabil.FieldByName('mcon_pcon_conta').Asstring;
        Linhas.Add(s);
...
//////////////////////////
{
    ...
    FTextRel.AddTitulo(space(margem)+Global.NomeSistema+' '+Global.VersaoSistema+space(86)+'Emissão:'+Datetostr(Sistema.hoje)+' '+timetostr(time)+'  Pg: [NumPg]',false,false,true );
    FTextRel.AddTitulo(FGeral.Centra('Detalhamento da Transação '+EdTransacao.text+cancelada,largura),false,false,true);
    FTextRel.AddTitulo(space(margem)+replicate('-',largura+3),false,false,true);
    FTextRel.AddTitulo(space(margem)+'Unidade : '+Unidade+' - '+FUnidades.Getnome(Unidade)+' - Usuário '+inttostr(codusuario)+' '+FUsuarios.GetNome(codusuario),false,false,true);
    FTextRel.AddTitulo(space(margem)+replicate('-',largura+3),false,false,true);
    FTextRel.AddTitulo(space(margem)+'Movimentação'+space(13)+'Tipo'+space(29)+'Documento'+space(02)+'Movim.'+
...
//////////////////////////
    ...
    FRel.AddTit('Detalhamento da Transação '+EdTransacao.text+cancelada);
    FRel.AddTit('Unidade : '+Unidade+' - '+FUnidades.Getnome(Unidade)+' - Usuário '+inttostr(codusuario)+
                ' '+FUsuarios.GetNome(codusuario)+' - Data de Lançamento : '+FGeral.FormataData(xdatalcto) );
    if (confmovimento>0) then begin
      FRel.AddTit('Configuração de Movimento : '+inttostr(confmovimento)+
                  ' - '+FConfMovimento.GetDescricao(confmovimento)+
                  ' Tipo de Movimento : '+TipoMov );
     end;
//    FRel.AddCol(800,1,'C','','','Movimentação'+space(13)+'Tipo'+space(29)+'Documento'+space(02)+'Movim.'+
//                       space(12)+'Valor'+space(01)+'Diversos','','',true);
    FRel.AddCol(150,1,'C','','','Movimentação','','',true);
...
// 10.01.23
    ...
    if not EdEspecies.IsEmpty then begin
      ...
      titentsai:=titentsai + ' - Espécies : '+EdEspecies.Text;
    end;
    ...
    if Global.Topicos[1510] then  begin
        ...
        if not FGeral.ConectaContax then  begin
              Avisoerro('Não foi possível conectar com o Contax');
              exit;
        end;
...
//      FRel.AddTit(Global.NomeSistema+' '+Global.VersaoSistema+space(78)+'Emissão:'+Datetostr(Sistema.hoje)+' '+timetostr(time)+'  Pg: [NumPg]',false,false,true );
      FRel.AddTit('Relação de Auditoria Fiscal '+titentsai);
...
//      if Global.UsaNfe='S' then
      if (Global.Usuario.Codigo=100) and ( EdEntSai.Text='E' )  then
         ...
         FRel.AddCol(150,1,'C','' ,''            ,'Situação Sefa'   ,''         ,'',false);
...
// 07.10.19 - Novicarnes - Simone
      ...
      FRel.AddCol( 50,0,'C','' ,''              ,'Série'            ,''         ,'',False);
      ...
      FRel.AddCol( 80,3,'N','' ,'##0.00'        ,'Redução'         ,''         ,'',False);
      FRel.AddCol( 80,3,'N','+',f_cr            ,'Base Icms'       ,''         ,'',False);
      FRel.AddCol( 60,3,'N','' ,'##0.00'        ,'Alíquota'        ,''         ,'',False);
...
// 21.09.18 - Novicarnes - Ketlen - Polli contadores
      ...
      if Global.topicos[1510] then begin
         ...
         FRel.AddCol(180,1,'C','' ,''              ,'Descrição Conta' ,''         ,'',False);
         FRel.AddCol(060,1,'C','' ,''              ,'CST Pis' ,''         ,'',False);
         FRel.AddCol( 80,1,'N','' ,''              ,'Reduzido'       ,''         ,'',False);
         FRel.AddCol(180,1,'C','' ,''              ,'Descrição Reduzido' ,''         ,'',False);
// 26.08.20
...
// 13.08.2021
              ...
              FRel.AddCel('Nota ainda não autorizada')
...
// 18.06.19  - acrescimo 'por dentro'
        ...
        vlrdesconto:=0; // nao precisa pois já está incluso na base
...
////////////
        ...
        if Q.FieldByName('moes_status').AsString='X' then
          ...
          FRel.AddCel(Q.FieldByName('moes_tipomov').AsString+' - Numeração Inutilizada')
        else
...
//      FRel.AddTit(Global.NomeSistema+' '+Global.VersaoSistema+space(78)+'Emissão:'+Datetostr(Sistema.hoje)+' '+timetostr(time)+'  Pg: [NumPg]',false,false,true );
//      FRel.AddTit('Relação de Cheques Recebidos'+titemaberto);
      FRel.AddTit('Relação de Cheques Recebidos '+FGeral.TituloRelCliRepre(Edcodtipo.asinteger,EdTipocad.text) );
      FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text));
{
      if (EdBomparai.asdate>1) and (EdBomparaf.asdate>1) then
        ...
        FRel.AddTit('Periodo Lançamento : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
}
      if ( not Edbomparai.isempty ) and (not EdDepositoi.isempty) and ( not Eddatai.isempty ) then
        FRel.AddTit('Periodo Bom para : '+FGeral.formatadata(Edbomparai.Asdate)+' a '+FGeral.formatadata(Edbomparaf.asdate)+
           ...
           ' - Periodo Lançamento : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate))
      ...
      FRel.AddCol( 90,0,'C','' ,''              ,'Banco Custódia'  ,''         ,'',false);
      while not Q.eof do begin
...
//      FRel.AddTit(Global.NomeSistema+' '+Global.VersaoSistema+space(78)+'Emissão:'+Datetostr(Sistema.hoje)+' '+timetostr(time)+'  Pg: [NumPg]',false,false,true );
      FRel.AddTit('Relação de Auditoria de Custos');
      FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text));
      FRel.AddTit('Periodo : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
      FRel.AddCol( 45,2,'C','' ,''              ,'Unidade'         ,''         ,'',false);
      ...
      FRel.AddCol( 80,3,'N','',f_cr            ,'Médio Compra' ,''         ,'',False);
      if Global.Usuario.OutrosAcessos[0010] then begin
        FRel.AddCol( 80,3,'N','',f_cr            ,'Médio Gerencial' ,''         ,'',False);
        FRel.AddCol( 80,3,'N','',f_cr            ,'Médio G.Cadastro' ,''         ,'',False);
      end;
      FRel.AddCol( 80,3,'N',''  ,f_cr             ,'Unitário'         ,''         ,'',False);
      FRel.AddCol( 80,3,'N',''  ,f_cr             ,'% Icms'           ,''         ,'',False);
      FRel.AddCol( 80,3,'N','+' ,f_cr            ,'Frete Proporcional'   ,''         ,'',False);
      FRel.AddCol( 70,3,'N',''  ,f_cr             ,'% Desconto'           ,''         ,'',False);
      FRel.AddCol( 70,3,'N',''  ,f_cr             ,'% Acréscimo'          ,''         ,'',False);
...
// 23.08.10
    ...
    titulo:='Comissão sobre Faturamento de '+FGeral.FormataData(Eddatai.asdate)+' a '+FGeral.FormataData(Eddataf.asdate)+
...
// 26.02.19 - Novicarnes
      ...
      FRel.AddCol(080,3,'N','+',f_cr            ,'Valor comissão'  ,''         ,'',false);
      FRel.AddCol(080,3,'N','',f_cr             ,'Perc. comissão'  ,''         ,'',false);
...
//      FRel.AddCol(120,0,'C','' ,''              ,'Tipo Movimento'  ,''         ,'',False);
      FRel.AddCol( 80,0,'D','' ,''              ,'Emissão'         ,''         ,'',False);
...
// ver pedir se quer relat. sobre consignação, pronta entrega ou "outro esquema"
...
//                  ' and '+FGeral.Getin('move_status','D','C')+
    ...
    if Q.Eof then
      ...
      FRel.AddCol(120,0,'C','' ,''              ,'Endereço'          ,''         ,'',False);
      FRel.AddCol(120,0,'C','' ,''              ,'Tipo Movimento'  ,''         ,'',False);
...
// 03.03.09
      ...
      FRel.AddCol( 80,3,'N',''  ,''             ,'Unitário médio'   ,''         ,'',False);
...
// sem codigo de devoluçao de consignaçao pois este somente é gerado quando é feito o acerto
//  else
//    sqltipomovto:='';
...
//////////////////////////////////////////////////////////////
{
    if global.usuario.codigo=300 then begin
       ...
       titespecial:=' - Remessas :'+remessas+' Devoluções :'+devolucoes;
...
//                ' order by move_unid_codigo,move_repr_codigo,move_tipo_codigo,move_esto_codigo,move_numerodoc' );
    ...
    if Q.Eof then
      ...
      FRel.AddTit('Relatório de Pronta Entrega em Aberto '+titespecial);
      FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text));
      FRel.AddTit('Periodo : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
      FRel.AddCol( 45,2,'C','' ,''              ,'Unidade'         ,''         ,'',false);
      FRel.AddCol( 45,1,'C','' ,''              ,'Codigo'          ,''         ,'',false);
...
// 26.09.05 - gambiarra devido a existencia de 'duas VB'...o sistema só trata a ref. a PE mas foi colocado
//            o tipo VB na conf. de venda 54 e usado o tipo de movimento como VB : é usado na consignação
//            checar se sera necessario crir outro tipo de VB para uso na consignação...
...
begin
  if aberto='S' then
    result:=' - Somente consignações em aberto'
  else
    result:='';
end;
...
///////////////
  with FRelGerenciais do begin
    ...
    if trim(EdCodtipo.text)='' then begin
      Avisoerro('Obrigatório escolher um cliente');
      exit
    end;
    Sistema.BeginProcess('Gerando Relatório');
...
//    else begin
      ...
      FTextRel.AddTitulo('Extrato de Informações Financeiras do Cliente',false,false,true);
...
///////////////////////////////////////
      ...
      if (qtdremessa+qtddevolucao) > 0 then begin
        ...
        FTextRel.AddLinha('Remessa    Devolução    Saldo             Valor',false,false,true);
...
// 02.02.15 - vivan -cecilia
      ...
      if  not QVd.eof then begin
        ...
        FTextRel.AddLinha(replicate('-',largura),false,false,true);
        FTextRel.AddLinha('Emissão          Numero         Valor',false,false,true);
      end;
...
/////////////////////      Q.close;
  ...
  // 27.05.10
      ...
      Q:=Sqltoquery( 'select * from pendencias'+
                  ...
                  sqltipocod+
                  sqlorder  );
//      FTextrel.Addlinha('Unidade Documento Portador             Emissão  Vencimento Baixa    Parc   Np          Valor   Atraso',true,false,true);
//      FTextRel.AddLinha(replicate('-',largura),false,false,true);
      totvalor:=0;
...
//        FTextRel.AddLinha(replicate('-',largura),false,false,true);
        FTextRel.AddLinha(space(39)+'TOTAL'+space(04)+FGeral.Formatavalor(totvalor,f_cr)+
                          space(02)+'Dias de atraso em média '+
...
//////////////////////
      if not Q.eof then begin
        ...
        FTextRel.AddLinha(replicate('-',largura),false,false,true);
        FTextrel.Addlinha('Unidade Documento Portador             Emissão  Vencimento Parc   Np          Valor   Atraso',true,false,true);
        FTextRel.AddLinha(replicate('-',largura),false,false,true);
      end;
      totvalor:=0;
      Sistema.beginprocess('Gerando relatório');
...
// 27.05.10
      ...
      Q.sql.text:='select * from cheques'+
                  ...
                  sqlorder ;
      Q.open;
      FTextrel.Addlinha('Unidade  Numero      Banco  '+space(11)+'Emissão  Vencimento            Valor   ',true,false,true);
      FTextRel.AddLinha(replicate('-',largura),false,false,true);
      totvalor:=0;
      while not Q.eof do begin
...
// Limite de credito
      ...
      FTextRel.AddLinha('Limite Disponível : '+FGeral.Formatavalor( FGeral.LimiteDisponivel(EdCodtipo.asinteger),f_cr)+
...
//        frel.addtit('Compras '+titulotipomov(tiposmovto));
        ...
        frel.addcol( 70,1,'D','' ,''              ,'Emissão'          ,''         ,'',false);
...
//        FRel.AddCol(100,1,'C','' ,''              ,'Copa'             ,''         ,'',False);
        frel.addcol( 80,3,'N','+' ,''             ,'Qtde'             ,''         ,'',false);
        frel.addcol( 80,3,'N',''  ,''             ,'Unitário'         ,''         ,'',false);
        frel.addcol( 80,3,'N','+' ,''             ,'Valor total'          ,''         ,'',false);
        ...
        if Global.Topicos[1302] then
          frel.addcol( 80,3,'N','+' ,''             ,'Peças'          ,''         ,'',false);
// 25.06.08
...
// 25.09.09
      ...
      if (EdSinana.text='S') then begin   // sintetico
        ...
        FRel.AddTit('Relatório de Vendas Sintético '+TituloTipoMov(tiposmovto));
        FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text));
        FRel.AddTit('Periodo : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
        FRel.AddCol( 45,2,'C','' ,''              ,'Unidade'         ,''         ,'',false);
        ...
        FRel.AddCol(100,1,'C','' ,''              ,'Cidade'          ,''         ,'',False);
        FRel.AddCol( 40,1,'C','' ,''              ,'Estado'          ,''         ,'',False);
        FRel.AddCol( 80,3,'N','' ,''              ,'População'       ,''         ,'',False);
...
// 26.10.11 - faltava o Abc em valor no sintetivo com usuario 'sem acesso'
        ...
        FRel.AddTit('Relatório de Vendas Analítico '+TituloTipoMov(tiposmovto));
        FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text));
        if pos( Global.CodRomaneioRemessaaOrdem,EdTipomov.text) > 0 then
          ...
          FRel.AddTit('Periodo : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
        FRel.AddCol( 90,1,'C','' ,''              ,'Transação'       ,''         ,'',false);
        FRel.AddCol( 45,2,'C','' ,''              ,'Unidade'         ,''         ,'',false);
...
// 24.06.08
        ...
        FRel.AddCol( 70,3,'N','+' ,''             ,'Peças'             ,''         ,'',False);
        FRel.AddCol( 60,3,'N','&'  ,''             ,'Unitário'         ,''         ,'',False);
        FRel.AddCol( 80,3,'N','+' ,''             ,'Valor Total'     ,''         ,'',False);
        FRel.AddCol( 80,3,'N','+' ,''             ,'Frete Unitário'     ,''         ,'',False);
...
begin
  ...
  with FRelGerenciais do begin
    ...
    if Q.Eof then
      ...
      Sistema.beginprocess('Gerando relatório SOMENTE DAS NOTAS COM ACRÉSCIMO NO TOTAL');
      totqtde:=0;totvalor:=0;
      while not Q.eof do begin
...
// 20.05.05
      ...
      if Lista.count>0 then begin
        ...
        FRel.AddTit('Relatório de Conferência de Acréscimos nas Vendas '+TituloTipoMov(tiposmovto));
        FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text));
        FRel.AddTit('Periodo : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
        FRel.AddCol( 45,2,'C','' ,''              ,'Unidade'         ,''         ,'',false);
...
//        FRel.AddCol(080,1,'C','' ,''              ,'Cor'             ,''         ,'',False);
        ...
        FRel.AddCol( 80,3,'N',''  ,''             ,'Uni.Líquido'      ,''         ,'',False);
        FRel.AddCol( 70,3,'N','' ,''              ,'Acres. Nota(%)'      ,''         ,'',False);
//        FRel.AddCol( 70,3,'N','' ,''              ,'Desc. Item(%)'      ,''         ,'',False);
        FRel.AddCol( 80,3,'N','+' ,''             ,'Valor Acréscimo'     ,''         ,'',False);
...
//          end;
        ...
        FRel.Video;
      end else Aviso('Não encontrado acréscimos no período');
...
// 27.05.10
    ...
    if Q.Eof then
      ...
      Lista:=Tlist.create;
      Listadoc:=TStringlist.create;
      Sistema.beginprocess('Gerando relatório SOMENTE DAS NOTAS COM DESCONTO NO TOTAL');
      totqtde:=0;totvalor:=0;
      while not Q.eof do begin
...
// 01.02.19
      ...
      if Lista.count>0 then begin
        ...
        FRel.AddTit('Relatório de Conferência de Descontos nas Vendas '+TituloTipoMov(tiposmovto));
        FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text));
        FRel.AddTit('Periodo : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
        FRel.AddCol( 45,2,'C','' ,''              ,'Unidade'         ,''         ,'',false);
        FRel.AddCol( 45,1,'N','' ,''              ,'Codigo'          ,''         ,'',false);
...
//        avisoerro('Produtos com custo zerado : '+copy(qprodutos,1,10));
      ...
      Sistema.beginprocess('Gerando relatório');
        ...
        FRel.AddTit('Relatório de Transferencias Corrigida pelo Custo da Matriz '+TituloTipoMov(tiposmovto));
        FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text));
        FRel.AddTit('Periodo : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
        FRel.AddCol( 45,2,'C','' ,''              ,'Unidade'         ,''         ,'',false);
        ...
        if EdSinana.text='A' then begin
          ...
          FRel.AddCol(120,0,'C','' ,''              ,'Descrição Produto'  ,''         ,'',False);
        end;
        FRel.AddCol( 80,1,'N','' ,''              ,'Documento'       ,''         ,'',False);
        FRel.AddCol( 50,1,'D','' ,''              ,'Emissão'         ,''         ,'',False);
        FRel.AddCol( 50,1,'D','' ,''              ,'Lançamento'      ,''         ,'',False);
        FRel.AddCol( 50,1,'D','' ,''              ,'Movimento'       ,''         ,'',False);
        FRel.AddCol( 80,3,'N','+' ,''             ,'Qtde'             ,''         ,'',False);
        FRel.AddCol( 80,3,'N','+' ,''             ,'Valor Total'     ,''         ,'',False);
        FRel.AddCol( 80,3,'N','+' ,''             ,'Valor Corrigido'     ,''         ,'',False);
        FRel.AddCol( 80,3,'N','+' ,''             ,'Diferença'     ,''         ,'',False);
...
begin
  with FRelGerenciais do begin
    ...
    Sistema.BeginProcess('Pesquisando movimento no período');
    statusvalidos:='N;E';
...
//            if Q.fieldbyname('move_tipomov').asstring=global.CodRemessaConsig then begin
      ...
      if tiporesumo='C' then
        ...
        FRel.AddTit('Relatório de Custo da Mercadoria Vendida - Tipos Impressos: '+TiposVenda+';'+TiposDev );
      ...
      FRel.AddCol(090,0,'C','' ,''              ,'Referência'         ,''         ,'',False);
      FRel.AddCol(170,0,'C','' ,''              ,'Descrição'       ,''         ,'',False);
      FRel.AddCol( 60,0,'N','' ,''              ,'Grupo'           ,''         ,'',False);
      FRel.AddCol(100,0,'C','' ,''              ,'Descrição Grupo'           ,''         ,'',False);
      FRel.AddCol( 60,0,'N','' ,''              ,'SubGrupo'           ,''         ,'',False);
      FRel.AddCol(120,0,'C','' ,''              ,'Descrição SubGrupo'           ,''         ,'',False);
      ...
      if pos(Global.CodSaidaAlmox,FRelGerenciais.EdTipomov.text)=0 then begin
        ...
        FRel.AddCol( 80,3,'N','+',''              ,'Devolução'              ,''         ,'',False);
        FRel.AddCol( 80,3,'N','+',''              ,'Devolução + ST'              ,''         ,'',False);
        FRel.AddCol( 80,3,'N','+',''              ,'Venda Liquida'          ,''         ,'',False);
      end else begin
          FRel.AddCol( 80,3,'N','+' ,''             ,'CMV Médio '     ,''         ,'',False);
      end;
// 30.07.09 - Abra - Contabilidade
      if pos(Global.CodSaidaAlmox,FRelGerenciais.EdTipomov.text)=0 then begin
        if Global.Usuario.OutrosAcessos[0010] then begin
...
// aqui deixar assim até criar campo especifico para cheques devolvidos
...
begin
  ...
  with FRelGerenciais do begin
    ...
    titulo:='Relatório de Inadimplência de Cheques';
    statusvalidos:='N';
    ...
    if EdDepositoi.AsDate>1 then begin
      ...
      periodo:=periodo+' Depósito : '+FGeral.FormataData(EdDepositoi.AsDate)+' a '+FGeral.FormataData(EdDepositof.AsDate);
    end else
      sqldatalan:='';
...
//                  ' and '+FGeral.Getin('move_status','D','C')+
      ...
      FRel.AddCol(100,0,'C','' ,''              ,'Descrição'           ,''         ,'',False);
...
// 02.06.16
      FRel.AddCol( 80,3,'N','+' ,''             ,'Peças'             ,''         ,'',False);
      FRel.AddCol( 80,3,'N','&'  ,''             ,'Unitário Médio'     ,''         ,'',False);
      FRel.AddCol( 80,3,'N','&'  ,''             ,'Peso Médio'     ,''         ,'',False);
      if EdConsFinal.Text='S' then begin
...
//                  ' where mpdd_datamvto>='+EdDatai.AsSql+' and mpdd_datamvto<='+EdDataf.AsSql+
    ...
    if Q.Eof then
      ...
      FRel.AddTit('Relatório de Atendimentos dos Pedidos de Venda'+subtit);
      FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text));
      FRel.AddTit('Periodo : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
      FRel.AddCol( 45,2,'C','' ,''              ,'Unidade'         ,''         ,'',false);
      FRel.AddCol( 60,1,'C','' ,''              ,'Cod.Repres.'     ,''         ,'',false);
...
//      FRel.AddTit('Relatório de Peças Pendentes dos Pedidos de Venda - Sem considerar PAC'+Gettitmotivos);
      FRel.AddTit('Relatório de Peças Pendentes dos Pedidos de Venda '+Gettitmotivos);
      FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text));
      FRel.AddTit('Periodo : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
      FRel.AddCol( 45,2,'C','' ,''              ,'Unidade'         ,''         ,'',false);
      FRel.AddCol( 60,1,'C','' ,''              ,'Cod.Repres.'     ,''         ,'',false);
...
//                  ' and '+FGeral.Getin('mpdd_situacao','E','C')+
    ...
    if Q.Eof then
      ...
      FRel.AddTit('Relatório de Pedidos de Venda');
      emaberto:='';
      if (fRelgerenciais.EdEmaberto.text='A') then
      ...
      FRel.AddCol( 80,1,'C','' ,''              ,'Condição'    ,''         ,'',false);
      FRel.AddCol( 80,3,'N','+' ,''             ,'Valor Total'             ,''         ,'',False);
      FRel.AddCol( 80,3,'N','+' ,''             ,'Peças Total'             ,''         ,'',False);
      FRel.AddCol( 60,1,'C','' ,''              ,'Situação'         ,''         ,'',false);
      FRel.AddCol( 80,1,'C','' ,''              ,'Cidade'         ,''         ,'',false);
      FRel.AddCol( 40,1,'C','' ,''              ,'UF'         ,''         ,'',false);
      FRel.AddCol( 80,1,'C','' ,''              ,'Usuário '               ,''         ,'',false);
...
//                  ' where mpdd_dataenviada>='+EdDatai.AsSql+' and mpdd_dataenviada<='+EdDataf.AsSql+
      ...
      titulo:='Relatório de Produtos mais vendidos por Produto';
...
//                  ' where mpdd_dataenviada>='+EdDatai.AsSql+' and mpdd_dataenviada<='+EdDataf.AsSql+
      ...
      titulo:='Relatório de Produtos mais vendidos por Representante + Produto';
    end;
    ...
    if Q.Eof then
      ...
      FRel.AddCol(090,1,'N','' ,''              ,'Produto'   ,''         ,'',false);
      FRel.AddCol(200,1,'C','' ,''              ,'Descrição Produto'   ,''         ,'',false);
...
//                  ' where mpdd_datamvto>='+EdDatai.AsSql+' and mpdd_datamvto<='+EdDataf.AsSql+
    ...
    if Q.Eof then
      ...
      FRel.AddTit('Relatório de Atendimentos dos Pedidos de Venda por Produto'+subtit+' - Somente dentro dos '+inttostr(FGeral.getconfig1asinteger('DIASPEDIDO'))+' dias');
...
// 28.01.15
    ...
    if Q.Eof then
      ...
      FRel.AddTit('Relatório de Impressão de Pedidos de Venda');
      FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text)+' '+FGeral.TituloRelCliRepre(EdCodtipo.asinteger,EdTipocad.text));
      FRel.AddTit('Periodo : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
      FRel.AddCol( 45,2,'C','' ,''              ,'Unidade'         ,''         ,'',false);
...
// mes final do relatorio ver proporcional data do fim do relatorou OU abate
           ...
           // if (Datetoano(DataAbate,true) > 1902) then begin  // proporcional ultimo mes..
                ...
                if not Qcc.eof then
                        ...
                        valor:=0;
               diasmes   := GetDiasmes( Datetomes(dataabate) );
               diaspropo := Datetodia(dataabate) ; // já vem descontado o dia do abate
               valor     := ( (valor/diasmes) * diaspropo );
               tvalor    := tvalor+ valor;
...
// 01.07.08  - aqui pois nao passa no 'execute' no inicio de .pas
    ...
    if (numero>0) and (unidade<>'') then begin
      ...
      xSa:='A';
      sqlperiodo:='';
// 18.07.11 - quem será q pediu ou porque foi alterado ??
...
// 30.07.19
 ...
 // 20.08.19
          ...
          FRel.AddCol( 60,3,'N','' ,f_cr            ,'Carcaça'        ,''         ,'',False);
...
// 22.09.16
          ...
          FRel.AddCol( 70,3,'N','+' ,f_cr       ,'Diferença'         ,''         ,'',False);
          FRel.AddCol( 50,3,'N','' ,''          ,'Dias'         ,''         ,'',False);
          FRel.AddCol( 70,3,'N','+' ,f_cr       ,'Ganho Peso'         ,''         ,'',False);
          FRel.AddCol(150,1,'C',''  ,''       ,'Produto Abatido'   ,''         ,'',false);
// 17.09.15
        end else if xTipomov='PA' then begin
// só para nao imprimir mais colunas...
        end else begin
//          FRel.AddCol( 40,3,'N',''  ,'##0.00'       ,'Unitário'              ,''         ,'',False);
          FRel.AddCol( 40,3,'N',''  ,''       ,'Unitário'              ,''         ,'',False);
//          FRel.AddCol( 70,3,'N','+' ,f_cr           ,'Valor Total'         ,''         ,'',False);
...
// 18.09.15
        ...
        if xTipomov=TipoEntradaAbate then begin
          ...
          FRel.AddTit('Relatório de Entrada de Abate '+Anasin);
          FRel.AddTit(FGeral.TituloRelUnidade(xUnidade));

        end else if xTipomov=TipoFazenda then begin

          FRel.Init('RelMovFazenda');
          FRel.AddTit('Relatório de Movimento '+Anasin+AbaVivos);
          if sqlbrinco='' then
          ...
          FRel.AddTit('Relatório de Pedidos de Venda '+Anasin);
          FRel.AddTit(FGeral.TituloRelUnidade(xUnidade));
        end;
        if numero=0 then
...
// 29.05.20
            ...
            FRel.AddCol( 80,3,'N','+' ,f_cr           ,'Diferença'         ,''         ,'',False);
            FRel.AddCol( 50,3,'N','' ,''              ,'Dias'         ,''         ,'',False);
            FRel.AddCol( 70,3,'N','+' ,f_cr           ,'Ganho Peso'         ,''         ,'',False);
// 30.08.19
...
// 09.09.17
          ...
          if xTipomov=TipoEntradaAbate then
            FRel.AddCol( 50,3,'N',''  ,'##0.00'       ,'Rend.%'              ,''         ,'',False);
          FRel.AddCol( 70,3,'N','+'  ,'######'       ,'Peças'                    ,''         ,'',False);
          FRel.AddCol( 70,3,'N',''  ,'##0.00'       ,'Valor Médio'              ,''         ,'',False);
          FRel.AddCol( 80,3,'N','+' ,f_cr           ,'Valor Total'         ,''         ,'',False);
        end;
      end;
...
// coluna 'diferença' apos coluna 'total abate'
//              FRel.AddCel( floattostr( (Q.Fieldbyname('Movd_pesocarcaca').AsCurrency*Q.Fieldbyname('Movd_vlrabate').AsCurrency)-
...
//          FRel.SetaSubTotal2('Descrição Produto')
...
//                ' order by move_unid_codigo,move_repr_codigo,move_tipo_codigo,move_esto_codigo,move_numerodoc' );
    ...
    if Q.Eof then
      ...
      if EdCodtipo.isempty then
        FRel.AddTit('Relatório de Consignações em Aberto '+titgrupo)
      else
        FRel.AddTit('Relatório de Consignações em Aberto - '+FGeral.Gettipoentidade(EdTipocad.text)+' - '+EdCodtipo.text+' - '+SetEdfavorecido.text+titgrupo);
      FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text));
      FRel.AddTit('Periodo : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
      FRel.AddCol( 45,2,'C','' ,''              ,'Unidade'         ,''         ,'',false);
      FRel.AddCol( 45,1,'C','' ,''              ,'Codigo'          ,''         ,'',false);
...
// 27.05.10
    ...
    titulo:='Comissão sobre Produtos no Faturamento de '+FGeral.FormataData(Eddatai.asdate)+' a '+FGeral.FormataData(Eddataf.asdate)+
            ' - Tipos Impressos: '+TiposVenda+';'+TiposDev ;
...
//      FRel.AddCol(080,3,'N','+',f_cr            ,'Total Produtos'     ,''         ,'',false);
      ...
      FRel.AddCol(140,0,'C','' ,''              ,'Descrição'       ,''         ,'',false);
// 19.10.16
...
//      FRel.AddTit(Global.NomeSistema+' '+Global.VersaoSistema+space(78)+'Emissão:'+Datetostr(Sistema.hoje)+' '+timetostr(time)+'  Pg: [NumPg]',false,false,true );
      FRel.AddTit('Relação de Auditoria Fiscal dos Itens das Notas'+titentsai);
      FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text));
      FRel.AddTit('Periodo : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
      FRel.AddCol( 45,2,'C','' ,''              ,'Unidade'         ,''         ,'',false);
      FRel.AddCol( 80,0,'C','' ,''              ,'Transação'       ,''         ,'',false);
...
// 11.11.14 - novi - angela
      ...
      FRel.AddCol( 80,3,'N','' ,f_cr           ,'Conferência'           ,''         ,'',False);
      FRel.AddCol( 50,0,'C','' ,''              ,'Cfop'            ,''         ,'',False);
      FRel.AddCol( 50,0,'C','' ,''              ,'Esp.'            ,''         ,'',False);
      FRel.AddCol( 50,0,'C','' ,''              ,'Série'            ,''         ,'',False);
      FRel.AddCol( 20,0,'C','' ,''              ,'UF'              ,''         ,'',False);
...
// 17.12.19
    ...
    //      FRel.AddTit(Global.NomeSistema+' '+Global.VersaoSistema+space(78)+'Emissão:'+Datetostr(Sistema.hoje)+' '+timetostr(time)+'  Pg: [NumPg]',false,false,true );
          FRel.AddTit('Relação de Notas Fiscais de Veículos da Frota');
          FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text)+TitVeiculo+Titconta );
          FRel.AddTit('Periodo : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
          FRel.AddCol( 45,2,'C','' ,''              ,'Unidade'         ,''         ,'',false);
          FRel.AddCol( 80,0,'C','' ,''              ,'Transação'       ,''         ,'',false);
          FRel.AddCol( 60,1,'C','' ,''              ,'Placa Veículo'     ,''         ,'',false);
          FRel.AddCol(100,1,'C','' ,''              ,'Veículo'         ,''         ,'',false);
          ...
          FRel.AddCol(150,1,'C','' ,''              ,'Descrição Item'   ,''         ,'',False);
          FRel.AddCol( 60,3,'N','',f_cr3           ,'Unitário Item'   ,''         ,'',False);
          ...
          FRel.AddTit('Custo Veículos da Frota por KM rodado');
...
// 12.12.19 - só pra dar eof e nao fazer nada...
...
//      FRel.AddTit(Global.NomeSistema+' '+Global.VersaoSistema+space(78)+'Emissão:'+Datetostr(Sistema.hoje)+' '+timetostr(time)+'  Pg: [NumPg]',false,false,true );
      FRel.AddTit('Relação de Auditoria Fiscal de Base do Icms dos Itens das Notas');
      FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text));
      FRel.AddTit('Periodo : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
      FRel.AddCol( 80,0,'C','' ,''              ,'Transação'       ,''         ,'',false);
      FRel.AddCol( 80,1,'D','' ,''              ,'Entrada/Emissão'  ,''         ,'',false);
      FRel.AddCol( 60,2,'N','' ,''              ,'Nota'     ,''         ,'',False);
      FRel.AddCol( 50,0,'C','' ,''              ,'Cfop'            ,''         ,'',False);
...
//      FRel.AddCol( 40,1,'C','' ,''              ,'Dif.'        ,''         ,'',false);
      ...
      FRel.AddCol( 80,3,'N','+',f_cr            ,'Contábil Nota'       ,''         ,'',False);
      FRel.AddCol( 80,3,'N','+',f_cr            ,'Contábil itens'      ,''         ,'',False);
      FRel.AddCol( 80,3,'N','+',f_cr            ,'Dif.Contábil'      ,''         ,'',False);
      FRel.AddCol( 80,3,'N','+',f_cr            ,'Valor IPI'      ,''         ,'',False);
//      FRel.AddCol( 60,3,'N','' ,'##0.00'        ,'Alíquota'        ,''         ,'',False);
//      FRel.AddCol( 80,3,'N','+',f_cr            ,'Valor Imposto'   ,''         ,'',False);
      for i:=0 to Lista.count-1 do begin
...
//////////////////////////////////////////
    ...
    sistema.beginprocess('separando quantidade por veículo');
    sqlperiodo:=' and mova_dtabate>='+FRelGerenciais.EdDatai.AsSql+' and mova_dtabate<='+FRelGerenciais.EdDataf.AsSql;
    if not FRelGerenciais.EdTran_codigo.isempty then
      titveiculos:='Veículo '+FRelGerenciais.EdTran_codigo.text+' - '+FTransp.GetNome(FRelGerenciais.EdTran_codigo.text)
    else
      titveiculos:='';
    if xsa='A' then
      Anasin:=' - Analítico'
...
// 27.05.10
    ...
    titulo:='Comissão sobre Entrada de Abate de '+FGeral.FormataData(Eddatai.asdate)+' a '+FGeral.FormataData(Eddataf.asdate);
...
// coletando as remessas da(s) venda(s) escolhida(s)
      ...
      for p:=0 to Lista.count-1 do begin
       ...
       if trim(remessas)='' then begin
         Avisoerro('Não encontrado remessas desta venda.  Checar se foi alterada');
         Q.close;
         Freeandnil(Q);
         Sistema.endprocess('');
         exit;
...
//////////////////
      ...
      Sistema.beginprocess('Pesquisando devoluçoes');
      Q.close;
      Freeandnil(Q);
...
//                  ' inner join movesto on ( moes_transacao=move_transacao )'+
                  ...
                  ' and '+FGeral.getin('moes_status','D;E','C')+
                  ' order by move_unid_codigo,move_repr_codigo,move_tipo_codigo,move_datamvto,move_esto_codigo' );
      Sistema.beginprocess('Somando devoluçoes');
      while not Q.eof do begin
...
// 18.05.06
      ...
      if  trim(devolucoes)<>'' then begin
        Sistema.beginprocess('Somando devoluçoes pesquisadas');
        Q:=sqltoquery('select * from movestoque '+
                  ' inner join movesto on ( moes_transacao=move_transacao and moes_tipomov=move_tipomov and moes_numerodoc=move_numerodoc and moes_tipo_codigo=move_tipo_codigo)'+
//                    ' inner join movesto on ( moes_transacao=move_transacao and moes_tipomov=move_tipomov and moes_numerodoc=move_numerodoc )'+
  //                  ' inner join movesto on ( moes_transacao=move_transacao )'+
...
//////////////////
      ...
      if not EdNumerodoc.IsEmpty then begin
        if length(devolucoes)>100 then
          FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text)+' - Devoluções:'+copy(devolucoes,1,100)+' mais ...' )
        else
          FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text)+' - Devoluções:'+devolucoes);
      end else
          FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text));
...
//      FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text)+' - Devoluções:'+devolucoes);
      FRel.AddTit('Periodo : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate)+' Período Remessas '+FGeral.formatadata(DataDevolucoes)+' a '+FGeral.formatadata(Datafinal) );
      FRel.AddCol( 45,2,'C','' ,''              ,'Unidade'         ,''         ,'',false);
      if not EdNumerodoc.isempty then begin
        FRel.AddCol( 45,1,'C','' ,''              ,'Codigo'          ,''         ,'',false);
        FRel.AddCol(120,1,'C','' ,''              ,'Representante'   ,''         ,'',false);
      ...
      if not EdNumerodoc.isempty then begin
        ...
        FRel.AddCol( 80,3,'N','+' ,''             ,'Diferença'     ,''         ,'',False);
        FRel.AddCol( 80,3,'N','+' ,''             ,'Vlr Diferença'     ,''         ,'',False);
      end else begin
        FRel.AddCol( 80,3,'N','+' ,''             ,'Venda'     ,''         ,'',False);
...
//      FRel.AddTit(Global.NomeSistema+' '+Global.VersaoSistema+space(78)+'Emissão:'+Datetostr(Sistema.hoje)+' '+timetostr(time)+'  Pg: [NumPg]',false,false,true );
      FRel.AddTit('Relação de Auditoria Fiscal por CFOP'+titentsai);
      FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text));
      FRel.AddTit('Periodo : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
      FRel.AddCol( 45,2,'C','' ,''              ,'Unidade'         ,''         ,'',false);
      FRel.AddCol( 80,0,'C','' ,''              ,'Transação'       ,''         ,'',false);
...
///////////////////////////////////////////////////////////
  with FRelGerenciais do begin
    ...
    if Lista.count=0 then begin
      Avisoerro('Não encontrado vendas no período');
      Sistema.EndProcess('');
      Q.close;
      Freeandnil(Q);
      exit;
    ...
    FRel.AddTit('Relatório de Análise de Venda por Cliente');
...
//      FRel.AddTit(Global.NomeSistema+' '+Global.VersaoSistema+space(78)+'Emissão:'+Datetostr(Sistema.hoje)+' '+timetostr(time)+'  Pg: [NumPg]',false,false,true );
      FRel.AddTit('Relatório de Lotes da Fazenda');
      FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text));
      FRel.AddTit('Periodo : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
//      FRel.AddCol( 45,2,'C','' ,''              ,'Unidade'         ,''         ,'',false);
//      FRel.AddCol( 80,0,'C','' ,''              ,'Transação'       ,''         ,'',false);
      ...
      FRel.AddCol(150,0,'C','' ,''              ,'Descrição Animal'            ,''         ,'',False);
      FRel.AddCol( 50,0,'C','' ,''              ,'Setor'            ,''         ,'',False);
      FRel.AddCol(150,0,'C','' ,''              ,'Descrição Setor'            ,''         ,'',False);
      ...
      FRel.AddCol( 80,3,'N','',f_cr             ,'% Projeção' ,''         ,'',False);
...
begin
  ...
  with FRelGerenciais do begin
    ...
    titulo:='Comissão Boiadeiros sobre Entrada de Abate de '+FGeral.FormataData(Eddatai.asdate)+' a '+FGeral.FormataData(Eddataf.asdate);
...
//                  ' and '+FGeral.GetIN('movd_status',statusvalidos,'C')+
    ...
    if Q.Eof then
      ...
      FRel.AddCol(140,0,'C','' ,''              ,'Caminhão'    ,''         ,'',false);
//      FRel.AddCol( 45,0,'C','' ,''              ,'Produto'         ,''         ,'',false);
//      FRel.AddCol(140,0,'C','' ,''              ,'Descrição'       ,''         ,'',false);
      FRel.AddCol(080,3,'N','+',f_cr            ,'KM'   ,''         ,'',false);
      FRel.AddCol(080,3,'N','+',f_cr            ,'Comissão KM'  ,''         ,'',false);
      FRel.AddCol(080,3,'N','',f_cr             ,'Por KM'  ,''         ,'',false);
      FRel.AddCol(080,3,'N','+',f_cr            ,'Cabeças'   ,''         ,'',false);
      FRel.AddCol(080,3,'N','+',f_cr            ,'Comissão Cab.'  ,''         ,'',false);
      FRel.AddCol(080,3,'N','',f_cr             ,'Por Cabeça'  ,''         ,'',false);
...
//      FRel.AddTit(Global.NomeSistema+' '+Global.VersaoSistema+space(78)+'Emissão:'+Datetostr(Sistema.hoje)+' '+timetostr(time)+'  Pg: [NumPg]',false,false,true );
      FRel.AddTit('Relatório de Resumo dos Lotes da Fazenda');
      FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text));
      FRel.AddTit('Periodo : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
//      FRel.AddCol( 45,2,'C','' ,''              ,'Unidade'         ,''         ,'',false);
//      FRel.AddCol( 80,0,'C','' ,''              ,'Transação'       ,''         ,'',false);
//      FRel.AddCol( 60,1,'D','' ,''              ,'Movimento'  ,''         ,'',false);
      FRel.AddCol( 60,2,'N','' ,''              ,'Lote'     ,''         ,'',False);
      FRel.AddCol( 50,0,'C','' ,''              ,'Baia'            ,''         ,'',False);
      FRel.AddCol(150,0,'C','' ,''              ,'Endereço'            ,''         ,'',False);
      FRel.AddCol( 80,3,'N','+',''              ,'Vivos' ,''         ,'',False);
      FRel.AddCol( 80,3,'N','+',''              ,'Abatidos' ,''         ,'',False);
      FRel.AddCol( 90,3,'N','+',''              ,'Não Encontrados' ,''         ,'',False);
      FRel.AddCol( 80,3,'N','+',''              ,'Total' ,''         ,'',False);
      FRel.AddCol(150,1,'C','' ,''              ,'Brincos Não Encontrados' ,''         ,'',False);
