...
//       Avisoerro('Reduzido '+inttostr(xcontareduzida)+' não encontrado no plano de contas');
...
// 18.10.12 - retirado por ainda nao usar...
  ...
  if not confirma('Confirma geração do arquivo do Sped Fiscal ?') then exit;
...
// 24.09.19
  GerarCodCta:=true;
  if not FGeral.ConectaContax then begin
     Avisoerro('Não foi possível conectar com a contabilidade. Cod_Cta não será informado !!');
     GerarCodCta:=false;
  end;
...
/// 21.09.18 - bloco é serviços e E110 é somente para icms
           end;
     end;
...
// 24.02.14 - Abra Cuiaba - notas de prestacao de serviços nao geram
// 19.02.15 - DAmama
  ...
  if Q.eof then begin
    Avisoerro('Nada encontrado para exportação.  Será gerado SEM movimento');
  end;
...
//                ver se é necessario fazer por unidade e ver se é do simples
...
//              SER:=strspace(Q.fieldbyname('moes_serie').asstring,3);
              ...
              DT_DOC:=Q.fieldbyname('moes_dataemissao').asdatetime;
              DT_E_S:=datanota;
// 19.09.2022 - Clessi - Receita Sc exige lançamento com valor zerado da entrada NFPe
...
// 26.06.17 - 'notas maxicorte' ração cujo unitario tem 'quase dízima'...
...
// 'APROVEITAMENTO DO CREDITO DE ICMS NO VALOR DE R$ 278,85'
              ...
              if trim(Q.fieldbyname('moes_chavenfe').asstring)<>'' then
                CHV_NFE:=copy(Q.fieldbyname('moes_chavenfe').asstring,1,44);
              VL_COFINS_ST:=0;
// 23.05.12 - Clessi - para gerar aqui pois NFe não gera o registro C170
...
/////////////////////////////////////////////////
// fatura de energia elétrica + Conta de Agua
////////////////////////////////////////////
...
//////////////// C510 - 'item' de energia elétrica  - NAO DEVE SER INFORMADO
...
// só gerar se for nota de prestação de serviço mas NÃo serie F que é
...
// detalhes do(s) volume(s) transportados, do transportador e placa do veículo
///////////////////////////////////////////////////////////////////////
...
//          if  GetModelo(Q.fieldbyname('moes_tipomov').asstring) <> ModeloNfe then begin
// 24.11.11 - gera se for NFe de Entrada OU se não for NFe
// 21.02.16 - gera se for NFe de Entrada E nao for emissao propria OU se não for NFe
...
// ver como calcular estes valores..se é apenas aplicar as aliquotas
// que tem no cadastro
...
//              ' and '+FGeral.GetIN('saes_esto_codigo',codigosprodutos,'C')+
        ...
        if QInventario.Eof then
          Avisoerro('Não encontrado inventário ref. '+EdMesano.text+' OU todos os itens zerados.  Bloco H não gerado');
...
//              ' and '+FGeral.GetIN('saes_esto_codigo',codigosprodutos,'C')+
             ...
             if QInventario.Eof then
                Avisoerro('Não encontrado inventário ref. '+EdMesano.text+' OU todos os itens zerados.  Bloco K não gerado');
/////////////////////////////////////////
...
//              codigosdeinsumos:=FComposicao.GetCodigosMat(QAcabado.FieldByName('move_esto_codigo').AsString);
              codigosdeinsumos:='';
// 09.03.19 - pegar somente se tiver composição cadastrada
// 30.01.20 - pegar o q tiver sido lançado ou importado na requisição como saida do almox..
...
///// Registro 1400. - entradas de produtor de 'qq espécie animal'
        ...
        for p:=0 to ListaRegE110.Count-1 do begin
          ...
          with RegistroE116New do begin
            ...
            if EdUnid_codigo.ResultFind.FieldByName('unid_uf').asstring='PR' then
              ...
              COD_REC:='144910014'   // icms a recolher décimo dia util
            else
...
//            PROC:='';
//            TXT_COMPL:='';
// campo somente a partir de 01.2011 - deverá precisar de atualizao da ACBR
...
// 01.06.10 - rever ..a classificacao é 'do sped' ou do plano da empresa
...
/////////////////
{
    if ind_ctav = 'A' then begin
      if (QPlan.Fieldbyname('pcon_classificacaosped').AsString='') or (QPlan.Fieldbyname('pcon_classificacaosped').AsString = null) then begin
         Texto.Lines.Add('Conta: '+QPlan.Fieldbyname('pcon_conta').AsString+' não associada ao plano referencial !');
      end;
      ...
      if (QPlan.Fieldbyname('pcon_spednaturezaconta').AsString = '') or (QPlan.Fieldbyname('pcon_spednaturezaconta').AsString = null) then begin
        Texto.Lines.Add('Conta: '+QPlan.Fieldbyname('pcon_conta').AsString+' não possui natureza !');
//        Sistema.EndProcess('Geração do Sped interrompida');
