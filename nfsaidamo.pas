...
//  if not Arq.TConfMovimento.Active then Arq.TConfMovimento.Open;
  ...
  if OP='A' then
    ...
    operacao:='Inclusão';
  FNotaSaidaMo.Caption:='Nota Fiscal de Mão de Obra - Saida - '+operacao;
...
//    Avisoerro('Atenção.   Checar preço de venda no cadastro.');
   ...
   if (not EdDtmovimento.isempty) then begin
     if  campoufentidade='forn_uf' then begin
       if ( trim(EdCliente.resultfind.fieldbyname('forn_cnpjcpf').AsString)='' ) then  begin
         Avisoerro('Obrigatório fornecedor com CNPJ/CPF');
         exit;
       end;
     end else begin
       if ( trim(EdCliente.resultfind.fieldbyname('clie_cnpjcpf').AsString)='' ) then  begin
         Avisoerro('Obrigatório cliente com CNPJ/CPF');
         exit;
       end;
     end;
   end;
...
//    Impr.ImprimeString(EdUnid_codigo.resultfind.fieldbyname('unid_bairro').asstring,false);
    ...
    Impr.ImprimeString('Ver qual numero controle terá',true);
...
// 01.090.9
       ...
       if (not QBusca.eof) and ( (OP='S') or (OP='I') ) then begin
         EdNUmerodoc.INvalid('Numero de nota encontrado em '+formatdatetime('dd/mm/yy',QBusca.fieldbyname('move_datamvto').asdatetime)+'.   Digitação não permitida');
...
//                                ' and mova_tipo_codigo='+EdCliente.assql+
            ...
            if QRoma.eof then
              EdPedido.invalid('Saida não encontrada ou com nota de venda já feita')
            else begin
              Grid.clear;
              Lista:=TList.create;
        ...
        s := msword.selection; //variavel para facilitar trabalho
{
        ...
        s.typeparagraph;
        s.typetext('Contratado  : '+strspace('VIMS - Esquadrias de Alumínio',50) );
