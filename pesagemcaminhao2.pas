...
begin
    ...
    Q1:=sqltoquery('select * from movcargas where movc_status='+stringtosql('N')+
                 ...
                 ' and movc_pesada = '+EdPesada.assql+
...
// 19.05.20
    QC:=sqltoquery('select * from movcargas where movc_status='+stringtosql('N')+
                 ...
                 ' and movc_tran_codigo='+EdTran_codigo.assql+
                 ' and movc_data = '+Datetosql(Sistema.hoje));
    if Qc.eof then novapesagem := true else novapesagem:=false;
///////////////
}
...
// 05.06.20
      Sistema.setfield('movc_pesada',EdPesada.AsInteger);
      Sistema.post();
    ...
    begin
       ...
       for I := 2 to 10 do begin
          ...
          QC:=sqltoquery('select * from movcargas where movc_status='+stringtosql('N')+
                 ' and movc_unid_codigo='+EdUnid_codigo.assql+
                 ' and movc_tran_codigo='+EdTran_codigo.assql+
                 ' and movc_pesada = '+inttostr( i )+
                 ' and movc_data = '+Datetosql(Sistema.hoje));
          if QC.Eof then begin

             pesada:=i; // nova pesada
             novapesagem := true;
             break;
...
//acho pesada completa tem q ver se ja tem proxima...
//                 pesada := i + 1 ;
//                 novapesagem := true;
//                 break;
...
begin
   ...
   QC:=sqltoquery('select * from movcargas where movc_status='+stringtosql('N')+
                 ' and movc_unid_codigo='+EdUnid_codigo.assql+
                 ' and movc_tran_codigo='+EdTran_codigo.assql+
                 ' and movc_pesada = '+inttostr(pesada)+
                 ' and movc_data = '+Datetosql(Sistema.hoje));
 {
  if QC.eof then begin
  ...
  // 05.06.20 - prever mais de uma pesada por dia do mesmo caminhão...

     EdPesoinicial.setvalue(Qc.FieldByName('movc_pesoi').AsCurrency);
     EdPesofinal.setvalue(Qc.FieldByName('movc_pesof').AsCurrency);
     novapesagem:=false;
     if Qc.FieldByName('movc_pesof').AsCurrency>0 then VerificaPesada;
     if novapesagem then begin
         EdPesoinicial.clear;
         EdPesofinal.clear;
     end;
