...
// 01.10.12 - vindo da pesagem....
  ...
  if OP='A' then
    ...
    operacao:='Reaproveitamento Numeração'
  else if op ='G' then
...
// 18.01.12 - para nao ficar errado a numeracao de nota
  ...
  CodigoImp:='00013';  // ver onde colocar o codigo da impressora ecf
                       // 12.07.11 - ver se realmente será usado
...
//////////////////// - 05.06.09
      ...
      Input('Informe a transação a ser alterada ','Transação ',transacaobusca,12,true);
...
// 12.08.2022
            if TiposFornecedor(EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring) then begin
              ...
              EdCliente.update;
              NotaTipocad:='F';  // q poderá ser somente C ou F
            end else begin
              FGeral.SetaEdEntidade(Edcliente,'C');
              campoufentidade:='clie_uf';
            end;

// 12.08.2022 - para não dar cliente não encontrado quando é fornec como em RS e outras...
...
//12.07.12 - Novicarnes = Clevis
            ...
            Avisoerro('Transação não encontrada ou cancelada');  // 03.10.08
...
// 18.04.19 - Giacomoni - Barbará  - cliente com 'sistema exigindo'...
...
// 23.01.20 - Novicarnes - venda de ração
...
// 03.04.20
       ...
       Q:=Sqltoquery('select move_qtde,move_numerodoc from movestoque'+
        ...
        EdMensagem.text:=EdMensagem.text+' Romaneios '+listaromaneios+' Município '+Fcidades.GetNome(EdCliente.resultfind.fieldbyname('Clie_cida_codigo_res').AsInteger);
    end;
...
// 01.04.13 - pela cidade do moes_clie_codigo
        ...
        FGeral.FechaQuery(Q);
        EdQtde.setvalue(rqtde);
        EdMensagem.text:=mens+' NF de Remessa a Ordem '+listaromaneios+' Município '+Fcidades.GetNome(strtointdef(EdCida_codigo.text,0) );
    end;
...
// 15.12.15
  ...
  if copy(EdNatf_codigo.text,1,1)<>'7' then begin
    if pos(copy(EdNatf_codigo.text,1,1),iniciook)=0 then begin
      Avisoerro('Cfop inválido para esta operação de saida.');
      exit;
    end;
  end;
//////////////////////
...
//    EdPerdesconto.enabled:=false;
    EdPerdesconto.setvalue(0);
// 23.09.2021 - Situacao Sicare - calcula st mas não icms
...
// 14.10.09
  if (Global.Topicos[1336]) and ( not ConstaListaProdutosAcabados(SetEdESTO_DESCRICAO.Items,EdProduto.Text) )
    ...
    then begin
    EdProduto.invalid('Produto não encontrado na produção.');
    exit;
  end;
// 05.04.10 - Abra
...
// 24.11.11 - no codigo de barra da balanca vem o valor total mas como o preço
// do kilo é . por ex.
...
// 27/10/04 - caso usar tabela recalcular preço de venda mas não "aparecer" o desconto ou acréscimo
// pois estará embutido no preço de venda
...
// 18.08.11 - Capeg
  if ( EdComv_codigo.ResultFind.fieldbyname('comv_tipomovto').asstring=Global.CodNotaRemessaaOrdem )
    ...
    if Global.Topicos[1469] then begin
       ...
       Sistema.GetPeriodo('Período para somar romaneios');
...
//       Sistema.GetPeriodo('Período para somar notas');
       ...
       if nffinal<nfinicial then begin
          Avisoerro('Número final menor que o inicial');
          exit;
       end;
...
// 04.11.2022 - prever icms diferido q não reduz a base e aliquota..somente pra efeito de calculo aplica na aliquota
         and
...
// 22.01.19 - Seip - voltado a pegar a tal 'aliquota fixa' interna de cada estado
             ...
             //08.06.19 - Sicare - voltado calculo peganedo aliquota q tá no produto..
...
// 29.10.14 - ajuste no calculo Da ST para usar 2 aliquotas de icms..a do icmsst é do estado destino
//            e a do icms 'normal' é do estado 'origem'... - giacomoni+asterio
...
//  end else begin   // 23.09.05 - retirado em 05.11.14 - Mirvane devolucao com ST e nao é ME
//      basesubs:=0;
//      icmssubs:=0;
  end;
...
/////////////////////    OP:='E';    // 20.12.19
// 02.06.20 - retirado esta EEEERRDAAA q só deu cagadaaaaaaaaaaaaaaaaa
...
//    EdPort_codigo.valid;
{
    ...
    if (Global.Topicos[1376]) then begin
      if trim(EdCliente.ResultFind.fieldbyname('clie_portadores').asstring)<>'' then begin
        ...
        Aviso('Portador(es) ainda não definidos neste cliente');
        Edcliente.SetFocus;
      end;
    end else
      }
...
//    EdVlrDesco.setfocus;
{
    ...
    EdFpgt_codigo.enabled:=true;
// 26.05.14 - vivan cobrança angela
...
// 17.05.18
  if (  Global.Topicos[1453] ) and ( EdFpgt_codigo.text<>FGeral.GetConfig1AsString('EdFpgtoavista') )
    ...
    EdFpgt_codigo.INvalid('Em portador carteira condição deve ser a vista');
    exit;
...
//          Avisoerro('Não foi possível abrir arquivo '+dbforcam.TableName);
      ...
      while (not dbforcam.Eof) and (ListaReq.Count>0) do begin
          if dbforcam.fieldbyname('codigo').asstring=PReq.obra then begin
            ...
            if not QEstoque.eof then begin
              ...
              Sistema.post;
              Sistema.commit;
// 29.04.08 - checa se precisa gravar nova grade de tamanho+cor - 'BARRAS EM PEDAÇOS'
/////////////////////////////////////
...
////        Avisoerro('Falta configurar o local do PEA nas configuração do sistema');
        exit;
      end else begin
...
//          Avisoerro('Não foi possível abrir arquivo '+dbforcam.TableName);
        ...
        Sistema.beginprocess('Checandos os acessórios da obra '+EdVendasmc.text);
        while not dbforcam.Eof do begin
          if dbforcam.FieldByName('codigo').asstring=obra then
...
//            avisoerro('Não encontrado no estoque a referência '+PReq.codigopea);
            refnaoachados:=refnaoachados+PReq.codigopea+';';
          FGeral.FechaQuery(Qe);
        end;
// 09.10.08
        if (trim(refnaoachados)<>'') and (trim(refnaoachados)<>';') then
           avisoerro('Não encontrado no estoque a(s) referência(s) '+refnaoachados);
      end;
   end;
...
// 09.10.20 - Novicarnes
   ...
   then begin
     ...
     if EdTran_codigo.ResultFind.fieldbyname('tran_cola_codigo').asstring = '' then begin
       Avisoerro('Obrigatório codigo do colaborador no cadastro do transportador');
       exit;
     end;
   ...
   if pos( Edcomv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring,Global.CodConhecimentoSaida ) > 0 then begin
     if Acbrnfe1.NotasFiscais.Count=0 then begin
       if trim(Grid71.Cells[01,01])='' then begin
         Avisoerro('Obrigatório informar as notas acobertadas pelo conhecimento');
         exit;
       end;
     end;
   end;
...
// 06.02.14 - Patoterra
   if ( global.topicos[1367] ) and ( EdCodEqui.IsEmpty ) then begin
     Avisoerro('Ainda não informado o codigo do equipamento');
     exit;
   end;
// 11.12.19 - A2z
...
// 22.01.16 - Patopapel
   if (EdComv_codigo.asinteger=FGeral.GetConfig1AsInteger('ConfMovNFCe')) and
     ...
     Input('Informar o CPF/CNPJ (OPCIONAL)','Número',xcpf,14,true);
   ...
   if ( pos(EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').AsString,Global.TiposSaida)>0 ) then begin
     if ( EdCliente.resultfind.fieldbyname(campoufentidade).asstring <> EdUnid_codigo.resultfind.fieldbyname('unid_uf').asstring  )then  begin
        if pos( copy(EdNatf_codigo.text,1,1),'6;7' ) = 0 then begin
          Avisoerro('UF do destinatário ou cfop incorreto(s) !');
          exit;
        end;
     end else begin
        if pos( copy(EdNatf_codigo.text,1,1),'6;7' ) > 0 then begin
...
// 21.06.16
  if (Global.topicos[1043]) and ( EdDtMovimento.asdate > 1 )  then begin
     ...
     if (debito=0) then begin
       Avisoerro('Falta configurar a conta de débito');
       exit;
     end else if (credito=0) then begin
       Avisoerro('Falta configurar a conta de crédito');
       exit;
     end;
  end;
// 15.05.17
   if (pos(EdUnid_codigo.resultfind.fieldbyname('unid_simples').AsString,'S;2')>0) and
      ...
      if ( confirma('Zerar valores de IPI e ST ?') ) then begin
         ...
         Edmensagem.text:='Devolução da NF '+inttostr(notadevolucao)+' de '+FGeral.formatadata(DataNotaDevolucao)+
...
// 28.10.11 - SM - Gris
                ...
                end else if (EdTotalNOta.AsCurrency>0) or ( pos(EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring,Global.CodNfeComplementoQtde+';'+Global.CodNfeComplementoIcms)>0) then // 18.11.09 devido nf serviços
...
// 04.03.13 - mudanças Capeg  - nfe remessa a ordem sempre pro mesmo 'cliente secretaria'
           if ( EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring=Global.CodNotaRemessaaOrdem )
              and
              ( pos(EdCliente.text,FGeral.getconfig1asstring('clientesjustica')+';'+FGeral.getconfig1asstring('clientessaude')) = 0 )
             then begin
// 05.06.13 - codigos que estiverem configuradas como sec. da justiça e saúde  hj 3 faz nfe normal)
             moes_clie_codigo:=EdCliente.AsInteger;
             EdMensagem.text:=EdMensagem.text+' CNPJ do colégio no.'+EdCliente.ResultFind.fieldbyname('clie_cnpjcpf').AsString;
             EdCliente.text:=inttostr( FGeral.GetConfig1AsInteger('clienferemordem') );
             EdCliente.validfind;
// 06.01.16 - retirado pois não é mai necessario - Leonir + Dani Ratko
...
begin
  ...
  if FileExists( Nomearquivo ) then begin
      ...
      if not Input('Informe','Transação',xtransacao,12,false) then exit;
      ...
      if Qu.Eof then begin
         Avisoerro('Transação não encontrada');
         exit;
      end;
...
begin
  if copy(EdNatf_codigo.Text,1,1) <> copy(EdNatf_codigop.Text,1,1) then
     EdNatf_codigop.Invalid('Cfop inválido');
end;
...
begin
  ...
  if copy(EdNatf_codigo.text,1,1)<>'7' then begin
    if pos(copy(EdNatf_codigo.text,1,1),iniciook)=0 then
      EdNatf_codigo.Invalid('Cfop inválido para esta operação de saida.');
  end else if copy(EdNatf_codigo.text,1,1)='7' then begin
// 06.04.20 - Seip
     if Confirma('Mudar o cfop também nos itens da nota ?' ) then begin
  ...
  if trim( FGeral.GetConfig1AsString('tipomovlote1')) <> '' then begin
    ...
    if Ansipos(strzero(Edcomv_codigo.AsInteger,4),FGeral.GetConfig1AsString('tipomovlote1') )>0 then begin
       ...
       if AnsiPos('LOTE',EdMensagem.text)=0 then begin
         ...
         Sistema.GetDataMvto('Data Fabricação');
         EdMensagem.text := EdMensagem.text + ' LOTE 1 : Data Fabricação : '+FGeral.formatadata(Sistema.DataMvto);
  ...
  if trim( FGeral.GetConfig1AsString('tipomovlote2')) <> '' then begin
    ...
    if Ansipos(strzero(Edcomv_codigo.Asinteger,4),FGeral.GetConfig1AsString('tipomovlote2') )>0 then begin
       ...
       if AnsiPos('LOTE',EdMensagem.text)=0 then begin
         ...
         Sistema.GetDataMvto('Data Fabricação');
         EdMensagem.text := EdMensagem.text + ' LOTE 2 : Data Fabricação : '+FGeral.formatadata(Sistema.DataMvto);
...
// 10.11.2022 - retirado ja q não puxa o serviço do pedido mas estava totalizando e dando erro
//               na autorização da nfe
// 21.02.20
        else if QPed.fieldbyname('mpdd_esto_codigo').asstring = xCodProdutoGeral then

             Avisoerro('Item ainda não cadastrado no estoque. Não importado do pedido')
...
// recalcular preço unitario pelo valor total pois pode variar o preço do produto
...
// 18.03.15 - Vivan
  if campoufentidade='clie_uf' then begin // 23.03.15 - vivan cecilia
    ...
    if ( Global.topicos[1229] )  and ( pos(EdCliente.ResultFind.FieldByName('clie_contribuinte').AsString,'1/2')=0 ) then
      EdCliente.invalid('Falta identificar se é 1-Simples  2-Icms Normal para cálculo do preço de venda');
  end;
  restricao1:=true;
  restricao2:=true;
  restricao3:=true;
...
// 23.09.17
      campo:=Sistema.GetDicionario('clientes','clie_Descontovenda');
//      if campo.Tipo<>'' then
// 22.05.18 - VidaNova - só aparecer no boleto o desconto nas obs na nota sai valor cheio
...
// 25.05.10 -
        if (Global.Topicos[1340]) and (EdTotalProdutos.AsCurrency>0)  then begin
          if EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring<>Global.CodVendaProntaEntrega then begin
            EdCliente.Invalid('Usar configuração de movimento VP-Venda Pronta Entrega para faturar este pedido');
            exit;
          end;
        end;
//        if (EdTotalServicos.AsCurrency>0) and (EdTotalProdutos.AsCurrency=0) then begin
//          Avisoerro('Pedido apenas com serviços.  Usar config. de movimento '+inttostr(FGeral.GetConfig1AsInteger('ConfMovSer'))+' ref. serviços' );
...
// 26.05.14 - vivan cobrança angela
  if (Global.Topicos[1413]) and (campoufentidade='clie_uf') then begin
      if (trim (EdCliente.ResultFind.fieldbyname('Clie_portadores').asstring)='') then begin
        EdCliente.Invalid('Ainda não definido portador(es) neste cliente');
        exit
      end;
  end;
// 28.11.14 - vivan cobrança angela
...
// 29.10.15 - Vivan
  if not ValidaTiposdeSaida(Edcomv_codigo.asinteger) then begin
     EdComv_codigo.invalid('Tipo de Venda não permitida');
     exit;
  end;
// 12.12.07
  NotaTipocad:='C';
...
//        EdComv_codigo.invalid('Impressora padrão não encontrada')
//      else
...
// 31.05.07 - mensagem padrão nf saida
...
// 11.09.19 - Vida Nova
{
  if trim( FGeral.GetConfig1AsString('tipomovlote1')) <> '' then begin
    ...
    if Ansipos(strzero(Edcomv_codigo.AsInteger,4),FGeral.GetConfig1AsString('tipomovlote1') )>0 then begin
       ...
       Sistema.GetDataMvto('Data Fabricação');
       EdMensagem.text := 'LOTE 1 : Data Fabricação : '+FGeral.formatadata(Sistema.DataMvto);
  ...
  if trim( FGeral.GetConfig1AsString('tipomovlote2')) <> '' then begin
    ...
    if Ansipos(strzero(Edcomv_codigo.Asinteger,4),FGeral.GetConfig1AsString('tipomovlote2') )>0 then begin
       ...
       Sistema.GetDataMvto('Data Fabricação');
       EdMensagem.text := 'LOTE 2 : Data Fabricação : '+FGeral.formatadata(Sistema.DataMvto);
...
// 07.04.20 - seip - 'anulação entrada importação'
// 24.07.20
...
//      Aviso('Obrigatório informar a chave em nota complementar de Icms');
...
// 15.05.17
      if confirma('Quer informar direto a transação ?') then
...
// 07.04.15
           ...
           Edmensagem.text:='Devolução da NF '+QN.fieldbyname('moes_numerodoc').AsString+' de '+FGeral.formatadata(QN.fieldbyname('moes_dataemissao').Asdatetime);
         end else
           Edmensagem.text:='Complemento de Icms da NF '+QN.fieldbyname('moes_numerodoc').AsString+' de '+FGeral.formatadata(QN.fieldbyname('moes_dataemissao').Asdatetime);
         EdCliente.text:=QN.fieldbyname('moes_tipo_codigo').AsString;
         EdCliente.validfind;
...
// 23.11.18 - para situação de devolver poucos itens de nf com muuuitos itens dai digita direto...
...
// 21.06.2021 - devolucao devereda
        ...
        Avisoerro('Transação '+trim(Lista[0])+'| não encontrada');
      end;
    end else begin
// 29.04.15
         if Edchavenfeacom.IsEmpty then begin
           Input('Infome chave da NFe a ser devolvida','Chave',xchaveref,44,false);
           EdChavenfeacom.text:=xchaveref;
           if Edchavenfeacom.IsEmpty then Aviso('Caso for fazer nf eletrônica não será autorizada sem a chave da nf-e a ser devolvida');
         end;
    end;
// 29.06.20
   Lista.free;
...
//    Edchavenfeacom.text:=chavenfeacom;
  ...
  if OP='I' then begin
// ver qual a condição para usar as 'saidas de abate' ou se cria permissao por usuario ou do sistema
      if ( Global.Topicos[1320] )  and
        ...
        Sistema.BeginProcess('Checando pedidos de venda frigorífico');
...
// ver se terá o 'rateio' nas saidas de abate dai volta situacal='N'
...
// 03.07.14 - Capeg - pega do mes anterior pois sempre é feito com data 'retroativa' a emissao destas notas
...
// 10.03.06
    ...
    if valor>0 then
      ...
      Avisoerro('Total da nota está zerado.  Cálculo não feito');
      peracre:=0;
    end;
...
begin
   if EdComv_codigo.ResultFind.fieldbyname('comv_tipomovto').asstring=Global.CodConhecimentoSaida then
     if EdEspecievolumes.IsEmpty then EdEspecievolumes.Invalid('Obrigatório informar tipo de volume');
end;
...
// 28.06.05
         ...
         if QBusca.eof then begin
           EdNUmerodoc.INvalid('Numero de nota não encontrado tipo '+EdComv_codigo.ResultFind.fieldbyname('comv_tipomovto').asstring);
...
// 01.090.9
       ...
       if (not QBusca.eof) and ( (OP='S') or (OP='I') ) then begin
         ...
         EdNUmerodoc.INvalid('Numero de nota encontrado em '+formatdatetime('dd/mm/yyyy',QBusca.fieldbyname('move_datamvto').asdatetime)+'.   Digitação não permitida');
//         EdNumerodoc.ClearAll(FNotaSaida,99);
         Grid.Clear;
...
// 08.03.20 - A2z - thais
          ...
          QBusca:=sqltoquery( 'select moes_transacao,moes_dataemissao from movesto '+
                              ...
                              ' and moes_status = ''N'''+
                              ' and moes_unid_codigo = '+EdUNid_codigo.assql );
          if not Qbusca.Eof then Aviso('ATENÇÃO !!! Numero já encontrado em '+FGeral.FormataData(QBusca.FieldByName('moes_dataemissao').AsDateTime) );
...
//         if abs( numero-EdNumerodoc.asinteger ) >1 then begin
         if numero<>EdNumerodoc.asinteger then begin
             EdNUmerodoc.INvalid('Ultima nota feita é número '+inttostr(numero)+'. Digitação não permitida');
//             EdNumerodoc.ClearAll(FNotaSaida,99);
             Grid.Clear;
         end;
       end;
// 31.08.05
       if EdComv_codigo.resultfind.fieldbyname('comv_tipomovto').asstring=Global.CodVendaAmbulante then begin
          ...
          if not Qbusca.eof then
             EdNUmerodoc.INvalid('Numero de nota '+Global.CodVendaAmbulante+' encontrado.   Digitação não permitida');
       end;
// 13.08.12
       if EdComv_codigo.resultfind.fieldbyname('comv_tipomovto').asstring=Global.CodVendaMagazine then begin
          ...
          if not Qbusca.eof then
             EdNUmerodoc.INvalid('Numero de nota '+Global.CodVendaMagazine+' encontrado.   Digitação não permitida');
       end;
     end;
     QBusca.close;
  end;
...
// 12.08.2022 - para alteracao de notas de saida para fornecedor q não tem o campo do representante ficando zerado...
...
// 31.07.19
    ...
    Avisoerro('Não permitido alterar nota com pendencia financeira já baixada');
  end;
  QPendencia.close;
...
//    if confirma('Buscar preço de venda do cadastro ? ') then
//      faz:=true
//    else
//      faz:=false;
//  end;
...
//    EdDtmovimento.invalid('Campo obrigatório para nota fiscal tipo '+EdComv_codigo.ResultFind.fieldbyname('comv_tipomovto').asstring)
...
// 08.12.16
    ...
    if (not Global.Usuario.OutrosAcessos[0319]) and (EdDtmovimento.isempty) then
       EdDtmovimento.Invalid('Usuário sem permissão para este tipo de nota')
    else if not Global.Topicos[1312] then
      bIncluiritemClick(FNotaSaida);
  end;
...
//                                ' and mova_tipo_codigo='+EdCliente.assql+
       ...
       if QRoma.eof then begin
           EdPedido.invalid('Saida (valores) não encontrada ou com nota de venda já feita');
       end else begin
...
// 28.09.12 - para ver se 'não confunde' com entradas de abate com mesmo numero
                                ' and mova_tipomov='+stringtosql(TipoSaidaAbate)+
//                                ' and mova_situacao=''N'''+
                                ' and mova_situacao=''P'''+
//                                ' and mova_tipo_codigo='+EdCliente.assql+
...
// 01.02.13
            ...
            if QRoma.eof then
              EdPedido.invalid('Saida não encontrada OU com nota de venda já feita OU mais de 30 dias')
            else begin
              Grid.clear;
              Lista:=TList.create;
...
/////////////////
      if (QPed.eof) and ( not Global.Topicos[1429] )  then
        ...
        EdPedido.invalid('Pedido não encontrado OU já baixado OU não autorizado produzir/faturar')
...
// 24.11.14
        if not Global.Usuario.OutrosAcessos[0335] then begin
           ...
           if QPed.fieldbyname('mped_nfvenda').AsInteger>0 then begin
             EdPedido.Invalid('Pedido já faturado na nf '+inttostr(QPed.fieldbyname('mped_nfvenda').AsInteger));
             exit;
           end;
          ...
          if ( ( QPed.FieldByName('mped_formaped').asstring='C' ) or
             ...
             (  AnsiPos(EdNatf_codigo.Text,'5102/6102')>0 ) then begin
              if QPed.FieldByName('mped_formaped').asstring='C' then
                ...
                EdPedido.invalid('Pedido referente BONIFICAÇÃO');
            exit;
          end;
          EdCliente.Enabled:=false;
          EdCliente.Valid;
...
//          EdFpgt_codigo.valid;
  ...
  //    EdPedido.invalid('Pedido ainda não liberado pelo financeiro para envio via PAC');
...
//                  ' and moes_status=''N'' and moes_tipomov='+stringtosql(Global.CodContrato) );
    ...
    if not q.Eof then begin
      EdPedido.invalid('Obra já encontrada na transação '+Q.fieldbyname('moes_transacao').asstring+'.  Cancelar primeiro');
      exit;
    end;
//    localexterno:=FGeral.Getconfig1asstring('localpea');
// 17.07.09
    ...
    valorcontrato:=0;
    if trim(localexterno)='' then begin
      EdPedido.invalid('Falta configurar o local do PEA na configuração geral do sistema');
      exit;
    end else begin
      dbforcam.FileName:=localexterno+'OBITENS.DBF';
//      dbforcam.TableName:=localexterno+'OBITENS.DBF';
...
begin
   ...
   if EdUNitario.isempty then
     ...
     EdUnitario.invalid('Preço de venda não pode ser zerado')
// 22.02.2011 - Abra
...
// 25.03.10
    ...
    begin
      ...
      if not ConstaListaProdutosAcabados(SetEdESTO_DESCRICAO.Items,codigo) then begin
        Avisoerro('Produto '+codigo+' '+FEstoque.GetDescricao(codigo)+' não produzido');
        result:=false;
      end;
    end;
...
// 28.10.2021
     ...
     if EdDevolucoesdm.isempty then
       EdDevolucoesdm.text:='999999';  // só pra dar eof nas devolucoes
...
begin
// 23.01.07 - somente até começar a fazer nota pelo sistema
  EdTotalNota.setvalue(EdTotalProdutos.ascurrency+EdValoripi.ascurrency);
...
begin
  Ed.Items.Clear;
  Ed.Items.Add('Itajaí');
  Ed.Items.Add('São Francisco do Sul');
  Ed.Items.Add('Paranaguá');
end;
...
//    formapag:='Boleto bancário entregue na assinatura do contrato';
...
//                FGEral.Formatavalor(valoritem,f_cr) );
      Produtos.Add(strspace(FEstoque.GetDescricao(QNota.fieldbyname('move_esto_codigo').asstring),45)+
                   ' em área de '+FGEral.Formatavalor(QNota.fieldbyname('move_qtde').asfloat,'###,##0.00')+' m2;' );
...
//////////////////
//        s.footnotes.add(msword.page,emptyparam,'Abra Alumínios Ltda - CNPJ 7610083/0001-80');
...
//        msword.ActiveDocument.Sections.Item(1).Footers.Item(1).Range.Text := 'Abra Alumínios Ltda - CNPJ 7610083/0001-80 - IE: 9035156787'+#13+
...
//        s.footnotes.add(msword.selection.range,emptyparam,'Abra Alumínios Ltda - CNPJ 7610083/0001-80');
//        s.footnotes.add(msword.selection.range,emptyparam,'Rob. BR 158. km 340');
        ...
        s.typetext('Quando a responsabilidade social é a obrigação que a empresa assume com a');
        s.typeparagraph;
//        s.typetext('"sociedade", fundamental  é  conhecer  a ABRA ALUMÍNIOS -  empresa  com');
        s.typetext('"sociedade", fundamental  é  conhecer  a ');
        s.Font.Bold := True;    //negrito
        s.typetext('ABRA ALUMÍNIOS');
        s.Font.Bold := False;    //negrito
        s.typetext(' - empresa com');
        s.typeparagraph;
        s.typetext('expressiva experiência, voltada às necessidades do cliente, com  profissionais');
        s.typeparagraph;
        s.typetext('especializados para assistir nas áreas de excelência de assessoria: ');
        s.typeparagraph;
        s.Font.Bold := True;    //negrito
        s.typetext('arquitetônica, técnica, comercial e  gerenciamento  de  obras.   Possui');
        s.typeparagraph;
        s.typetext('também  área  fabril  com  ferramentais e  maquinários  de  alto  nível');
        s.typeparagraph;
        s.typetext('tecnológico em sua maioria de procedência  italiana, que  garantem  a');
        s.typeparagraph;
        s.typetext('linha de produção, otimização e precisão dos produtos. Equipe técnica');
        s.typeparagraph;
        s.typetext('de produção e instalação capacitada de forma teórica e prática apta  a');
        s.typeparagraph;
        s.typetext('garantir excelência ao processo.');
        s.typeparagraph;
        s.typeparagraph;
        s.typeparagraph;
        s.Font.Bold := True;    //negrito
        s.typetext('A ABRA ALUMÍNIOS');
        s.Font.Bold := False;    //negrito
        s.typetext(' atua desde 1981,tendo na execução de seus serviços a');
        s.typeparagraph;
        s.typetext('eqüidade e o compromisso de atendimento a satisfação dos clientes,mediante');
        s.typeparagraph;
        s.typetext('um trabalho coordenado de  respeito e  confiança  entre  clientes,  dirigentes,');
        s.typeparagraph;
        s.typetext('funcionários, fornecedores e colaboradores.');
        s.typeparagraph;
        s.typeparagraph;
        s.typeparagraph;
        s.Font.Bold := True;    //negrito
        s.typetext('NOSSA MISSÃO:');
        s.Font.Bold := False;    //negrito
        s.typetext('Desenvolver e produzir soluções em alumínio e vidro para o');
        s.typeparagraph;
        s.typetext('segmento  da  construção   civil,  com   inovação  tecnológica  e  qualificação');
        s.typeparagraph;
        s.typetext('profissional, agregando valores às edificações; visando a lucratividade através');
        s.typeparagraph;
        s.typetext('da satisfação dos clientes, colaboradores, fornecedores, sócios e comunidade,');
        s.typeparagraph;
        s.typetext('fomentando a melhoria  da  qualidade de  vida  com  responsabilidade  sócio-');
        s.typeparagraph;
        ...
        s.typeparagraph;
///////////////////
        s.typetext('É com esse comprometimento que a empresa ABRA ALUMÍNIOS  apresenta  a');
        s.typeparagraph;
        s.typetext('proposta dos produtos e serviços, que  assegurem   a  qualidade  do  produto');
        s.typeparagraph;
...
//        s.footnotes.add(msword.selection.range,emptyparam,'Rob. BR 158. km 340');
        ...
        s.typetext('A ABRA');
        s.Font.Bold := False;    //negrito
        s.typetext(' - Departamento técnico/comercial agradece a oportunidade de oferecer');
        s.typeparagraph;
        s.typetext('seus produtos e serviços e, após análise técnica do que lhe foi solicitado, dispõe');
        s.typeparagraph;
        ...
        s.typeparagraph;
        s.typetext('anodização ou pintura eletrostática a pó;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('2) Perfis extrudados com espessuras que garantam um  comportamento estrutural');
        s.typeparagraph;
        s.typetext('adequado e precisão dimensional;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('3) Perfis extrudados, originais ALCOA;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('4) Tratamento de superfície do aluminio em conformidade com as normas vigentes');
        s.typeparagraph;
        s.typetext('da ABNT e ISO, garantindo vida útil prolongada, sem a perda de suas  característi-');
        s.typeparagraph;
        s.typetext('cas;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('5) Componentes de fornecedores devidamente homologados às linhas ALCOA;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('6) Acessórios de  fornecedores  homologados  às  linhas  ALCOA,  com  resistência,');
        s.typeparagraph;
        s.typetext('ergonomia e design;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('7) Guarnições e gaxetas em borracha de EPDM, produto em conformidade com  as');
        s.typeparagraph;
        s.typetext('normas vigentes da ABNT E ISO,  garantindo  vedação  ao  ar  e  a  água, fixação,');
        s.typeparagraph;
        s.typetext('dilatação, sustentação e absorção as tensões mecânicas e  ruídos  evitando  defor-');
        s.typeparagraph;
        s.typetext('mações, ressecamento e retração do produto aplicado;');
        s.typeparagraph;
        s.typeparagraph;
        s.typeparagraph;
        s.typeparagraph;
        ...
        s.typeparagraph;
        s.typetext('produtos alto desempenho tecnológico e garantia de permanência  de seu  portfólio,');
        s.typeparagraph;
        s.typetext('para eventuais reposições;');
        s.typeparagraph;
        s.typeparagraph;
        s.Font.Bold := True;    //negrito
        ...
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('1) Caixilhos, sistemas de fachada, gradis, portões e grades, são  produtos  originais');
        s.typeparagraph;
        s.typetext('do portfólio ALCOA, sendo estas linhas protegidas por patentes;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('2) As linhas aplicadas com suas tipologias, dimensões e composições de montagem,');
        s.typeparagraph;
        s.typetext('foram definidas através de softwares de planejamento e cálculo estrutural de  caixi-');
        s.typeparagraph;
        s.typetext('lhos e gradis,sendo os cálculos de conformidade com as NBR 10.821 e NBR 14.718;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('3) Produtos em conformidade com a NBR 10.821, a qual regulamenta as  condições');
        s.typeparagraph;
        s.typetext('mínimas exigíveis, de desempenho estrutural, estanqueidade a água e  vedação  ao ar;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('4) Caixilhos de contramarco fornecidos com perfis resistentes ao manuseio e chum-');
        s.typeparagraph;
        s.typetext('bamento, montagens à 45º fechados com conexões de aluminio e grapas de fixação');
        s.typeparagraph;
        s.typetext('em aço galvanizado; garantindo com isso a possibilidade de excelência na aplicação');
        s.typeparagraph;
        s.typetext('do produto, quanto ao nível, prumo, esquadrejamento e estanqueidade a água;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('5) Produtos e soluções em alumínio e vidro, confeccionados através de maquinários');
        s.typeparagraph;
        s.typetext('específicos a utilização, com precisão de corte,  usinagem  e  montagem,  a  fim  de');
        s.typeparagraph;
        s.typetext('garantir qualidade ao produto final;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('6) Produtos de baixa manutenção e elevada vida útil;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('7) Alto nível  de  desempenho  energético,  atenuação  acústica  e  térmica,  em  se');
        s.typeparagraph;
        s.typetext('projetado e aplicado como solução de caixilho + vidro;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('8) Os  produtos  especificados  detêm  inovações  tecnológicas  e  design  inovador,');
        s.typeparagraph;
        s.typetext('proporcionando ergonomia, conforto, sofisticação e modernidade;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('9) Os  produtos   proporcionam   bem  estar  ao  ambiente,   agregando   valor   às');
        s.typeparagraph;
        s.typetext('edificações;');
        s.typeparagraph;
        s.typeparagraph;
        s.Font.Bold := True;    //negrito
        ...
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('1) O departamento técnico fornecerá  relatório  detalhado de todos  os  produtos,  ao');
        s.typeparagraph;
        s.typetext('engenheiro executor, para projetos e obras especiais,também será fornecido  projeto');
        s.typeparagraph;
        s.typetext('executivo, dos produtos especiais;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('2) O   departamento   de   Gerenciamento   de   Produção   e   Obras  (GPO)  fará  o');
        s.typeparagraph;
        s.typetext('acompanhamento   de   todas   as   fases  que   se  corelacionarem  à  aplicação  dos');
        s.typeparagraph;
        s.typetext('produtos;');
        s.typeparagraph;
        s.typeparagraph;
        ...
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('3) Os caixilhos de contramarco serão fornecidos, em fase  de  chapisco,  emboço  ou');
        s.typeparagraph;
        s.typetext('reboco. Sendo a instalação dos mesmos  de  responsabilidade  do  contratante;  com');
        s.typeparagraph;
        s.typetext('prévia explicação verbal e escrita ao mestre de obra;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('4) Os caixilhos das esquadrias ou outros produtos serão instalados, após a aplicação');
        s.typeparagraph;
        s.typetext('de: cal fino,  massa  corrida,  massa  acrílica,  gesso,  selador  externo,  textura  ou');
        s.typeparagraph;
        s.typetext('grafiato;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('4.1) Nesta etapa não  deve  existir  trânsito  de  materiais  pesados  na  obra,  como');
        s.typeparagraph;
        s.typetext('carriolas, caixas de argamassa, placas de granito ou pisos do tipo cerâmico, a fim de');
        s.typeparagraph;
        s.typetext('garantir as características do produto acabado.');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('5) Caso o contratante não acorde com o cronograma de instalação, descrito no  item');
        s.typeparagraph;
        s.typetext('4, fica à  possibilidade  do  fornecimento  antecipado  dos  produtos,  com  o  devido');
        s.typeparagraph;
        s.typetext('reconhecimento do termo de responsabilidade dos produtos, onde se  caracterizando');
        s.typeparagraph;
        s.typetext('mau uso do  produto, através  de  sujeiras  provenientes  de  argamassa  ou  gesso,');
        s.typeparagraph;
        s.typetext('amassamento, riscos, componentes e acessórios quebrados, o  contratante  perderá');
        s.typeparagraph;
        s.typetext('as garantias do produto e acarretará com os custos de manutenção;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('6) Os produtos serão fabricados em nosso parque fabril, onde  ficarão  devidamente');
        s.typeparagraph;
        s.typetext('estocados, aguardando cronograma de instalação;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('7) Após  entrega  dos  caixilhos  de  contramarco,  com  a  devida  conferência  pelo');
        s.typeparagraph;
        s.typetext('responsável da obra, consideraremos estas como  medidas  oficiais  para fabricação');
        s.typeparagraph;
        s.typetext('dos caixilhos das esquadrias; caso alguma alteração venha  a  ser feita  posterior  a');
        s.typeparagraph;
        s.typetext('esta etapa, ficam estas alterações sob  inteira  responsabilidade  do  contratante,  e');
        s.typeparagraph;
        s.typetext('suscetíveis de aditivos aos valores do contrato;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('8)Após as etapas de pintura interna e externa da obra, será efetuada a conclusão de');
        s.typeparagraph;
        s.typetext('instalação e entrega dos produtos, com aplicação de remates (quando contratados),');
        s.typeparagraph;
        s.typetext('vedações com selantes de silicone e  polioretano;  também  a  regulagem  final  das');
        s.typeparagraph;
        s.typetext('esquadrias, acessórios e automações (quando contratados);');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('9) Todo processo de fabricação e instalação serão efetuados por equipes próprias de');
        s.typeparagraph;
        s.typetext('trabalhos, com qualificação profissional e conhecimento tecnológico;');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('10) A vistoria e entrega definitiva dos  produtos  contratados  serão  efetuados  com');
        s.typeparagraph;
        s.typetext('acompanhamento do contratante ou responsável legal  pela  obra,  com  entrega  do');
        s.typeparagraph;
        s.typetext('certificado de garantia e manual de uso limpeza e conservação das esquadrias;');
        s.typeparagraph;
        s.typeparagraph;
        s.Font.Bold := True;    //negrito
        s.typetext('IV - APRESENTAÇÃO DOS PRODUTOS:');
        s.Font.Bold := False;    //negrito
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('Consolidação da proposta:'+strspace(cobra,12)+', com descrições individuais em anexo:');

//        s.typetext('Contratado  : '+strspace('ABRA Alumínios Ltda.',50) );
//        s.Font.Bold := False;    //negrito
...
//        s.moveright;  //move para direita
        ...
        s.typeparagraph;
        s.Font.Bold := True;    //negrito
        s.typetext('V - CONDIÇÕES GERAIS:');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('1)Condição e Forma de Pagamento:');
        s.Font.Bold := False;    //negrito
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('   '+formapag);
        ...
        s.typeparagraph;
        s.Font.Bold := True;    //negrito
        s.typetext('1.1)Tributação:');
        s.Font.Bold := False;    //negrito
        s.typeparagraph;
        s.typetext('A) Para  a  formação  do  preço   de   venda    desta    proposta,    considerou-se   a');
        s.typeparagraph;
        s.typetext('emissão direta das Notas Fiscais dos insumos, em nome do contratante,  cabendo  a');
        s.typeparagraph;
        s.typetext('empresa contratada somente os valores referentes a prestação de serviço.');
        s.typeparagraph;
        s.typetext('B) Para a formação do preço de venda desta proposta, considerou-se como natureza');
        s.typeparagraph;
        s.typetext('da operação "venda de mercadoria".');
        s.typeparagraph;
        s.Font.Bold := True;    //negrito
        s.typetext('2) Composição do valor do produto:');
        s.Font.Bold := False;    //negrito
        s.typeparagraph;
        s.typetext('Está prevista no  cálculo dos  caixilhos  a  fabricação  total  dos  produtos,  a  fim  de');
        s.typeparagraph;
        s.typetext('otimizar o quantitativo  dos  insumos  e  os  custos  operacionais.  Portanto  qualquer');
        s.typeparagraph;
        s.typetext('desmembramento ou  alteração  nas  descrições  individuais  desta   proposta   ficará');
        s.typeparagraph;
        s.typetext('sujeita a aplicação de aditivos aos valores do contrato.');
        s.typeparagraph;
        s.typeparagraph;
        s.typeparagraph;
        s.typeparagraph;
        ...
        s.Font.Bold := False;    //negrito
        s.typeparagraph;
        s.typetext('Conforme cronograma da  obra,  considerando o  descrito  no  item  4  do  subtítulo:');
        s.typeparagraph;
        ...
        s.Font.Bold := False;    //negrito
        s.typeparagraph;
        s.typetext('Dez (10) dias, seguidos à data da proposta.');
        s.typeparagraph;
        s.Font.Bold := True;    //negrito
        s.typetext('5) Frete:');
        s.Font.Bold := False;    //negrito
        ...
        s.typeparagraph;
        s.typetext('B) FOB (por conta do contratado), frota própria.');
        s.typeparagraph;
        s.Font.Bold := True;    //negrito
        s.typetext('6) Garantia:');
        s.Font.Bold := False;    //negrito
        s.typeparagraph;
        s.typetext('Contido em nosso Termo de garantia e Manual de uso, limpeza e conservação.');
        s.typeparagraph;
        s.Font.Bold := True;    //negrito
        s.typetext('7) Não incluso a esta proposta:');
        s.Font.Bold := False;    //negrito
        s.typeparagraph;
        s.typetext('A) limpeza e correção de imperfeições nos contramarcos;');
        s.typeparagraph;
        s.typetext('B) Instalação e requadro da alvenaria nos contramarcos;');
        s.typeparagraph;
        ...
        s.typeparagraph;
        s.Font.Bold := True;    //negrito
        s.typetext('VI - Formalização da Proposta:');
        s.Font.Bold := False;    //negrito
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('Havendo a aceitação dos produtos e serviços descritos neste  documento  em  forma');
        s.typeparagraph;
        s.typetext('de Proposta, deverá ser assinado  no  campo  "De acordo"  e  rubricado  nas  demais');
        s.typeparagraph;
        s.typetext('páginas, pelos representantes legais da empresa ou, em se tratando de pessoa física');
        s.typeparagraph;
        s.typetext('pelo próprio individuo a ser emitida a Nota Fiscal; e uma das  vias  retorna  a  Abra');
        s.typeparagraph;
        s.typetext('Alumínios, a qual oficializará  nossa  relação  de  trabalho  constituindo-se,  assim,');
        s.typeparagraph;
        s.typetext('documento formal para todos os efeitos legais.');
        s.typeparagraph;
        s.typeparagraph;
        s.typetext('Ficamos a disposição para eventuais esclarecimentos e negociações. Para  dar  inicio');
        s.typeparagraph;
...
//        msword.Quit  // finaliza aplicaçãoend;
    FGeral.FechaQuery(QNota);
...
// 09.06.14 - Granzoto - para ter um 'cfop' base para buscar o cfop correto na tabela de CST
//   EdNatf_codigo.text:=Arq.TConfMovimento.fieldbyname('comv_natf_estado').asstring;
// 15.02.2023 - Guiber - retirado pois já vem certo do pedido...vamos ver...
////////////////
end;
...
begin
  if EdQtdeNf.ascurrency>Texttovalor(Grid.cells[grid.getcolumn('move_qtde'),grid.row]) then
    EdQtdenf.invalid('Quantidade inválida');
...
// 10.06.09
    if EdComv_codigo.ResultFind.fieldbyname('comv_tipomovto').asstring=Global.CodRemessaIndPropria then begin
        EdMensagem.text:='Remessa Ref. NF(s) '+trim(EdVendasmc.text)+' p/ industrialização '+EdMensagem.text;
// 24.08.12 - Abra escolhendo 'trocentas notas'...
        EdMensagem.text:=copy(EdMensagem.text,1,EdMensagem.MaxLength);
        EdMens_codigo.text:='';  // para que quando passar pelo codigo da mensagem não 'recolocar' a mensagem 'normal'
    end;
...
//  if ( Global.Topicos[1326] ) and ( EdCertificado.text='S' ) then begin
  if ( Global.Topicos[1326] ) and ( not EdCertificado.isempty ) and (  EdCertificado.text<>'N' ) then begin
    ...
    if (credito<EdQtde.AsCurrency) and ( not Global.Usuario.OutrosAcessos[0316] ) then
      EdCertificado.invalid('Disponível somente '+floattostr(credito)+' metros cúbicos de madeira certificada');
  end;
...
//    EdUNitarionf.invalid('Unitário inválido');
    ...
    if trim(produto)<>'' then begin
      ...
      if not QTrib.eof then begin
        ...
        if not Achou then begin
          QTrib.close;
          result:=false;
          Avisoerro('Unidade '+xunid_codigo+' não configurada para este ncm '+ncm);
          break;
        end;
        ...
        QTrib.close;
        result:=false;
        Avisoerro('NCM '+ncm+' não encontrado');
        break;
      end;
      QTrib.close;
      if aliicms<>aliicmsncm then begin
        Avisoerro('Produto '+produto+' com icms '+currtostr(aliicms)+' correto é icms '+currtostr(aliicmsncm)) ;
        result:=false;
        break;
      end;
      if cst<>cstncm then begin
        Avisoerro('Produto '+produto+' com cst '+cst+' correto é cst '+cstncm) ;
        result:=false;
        break;
      end;
      if cfop<>cfopncm then begin
        Avisoerro('Produto '+produto+' com cfop '+cfop+' correto é cfop '+cfopncm) ;
        result:=false;
        break;
      end;
    end;
...
// peso bruto deixa o q foi digitado pois nao tem a informaçao nos itens da nota
// e nem em outra tag...
...
// caso quiser lançar manual as notas....
