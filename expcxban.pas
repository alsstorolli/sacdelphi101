...
// 25.06.18 - Vida Nova - Lançamentos ref. baixa cobrança bancária 'nunca nem vi'..
          ( AnsiPos( Q.FieldByName('movf_tipomov').AsString,Global.CodJurosRecebidos+';'+Global.CodPendenciaFinanceira) = 0)
          then begin

            result:=false;
    // 28.11.05 exceção devido as tarifas bancarias lançadas mas sem conta de receita/despesas
...
// 09.08.16 - ajustes devido a baixa de cobrança bancaria
/////////////////////////////////////////////////////////
          ...
          if pos( 'Tarifa ban',FHistoricos.GetDescricao(Q.FieldByName('movf_hist_codigo').AsInteger)+' '+Q.FieldByName('movf_complemento').AsString ) >0 then
            ...
            result:=true
// para prever juros / descontos da baixa de cobrança
...
/////////////////////////////////////////////////////////////////
  // 04.04.05 - retirado esta checagem - NUNCA MAIS retirar pois dobra os lançamentos
...
//          showmessage('Não encontrado como transacao  de baixa '+transacaobaixa);
...
//          if Global.Usuario.Codigo=100 then
//            showmessage('Não encontrado como transacao normal '+transacaobaixa);
         result:=0;
        end;
      end;
    end;
...
//     Aviso('Atenção !  Último período foi de '+dbfexp.FieldByName('dataini').Asstring+' a '+dbfexp.FieldByName('datafinm').asstring );
...
// 28.11.16 - Vida Nova  - baixa varios clientes de uma vez
  ...
  texto.clear;
{
  Sistema.beginprocess('Eliminando lançamentos da última exportação.  Arquuivo '+nomepath+nomedbf);
  ...
  dbfexp.open;
  dbfexp.First;
  Sistema.beginprocess('Percorrendo lançamentos');
...
// ver onde colocar esta configuração da empresa no viasoft
...
//                   credito:=FCadcli.GetContaExp(codforne)
                   if EdMesano.isempty then begin   // 17.03.08 - vanessa - novicarnes - checa se é baixa de pendecia de venda a associado
...
//////////////////
// 25.11.09 - ajustes para fazer multiplo com um lançamento no caixa/banco e diversos em
//          cada cliente ou fornecedor
//////////////////
            if (debito=0) or (credito=0) then
//               Texto.lines.add(' transacao '+Q.fieldbyname('movf_transacao').asstring+' dia '+Q.fieldbyname('movf_datamvto').asstring+' débito '+inttostr(debito)+' crédito '+inttostr(credito));
               Texto.lines.add('Ent.op. '+Q.fieldbyname('movf_operacao').asstring+' dia '+
                            Q.fieldbyname('movf_datamvto').asstring+' débito '+inttostr(debito)+
                            ' crédito '+inttostr(credito)+' conta '+Q.fieldbyname('movf_plan_conta').asstring+
                            ...
                            ' histórico '+copy(Q.fieldbyname('movf_complemento').asstring,1,30));
...
// 02.11.07 - transferencias  - quando primeiro é 'encontrado' a saida e nao a entrada...
...
//                   end else
                     ...
                     debito:=0;
// 01.09.09 - baixa de lançamentos ref. adiantamento de salarios...'trato vip' - Abra - Ligiane
...
//////////////////
            if ( (debito=0) or (credito=0) ) and ( EdSistema.text<>'04'  ) then
               Texto.lines.add('Sai.op. '+Q.fieldbyname('movf_operacao').asstring+' dia '+
                Q.fieldbyname('movf_datamvto').asstring+' débito '+inttostr(debito)+' crédito '+inttostr(credito)+
        ...
        if ( (debito>0) and (credito>0) ) or ( (cdebito<>'') and (ccredito<>'') ) then begin
          if ( (debito<=99999) and (credito<=99999) and (Debito<>credito)  and (EdSistema.text='02') ) or
             ...
             if Q.fieldbyname('movf_datamvto').Asdatetime>1 then begin
              ...
              Texto.lines.add(' operação '+Q.fieldbyname('movf_operacao').asstring+' com problemas na data.  Lançado na data final');
...
//        end ; // (debito>0) and (credito>0)  // 24.05.09
            ...
            Texto.lines.add('D:'+inttostr(debito)+' C:'+inttostr(credito)+' doc. '+Q.fieldbyname('movf_numerodcto').Asstring+
               ' '+Q.fieldbyname('movf_tipocad').AsString+':'+Q.fieldbyname('movf_tipo_codigo').Asstring+' '+copy(FGeral.GetNomeRazaoSocialEntidade(Q.fieldbyname('movf_tipo_codigo').AsInteger,Q.fieldbyname('movf_tipocad').AsString,'R'),1,30)+
              ' sem conta' );
// 25.11.09 - lançamento ref. baixa multipla diversos clientes OU fornecedores
///////////////////////////////////////////////////////////////////
...
// 21.03.18 - Novicarnes
  if EdSistema.Text='03' then begin
    ...
    if (not ChecaValores) and ( Global.Usuario.Codigo<>100 ) then begin
      Avisoerro('Exportação cancelada');
//      DeleteFile( EdPasta.text+'\'+nomearqtexto+EdUnid_codigo.text+'.txt')
  ...
  Sistema.endprocess('Exportados '+inttostr(n)+' lançamentos  Valor '+formatfloat(f_cr,valortotal));
...
begin
   ...
   Ftextrel.AddTitulo('Relatório de Erros na Exportação Contábil do Caixa Bancos',false,false,true);
...
begin
    ...
    if EdSistema.text<>'03' then begin
      Avisoerro('Relatório disponível somente para exportação para o Contax');
      exit;
    end;
    ...
    nomearqtexto:='CTBCX'+FormatDatetime('mmyyyy',EdTermino.Asdate);
    Sistema.beginprocess('Conferindo totais de débito e crédito');
    try
    ...
    Sistema.endprocess('');
    lista.free;
    if valorc<>valord then Avisoerro('Atenção.  Verificar lançamentos sem conta.    Transação com diferença de '+FGeral.Formatavalor(abs(valord-valorc),f_cr));
...
// 10.08.16
    ...
    if lista.count>0 then begin
      ...
      FRel.AddCol(250,1,'C','' ,''              ,'Histórico'         ,''         ,'',false);
...
begin
   if (EdSistema.text='03') and (not EdContas.IsEmpty) then
     EdSistema.invalid('Para exportar pro Contax não é permitido escolher contas')
