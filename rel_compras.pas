...
//  FRelCompras.PRel.enableedits;
  ...
  with FRelCompras do begin
    ...
    if tp=1 then
      Caption:='Impressão Pedido de Compra'
    else if tp=2 then
      Caption:='Relatório de Recebimento de Mercadoria'
    else if tp=3 then
      Caption:='Extrato Detalhado de Matéria Prima'
    else if tp=4 then
      Caption:='Extrato Resumido de Matéria Prima'
    else if tp=5 then
      Caption:='Relatório para Contagem Recebimento de Mercadoria';
...
begin
  with FRelCompras do begin
    ...
    Sistema.beginprocess('Pesquisando movimentação de materia prima');
...
//                  ' order by move_unid_codigo,move_esto_codigo,move_tipomov');
   ...
   if Q.eof then begin
    ...
    FRel.AddTit('Extrato de Matéria Prima');
...
//    FRel.AddCol(060,1,'C','' ,''              ,'Cod.Copa'        ,''         ,'',False);
    ...
    FRel.AddCol( 60,3,'N','' ,f_cr            ,'Unitário'       ,''         ,'',False);
...
begin
  ...
  with FRelCompras do begin
    ...
    Sistema.beginprocess('Pesquisando pedidos de compra no período');
    if trim(EdUnid_codigo.text)<>'' then begin
...
//                    sqlunidade+sqltipomovto+sqldatacont+sqlproduto+sqlnumerodoc+sqlcodtipo+
   ...
   if Q.eof then begin
    ...
    FRel.AddTit('Relação de Recebimento de Compras - Previsão de entrega : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
    FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text)+' Fornecedor '+FGeral.GetNomeRazaoSocialEntidade(EdCodtipo.asinteger,Edtipocad.text,'N') );
    titulo:='';
    if not EdNumerodoc.isempty then
    ...
    if trim(titulo)<>'' then
      FRel.AddTit(titulo);
    FRel.AddCol( 35,2,'C','' ,''              ,'Unid'         ,''         ,'',false);
    FRel.AddCol( 60,1,'D','' ,''              ,'Lançam.'  ,''         ,'',false);
//    FRel.AddCol( 60,1,'D','' ,''              ,'Movimento'    ,''         ,'',false);
//    FRel.AddCol( 60,1,'D','' ,''              ,'Emissão'    ,''         ,'',false);
    ...
    FRel.AddCol( 80,1,'N','' ,''              ,'Produto'          ,''         ,'',false);
    FRel.AddCol(130,1,'C','' ,''              ,'Descrição'       ,''         ,'',false);
//    FRel.AddCol(060,1,'C','' ,''              ,'Cod.Tam'         ,''         ,'',False);
...
//    FRel.AddCol(050,1,'C','' ,''              ,'Copa'           ,''         ,'',False);
    ...
    FRel.AddCol( 70,3,'N','',f_cr            ,'Unitário'       ,''         ,'',False);
...
// 27.07.06
    ...
    FRel.AddTit('Extrato de Matéria Prima Resumido');
    FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text)+' Fornecedor '+FGeral.GetNomeRazaoSocialEntidade(EdCodtipo.asinteger,Edtipocad.text,'N') );
    FRel.AddTit('Periodo : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
    FRel.AddCol( 35,2,'C','' ,''              ,'Unid'         ,''         ,'',false);
    FRel.AddCol( 80,1,'N','' ,''              ,'Produto'          ,''         ,'',false);
