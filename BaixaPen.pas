...
// 18.05.20  - Novicarnes - 'as vezes puxava as vezes nao...'
    if ( Global.Topicos[1511] )
              ...
              ( Ansipos('MANUTEN', FPlano.GetDescricao( strtointdef(Grid.Cells[Grid.getcolumn('pend_plan_conta'),Grid.row],0) ) ) >0 )
               or
// 25.01.2021 - Novicarnes - Sandro
              ( Ansipos('IPVA', FPlano.GetDescricao( strtointdef(Grid.Cells[Grid.getcolumn('pend_plan_conta'),Grid.row],0) ) ) > 0 )
               or
              ( Ansipos('COMBUST', FPlano.GetDescricao( strtointdef(Grid.Cells[Grid.getcolumn('pend_plan_conta'),Grid.row],0) ) ) > 0 )
              )
                then  begin

// 12.11.20 - Novicarnes - Sandro - futuramente ver se cria config. para contas 'q não valem'...
        if ( strtointdef(Grid.Cells[Grid.getcolumn('pend_plan_conta'),Grid.row],0) <> 18 )
           and
           ( strtointdef(Grid.Cells[Grid.getcolumn('pend_plan_conta'),Grid.row],0) <> 309 )
           then
...
// 19.08.20 - baixa 'adiantada' ou 'futura' - sandro novicarnes
//    else if not FGeral.ValidaMvto(EdDtBaixa) then
//            EdDtBaixa.Invalid('')
//         else
...
// 06.06.07
      if ( texttovalor(Grid.cells[Grid.getcolumn('pend_plan_conta'),Grid.row])<>0 ) and
         ( texttovalor(Grid.cells[Grid.getcolumn('pend_plan_conta'),Grid.row])<>99999 ) and
// 09.12.20 - rateio de nf de entrada em mais de uma conta de despesa
         ( texttovalor(Grid.cells[Grid.getcolumn('pend_plan_conta'),Grid.row])<>9999 )
        then begin
...
// 13.09.22
  end else if ( (EdVAlor.ascurrency+EdDescontos.ascurrency)<valortitulos ) then begin
...
var QBxparcial:TSqlquery;
    ...
    function GetNumeroPedido( xOp :string  ):string;
    //////////////////////////////////////////////////
    var QPed:TSqlquery;
        xt  :string;
    ...
    begin
       ...
       QPed := Sqltoquery('select pend_transacao from pendencias where pend_operacao='+stringtosql(xOP)+
                          ' and pend_status <> ''C''');
       xt := QPed.fieldbyname('pend_transacao').asstring;
       FGeral.FechaQuery(QPed);
       QPed := Sqltoquery('select moes_pedido from movesto where moes_transacao = '+Stringtosql(xt)+
                          ' and moes_status <> ''C''');
       result := inttostr(QPed.fieldbyname('moes_pedido').asinteger);
       FGeral.FechaQuery(QPed);
...
// 07.05.15
    EdChequerec.text:='';
// 16.10.20  - devereda
    if Global.Topicos[1515] then

       EdPedido.text := GetNumeroPedido( EdOperacao.text  );
...
// 19.12.13 - Metalforte - Rosangela
  ...
  end else if (EdVlrAtualizado.ascurrency>EdVAlor.ascurrency) and (marc>=2)
// 15.02.2022
              and (  EdRecpag.text = 'P' )
  then begin
...
// 13.09.2022 - baixa parcial pelo total com diversos titulos
// gera 'nova pendencia' para pode ficar em aberto pois não pode gerar com status 'P' pois nao
//  ficará nenhum titulo pendente
      if TipoBaixaMultipla='BQ' then begin
            ...
            if EdRecpag.text='P' then
              ...
              Sistema.SetField('Pend_DataMvto',EdDtBaixa.AsDate);
            Sistema.SetField('Pend_DataVcto',EdDtBaixa.AsDate);
            Sistema.SetField('Pend_DataEmissao',EdDtBaixa.AsDate);
            Sistema.SetField('Pend_Plan_Conta',EdPlan_conta.AsInteger);
            Sistema.SetField('Pend_Tipo_Codigo',EdCodtipo.Asinteger);
            Sistema.SetField('Pend_Unid_Codigo',Unidade);
            if EdTipocad.Text='C' then

              Fpgt_codigo:=EdCodTipo.ResultFind.FieldByName('clie_fpgt_codigo').asstring

            else

              Fpgt_codigo:=EdCodTipo.ResultFind.FieldByName('forn_fpgt_codigo').asstring;

            Sistema.SetField('Pend_Fpgt_Codigo',Fpgt_codigo);
            Sistema.SetField('Pend_Port_Codigo',EdPort_codigo.Text);
            Sistema.SetField('Pend_Hist_Codigo',EdHist_codigo.asinteger);
            Sistema.SetField('Pend_Moed_Codigo','');
            Sistema.SetField('Pend_Repr_Codigo',0);
            Sistema.SetField('Pend_TipoCad'    ,EdTipocad.text);
            Sistema.SetField('Pend_Complemento','Saldo Periodo '+FGeral.FormataData(Sistema.Datai)+' a '+FGeral.FormataData(Sistema.Dataf));
            Sistema.SetField('Pend_NumeroDcto',inttostr(EdCodtipo.AsInteger+Datetomes(EdDtBaixa.AsDate)+Datetoano(EdDtbaixa.AsDate,true)));
            Sistema.SetField('Pend_Parcela','1');
            Sistema.SetField('Pend_NParcelas',1);
            Sistema.SetField('Pend_RP',Edrecpag.text);
            Sistema.SetField('Pend_DataCont',EdDtMovimento.asdate);
            Sistema.SetField('Pend_Valor',valortitulos-EdValor.AsCurrency);
            Sistema.SetField('Pend_ValorTitulo',valortitulos-EdValor.AsCurrency);
            Sistema.SetField('Pend_Juros',0);
            Sistema.SetField('Pend_Multa',0);
            Sistema.SetField('Pend_Mora',0);
            Sistema.SetField('Pend_Acrescimos',0);
            ...
            Sistema.SetField('Pend_TransBaixa','');  // no. transacao da baixa
            Sistema.SetField('Pend_ContaBaixa',0);
//            Sistema.SetField('Pend_DataBaixa','');
//            Sistema.SetField('pend_usubaixa',Global.Usuario.codigo);   // 20.06.05
//            Sistema.SetField('pend_observacao',(EdQuempagou.text));
            Sistema.SetField('pend_observacao',TipoBaixaMultipla);
            Sistema.SetField('Pend_Impresso',0);   // numero do impresso relacionado
            Sistema.SetField('Pend_ImprDcto','');  // S ou N se o impresso já foi enviado para impressão
            Sistema.SetField('Pend_LoteCNAB',0);  // no. lote para exportação bancária (CNAB )
            Sistema.Post;
...
//  end else if (EdBaixaparcial.text='S') and (marc>1) then begin  // 11.09.08
  end else if (EdBaixaparcial.text='S') and (marc>1) and (baixouparcial) then begin  // 13.09.2022

     TipoBaixaMultipla:='BP';   // Baixa Multipla Parcial
     BaixaDiversasBP;

// 13.09.2022 - aqui para o caso nao baixar parcial somente no total sem informar em algum titulo
  end else if (EdBaixaparcial.text='S') and (marc>1) and (not baixouparcial) then begin

      TipoBaixaMultipla:='BQ';   // Baixa Multipla Total com Parcial somente no total
...
// 27.10.05
    ...
    if marc>1 then begin
      ...
      if EdBaixaParcial.Text='S' then valortitulo:=EdValor.AsCurrency;
...
//      FImpressao.ImprimeReciboPen(Grid.Cells[Grid.Getcolumn('pend_operacao'),Grid.row]);
      if TipoBaixaMultipla='BQ' then

         FImpressao.ImprimeReciboPen(global.UltimaOperacaoBaixada,TipoBaixaMultipla,valortitulos-EdValor.AsCurrency)
