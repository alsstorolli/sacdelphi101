...
// 26.07.13 - Metalnorte - aqui jamantao só em 14.08.13
    campo:=Sistema.GetDicionario('estoque','esto_fami_descricao');
    if Campo.Tipo<>'' then begin
      Edesto_fami_descricao.TableName:='estoque';
  //    SetEdfami_descricao.TableField:='esto_fami_descricao';
...
begin
  ...
  if retornamsg='S' then
    Result:=Codigo+' Não encontrado no estoque geral'
  else
    Result:='';
...
var Codigo:String;
    ...
    var Q:TSqlQuery;
    begin
      Sistema.beginprocess('Checando dependências');
      ...
      if Result then AvisoErro('Encontrados lançamentos com o produto escolhido no arquivo '+Msg);
      Q.Close;Q.Free;
      Sistema.endprocess('');
    end;
...
// 01.07.05
  if not Achou then Achou:=AchaTabela('Movestoque','Move_Esto_Codigo','Movimentação produtos','Move_Status');
  if not Achou then begin
    if not Confirma('Confirma exclusão do codigo '+codigo+' ?') then exit;
...
begin
//  Grid.Report('CadEstoque','Relação de Produtos','','');
//  Frel.Reportfromsql('select * from estoques','CadEstoque','Relação de Produtos');
{
...
// 05.12.2022 - guiber ajudou a achar esta 'não conformidade'
// 27.01.2023 - guiber(vivi) ajudou novamente a achar esta 'outra não  conformidade'
     if ie = 'I' then begin
...
// 26.03.12 - usado quando o estoque é filtrado por unidade
...
////////////////////////////////////
   ...
   Sistema.SetMessage('Validando as informações');
...
// 09.06.11 - devido aos 'sumiços' de produtos
...
begin
  if ( trim(GridGRade.Cells[GridGrade.Col,0])='' ) or ( trim(GridGRade.Cells[0,GridGrade.Row])='' ) then begin
    Avisoerro('Não permitido a digitação');
    exit;
  end;
  Ativaedit;
end;
...
begin
  if key = #13 then
    ...
    avisoerro('Usar somente números');
    Gridgrade.SetFocus;
  end;
end;
...
begin
  if EdEsto_grad_codigo.asinteger=0 then begin
    Avisoerro('Grade não gravada !!!   Codigo da grade está vazio');
    exit;
  end;
//  if OP<>'I' then begin
//    Executesql('Update Estgrades set esgr_status=''C'' where esgr_status=''N'''+
...
//      core_codigo:=strtoint(Listacoluna[linha-1]);
      ...
      showmessage('erro do q não é integer valido ...');
    end;
...
//    Sistema.SetField('esgr_codigocoluna',EdEsto_grad_codigo.ResultFind.fieldbyname('grad_codigocoluna').AsInteger);
    Sistema.SetField('esgr_codigolinha',linha);   // estes linha e coluna são a posicao no grid da grade
    ...
    Sistema.Post('');
// aqui nao grava codigo de barra  pois será pedido na opção dados grade

    except
      showmessage('erro gravando estgrades');
    end;

// por enquanto não está previsto dusa quantidades para o estoque por grade
{
    try
    Sistema.Insert('Estoqueqtde');
    Sistema.Setfield('esqt_status','N');
...
// 22.01.14 - no ferro o peso já da barra de 6 metros....
...
// 24.02.17
  ...
  EdEsto_categoria.Update;
    ...
    if OP='A' then
      Avisoerro('Atenção !  Item '+edesto_codigo.text+' não encontrado na tabela estoqueqtde.  NÃO GRAVAR !!!');
...
begin
  preco:=0;
// 12.10.05 - unidades 'nao toke' tem q buscar o preço de venda na própria unidade
  unidadesnao:='100;900';
  if pos( Global.CodigoUnidade,unidadesnao )>0 then
    unidadebusca:=Global.codigounidade
// 04.03.13 - Vivan Financeiro - Lindacir - usar preço da matriz ( 001 )
...
// 24.03.14 - Lindacir - para o caso de incluir em outra unidade e alterar o preço de venda antes de transferir
  if Q.eof then begin
    FGeral.FechaQuery(Q);
    Q:=sqltoquery('select esqt_vendavis from EstoqueQtde where esqt_status=''N'' and esqt_esto_codigo='+StringtoSql(Produto)+
...
//               and (Tipo='F')
            ...
            if not Global.topicos[1229] then begin
    // 25.01.06
    // procuara a margem dentro do estado nao usando o estado do cliente quando a venda é fora do estado
...
//  if not Arq.TCodigosFiscais.active then Arq.TCodigosFiscais.open;
  codigoestoque:=trim(codigoestoque);   // um codigo veio '1279 '..com espaço no final..vai saber...
...
// 04.08.20 só se nao for devolucao....
...
// 15.10.2021 - ao inves de usar o subgrupo configurado nas config. gerais usar o q está no produto
//              para não ter q ficar mudando na config. cfe a aliquota do icms da nota sendo devolvida...
//        QSubgrupo:=sqltoquery('select * from subgrupos where sugr_codigo='+TEst.fieldbyname('esto_sugr_codigo').Asstring);
// 18.03.2022
        QSubgrupo:=sqltoquery('select * from subgrupos where sugr_codigo='+IntToStr(subgrupodevcompra));
        if not QSubgrupo.eof then begin
...
/////////////////////////////////////////////////
        ...
        end else begin  // destinatario é cliente
...
///////////////////////
    ...
    Avisoerro('Codigo ['+codigoestoque+'] não encontrado no estoque  ( Aliquota Icms )');
...
// 18.03.2022 - para deixar independente do q tá nas config.
...
// 08.02.18
         if (codsittrib>0) and ( mens='') then
           Avisoerro('Não encontrado sit. tributária '+inttostr(codsittrib)+' codigo '+codigoestoque);  // 22.07.05
      end;
...
// 01.09.05 - retirado em 26.07.18 - em avaliação
    {
...
//       result:='900'
    ...
    if mens='' then
      ...
      Avisoerro('Codigo ['+codigoestoque+'] não encontrado no estoque (Situação Tributária)');
...
// 08.08.11
  if PEdits.Visible then begin
    Avisoerro('Não permitido durante a inclusão/alteração do produto');
    exit;
  end;
  if EdEsto_codigo.IsEmpty  then begin
    Avisoerro('Não permitido com o campo codigo vazio');
    exit;
  end;
//  if EdEsto_grad_codigo.AsInteger>0 then begin
//    EdEsto_grad_codigo.ValidFind;
...
begin
 if EdEsto_qtdeminimo.AsCurrency>EdEsto_qtdemaximo.AsCurrency then
   EdEsto_qtdemaximo.Invalid('Quantidade máxima inválida');
end;
...
// 18.03.2022 - pra ficar independente do q tá nas configuracoes
...
// 27.07.06  - retirado ate implantar a grade q dará o custo por tamanho cor e COPA...
...
begin
   ...
   if atuvenda then begin
     if confirma('Confirma novo preço de venda ? ') then begin
...
//     Edesto_sitt_codestado.invalid('Codigo de situação tributária inválido para unidade '+global.CodigoUnidade);
// 22.02.11
   if not ValidaCodCst(EdEsto_sitt_codestado.asinteger,'S') then
     Edesto_sitt_codestado.invalid('Codigo de situação tributária inválido para nota de saida');
end;
...
//
//    Avisoerro('Usuário sem permissão para atualizar preço de venda');
//    exit;
//    podealterar:='N';
//  end;
...
begin
  ...
  if Ed<>nil then begin
    if pos(',',Ed.text)>0 then begin
       Ed.Invalid('Codigo com caracter inválido');
       result:=false;
...
// 30.08.10 - retirado - 'codigos Bavi'
    ...
    if pos(',',produto)>0 then begin
      Avisoerro('Codigo com caracter inválido');
      result:=false;
    end;
  end;
...
// 10.09.19 -  Para não calcular ipi nas vendas de empresas do simples mesmo
//             q tiver a aliquota no ncm
    ...
    Avisoerro('Codigo '+codigoestoque+' não encontrado no estoque (Alíquota Ipi)');
...
begin
  ...
  if not Input('Digitação de unidade','Unidade para cadastro',unidade,3,false) then exit;
  if unidade=Global.CodigoUnidade then begin
    Avisoerro('Unidade para duplicar não pode ser a atual');
    exit;
  end;
  Q:=sqltoquery('select unid_codigo from unidades where unid_codigo='+stringtosql(unidade));
  if Q.eof then begin
    Avisoerro('Unidade '+unidade+' não encontrada');
    Q.close;
    exit;
  end else begin
    if pos( unidade,Global.Usuario.UnidadesMvto )=0 then begin
      Avisoerro('Usuário sem permissão para acessar a unidade '+unidade);
      Q.close;
      exit;
    end;
    FGeral.Fechaquery(Q);
    Q:=sqltoquery('select esqt_esto_codigo from estoqueqtde where esqt_unid_codigo='+stringtosql(unidade)+
                  ' and esqt_status=''N'' and esqt_esto_codigo='+EdEsto_codigo.assql);
    if not Q.eof then begin
      Avisoerro('Produto '+EdEsto_codigo.text+' já cadastrado na unidade '+unidade);
      Q.close;
      exit;
    end;
    if confirma('Confirma duplicação do item para unidade '+unidade) then begin
...
// 09.07.20 - Guiber com produtos configurado errados na unidade 002
      ...
      try
        sistema.commit;
        Aviso('Duplicação efetuada');
      except
        Avisoerro('Problemas na duplicação');
      end;
    end;
    Q.close;
  end;
...
begin
   if ( EdEsto_vendamin.ascurrency>0 ) and ( EdEsto_vendavis.ascurrency>0 ) then begin
     if EdEsto_vendamin.ascurrency>EdEsto_vendavis.ascurrency then
       EdEsto_vendamin.invalid('Preço de venda mínimo tem que ser menor que preoço a vista');
   end;
end;
...
// 17.10.08
        ...
        if copy(QUsuario.FieldByName('Usua_OutrosAcessos').AsString,301,1)<>'S' then begin
           if copy(QUsuario.FieldByName('Usua_OutrosAcessos').AsString,314,1)<>'S' then
             Avisoerro('Usuário sem permissão para faturamento abaixo do preço mínimo.  Solicitar autorização depois prosseguir!');
           QUsuario:=sqltoquery('select Usua_OutrosAcessos from usuarios where usua_codigo='+inttostr(usuario));
...
//                                 copy(QUsuario.FieldByName('Usua_OutrosAcessos').AsString,302,4000));
               ...
               sistema.Commit;
               FGeral.GravaLog(14,'Produto '+produto+' venda mínima '+floattostr(Q.fieldbyname('esqt_vendamin').ascurrency)+' preço praticado '+floattostr(venda));
             end else begin
               result:=false;
               Avisoerro('Usuário Ainda sem permissão para faturamento abaixo do preço mínimo');
             end;
           end else
             result:=false;
...
// 30.07.08 - 'rastrear' vendas abaixo do minimo 'sem autorizacao'
          FGeral.GravaLog(99,inttostr(usuario)+' com permissão. '+produto+' venda mínima '+floattostr(Q.fieldbyname('esqt_vendamin').ascurrency)+' preço praticado '+floattostr(venda));
          result:=true;
        end;
      end else
...
// 23.09.2021 - Vando - Sicare   - assi nao dá certo pois no produto não tem % pra ST ver como tratar..
         ieST := FUnidades.GetieST(Global.codigounidade);

         if ( aliicms > 0 ) and ( trim(ieST)='' )  then begin

            Avisoerro('Produto '+produto+' com icms = '+currtostr(aliicms)+' %.  Alterar para 0 para pode usá-lo');
            result := false;
...
// 21.09.18 - refeito de outra forma senão pega codigo errado na novicarnes
      if tam=0 then begin
...
begin
  ...
  if not QBusca.Eof then begin
    ...
    Avisoerro('Codigo '+codigoestoque+' não encontrado no estoque (NcmIpi)' );
end;
...
begin
  ...
  if not QBusca.Eof then begin
      if comp<QBusca.FieldByName('esto_compminimo').asfloat then begin
        Avisoerro('Comprimento mínimo para aproveitamento é '+QBusca.FieldByName('esto_compminimo').asstring);
        result:=false;
      end;
  end;
  FGeral.FechaQuery(QBusca);
...
begin
  ...
  if not QBusca.Eof then begin
    ...
    Avisoerro('Codigo '+codigoestoque+' não encontrado no estoque');
  FGeral.FechaQuery(QBusca);
end;
...
begin
  if Global.Topicos[1209] then begin
    Result:='Não encontrado no estoque geral';
    if Trim(Codigo)<>'' then begin
     Q:=sqltoquery('select esto_referencia from estoque where esto_codigo='+stringtosql(codigo));
     if not Q.eof then
       result:=Q.FieldByName('Esto_referencia').AsString;
...
begin
   ...
   if not FileExists(opd.FileName) then begin
     Avisoerro('Arquivo '+opd.FileName+' não encontrado');
     exit;
   end;
...
begin
  ...
  if not QBusca.Eof then begin
    ...
    Avisoerro('Codigo '+codigoestoque+' não encontrado no estoque ( CSTIPI )');
end;
// 18.06.21
...
begin
   ...
   if not Q.eof then begin
    ...
    if campo.Tipo<>'' then begin
     ...
     result:=false;
     Avisoerro('Codigo '+inttostr(codigocst)+' de situação tributária não encontrado');
   end;
   FGERal.FechaQuery(Q);
end;
...
// 22.02.11
   if not ValidaCodCst(EdEsto_sitt_forestado.asinteger,'N') then
     Edesto_sitt_forestado.invalid('Codigo de situação tributária inválido para nota de saida');
...
//           strspace(copy(Q.fieldbyname('esto_descricao').asString,26,25),25)+
           ...
           strzero(0,1)+    // versão do preço/indicador de uso..nao sei o que é...
           ...
           strzero(codconserva,4)+    // codigo do conservação
...
RRRRRR	Código da Inf. Extra do item	(6 bytes)
"000000"  = não haverá associação
FFF	Código da Imagem do Item	(3 Bytes)
"000"  = não haverá associação
IIII	Código da Informação Nutricional	(4 Bytes)
...
CF	Código do Fornecedor	(4 Bytes)
"0000"  = não haverá associação	
L	Lote	(12 Bytes)
G	Reservado	(11 Bytes)
Z	Versão do preço / Indicador de uso	(1 Byte)
R	Bytes Reservados	(2 Bytes)
/////////////////////////////////////campos da versa 2
CS	Código do Som"0000"  = não haverá associação	(4 Bytes)
CT	Código da Tara"0000"  = não haverá associação	(4 Bytes)
FRAC	Código do Fracinador"0000"  = não haverá associação	(4 Bytes)
CE1	Código do Campo Extra 1"0000"  = não haverá associação	(4 Bytes)
CE2	Código do Campo Extra 2"0000"  = não haverá associação	(4 Bytes)
CONS	Código da Conservação"0000"  = não haverá associação	(4 Bytes)
...
//////////////////
{
CCCC	Código da Informação Nutricional	(4 Bytes)
A	Reservado	(1 Byte)
BBB	Quantidade	(3 Bytes)
D	Unidade de Porção	(1 Byte)
 	[0] => Unidade de porção em gramas(g)
 	[1] => Unidade de porção em mililitros(ml)
 	[2] => Unidade de porção em unidades(un)
...
GG	Medida Caseira Utilizada	(2 Bytes)
 	...
 	[01] => Colher(es) de Café	[15] => Copo(s)
[02] => Colher(es) de Chá	[16] => Porção(ões)
[03] => Xícara(s)	[17] => Tablete(s)
[04] => De Xícara(s)	[18] => Sachê(s)
[05] => Unidade(s)	[19] => Almôdega(s)
[06] => Pacote(s)	[20] => Bife(s)
[07] => Fatia(s)	[21] => Filé(s)
[08] => Fatia(s) Fina(s)	[22] => Concha(s)
[09] => Pedaço(s)	[23] => Bala(s)
[10] => Folha(s)	[24] => Prato(s) Fundo(s)
[11] => Pão(es)	[25] => Pitada(s)
[12] => Biscoito(s)	[26] => Lata(s)
 	[13] => Bisnaguinha(s)
HHHH	Valor Calórico	(4 Bytes)
III	Carboidratos	(3 Bytes)
J	Menor que 1 g de Carboidrato	(1 Byte)
 	[0] => Não
 	[1] => Sim
LL	Proteínas	(2 Bytes)
M	Menor que 1 g de Proteína	(1 Byte)
 	[0] => Não
 	[1] => Sim
...
ZZ	Valor Diario das Proteínas	(2 Bytes)
...
##	Valor Diario do Cálcio	(2 Bytes)
**	Valor Diario do Ferro	(2 Bytes)
$$	Valor Diario do Sódio	(2 Bytes)
}
//////////////////
...
// CCCCCCB(100)I(56)I(56)I(56)I(56)I(56)I(56)I(56)I(56)I(56)I(56)I(56)I(56)I(56)I(56)I(56) (+CR +LF)
      if bal='TOLEDO' then begin  // 03 a 5 linhas de 56 bytes cfe a etique escolhida...
        tamlinha:=56;
// talvez usar a config. geral ou o conceito de 'específico'
{
...
//////////////////////////////////////////////////////
  ...
  Q:=Sqltoquery('select * from estoque '+
                ...
                ' order by esto_descricao');
  if Q.eof then begin
    Avisoerro('Nenhum produto configurado com codigo de informação nutricional, ingredientes ou conservação');
    exit;
  end;
...
begin
  ...
  if not TEstoque.eof then begin
    ...
    if not QBusca.Eof then begin
      codsittribestado:=QBusca.FieldByName('esqt_sitt_codestado').asinteger;
// por enquanto trazer somente baseado 'no do estado' pois PIS é federal
//      if UF=Arq.TUnidades.fieldbyname('unid_uf').AsString then
        codsittrib:=QBusca.FieldByName('esqt_sitt_codestado').asinteger;
//      else
//        codsittrib:=QBusca.FieldByName('esqt_sitt_forestado').asinteger;
// 09.07.13
      ...
      if ( trim(sqlcst)<>'' ) then begin
         ...
         if not Tcst.eof then
           ...
           result:='01';
           Avisoerro('Não encontrado sit. tributária PIS (Entrada ) '+inttostr(codsittrib)+' codigo '+codigoestoque);
         end;
         TCst.Close;
...
// 08.07.20
         if mens='' then
           Avisoerro('Não encontrado sit. tributária PIS '+inttostr(codsittrib)+' codigo '+codigoestoque);
...
// 08.07.20
    if mens='' then
       Avisoerro('Codigo ['+codigoestoque+'] não encontrado no estoque (Situação Tributária PIS)');
...
///////////  if not Arq.TSittributaria.active then Arq.TSittributaria.open;
  ...
  if not TEstoque.eof then begin
    ...
    if not QBusca.Eof then begin
      codsittribestado:=QBusca.FieldByName('esqt_sitt_codestado').asinteger;
// por enquanto trazer somente baseado 'no do estado' pois Cofins é federal
//      if UF=Arq.TUnidades.fieldbyname('unid_uf').AsString then
        codsittrib:=QBusca.FieldByName('esqt_sitt_codestado').asinteger;
//      else
//        codsittrib:=QBusca.FieldByName('esqt_sitt_forestado').asinteger;
      ...
      if ( trim(sqlcst)<>'' ) then begin
         ...
         if not Tcst.eof then
           ...
           result:='01';
//           Avisoerro('Não encontrado sit.  tributária COFINS (Entrada ) '+inttostr(codsittrib)+' codigo '+codigoestoque);
         end;
         TCst.Close;
         ...
         result:='01';
//         Avisoerro('Não encontrado sit. tributária COFINS '+inttostr(codsittrib)+' codigo '+codigoestoque);
      end;
    end else begin
// 14.03.16 - prever banco de dados 'detonado' quando nao encontra o produto no estoqueqtde
      TCst:=sqltoquery('select * from sittrib where sitt_codigo='+inttostr(FUnidades.GetSittDentro(unidade)));
...
// 08.07.20
    if mens='' then
      Avisoerro('Codigo ['+codigoestoque+'] não encontrado no estoque (Situação Tributária COFINS)');
   ...
   if not Tcst.eof then
     ...
     IF xes='S' then begin
       ...
       if not Tcst.eof then
         ...
         result:='01';
         if trim(xcst)<>'' then
           Avisoerro('Não encontrado sit. tributária COFINS CST Icms '+xcst+' '+xes);
       end;
     end else begin
       result:='01';
       if trim(xcst)<>'' then
...
//   if trim(result)='' then Avisoerro('sit. tributária COFINS em branco para CST Icms '+xcst+' '+xes);
   ...
   if not Tcst.eof then
     ...
     IF xes='S' then begin
       ...
       if not Tcst.eof then
         ...
         result:='01';
         if trim(xcst)<>'' then
//           Avisoerro('Não encontrado sit. tributária PIS CST Icms ['+xcst+'] '+xes+' '+xtransacao);
       end;
     end else begin
       result:='01';
       if trim(xcst)<>'' then
...
//       if trim(cstpis)<>'' then result:=cstpis;
 //   end;
//   if trim(result)='' then Avisoerro('sit. tributária PIS em branco para CST Icms ['+xcst+'] '+xes);
...
//      Aviso('Não encontrado '+EdPesquisa.Text )
       Arq.TEstoque.Close;
...
begin
  if trim(EdEsto_referencia.text)<>'' then begin
    ...
    if (not Q.Eof) and (Q.Fieldbyname('esto_referencia').AsString<>EdEsto_referencia.Text) then
      EdEsto_referencia.Invalid('Referência pertence ao codigo '+Q.Fieldbyname('esto_codigo').AsString)
    else
     Q.Close;
    Freeandnil(Q);
  end;
...
/////////////////////////////////////////////////////////////////
begin
  if ( Global.Topicos[1228] ) and ( EdEsto_cipi_codigo.isempty ) then EdEsto_cipi_codigo.invalid('Obrigatório informar codigo do NCM');
end;
...
// 27.03.16
  ...
  if not QBusca.Eof then begin
    ...
    Avisoerro('Codigo '+codigoestoque+' não encontrado no estoque (CestNcm)' );
end;
