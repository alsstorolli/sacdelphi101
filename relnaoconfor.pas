...
//                  ' order by paca_unid_codigo,paca_data,paca_numeroata,paca_seq');
    ...
    FRel.AddTit('Relatório de Plano de Ação - Período ref. Previsão: '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
//    FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text) );
    FRel.AddTit('Unidade(s):'+EdUnid_codigo.Text +' - Período ref. Encerramento: '+FGeral.formatadata(Edencerrai.Asdate)+' a '+FGeral.formatadata(Edencerraf.asdate));
    titulo:='';
    if not EdNumerodoc.isempty then
      titulo:='Plano de Ação '+EdNumerodoc.text;
    if EdPendentes.text='S' then
    ...
    FRel.AddCol(100,1,'C','' ,''              ,'Responsável'            ,''         ,'',false);
...
//                  ' order by paca_unid_codigo,paca_data,paca_numeroata,paca_seq');
    ...
    FRel.Init('RelPlanosdeAcao');
    FRel.AddTit('Relatório de Plano de Ação - Período : '+FGeral.formatadata(EdDatai.Asdate)+' a '+FGeral.formatadata(EdDataf.asdate));
//    FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text) );
...
//                     ' and '+FGeral.Getin('move_unid_codigo',EdUnid_codigo.text,'C')+
    ...
    FRel.AddTit('Relatório de Indicadores de Qualidade');
//    FRel.AddTit('Unidade : '+Unidade+' - '+FUnidades.Getnome(Unidade)+FGeral.TituloRelCliRepre(EdCodtipo.asinteger,EdTipocad.text));
    ...
    if not EdUsua_codigo.isempty then
      FRel.AddTit('Responsável : '+EdUsua_codigo.text+' - '+Edusua_nome.text );
