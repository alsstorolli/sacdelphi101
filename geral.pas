...
uses
  ...
  Dialogs,SqlExpr, StdCtrls, Mask, SQLEd, ExtCtrls, CheckDoc,SqlSis,Math,SqlGrid,
  AppEvnts, alabel, Grids, SqlDtg, Barcode, SqlFun, ShellAPI,
  ...
  ACBrIBPTax, Kr_Hint, IdPOP3, ACBrMail, DbxDevartPostgreSQL,
  IdExplicitTLSClientServerBase, IdSMTPBase, ACBrBarCode, ACBrValidador, ACBrDFe,
  PcnConversaoNfe, pcnNFe, ACBrMDFe, ACBrCTe, ComObj, ACBrGNRE2, ACBrDFeReport,
...
//    Excel: TExcelApplication;
  ...
  public                                                     // 04.05.10
    ...
    function GetTransacao(copiaclipboard:string='S'):String;
// 20.06.16
    ...
    procedure GravaMestreConsignacao(Emissao: TDatetime; Cliente:TSqlEd ;
              Representante: integer; Unidade, TipoMovimento, Transacao : string ; Numero: Integer ;
              Valortotal:currency ; Tabela:Integer ; OP:string='I' ; Magazine:string='N';
              xtransacanota:string='';xobs:string=''; xobs1:string=''; xprecor:string='' );
...
//    procedure GeraListaCstPerc(Cst: string; Perc,contabil,base,reducao,isentas,outras,basesubs: currency);
    procedure GeraListaCstPerc(Cst: string; Perc,contabil,base,reducao,isentas,outras,basesubs: currency ;
              ListaCstPerc:TList=nil ; TpImposto:string='I' ; Cfop:string='' ; valoricms:currency=0);
    procedure GravaMestreNFSaida(Emissao,Saida: TDatetime; Cliente: TSqlEd;
              ...
              xvlroutrasdespesas:currency=0 ; xNomeObra:string=''; carga:integer=0 ; RetIR:currency=0 ; RetPIS:currency=0 ;
              RetCofins:currency=0 ; RetCSL:currency=0 ; RetIss:currency=0 ; xtomador:integer=0 ;
              cseto_codigo:string=''; xGrid:TSqlDtGrid=nil );
    ...
    procedure GravaItensNFSaida(Emissao: TDatetime; Cliente:TSqlEd ;
              ...
              rtipocad:string='C' ; xPedidos:string='' ; Saida:TDateTime=0 );
    ...
    procedure GravaMovfin(Transacao,Unidade,EntSai,CompleHist:string ; Emissao,Movimento,Bompara:TDatetime ; Numero,CodHist,Cheque,Conta:integer ; valorparcela:currency ; Contarecdes :integer ;
                          ...
                          transacaocontax:string=''; ContaTrans:integer=0 ; Port_codigo:string='');
    ...
    procedure GravaMestreNFCompra(Emissao,Entrada: TDatetime; Fornecedor: TSqlEd; TipoCad: String ;
              ...
              xvlrINss:currency=0;xvlrpis:currency=0;xvlrcofins:currency=0;xvlrcsll:currency=0;xvlrir:currency=0;xvlriss:currency=0 ;
              xXMLCTE:widestring='' ; xPesobru:currency=0 ; xespecievol:string='' ; xqtdevol:currency=0; xinsumos:currency=0;
              cequi_codigo:string='');
    ...
    procedure GravaItensNFCompra(Emissao,Entrada: TDatetime; Fornecedor:TSqlEd ;
              ...
              ArqXml:string='' ; xtotalicmsst:currency=0; cequi_codigo:string='' ; ListaIguais:TList=nil);
...
// 12.05.06
    procedure PulalinhaRel(colunas:integer ; coluna1:integer=0 ; dados1:string='' ; coluna2:integer=0 ; dados2:string='' ;
              ...
              coluna6:integer=0 ; dados6:string='';coluna7:integer=0 ; dados7:string='';
              coluna8:integer=0 ; dados8:string='');
// 23.05.06
...
// 06.03.2023 - Devereda
    function Checaetraznumfalta(numero: integer; serie,especie,unidade: string;datareferencia: TDatetime): integer;
...
//    procedure EnviaEmail(ListaTitulos,ListaCabecalho1,Listacabecalho2:TStringlist ; nomerel:string);
    ...
    function GetValorJuros(capital:currency ; vencimento,baixa:TDatetime ; RecPag:string; Emissao:TDatetime=0;xTipomov:string='' ):currency;
...
// 26.11.12
    procedure GravaNotacomxml( NotaFiscal: TACBrNFe ;xarquivoxml,xunidade:string;ListaMuni:TList=nil;aviso:boolean=false;comita:boolean=true);
// 03.05.13 - Vivan
...
// 29.10.14 - Giacomoni
    function GetAliquotaFixaIcmsEstado(xUF:string;xcodigo:integer=0;xtipocad:string='C'):currency;
// 18.03.15 - Vivan
    function ValidaCadastro(xsituacao:string):boolean;
// 16.09.15
   procedure EnviaEMailcomAnexo(xemail: string; xListaAnexos:TStringList ; xCorpoEmailTexto: TStrings;xtipo:string='NF-e';xassunto:string='');
// 17.09.15
...
// 15.02.17
   function GetContaContax(pconta:integer):string;
// 02.03.17
   procedure GravaNotaCancelada( xarq,xunidade:string; aviso:boolean );
// 02.06.17
    function SaldoAnteriorRecDes(conta:integer;unidade,campo:string;DataInicial:TDatetime):currency;
// 05.12.17
    procedure GravaMovimentoEstoque(xTransacao,Codigo, UNidade, EntSai,  TipoMovimento: string;xNumero,Cliente:integer;
              Qtde: currency; Q: TSqlquery; Qtdeprev,  Pecas: currency);
// 04.08.18
    procedure ConfiguraCriptografiaAcbrNfe( AcbrNfex:TAcbrNfe; cUF:string='PR' ) ;
    procedure ConfiguraACBrNFe (  AcbrNfex:TAcbrNfe ; qnf:string='NFe' );
// 25.07.20
    procedure ConfiguraCriptografiaAcbrGNRE( AcbrGNREx:TAcbrGNRE; cUF:string='PR' ) ;
    procedure ConfiguraACBrGNRE (  AcbrGNREx:TAcbrGNRE ; xUnidade:string );

// 09.04.19
    procedure ConfiguraRespTecnicoAcbrNfe( xInfRespTec:TInfRespTec ) ;
// 12.04.19
    function Mes( num:integer ) : string;
    procedure ConfiguraCriptografiaAcbrMdfe( AcbrMdfex:TAcbrMdfe ) ;
// 17.06.20
    procedure ConfiguraCriptografiaAcbrCTe( AcbrCTex:TAcbrCte ) ;
// 10.10.19
    function GetSaldoInicialMovCon(Conta:Integer;Unidade:String;Data:TDateTime):Currency;
// 05.11.19 - Novicarnes -relat. IR produtor
    function GetReduzidooutraUnidadeContax(pconta:integer;xunidade:string):integer;
// 04.02.20
    Procedure SetDefaultPrinter(PrinterName: String);
// 03.03.20
    Function LeiLGPDP:boolean;
// 17.12.20
    Procedure ConfiguraMargemNFe( xImp : TACBrNFeDANFeRL );
// 25.03.2021 - Calculo da ST nas compras usando MVA
    function GetAliquotaIcmsEstadoEntradas(xUF:string):currency;
// 11.02.23
    procedure GetTributacaoNCM(QTrib:TSqlquery;xcfop:string;xUnid_codigo:string;var aliicmsncm:currency;var cstncm:string;var cfopncm:string;var achou:boolean);
...
type TGlobal=record
     ...
     NomeSistema,VersaoSistema,UltimoFormAberto,UnidadesNCM     :String;
     Usuario:TUsuario;
     ...
     ContaDescontosobtidos,ContaDescontosTarifaBan,ContaDescontosConcTarifaBan,
     VersaoBanco:Integer;
     ...
     CodDevolucaoTributada,CodFaturaAgua,CodNfeComplementoQtde,CodNfeComplementoIcms,CodNfeComplementoValorProdutor,
     CodNfeComplementoIcmsE,CodNfeComplementoIPI,CodCedulaProdutoRural,CodNfeComplementoIcmsECliente,
     CodNfeComplmentoIcmsEcliente,CodComplementoValorE,CodComplementoValorS :string;
     DataSistema:TDatetime;
     PlanoFiltrado,CodDevolucaoCompraProdutor,Impressora,CodPrestacaoServicosE,CodOrdemdeServico,
     CodManutencaoEquipamento,CodEntradaProdutor,CodCompraProdutorMerenda,
     CodOrcamento,CodDevolucaoTributadaCliente,CodDevolucaoBonificacao,
     CodEntradaNFPe:string;
end;

type TCstPerc=record
     cst,tpimposto,cfop:string;
     perc,contabil,base,reducao,isentas,outras,basesubs,vicms:currency;
end;
...
uses Hist, Usuarios, Init, Menuinicial, Arquiv,
  ...
  expnfetxt, codigosipi, nfcompra, grupos, portador, importanfe, vendabalcao,
  ACBrDFeSSL, Printers, checabalanca, blcksock;
...
// 12.04.05 - conta padrao de Devolução de Venda do plano gerencial
...
{
// 17.08.05
  Global.ContaJurosRecebidos:=13;          100
...
// 10.03.08
// 23.09.05
  Global.TiposRelVenda:=Global.CodVendaConsig+';'+global.CodVendaDireta+';'+global.CodVendaProntaEntrega+';'+
...
// 28.09.05 - para liberar digitação do valor unitario
...
// 28.03.08
  Global.TiposRelCompra:=Global.CodCompra+';'+Global.CodCompraSemMovEstoque+';'+
                ...
                Global.CodEntradaNFPe;

  Global.PlanoFiltrado:='N';
////////////////////////////////
// movMOVestoque e movMOVentrada para ver quando movimenta ou não quantidades do estoque
  ...
  Global.TiposMovMovEstoque:=Global.CodRemessaConsig+';'+Global.CodDevolucaoConsig+';'+Global.CodVendaDireta+';'+
                             ...
                             Global.CodEntradaProdutor+';'+Global.CodCompraProdutorMerenda+';'+Global.CodDevolucaoTributadaCliente+';'+
...
//  Global.TiposMovMovEntrada:=Global.CodDevolucaoConsig+';'+Global.CodCompra+';'+Global.CodAcertoEsEnt+';'+
  Global.TiposMovMovEntrada:=Global.CodDevolucaoConsig+';'+Global.CodAcertoEsEnt+';'+
                             ...
                             Global.CodCompraProdutor+';'+Global.CodDesossaEnt+';'+';'+Global.CodEntradaAlmox+';'+
  ...
  Global.TiposGeraFinanceiro:=Global.CodVendaDireta+';'+Global.CodCompra+';'+Global.CodDevolucaoCompra+';'+
                              ...
                              Global.CodCedulaProdutoRural+';'+Global.CodComplementoValorE+';'+
// 06.08.20 - Novicarnes
                              Global.CodDevolucaoTributada+';'+Global.CodComplementoValorS ;
  ...
  Global.TiposNaoCalcSubsTrib:=global.CodVendaMostruario+';'+global.codretornomostruario+';'+global.CodRetornoConsigMercanil+
                               ...
                               Global.CodRomaneioRemessaaOrdem+';'+Global.CodFaturaAgua+';'+Global.CodCedulaProdutoRural;

//25.08.06 - VENDA de mostruario tem subst...REMESSA não...
// global.CodVendaMostruarioII
  Global.TiposNaoCalcIcms:=Global.CodTransfSaidaTempo+';'+Global.CodTransfEntradaTempo+';'+Global.CodTransfEntRetTempo+';'+
                           ...
                           ';'+Global.CodNfeComplementoValorProdutor+';'+Global.CodNfeComplementoIcmsE+';'+Global.CodNfeComplementoIPI+';'+
                           Global.CodCedulaProdutoRural+';'+Global.CodComplementoValorE+';'+
                           Global.CodEntradaNFPe;
// 18.03.10
  Global.TiposNaoCalcIpi:=Global.CodTransfSaidaTempo+';'+Global.CodTransfEntradaTempo+';'+Global.CodTransfEntRetTempo+';'+
                           ...
                           Global.CodNfeComplementoIcmsE+';'+Global.CodNfeComplementoIPI+';'+Global.CodCedulaProdutoRural+';'+
                           Global.CodNfeComplementoIcmsECliente+';'+Global.CodComplementoValorE;


//  +Global.CodRomaSerie4;

///     CodVendaLiqSerie4 // nao movimenta o estoque so 'acerta' o financeiro
  Global.MargemSubsTrib:=40;  // ate ver onde deixar configurável...
  Global.CodigoSubsTrib:='4';   // ate ver se poe configuraçao
  Global.TiposEntrada:=Global.TiposMovMovEntrada+';'+Global.CodConhecimento+';'+Global.CodDevolucaoSerie5+';'+Global.CodDevolucaoRoman+
                       ...
                       Global.CodNfeComplementoIcmsE+';'+Global.CodNfeComplementoIcmsECliente+';'+
                       Global.CodComplementoValorE+';'+Global.CodCedulaProdutoRural+';'+
                       Global.CodEntradaNFPe;
  ...
  Global.TiposSaida:=Global.CodRemessaConsig+';'+Global.CodVendaDireta+';'+
                     ...
                     Global.CodNfeComplementoIcms+';'+Global.CodNfeComplementoIPI+';'+Global.CodDevolucaoTributadaCliente+';'+
                     Global.CodComplementoValorS;
...
// 08.05.17
//                         ';'+Global.CodPrestacaoServicos+';'+Global.CodCedulaProdutoRural;
// 11.02.19 liberado pra Eticon
                         Global.CodCedulaProdutoRural;
// 08.06.11
...
// 01.06.20 - retirado fornecedores
/////////////////////////
  end;
end;
...
// 09.03.20  - Pequim
     if Global.Usuario.OutrosAcessos[0519] then begin
        ...
        if Global.Contadores.Timer mod 2=0 then FChecaBalanca.execute;
...
// 06.04.17 - devereda
//        FVendaBalcao.EdDtMovimento.setdate(Sistema.Hoje);
     end;
  end;
...
function TFGeral.GetTransacao(copiaclipboard:string='S'):String;
begin
...
// 14.09.17
  if (trim(result)<>'') and (copiaclipboard='S') then
...
begin
  ...
  if not Arq.TImpressos.Locate('Impr_Codigo',CodImpresso,[]) then begin
     AvisoErro('Impresso não cadastrado: '+CodImpresso);
     Exit;
  end;
...
begin
  if sistema.inicializado then begin
    ...
    avisoerro('Sistema ainda não inicializado');
end;


// 03.03.20 - Lei geral de proteção a dados pessoais
function TFGeral.LeiLGPDP: boolean;
/////////////////////////////////////
begin

   if ( Global.Topicos[1116]) then  begin

      if ( Global.Usuario.OutrosAcessos[0066] ) then
         result := false
      else
         result:=true;
...
//  if Edit.Items.Count=0 then begin
    Edit.Items.Clear;
     ...
     Lista.Add('SP - São Paulo');
     Lista.Add('RJ - Rio De Janeiro');
     Lista.Add('ES - Espírito Santo');
     ...
     Lista.Add('CE - Ceará');
     Lista.Add('PI - Piauí');
     Lista.Add('MA - Maranhão');
     Lista.Add('BA - Bahia');
     Lista.Add('PB - Paraíba');
...
begin
  if Categoria='C' then begin
     ...
     EdCodigo.MessageStr:='Código ou CNPJ/CPF do cliente';
     EdCodigo.TagStr:='C';
     EdCodigo.Visible:=True;
     EdCodigo.Width:=95;
     ...
     EdCodigo.MessageStr:='Código ou CNPJ/CPF do fornecedor';
     ...
     EdCodigo.Visible:=True;
     EdCodigo.Title:='Funcionário';
     EdCodigo.ShowForm:='FFuncionarios';
     EdCodigo.MessageStr:='Código ou CPF do funcionário';
     EdCodigo.TagStr:='I';
     EdCodigo.Visible:=True;
     EdCodigo.Width:=95;
     EdCodigo.ValueFormat:='';
     ...
     EdCodigo.MessageStr:='Código da unidade';
     EdCodigo.TagStr:='U';
     EdCodigo.Visible:=True;
     EdCodigo.Width:=30;
...
// 17.12.20
// para ajeitar impressao em pdf e física da nfe
procedure TFGeral.ConfiguraMargemNFe(xImp: TACBrNFeDANFeRL );
/////////////////////////////////////////////////////////////
begin

  xImp.MargemDireita  := 5.1;
  xImp.MargemEsquerda := 6;
  xImp.MargemInferior := 8;
  xImp.MargemSuperior := 8;


end;

// 09.04.19
procedure TFGeral.ConfiguraRespTecnicoAcbrNfe(xInfRespTec:TInfRespTec);
///////////////////////////////////////////////////////////////////
begin
...
begin
  ...
  if Cat='F' then begin
     ...
     if Q.IsEmpty then begin
        EdCodigo.Invalid('Fornecedor não cadastrado');
        Result:=False;
     end else begin
     ...
     if Q.IsEmpty then begin
        EdCodigo.Invalid('Cliente não cadastrado');
        Result:=False;
     end else begin
     ...
     if Q.IsEmpty then begin
        EdCodigo.Invalid('Funcionário não cadastrado');
        Result:=False;
     end else begin
     ...
     if not Found then begin
        EdCodigo.Invalid('Unidade não cadastrada');
        Result:=False;
     end else begin
...
//     if not FGeral.CheckCNPJCPF.Result then begin
     ...
     ACBrValidador1.Documento:=S;
     if not ACBrValidador1.Validar then
        Edit.Invalid('CPF inválido');
     ...
     if (Global.Topicos[1104]) and (Edit.text='00000000000000') then begin   // 24.05.07
       ...
       ACBrValidador1.Documento:=S;
       if not ACBrValidador1.Validar then
          Edit.Invalid('CNPJ inválido');
     end;
  end else begin
     Edit.Invalid('Conteúdo inválido');
  end;
end;
...
begin
  ...
  if t=11 then begin
     ACBrValidador1.TipoDocto:=docCPF;
     ACBrValidador1.Documento:=S;
     if not ACBrValidador1.Validar then begin
     ...
     ACBrValidador1.Documento:=S;
     if not ACBrValidador1.Validar then begin
          Avisoerro('CNPJ inválido');
       end else xret:=true;
     end;
  end else begin
     Avisoerro('Conteúdo inválido');
  end;
  result:=xret;
end;
...
begin
  ...
  if not Result then EdUnidade.Invalid('Unidade de movimento inválida para o usuário do sistema');
end;
...
begin
  ...
  if Global.Usuario.Desenvolvimento then Result:=True;
  if not Result then EdConta.Invalid('Conta caixa inválida para o usuário do sistema');
end;
...
// 26.08.20
  if (EdDAta.asdate>Global.DataSistema) and ( AnsiPos('NOVICARNE',Uppercase(Global.NomeUnidade))=0 )  then begin
...
// 29.03.10 - dias somente para caixa bancos e nota saida
    if ( FGeral.GetConfig1Asinteger('DIASUSOSAC')>0 )  and ( pos('emissao',EdData.Name)>0 ) or
      ...
      if (EdData.asdate<data) and ( not Global.Usuario.OutrosAcessos[0043]) then begin
        EdData.Invalid('Data de movimento fora do número de dias permitido para uso. Data limite '+FormataData(data));
        Result:=false;
      end else
        Result:=true;
      ...
      if FGeral.GetConfig1AsDate('DtInicioMvto') > 1 then begin
        ...
        if FGeral.GetConfig1AsDate('DtterminoMvto') > 1 then
          ...
          Result:=true;
      if not Result then EdData.Invalid('Data de movimento fora do período permitido para uso');
...
begin
  ...
  if not Result then EdData.Invalid('Data contábil fora do período configurado');
  if (Result) and (Global.DataUltimoEncerrPerCont>0) then begin
     if EdData.AsDate<Global.DataUltimoEncerrPerCont then begin
        Result:=False;
        EdData.Invalid('Data de contábil pertence a período contábil já encerrado');
     end;
  end;
end;
    ...
    Avisoerro('Data posterior a data de validade fiscal do sistema');
      ...
      if FGeral.GetConfig1AsDate('Dtinicionotas') > 1 then begin
        ...
        if FGeral.GetConfig1AsDate('Dtterminotas') > 1 then
          ...
          Result:=true;
      if not Result then EdData.Invalid('Data de movimento fora do período FISCAL permitido para uso');
...
// 15.02.17
function TFGeral.GetContaContax(pconta: integer): string;
//////////////////////////////////////////////////////////
var Q:TSqlquery;
begin
   Q:=sqltoquerycontax('select pcon_descricao from planocon where pcon_conta='+inttostr(pconta));
   if not Q.Eof then result:=Q.FieldByName('pcon_descricao').AsString else result:='';
   FGeral.FechaQuery(Q);
end;
...
// 08.12.20
                 tvDatelng:Ed.SetDate( GetConfig1AsDAte(Ed.TableField ) );
...
// 08.12.20
            if (Ed.TypeValue=tvDateLng) and (Ed.AsDate<>GetConfig1AsDate(Ed.TableField)) then GravaConfig1(Ed.TableField,'D',DatetoStr_4(Ed.AsDate));
         end;
      end;
  end;
end;
...
// 01.06.20 - para nao  gravar \sac\logo.bmp como \\sac\\logo.bmp
     if Ansipos('\',conteudo) > 0 then
       Sistema.Conexao.ExecuteDirect( 'INSERT INTO Config1 (Cfg1_Nome,Cfg1_Tipo,Cfg1_Conteudo) VALUES('+StringToSql(NomeCampo)+','+StringToSql(Tipo)+','+chr(39)+( Conteudo )+chr(39)+')')
     else
...
// 22.01.18 - para nao  gravar \sac\logo.bmp como \\sac\\logo.bmp
     if Ansipos('\',conteudo) > 0 then
       Sistema.Conexao.ExecuteDirect( 'UPDATE Config1 SET Cfg1_Conteudo='+chr(39)+( Conteudo )+chr(39)+' WHERE Cfg1_Nome='+StringToSql(NomeCampo) )
     else
       Sistema.Conexao.ExecuteDirect( 'UPDATE Config1 SET Cfg1_Conteudo='+StringToSql( Conteudo)+' WHERE Cfg1_Nome='+StringToSql(NomeCampo) );
     Sistema.Commit;
  end;
...
// 04.02.20
procedure TFGeral.SetDefaultPrinter(PrinterName: String);
//////////////////////////////////////////////////////////
var
  I: Integer;
  Device : PChar;
...
begin
  if m<>'' then m:=' '+m;
  Result:='Período'+m+': '+DateToStr_(Sistema.Datai)+' a '+DateToStr_(Sistema.Dataf);
end;
...
// 10.10.19
function TFGeral.GetSaldoInicialMovCon(Conta: Integer; Unidade: String;  Data: TDateTime): Currency;
//////////////////////////////////////////////////////////////////////////////////////////////////
var DataUltimoEncerramento:TDateTime;
    Q:TSqlQuery;
    ...
    var Q:TSqlQuery;
        w:String;
    begin
      w:=' WHERE Mcon_DataMvto>'+DateToSql(DataUltimoEncerramento);
      w:=w+' AND Mcon_DataMvto<'+DateToSql(Data);
      w:=w+' AND Mcon_Pcon_Conta='+IntToStr(Conta);
      if Unidade<>'' then w:=w+' AND Mcon_Unid_Codigo='+StringToSql(Unidade);
      w:=w+' AND Mcon_Status='+StringToSql('N');
      w:=w+' AND Mcon_DC='+StringToSql(DC);
      Q:=SqlToQueryContax('SELECT SUM(Mcon_Valor) AS Total FROM MovCon'+w);
...
begin
  ...
  if Unidade<>'' then begin
     Q:=SqlToQuerycontax('SELECT * FROM SaldosCon WHERE Scon_Data='+DateToSql(Global.DataMinima)+' AND Scon_Pcon_Conta='+IntToStr(Conta)+' AND SCon_Unid_Codigo='+StringToSql(Unidade));
  end else begin
     Q:=SqlToQuerycontax('SELECT * FROM SaldosCon WHERE Scon_Data='+DateToSql(Global.DataMinima)+' AND Scon_Pcon_Conta='+IntToStr(Conta));
  end;
  if not Q.isEmpty then begin
     Result:=Q.FieldByName('Scon_Valor').AsFloat;
     if Q.FieldByName('Scon_DC').AsString='D' then Result:=Result*-1;
  end;
  Q.Close;Q.Free;
  if Unidade<>'' then begin
     Q:=SqlToQuerycontax('SELECT MAX(Scon_Data) AS Scon_Data FROM SaldosCon WHERE SCon_Data<='+DateToSql(Data)+' AND Scon_Pcon_Conta='+IntToStr(Conta)+' AND SCon_Unid_Codigo='+StringToSql(Unidade));
  end else begin
     Q:=SqlToQuerycontax('SELECT MAX(Scon_Data) AS Scon_Data FROM SaldosCon WHERE SCon_Data<='+DateToSql(Data)+' AND Scon_Pcon_Conta='+IntToStr(Conta));
  end;
  DataUltimoEncerramento:=Q.FieldByName('Scon_Data').AsDateTime;
  if DataUltimoEncerramento=0 then DataUltimoEncerramento:=Global.DataMinima;
  Q.Close;Q.Free;
  if (DataUltimoEncerramento>Global.DataMinima) and (DataUltimoEncerramento<=Data) then begin
     if Unidade<>'' then begin
        Q:=SqlToQueryContax('SELECT * FROM SaldosCon WHERE Scon_Data='+DateToSql(DataUltimoEncerramento)+' AND Scon_Pcon_Conta='+IntToStr(Conta)+' AND Scon_Unid_Codigo='+StringToSql(Unidade));
     end else begin
        Q:=SqlToQueryContax('SELECT * FROM SaldosCon WHERE Scon_Data='+DateToSql(DataUltimoEncerramento)+' AND Scon_Pcon_Conta='+IntToStr(Conta));
     end;
     if not Q.isEmpty then begin
        Result:=Q.FieldByName('Scon_Valor').AsFloat;
        if Q.FieldByName('scon_debcre').AsString='D' then Result:=Result*-1;
     end;
     Q.Close;Q.Free;
  end;
  if (Data-1)>DataUltimoEncerramento then begin
    ...
    begin
      ...
      try
        if (Pos(Mat[p1].Dado,'*/')>0) or (Pos(Mat[p2].Dado,'*/')>0) then begin
           AvisoErro('Erro de operadores na fórmula: '+Formula);
           Result:=False;
           Exit;
        end;
        ...
        AvisoErro('Erro na fórmula: '+Formula);
        Result:=False;
        Exit;
      end;
    end;
...
begin
  ...
  if Nivel<>1 then begin
     AvisoErro('Erro de parenteses na fórmula: '+Formula);
     Result:=False;
     Exit;
  end;
  for i:=1 to Max do begin
      ...
      for p:=1 to Length(v) do begin
          if not (v[p] in ['0'..'9',' ','+','-','/','*','.',',']) then begin
             AvisoErro('Erro de variáveis na fórmula: '+Formula);
             Result:=False;
             Exit;
          end;
      end;
...
begin
  if (Global.Topicos[1421]) and (TSqlEd(Edit).IsEmpty) then begin
     TSqlEd(Edit).Invalid('Número de documento deve ser informado');
  end;
end;
...
var Lista:TStringList;
    i:Integer;
begin
  Aviso('Opção em desenvolvimento ainda não disponível');
  exit;
  if not Confirma('Confirma a reorganização do banco de dados') then Exit;
...
// verificar
  ...
  Lista.Free;
  GravaLog(1,'');
  Sistema.EndProcess('Reorganização Efetivada');
end;
...
begin
  if (Global.Usuario.Suporte) or (Global.Usuario.SenhaDiaria) then begin
     AvisoErro('Usuário suporte, não permitido solicitar uso exclusivo');
     Exit;
  end;
  if not FUsuarios.GetSenhaProcesso then Exit;
  Q:=SqlToQuery('SELECT Ctrl_UsuExclusivo FROM Controle WHERE Ctrl_Registro=1');
  ...
  if u=Global.Usuario.Codigo then begin
     AvisoErro('Atenção, sistema já está com uso exclusivo para o usuário atual');
     Exit;
  end;
  if u>0 then begin
     AvisoErro('Atenção, sistema já está com uso exclusivo para o; usuário: '+IntToStr(u)+' - '+FUsuarios.GetNome(u));
     Exit;
  end;
  if Confirma('Confirma a solicitação de uso exclusivo') then begin
     Sistema.Edit('Controle');
     Sistema.SetField('Ctrl_UsuExclusivo',Global.Usuario.Codigo);
     Sistema.Post('Ctrl_Registro=1');
     Sistema.Commit;
     Aviso('Efetivada solicitação para uso exclusivo');
  end;
end;
...
begin
  ...
  if Confirma('Confirma a liberação de uso exclusivo') then begin
     Sistema.Edit('Controle');
...
begin
  if (CodigoEvento<=0) or (CodigoEvento>High(Global.CodigosLog)-1) or (Global.CodigosLog[CodigoEvento]='') then begin
     AvisoErro('Código inválido para evento de log');
     Exit;
  end;
  Sistema.Insert('Log');
...
begin
  ...
  Global.CodigosLog[002]:='Alteração período alteração informações';
  Global.CodigosLog[003]:='Cancelamento de transação';
  Global.CodigosLog[004]:='Alteração movimento financeiro';
  Global.CodigosLog[005]:='Cancelamento Pedido de Venda';
  Global.CodigosLog[006]:='Alteração custo médio gerencial via cadastro';
  Global.CodigosLog[007]:='Alteração nf saida';
  Global.CodigosLog[008]:='Manutenção config. de movimentos';
  Global.CodigosLog[009]:='Manutenção cadastro CFOPs ( naturezas fiscais )';
  Global.CodigosLog[010]:='Venda/Devolução indicando remessa sem produto )';
  Global.CodigosLog[011]:='Numeração de nota perdida';
  Global.CodigosLog[012]:='Liberado Crédito com cheque devolvido';
  Global.CodigosLog[013]:='Alteração preço de venda via cadastro';
// 22.05.07
  Global.CodigosLog[014]:='Nota de venda abaixo do preço mínimo';
  Global.CodigosLog[015]:='Autorizado nota de venda abaixo do preço mínimo';
// 23.05.07
  Global.CodigosLog[016]:='Autorizado venda para cliente com restrição de crédito';
// 31.05.07
  Global.CodigosLog[017]:='Utilizado opção igualar cadastro usuários';
// 05.06.07
  Global.CodigosLog[018]:='Liberado venda mesmo acima limite crédito';
// 09.08.07
  Global.CodigosLog[019]:='Alterado cidade, cfop nota fiscal';
// 12.09.07
  Global.CodigosLog[020]:='Exclusão entrada de abate';
// 03.10.08
  Global.CodigosLog[020]:='Gravação cadastro de usuários';
// 20.04.09
  Global.CodigosLog[021]:='Alteração Valor pendencia financeira';
// 25.08.09
  Global.CodigosLog[022]:='Alteração de Opção de Orçamento de Obra Aprovada';
// 17.02.11
  Global.CodigosLog[023]:='Exclusão de codigo do estoque';
// 23.05.11
  Global.CodigosLog[024]:='Faturamento abaixo do mínimo por cidade';
// 08.06.11
  Global.CodigosLog[025]:='Gerou nota com diferença de peças/peso';
  Global.CodigosLog[026]:='Gravou peso da balança menor que o mínimo do pedido';
// 20.10.11
  Global.CodigosLog[027]:='Numero de NFe inutilizado';
// 03.05.13
  Global.CodigosLog[028]:='Alteração limite de crédito do cliente';
// 16.12.13
  Global.CodigosLog[029]:='Alteração campo situação do cliente';
// 28.08.14
  Global.CodigosLog[030]:='Alteração do vencimento de pendência financeira';
// 11.11.14
  Global.CodigosLog[031]:='Equipamento com manutenção bloqueada';
// 19.04.16
  Global.CodigosLog[032]:='Alteração da DESCRIÇÃO no cadastro de produtos';
// 11.10.17
  Global.CodigosLog[033]:='Problema na leitura de peso na balança de entrada de abate';
// 10.03.20
  Global.CodigosLog[034]:='Excluído item da composição de custos';
  Global.CodigosLog[035]:='Excluído toda composição de custos';
// 26.05.20
  Global.CodigosLog[036]:='Gravado a configuração geral do sistema';
// 09.07.20
  Global.CodigosLog[037]:='usado opção DUPLICAR do estoque';
// 16.11.20
  Global.CodigosLog[038]:='BRINCO na ENTRADA DE ABATE não encontrado no FAZENDA';
// 17.11.20
  Global.CodigosLog[039]:='BRINCO na ENTRADA DE ABATE usado em outra entrada de abate vinda da FAZENDA';
// 21.02.2022
  Global.CodigosLog[040]:='Ajustado data de EMISSÃO na VENDA BALCÃO após meia-noite';
// 23.05.2023
  Global.CodigosLog[041]:='AJUSTE 02 ref. data de EMISSÃO na VENDA BALCÃO após meia-noite';
// 13.02.2023
  Global.CodigosLog[042]:='Alteração de informação no cadastro de PRODUTOS';
// 06.03.2023
  Global.CodigosLog[043]:='Encontrado numeração perdida e usado número para autorizar nota';

  Global.CodigosLog[099]:='Uso Temporário';
...
begin
  ...
  try
     ...
     Result:=strtointdef(Q.FieldByName('Numero').AsString,0)+1;
  end;
   ...
   for i:=0 to xGrid.Columns.Count-1 do begin
     ...
     for p:=0 to xPainel.ComponentCount-1 do begin
       if xPainel.Components[p] is TSqlEd then begin
         if Uppercase( TSqlEd( xPainel.Components[p] ).TableField ) = Uppercase( campocoluna )  then begin

           xGrid.Columns[i].Title.Caption:=TSqlEd( xPainel.Components[p] ).Title ;
//           if ( not Global.Usuario.OutrosAcessos[0066] ) then
//              xGrid.Columns[i].Fieldname:= '##$$';
...
// 15.02.17
   xGrid.DrawingStyle:=gdsClassic;
///   xGrid.Color:=clyellow;
//   xGrid.Font.Color:=clblue;  // 24.03.17 - devereda
   xGrid.FixedColor:=claqua;

//   if ( not Global.Usuario.OutrosAcessos[0066] ) then begin
   if LeiLGPDP then begin


     for i:=0 to xGrid.Columns.Count-1 do begin

//        xGrid.Columns[i].Field.Text :='##$$';
//        if ( AnsiPos( 'NOME' ,Uppercase(xGrid.Columns[i].fieldname)) > 0 )  or
//           ( AnsiPos( 'RAZAOSOCIAL' ,Uppercase(xGrid.Columns[i].fieldname)) > 0 )  or
          if ( AnsiPos( 'FONE' ,Uppercase(xGrid.Columns[i].fieldname)) > 0 )  or
           ( AnsiPos( 'CPF' ,Uppercase(xGrid.Columns[i].fieldname)) > 0 )  or
           ( AnsiPos( 'CNPJ' ,Uppercase(xGrid.Columns[i].fieldname)) > 0 )  or
           ( AnsiPos( 'EMAIL' ,Uppercase(xGrid.Columns[i].fieldname)) > 0 )  or
           ( AnsiPos( 'ENDERECO' ,Uppercase(xGrid.Columns[i].fieldname)) > 0 )  then
//           ( AnsiPos( 'DESCRICAO',Uppercase(xGrid.Columns[i].fieldname)) > 0 )  then

           xGrid.Columns[i].field.EditMask :='##**';
...
// 08.10.2012 // senao tem portado identificado mostra 2 vezes 'mesma coisa'
        ...
        exit;
// 21.01.13 - Vivan - para não imprimir contas a receber ref. recebimento com cartão de credito
      sqlportadorcartao:='';
...
// 13.02.13 - Vivan Cobrança - RC soma até a data atual
        dataf:=Sistema.hoje;
...
// 15.05.13 - remessas 'troca de crédito' não soma pra limite
...
// 24.05.13 - null...sem comentarios...
         ...
         FGeral.Fechaquery(Q);
// 14.02.13 - Vivan Cobrança - DC e DR não estava deduzindo do credito
//////////////////////////////////////////////////////////////////////
        Q:=sqltoquery('select sum(moes_vlrtotal) as remessas from movesto '+
...
//          Avisoerro('Boleto bancário em aberto '+numerodcto+' vencido em '+formatdatetime('dd/mm/yy',datavcto)+' valor '+FGeral.Formatavalor(valor,f_cr)+' Total atrasado '+FGeral.Formatavalor(valoraberto-valorbxparcial,f_cr))
          Avisoerro('Boleto bancário em aberto '+numerodcto+' vencido em '+formatdatetime('dd/mm/yy',datavcto)+' valor '+FGeral.Formatavalor(valor,f_cr)+' Total atrasado '+FGeral.Formatavalor(valoraberto,f_cr))
        else if portador='LIM' then
//          Avisoerro('Limite de crédido de '+FGeral.Formatavalor(Edit.resultfind.fieldbyname('Clie_limcredito').ascurrency,f_cr)+' ultrapassado.  Total em aberto '+FGeral.Formatavalor(valoraberto+valordoc-valorbxparcial,f_cr))
          Avisoerro('Limite de crédido de '+FGeral.Formatavalor(Edit.resultfind.fieldbyname('Clie_limcredito').ascurrency,f_cr)+' ultrapassado.  Total em aberto '+FGeral.Formatavalor(valoraberto+valordoc,f_cr))
// 18.11.13
    ...
    begin
      ...
      if minimo>0 then begin
        ...
        if (venda-devolucao)<minimo then begin
          result:=false;
          Avisoerro('Valor último mes : Venda '+floattostr(venda)+' Devolução '+floattostr(devolucao));
        end;
        FGeral.Fechaquery(Q);
      end else
        result:=true
...
//      Aviso('Ver validações de clientes');
...
// 04.08.06
      if Edit.ResultFind.FieldByName('clie_situacao').AsString='R' then begin
        Avisoerro('Cliente registrado.   Solicitar depósito antecipado.');
      end else if Edit.ResultFind.FieldByName('clie_situacao').AsString='B' then begin
        Avisoerro('Cliente bloqueado.   Solicitar autorização.');
        result:=false;
      end;
    end;
...
// 21.05.12 - Janina pediu pra liberar no cnpj RC e VC
      if (Edit.ResultFind.FieldByName('clie_tipo').AsString<>'F') then begin
          if not Global.topicos[1026] then begin
            Avisoerro('Venda '+Global.CodVendaSerie4+' somente permitida para pessoa física; tipo cliente : '+Edit.ResultFind.FieldByName('clie_tipo').AsString);
            result:=false;
          end else
            result:=true;
  ...
  //  17.01.06 - liberado pedido de venda para pessoa juridica pois é venda direta os pedidos
  ...
  //      if Edit.Resultfind.fieldbyname('clie_tipo').asstring<>'F' then begin
  //        Avisoerro('Remessa permitida somente para pessoa jurídica');
          result:=true;
  //      end;
...
// 21.05.12 - Janina pediu pra liberar no cnpj RC e VC
        if not Global.topicos[1026] then begin
          Avisoerro('Remessa permitida somente para pessoa física');
          result:=false;
        end else
          result:=true;
...
//             then begin  // 12.05.06
           Avisoerro('Remessa de consignação permitida somente por '+inttostr(FGeral.getconfig1asinteger('DIASCONSIG'))+' dias');
...
// Rever possível validaçoes de clientes
    result:=true;
  end;
end;
...
//      Aviso('Ver validações de representantes');
    result:=true;
  end else
    result:=false;
end;
...
// 13.06.05 - opção para NAO checar o estoque e baixa somente quando grava o documento
    ...
    try
      ...
      Avisoerro('Problema na função tem estoque');
...
// 18.04.06 - tipo de movimento para retorno de industrialização com serviços só pra financeiro- rolos tipo manoel
...
// 23.01.17
   Global.CodNfeComplementoIcmsE:='IE';
// 12.09.17
   Global.CodNfeComplementoIPI:='IP';
// 03.11.17
   Global.CodCedulaProdutoRural:='PR';
// 01.11.18
   Global.CodOrcamento:='OR';
// 18.03.19
   Global.CodNfeComplementoIcmsECliente:='C2';
// 19.03.2019
   Global.CodComplementoValorE:='CV';
// 23.06.19 - 'rolo seip devolucao importacao'
   Global.CodDevolucaoTributadaCliente:='DZ';
// 03.12.20
   Global.CodDevolucaoBonificacao:='DY' ;
// 20.08.2021 - NFPe
   Global.CodEntradaNFPe:='NE';
// 28.12.2022
   Global.CodComplementoValorS:='CZ';
...
begin
 if tipomovto=Global.CodPrestacaoServicos then
   result:=if_s(not reduzido,'Prestação Serviços',Global.CodPrestacaoServicos)
 else if tipomovto=Global.CodPrestacaoServicosE then
   result:=if_s(not reduzido,'Prestação Serviços - Entrada',Global.CodPrestacaoServicosE)
 else if tipomovto=Global.CodRemessaConsig then
   result:=if_s(not reduzido,'Remessa de Consignação',Global.CodRemessaConsig)
 else if tipomovto=Global.CodDevolucaoConsig then
   result:=if_s(not reduzido,'Devolução de Consignação',Global.CodDevolucaoConsig)
 else if tipomovto=Global.CodDevolucaoTroca then
   result:=if_s(not reduzido,'Devolução de Troca',Global.CodDevolucaoTroca)
   ...
   result:=if_s(not reduzido,'Venda Líquida Serie 4',Global.CodVendaLiqSerie4)
   ...
   result:=if_s(not reduzido,'Devolução de Compra',Global.CodDevolucaoCompra)
   ...
   result:=if_s(not reduzido,'Devolução Romaneio de Retorno Serie 5',Global.CodDevolucaoSerie5)
...
//   result:=if_s(not reduzido,'Acerto Est. - Saida',global.CodAcertoEsEnt)
   ...
   result:=if_s(not reduzido,'Transferência - Entrada',global.CodTransfEntrada)
 else if tipomovto=Global.CodTransfSaida then
   result:=if_s(not reduzido,'Transferência - Saida',global.CodTransfSaida)
 else if tipomovto=Global.CodTransfEnt then
   result:=if_s(not reduzido,'Transferência - Entrada sem movimento',global.CodTransfEnt)
 else if tipomovto=Global.CodTransfSai then
   result:=if_s(not reduzido,'Transferência - Saida sem movimento',global.CodTransfSai)
   ...
   result:=if_s(not reduzido,'Venda Transferência',Global.CodVendaTransf)
...
// 06.05.05
   ...
   result:=if_s(not reduzido,'Remessa p/ Industrialização',Global.CodRemessaInd)
   ...
   result:=if_s(not reduzido,'Devolução Pronta Entrega',Global.CodDevolucaoProntaEntrega)
 else if tipomovto=Global.CodDevolucaoInd then
   result:=if_s(not reduzido,'Retorno Industrialização',Global.CodDevolucaoInd)
 else if tipomovto=Global.CodDevolucaoRoman then
   result:=if_s(not reduzido,'Devolução Romaneio Ambulante',Global.CodDevolucaoRoman)
 else if tipomovto=Global.CodDevolucaoVenda then
   result:=if_s(not reduzido,'Devolução Venda',Global.CodDevolucaoVenda)
 else if tipomovto=Global.CodDevolucaoImob then
   result:=if_s(not reduzido,'Devolução Imobilizado',Global.CodDevolucaoImob)
 else if tipomovto=Global.CodDevolucaoConsigMerc then
   result:=if_s(not reduzido,'Devolução Consignação Mercantil',Global.CodDevolucaoConsigMerc)

 else if tipomovto=Global.codDevolucaoBonificacao then
   result:=if_s(not reduzido,'Devolução Remessa em Bonificação',Global.CodDevolucaoBonificacao)

 else if tipomovto=Global.CodConsigMercantil then
   result:=if_s(not reduzido,'Remessa Consignação Mercantil',Global.CodConsigMercantil)
 else if tipomovto=Global.CodRetornoConsigMercanil then
   result:=if_s(not reduzido,'Retorno Consignação Mercantil',Global.CodRetornoConsigMercanil)
 else if tipomovto=Global.CodVendaConsigMercantil then
   result:=if_s(not reduzido,'Venda Consignação Mercantil',Global.CodVendaConsigMercantil)
 else if tipomovto=Global.CodVendaMostruario then
   result:=if_s(not reduzido,'Venda Mostruário',Global.CodVendaMostruario)
   ...
   result:=if_s(not reduzido,'Retorno Mostruário',Global.CodRetornoMostruario)
   ...
   result:=if_s(not reduzido,'Venda Peça Problema',Global.CodVendaPecaProblema)
 else if tipomovto=Global.CodVendaREFinal then
   result:=if_s(not reduzido,'Venda Final Série 4',Global.CodVendaREFinal)
 else if tipomovto=Global.CodVendaRE then
   result:=if_s(not reduzido,'Venda Parcial Série 4',Global.CodVendaRE)
 else if tipomovto=Global.CodVendaREBrinde then
   result:=if_s(not reduzido,'Venda Brinde Série 4',Global.CodVendaREBrinde)
// 01.08.05
   ...
   result:=if_s(not reduzido,'Devolução Igual Venda',Global.CodDevolucaoIgualVenda)
   ...
   result:=if_s(not reduzido,'Venda Mostruário com Financeiro',Global.CodVendaMostruarioII)
 else if tipomovto=Global.CodDevolucaoCompraSemEstoque then
   result:=if_s(not reduzido,'Devolução de Compra Sem Estoque',Global.CodDevolucaoCompraSemEstoque)
   ...
   result:=if_s(not reduzido,'Contagem Balanço Saida',Global.CodContagemBalancoS)
 else if tipomovto=Global.CodContagemBalancoE then
   result:=if_s(not reduzido,'Contagem Balanço Entrada',Global.CodContagemBalancoE)
 else if tipomovto=Global.CodTransfEntradaTempo then
   result:=if_s(not reduzido,'Transferência - Temporada Entrada',global.CodTransfEntradaTempo)
 else if tipomovto=Global.CodTransfSaidaTempo then
   result:=if_s(not reduzido,'Transferência - Temporada Saida',global.CodTransfSaidaTempo)
   ...
   result:=if_s(not reduzido,'Pedido de Venda de Mostruário',global.CodPedVendaMostruario)
 else if tipomovto=Global.CodBaixaMatEnt then
   result:=if_s(not reduzido,'Mov Matéria Prima - Entrada',global.CodBaixaMatEnt)
 else if tipomovto=Global.CodBaixaMatSai then
   result:=if_s(not reduzido,'Mov Matéria Prima - Saida',global.CodBaixaMatSai)   //  07.12.05
   ...
   result:=if_s(not reduzido,'Transferência - Retorno Temporada Entrada',global.CodTransfEntRetTempo)
 else if tipomovto=Global.CodTransfSaiRetTempo then
   result:=if_s(not reduzido,'Transferência - Retorno Temporada Saida',global.CodTransfSaiRetTempo)
// 09.02.05
   ...
   result:=if_s(not reduzido,'Pedido de Venda de Mostruário Consignada',global.CodPedVendaMostruarioConsig)
 else if tipomovto=Global.CodDevolucaoVendaConsig then
   result:=if_s(not reduzido,'Devolução Venda Consignada',Global.CodDevolucaoVendaConsig)
 else if tipomovto=Global.CodRetornoInd then
   result:=if_s(not reduzido,'Retorno Indust. sem serviço no fiscal ',Global.CodRetornoInd)
 else if tipomovto=Global.CodRetornocomServicos then
   result:=if_s(not reduzido,'Retorno Serviço Industrialização ',Global.CodRetornocomServicos)
 else if tipomovto=Global.CodRetornoMercDepo then
   result:=if_s(not reduzido,'Retorno Mercadoria Depósito ',Global.CodRetornoMercDepo)
 else if tipomovto=Global.CodLanCaixaBancos then
   result:=if_s(not reduzido,'Lançamento Caixa/bancos'          ,Global.CodLanCaixaBancos)
// 04.07.06
...
// 20.08.2021
 else if tipomovto=Global.CodEntradaNFPe then
   result:=if_s(not reduzido,'Entrada de NFP-e Produtor Rural',Global.CodEntradaNFPe)
   ...
   result:=if_s(not reduzido,'Requisição Almoxarifado',Global.CodRequisicaoAlmox)
   ...
   result:=if_s(not reduzido,'Remessa em Demonstração',Global.CodRemessaDemo)
   ...
   result:=if_s(not reduzido,'Compra Indústria',Global.CodCompraIndustria)
 else if tipomovto=Global.CodDevolucaoSaida then
   result:=if_s(not reduzido,'Devolução Saida',Global.CodDevolucaoSaida)
   ...
   result:=if_s(not reduzido,'Devolução Sem Financeiro',Global.CodDevolucaoSemFinan)
 else if tipomovto=Global.CodDevolucaodeRemessa then
   result:=if_s(not reduzido,'Devolução de Remessa',Global.CodDevolucaodeRemessa)
   ...
   result:=if_s(not reduzido,'Entrada para Industrialização',Global.CodEntradaInd)
 else if tipomovto=Global.CodRemessaIndPropria then
   result:=if_s(not reduzido,'Remessa Industrialização Própria',Global.CodRemessaIndPropria)
 else if tipomovto=Global.CodEntradaBrinde then
   result:=if_s(not reduzido,'Entrada Bonificação, Brinde ou Doação',Global.CodEntradaBrinde)
 else if tipomovto=Global.CodContratoDoacao then
   result:=if_s(not reduzido,'Venda Contrato Doação',Global.CodContratoDoacao)
 else if tipomovto=Global.CodCompraProdutorReclassifica then
   result:=if_s(not reduzido,'Compra de Produtor Rural Reclassificação',Global.CodCompraProdutorReclassifica)
   ...
   result:=if_s(not reduzido,'Devolução Compra de Produtor',Global.CodDevolucaoCompraProdutor)
 else if tipomovto=Global.CodRemessaDemoClientes then
   result:=if_s(not reduzido,'Remessa em Demonstração (Clientes)',Global.CodRemessaDemoClientes)
   ...
   result:=if_s(not reduzido,'Devolução Simbólica de Consignação',Global.CodDevolucaoSimbolicaConsig)
   ...
   result:=if_s(not reduzido,'Estorno de NF-e de Venda não cancelada',Global.CodEstornoNFeSai)
 else if tipomovto=Global.CodDevolucaoTributada then
   result:=if_s(not reduzido,'Devolução com Crédito Icms/IPI',Global.CodDevolucaoTributada)
 else if tipomovto=Global.CodDevolucaoTributadaCliente then
   result:=if_s(not reduzido,'Devolução com Crédito Icms/IPI para cliente',Global.CodDevolucaoTributadaCliente)
 else if tipomovto=Global.CodFaturaAgua then
   result:=if_s(not reduzido,'Conta de Água',Global.CodFaturaAgua)
   ...
   result:=if_s(not reduzido,'Entrada Complemento de Valor NF Produtor',Global.CodNfeComplementoValorProdutor)
 else if tipomovto=Global.CodComplementoValorE then
   result:=if_s(not reduzido,'Entrada Complemento de Valor NF Compra',Global.CodComplementoValorE)
 else if tipomovto=Global.CodComplementoValorS then
   result:=if_s(not reduzido,'Saida Complemento de Valor NF Venda',Global.CodComplementoValorS)
 ...
 else if tipomovto=Global.CodNfeComplementoIPI then
   result:=if_s(not reduzido,'Complemento de IPI ',Global.CodNfeComplementoIPI)
 else if tipomovto=Global.CodNfeComplementoIcmsE then
   result:=if_s(not reduzido,'Complemento de Icms - Entrada',Global.CodNfeComplementoicmsE)
 else if tipomovto=Global.CodNfeComplementoIcmsECliente then
   result:=if_s(not reduzido,'Complemento de Icms - Entrada - Cliente',Global.CodNfeComplementoicmsECliente)
 ...
 else if tipomovto=Global.CodCedulaProdutoRural then
   result:=if_s(not reduzido,'Cédula de Produto Rural - CPR',Global.CodCedulaProdutoRural)
...
begin
  Ed.Items.clear;
  Ed.Items.Add(Global.CodPrestacaoServicos+' - Prestação Serviços' );
  Ed.Items.Add(Global.CodPrestacaoServicosE+' - Prestação Serviços Entrada' );
  Ed.Items.Add(Global.CodRemessaConsig+' - Remessa consignação' );
  Ed.Items.Add(Global.CodRemessaProntaEntrega+' - Remessa de pronta entrega' );
  Ed.Items.Add(Global.CodConsigMercantil+' - Remessa Consignação Mercantil' );
  Ed.Items.Add(Global.CodRemessaInd+' - Remessa p/ Industrialização');
  Ed.Items.Add(Global.CodRemessaIndPropria+' - Remessa de Industrialização Própria');
  Ed.Items.Add(Global.CodRemessaConserto+' - Remessa p/ Conserto');
  Ed.Items.Add(Global.CodRemessaDemo+' - Remessa em Demonstração (Fornec.)');
  Ed.Items.Add(Global.CodRemessaDemoClientes+' - Remessa em Demonstração (Clientes)');
  ...
  Ed.Items.Add(Global.CodNfeComplementoValorProdutor+' - Complemento de Valor Entrada Produtor');
  Ed.Items.Add(Global.CodComplementoValorE+' - Complemento de Valor Entrada Compras');
  Ed.Items.Add(Global.CodComplementoValorS+' - Complemento de Valor Saida Vendas');
  Ed.Items.Add(Global.CodNfeComplementoIcms+' - Complemento de Icms');
  Ed.Items.Add(Global.CodNfeComplementoIcmsE+' - Complemento de Icms - Entrada');
  Ed.Items.Add(Global.CodNfeComplementoIcmsECliente+' - Complemento de Icms - Entrada - Cliente');
  Ed.Items.Add(Global.CodNfeComplementoIPI+' - Complemento de IPI');
//   Global.CodConsigMercantil:='MC';
  Ed.Items.Add(Global.CodDevolucaoConsig+' - Devolução de consignação');
  Ed.Items.Add(Global.CodRetornoConsigMercanil+' - Retorno de consignação mercantil');
//   Global.CodRetornoConsigMercanil:='MR';
  Ed.Items.Add(Global.CodDevolucaoCompra+' - Devolução compra');
  Ed.Items.Add(Global.CodDevolucaoInd+' - Retorno Industrialização');
  Ed.Items.Add(Global.CodRetornoMercDepo+' - Retorno Mercadoria Depósito');
  Ed.Items.Add(Global.CodDevolucaoProntaEntrega+' - Devolução Pronta Entrega');
  Ed.Items.Add(Global.CodDevolucaoTroca+' - Devolução de troca na consignação');
  Ed.Items.Add(Global.CodDevolucaoRoman+' - Devolução Romaneio Ambulante');
  Ed.Items.Add(Global.CodDevolucaoVenda+' - Devolução de Venda');
  Ed.Items.Add(Global.CodDevolucaoVendaConsig+' - Devolução de Venda Consignada');
  Ed.Items.Add(Global.CodDevolucaoConsigMerc+' - Devolução Consignação Mercantil');
  Ed.Items.Add(Global.CodDevolucaoIgualVenda+' - Devolução Igual Venda');
  Ed.Items.Add(Global.CodDevolucaoCompraSemEstoque+' - Devolução Compra Sem Estoque');
  Ed.Items.Add(Global.CodDevolucaoSaida+' - Devolução Saida');
  Ed.Items.Add(Global.CodDevolucaoImob+' - Devolução Imobilizado');
  Ed.Items.Add(Global.CodDevolucaoSemFinan+' - Devolução Sem Financeiro');
  Ed.Items.Add(Global.CodDevolucaoCompraProdutor+' - Devolução de Compra de Produtor');
  Ed.Items.Add(Global.CodDevolucaoSimbolicaConsig+' - Devolução Simbólica de Consignação');
  Ed.Items.Add(Global.CodDevolucaodeRemessa+' - Devolução de Remessa');
  Ed.Items.Add(Global.CodDevolucaoTributada+' - Devolução c/Icms e Ipi');
  Ed.Items.Add(Global.CodDevolucaoTributadaCliente+' - Devolução c/Icms e Ipi para Cliente');

  Ed.Items.Add(Global.codDevolucaoBonificacao+' - Devolução de Remessa em Bonificação');

  Ed.Items.Add(Global.CodRomaSerie4+' - Romaneio de Retorno');
  Ed.Items.Add(Global.CodDevolucaoSerie5+' - Devolução Romaneio de Retorno');
  Ed.Items.Add(Global.CodRetornoMostruario+' - Retorno Mostruário');
  Ed.Items.Add(Global.CodRetornoRemessaConserto+' - Retorno Remessa p/ Conserto');
// 31.07.12 - Clessi - Estorno de NFe de Saida
  Ed.Items.Add(Global.CodEstornoNFeSai+' - Estorno NFe de Venda Não Cancelada');
  ...
  Ed.Items.Add(Global.CodVendaMostruario+' - Venda Mostruário');
// 07.11.07
  Ed.Items.Add(Global.CodContrato+' - Venda Contrato');
// 10.08.09
  Ed.Items.Add(Global.CodContratoNota+' - Venda Contrato Nota');
// 04.09.09
  Ed.Items.Add(Global.CodContratoDoacao+' - Venda Contrato Doação');
// 28.11.07
  ...
  Ed.Items.Add(Global.CodVendaLiqSerie4+' - Venda Líquida Serie 4');
  Ed.Items.Add(Global.CodVendaConsigMercantil+' - Venda Consignação Mercantil');
  Ed.Items.Add(Global.CodVendaPecaProblema+' - Venda Peça Problema');
  Ed.Items.Add(Global.CodVendaRE+' - Venda Parcial Série 4');
  Ed.Items.Add(Global.CodVendaREFinal+' - Venda Final Série 4');
  Ed.Items.Add(Global.CodVendaREBrinde+' - Venda Brinde Série 4');
  Ed.Items.Add(Global.CodVendaBrindeConsig+' - Venda Brinde Consignado');
  Ed.Items.Add(Global.CodVendaTransf+' - Venda Transferência');
//  Ed.Items.Add(Global.CodVendaImobilizado+' - Venda Transferência');
  Ed.Items.Add(Global.CodVendaSemfinan+' - Venda sem Financeiro');
  Ed.Items.Add(Global.CodVendaMostruarioII+' - Venda Mostruário com Financeiro');
...
// 14.06.16
  Ed.Items.Add(Global.CodPedVendaMostruarioConsig+' - Venda Mostruário Consignado');
//   Global.CodRetornoMostruario:='RM';
//   Global.CodVendaMostruario:='SM';
//   Global.CodVendaConsigMercantil:='MV';
...
// 04.07.08
  ...
  Ed.Items.Add(Global.CodCompraProdutorReclassifica+' - Compra Produtor Rural Reclassificação');
  Ed.Items.Add(Global.CodCompraProdutorMerenda+' - Compra Produtor Rural Merenda');
  Ed.Items.Add(Global.CodEntradaNFPe+' - Entrada NFP-e Produtor Rural');

  Ed.Items.Add(Global.CodCompraIndustria+' - Compra Indústria');
// 23.05.08 e só em 30.09.08 'disponibilizou'...jamanta...
  ...
  Ed.Items.Add(Global.CodEntradaInd+' - Entrada para Industrialização');
// 15.06.09
  Ed.Items.Add(Global.CodEntradaBrinde+' - Entrada Bonificação,Brinde ou Doação');
// 11.03.14
  Ed.Items.Add(Global.CodFaturaAgua+' - Conta de Água');
  ...
  Ed.Items.Add(Global.CodContagemBalancoE+' - Contagem Balanço (entrada)');
  Ed.Items.Add(Global.CodContagemBalancoS+' - Contagem Balanço (saida)');
  Ed.Items.Add(Global.CodTransfEntrada+' - Transferência estoque (entrada)');
  Ed.Items.Add(Global.CodTransfSaida+' - Transferência estoque (saida)');
  Ed.Items.Add(Global.CodTransfEnt+' - Transferência estoque (entrada sem movimento)');
  Ed.Items.Add(Global.CodTransfSai+' - Transferência estoque (saida sem movimento)');
  Ed.Items.Add(Global.CodTransImob+' - Transferência de Imobilizado Saida');
  Ed.Items.Add(Global.CodTransImobE+' - Transferência de Imobilizado Entrada');
  Ed.Items.Add(Global.CodTransfEntradaTempo+' - Transferência estoque temporada (entrada)');
  Ed.Items.Add(Global.CodTransfSaidaTempo+' - Transferência estoque temporada (saida)');
  Ed.Items.Add(Global.CodTransfEntRetTempo+' - Transferência estoque Retorno temporada (entrada)');
  Ed.Items.Add(Global.CodTransfSaiRetTempo+' - Transferência estoque Retorno temporada (saida)');
  Ed.Items.Add(Global.CodTransMatConsumoE+' - Transferência Material Consumo Entrada');
  Ed.Items.Add(Global.CodTransMatConsumoS+' - Transferência Material Consumo Saida');
...
// 18.04.06
  Ed.Items.Add(Global.CodRetornoInd+' - Retorno Indust. sem serviço fiscal');
// 20.04.06
  Ed.Items.Add(Global.CodRetornocomServicos+' - Retorno Serviço Industrialização');
// 31.10.07
...
// 22.11.07
  Ed.Items.Add(Global.CodRequisicaoAlmox+' - Requisição Almoxarifado');
// 11.03.09
...
// 06.10.09
  ...
  Ed.Items.Add(Global.CodCedulaProdutoRural+' - Cédula de Produto Rural - CPR');
...
// 13.06.05 - opção de baixar o estoque somente quando grava e nao durante a digitacao
...
// 25.06.19 - para vincULAR manutenção de equipamentos com nota lançada
    Sistema.SetField('moes_transacerto',xtransacanota);
// 11.07.19
    Sistema.SetField('moes_mensagem',xobs);
// 06.10.2021 - A2z
    Sistema.SetField('moes_devolucoes',xobs1);
// 13.10.2021 - A2z
    Sistema.SetField('moes_rcmagazine',xprecor);
...
// 05.10.2021 - liberado para  alterar a data da remessa na alteracao
//              Fama - Janina
    Sistema.SetField('moes_vlrtotal',Valortotal);
    Sistema.SetField('moes_tabp_codigo',Tabela);
    Sistema.SetField('moes_usua_codigo',Global.Usuario.Codigo);
...
// 05.10.2021 - Fama - Janina
////////////////////////////////
    Sistema.Edit('Movestoque');
    Sistema.SetField('move_datamvto',Emissao);
    Sistema.Post('move_transacao='+stringtosql(transacao));
...
//  if (Global.Topicos[1221]) and (Ed<>nil) then ed.Text:=copy(codigoproduto,1,12);
  if (Global.Topicos[1221]) and (Ed<>nil) and (copy(codigoproduto,1,6)='000000') then ed.Text:='0'+copy(codigoproduto,1,11);
// 31.08.17 - janinia
  if (Global.Topicos[1221]) and (Ed<>nil) and (copy(codigoproduto,1,3)='900')
     then ed.Text:=copy(codigoproduto,1,12);
// 06.03.13 - Benato  - só pra passar pelo edit do codigo e nao pesquisar pelo codigo de barra 'normal'
...
// 20.03.14 - Vivan
  if (Global.Topicos[1225]) and ( copy(codigoproduto,1,1)='2' ) and ( length(trim(codigoProduto))=13 ) then
    result:=false ;
// 15.05.14 - Devereda - codigo de barras com 8 digitos e começando por 789
  if ( copy(CodigoProduto,1,3)='789' ) and (length(trim(CodigoProduto))=8 ) then result:=true;
// 28.08.14 - Devereda - codigo de barras com 8 digitos e começando por 'qq coisa'
...
// 07.01.2014
          ...
          ' and '+FGeral.Getin('move_tipomov',tipomov,'C')+
          ' and substr(moes_transacao,1,1)<>''I'''+   // para não confundir com remessas ou devolucçeos importadas
          ' and substr(moes_transacao,1,1)<>''N'''+   // para não confundir com remessas ou devolucçeos importadas
          ' and substr(move_transacao,1,1)<>''I'''+   // para não confundir com remessas ou devolucçeos importadas
          ' and substr(move_transacao,1,1)<>''N'''+   // para não confundir com remessas ou devolucçeos importadas
...
begin
  ...
  if not QBusca.Eof then begin
    ...
    Avisoerro('Produto '+codigo+' não encontrado em estoqueqtde na unidade '+unidade+'.  NAO MOVIMENTADO');
end;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// 12.04.19
function TFGeral.Mes(num: integer): string;
///////////////////////////////////////
begin
  if num = 1 then
  MES:='Janeiro';
...
//    TipoMovimento será usado para ver se gera nf de venda consignada
...
// grava o mestre do produtos q foram digitados no retorno de consignação
      Sistema.beginprocess('Gravando mestre da devolução');
      Sistema.Insert('Movesto');
      Sistema.SetField('moes_transacao',Transacao);
      Sistema.SetField('moes_operacao',GetOperacao);
      Sistema.SetField('moes_status','N');
...
// grava o detalhe dos produtos q foram digitados no retorno de consignação
  Sistema.beginprocess('Gravando detalhe da devolução');
  for linha:=1 to Grid.rowcount do begin
    if trim(Grid.Cells[Grid.getcolumn('move_esto_codigo'),linha])<>'' then begin
...
// devolver ao estoque o q voltou
        ...
        aviso('Codigo '+Grid.Cells[grid.GetColumn('move_esto_codigo'),linha]+' não encontrado no estoque');
      end;
    end;
  end;   // ref. ao grid
...
// uma hipotese era acerto 'sem Vc' , isto é ,devolveu tudo
...
// 29.06.06 - aqui em 09.05.11 pra poder ver se é do simples naciona dai muda cst
////////////////
        mensagemfixa:='';
        simples:='N';
...
// 12.09.2022 - retirado estas 2 linhas devido erro na gravacao campo C 1000
//          na ane chapeco
...
// 26.01.05 - retirado para ver se este é o problema da nf com vlr dif. do edit no ret. consig.
...
// 31.08.06 - gravação separando pela grade...gravar somente um codigo e suas 'derivações' por vez...
...
// 09.05.11 - Nfe de VC - co..sn - cst do simples nacional
            ...
            Sistema.SetField('move_usua_codigo',Global.Usuario.codigo);
            Sistema.SetField('move_vendabru',venda);
  // 25.10.05 / 11.10.2022 - truncado em 1000
            Sistema.SetField('move_devolucoes',copy(Devolucoes,1,1000));
//          Sistema.SetField('move_perdesco',percdesconto);
// 20.05.05 - retirado gravação do desconto por item pois na nf VC so tem desconto "geral"
...
//            basesubs:=basesubs+( totalitem*(1+(Global.MargemSubsTrib/100)) );
  ...
  // ver campos de tributação,  bases de calculos, etc
  // ver quando tem reduçao da base de calculo e reduzir
...
// 17.11.04 - isento de icms
        ...
        end else begin    // se qtde>0 , isto é , se o saldo do produto for maior q zero
...
// Gerando o arquivo de "mestre" da movimentação da nf de venda
/////////////////////////////////////////////////////////////////////////////////////
...
///////////          totalnf:=xtotalnf;
// 13.12.13 0 retirado pois junto com sandra constatado que totalnf tá certo
...
// 11.10.2022 - truncado em 1000
        Sistema.SetField('moes_remessas',copy(Remessas,1,1000));
    //    Sistema.SetField('moes_tabp_codigo',Tabela);
    //    Sistema.SetField('moes_tabaliquota',FTabela.GetAliquota(Tabela));
        Sistema.SetField('moes_usua_codigo',Global.Usuario.Codigo);
...
//  desmudado pra deixar o total de produtos sem a substituição
...
// 25.10.05
// 11.10.2022 - truncado em 1000
        Sistema.SetField('moes_devolucoes',copy(Devolucoes,1,1000));
...
///////////////
  ...
  // Gerando o arquivo de bases de cálculo conforme as situacoes tributarias existentes
  /////////////////////////////////////////////////////////////////////////////////////
        Sistema.beginprocess('Gravando bases de cálculo da nota fiscal');
        if Movimento>1 then begin
          if TotalBaseicms>0 then
...
// ver se for a vista jogar na conta caixa ( configuração na unidade )
      nparcelas:=FCondPagto.GetNumeroParcelas(Condicao);
// verificar se é a vista com apenas uma parcela OU se somente a primeira parcela é a vista ( ou parte do total )
      valorparcela:=0;
...
// 26.10.13                                               // 30.10.13
        if ( totalnf-valorparcela>1 ) and (percdesconto=0) and (FCondPagto.GetAvPz(Condicao)='V') then begin
          valorparcela:=totalnf;  // 04.11.13 - da dif. devido a media de preço
        end;
...
//        valortotal:=valortotal-valorparcela;
      ...
      acumulado:=0;
      Sistema.beginprocess('Gravando pendências financeiras');
...
// 04.06.04 - senao ainda não tem nada gravado no banco para mudar o status                                                                                  f
//////////////////////////////////////////////////////////////////////////////////////////////
...
// 25.08.11 - pra ver se 'para de pular' a numeracao as vezes na Fama Confeccoes
        ...
        Sistema.endprocess('Gravação Interrompida.  Problemas para gravar no banco de dados');
        exit;
      end;

      Sistema.beginprocess('Gravando as remessas como já totalmente devolvidas');
...
// trato com as VT ja fechadas na pronta entrega
      Sistema.beginprocess('Gravando as vendas de transferência como já totalmente devolvidas');
      Sistema.Edit('movesto');
...
//                         ' and '+FGeral.estaemaberto(QProdDevolvido.fieldbyname('move_remessas').AsString,EdRemessas.text) and
     ...
     Sistema.beginprocess('Baixando devoluções');
     sqldevosemm:='';
...
var p,x:integer;
begin
// confirmar se para substituição tributaria "é mais um campo ou mais um registro ou muda a base e valor do icms"
  ...
  if x<0 then begin
    ...
    pcstperc.cfop:=cfop;
// 04.02.17 - exclusivo para uso em nfe de complemento de icms entrada
    Pcstperc.vicms:=valoricms;
...
// 24.05.07 - percentual de redução na precisa acumular
...
///////////////////////////////////////////////////////////////////////////////////////////
  ...
  CodigoMov:integer=0 ; rtipocad:string='C' ; xPedidos:string='' ; Saida:TDateTime=0);
...
// 14.04.18 - benato para poder mostrar 'de onde' veio a baixa da carne bovina
              Sistema.setfield('move_nroobra',Grid.Cells[grid.GetColumn('move_esto_codigo'),linha]);

              Sistema.Post;
// 02.06.12 - Benato - retirado em 20.09.12 pois só comita no final da transacao só baixando 'o último'...
...
// 15.06.05 - opção de baixar o estoque somente na gravacao
...
//      Arq.TEstoqueqtde.Locate('esqt_unid_codigo;esqt_esto_codigo',Vararrayof([Unidade,Grid.Cells[grid.GetColumn('move_esto_codigo'),linha]]),[]);
//        avisoerro('Não encontrou o produto '+Grid.Cells[grid.GetColumn('move_esto_codigo'),linha]+' na unidade '+unidade);
// 22.08.06
...
// 15.01.19 - Vida Nova - Alexsandra
      else if Global.Topicos[1054] then
        Sistema.SetField('move_datamvto',Saida)
      else
        Sistema.SetField('move_datamvto',Emissao);
...
// 14.08.19  - A2z - Equipamentos  - mudança para permitir varios por nf de saida...
    if ( global.topicos[1367] ) then
       Sistema.SetField('move_equi_codigo',Grid.Cells[Grid.getcolumn('move_equi_codigo'),linha]);
// 08.11.2022 - Alutech - devolucao de venda com icms diferido
    if AnsiPos( TipoMovimento,TiposDevolucao ) > 0 then

       if TextToValor(Grid.Cells[Grid.getcolumn('move_redubase'),linha]) > 0 then

          Sistema.SetField('move_sitt_codigo',Grid.Cells[Grid.getcolumn('codsitrib'),linha]);

    Sistema.Post('');

//      Sistema.Edit('estoqueqtde');
//      Sistema.Setfield('esqt_dtultvenda',Emissao);
// 13.06.05 - opção de baixar o estoque somente na gravacao
//      if ( Global.Topicos[1201] ) and ( pos( TipoMovimento,Global.TiposMovMovEstoque ) > 0  ) then begin

// 13.04.18 - benato - para nao baixar estoque
      if ( Global.Topicos[1201] ) and ( pos( TipoMovimento,Global.TiposMovMovEstoque ) > 0  )
          and
          ( not Global.Topicos[1234] )
        then begin
...
/////////////////////////////////////////////////////////////////////////////
// 22.04.09 - colocado opção de cor e tamanho na nota de saida
...
// 18.06.19 - calculo 'por dentro'
//        totalitem:=totalitem+FGEral.Arredonda( totalitem/((100-Peracre)/100) ,2  );
// 06.08.19  - pois ja 'embute' o valor do acrescimo...senao soma duas vezes 'o capital'
        totalitem:=FGEral.Arredonda( totalitem/((100-Peracre)/100) ,2  );
...
// 15.12.11 - lançar nota de servico serie F na mesma opcao de nf modelo 1
...
// 13.07.06
  ...
  // codigo de valores fiscais ( 1,,5 da impressao do livro fiscal )
//        Sistema.SetField('Movb_CVF',cvf);    // checar - talvez nao precise
// ver como somar o frete + seguro no valor contabil agora o somente na geração para o livro fiscal
        if (CodigoMov=FGeral.GetConfig1Asinteger('ConfNfComple')) and
...
Var Q,QUnidade:TSqlquery;
    Especie,serie,mensagemfixa,TipoSaidaAbate,MensagemCliente,TransacaoContax,clifor,
    tipocad,campos,valores,
    xsetores:string;
    posmsgcliente,debito,credito,
    i:integer;
...
// 11.11.2021
    if ( Global.Topicos[1519] ) and ( FGeral.GetConfig1AsString('clieconsumidor') <> Cliente.text )   then

       mensagemfixa := Cliente.text + ' - '+ Cliente.ResultFind.fieldbyname('clie_nome').AsString;
...
// 09.11.05 - 12.08.08 + RI
    if pos(Tipomovimento,global.coddevolucaocompra+';'+global.coddevolucaocompraSemestoque+';'+Global.CodRemessaConserto+';'
      ...
      +';'+Global.CodNfeComplementoIPI)>0 then begin
      Sistema.SetField('moes_estado',Cliente.ResultFind.fieldbyname('forn_uf').AsString);
...
// 15.01.19 - Vida Nova - Alexsandra
      if Global.Topicos[1054] then
        Sistema.SetField('moes_datamvto',Saida)
      else
        Sistema.SetField('moes_datamvto',Emissao);
    end else
      Sistema.SetField('moes_datamvto',Emissao);
...
// 11.11.2021
    if Global.Topicos[1519] then

       Sistema.SetField('moes_clie_codigo',FGeral.GetConfig1AsString('clieconsumidor'));
...
//    end;
// 15.02.10 - notas com serviços e produtos
    Sistema.SetField('moes_baseiss',valorservicos);
    Sistema.SetField('moes_valorpis',retpis);
    Sistema.SetField('moes_valorcofins',retcofins);
// 12.06.17 - SM
    Sistema.SetField('moes_valorir',retir);
    Sistema.SetField('moes_valorcsl',retcsl);
    if valoriss>0 then begin
      Sistema.SetField('moes_valoriss',valoriss);
      Sistema.SetField('moes_periss',perciss);
    end else begin
      Sistema.SetField('moes_valoriss',retiss);
...
// 19.09.16
   ...
   else if ( Movimento > 1 )  then
      Sistema.SetField('movf_transacaocontax',transacaocontax);

// 01.09.2022
   if (Global.topicos[1520]) then
      Sistema.SetField('movf_port_codigo',Port_codigo);
...
// 02.12.16 - unid_caixa é da contabilidade nao do sac
      caixafilial:=QUnid_codigo.fieldbyname('unid_contacontabil').asinteger;
// lançamento simples
      campos:='mcon_transacao,mcon_operacao,mcon_status,mcon_unid_codigo,mcon_pcon_conta,mcon_datamvto,mcon_datalcto,mcon_dc,'+
              'mcon_valor,mcon_zeramento,mcon_hist_codigo,mcon_complemento,mcon_numerodcto';
      clifor:=Complehist;
...
// 05.12.16 - contabilizando usando as 'contas correntes' de cada plano
            if ContaTrans>0 then
               credito:=QUnid_codigo.fieldbyname('Unid_ctbtransnumecre').asinteger;
...
// Novicarnes - Leonir - lançar a baixa de venda a associado no ativo usando o campo conta01 do cad. de cliente
                   end else if FCadcli.Getecooperado(codforne) then begin
                     credito:=FCadcli.GetContaExp(codforne,'','XY',tipomov);
                   end else begin
...
// 05.12.16 - contabilizando usando as 'contas correntes' de cada plano
            if ContaTrans>0 then debito:=QUnid_codigo.fieldbyname('Unid_ctbtransnumecre').asinteger
            else if ( Global.Topicos[1253] ) and (tipomov=Global.CodPendenciaFinanceira) then begin
               codforne:=tipo_codigo;
               tipocad:=TipoCad;
               if codforne>0 then begin
                 if tipocad='C' then begin
...
// Novicarnes - Leonir - lançar a baixa de venda a associado no ativo usando o campo conta01 do cad. de cliente
                     ...
                     debito:=FCadcli.GetContaExp(codforne,'','XY',tipomov);
                   end else begin
...
// 04.10.16 - para o caso do 'fornecedor 24' q nao tem conta contabil
  ...
  // conta de compensacao
      ...
      if (debito>0) and (credito>0) and (debito=credito)  then begin
        Avisoerro('Contas de débito e crédito igual a '+inttostr(debito)+'.  Sistema será finalizado.' );
        Application.Terminate;
      end;
...
// 05.12.17
procedure TFGeral.GravaMovimentoEstoque(xTransacao,Codigo, UNidade, EntSai,  TipoMovimento: string;xNumero,Cliente:integer;
          Qtde: currency; Q: TSqlquery; Qtdeprev,  Pecas: currency);
///////////////////////////////////////////////////////////////////////
var TEstoqueQTde,TEstoque,Qcliente:TSqlquery;
    rtipocad:string;
begin

      TEstoque:=sqltoquery('select * from estoque where esto_codigo='+Stringtosql(codigo));
      TEstoqueQtde:=sqltoquery('select * from estoqueqtde where esqt_esto_codigo='+Stringtosql(codigo)+
                               ' and esqt_unid_codigo = '+Stringtosql(Unidade)+
                               ' and esqt_status = ''N''');
      QCliente:=sqltoquery('select * from clientes where clie_codigo='+inttostr(cliente));
      rTipocad:='C';
      Sistema.Insert('Movestoque');
      Sistema.SetField('move_esto_codigo',Codigo);
{
      Sistema.SetField('move_tama_codigo',strtointdef(Grid.Cells[Grid.getcolumn('codtamanho'),linha],0));
      Sistema.SetField('move_core_codigo',strtointdef(Grid.Cells[Grid.getcolumn('codcor'),linha],0));
      Sistema.SetField('move_copa_codigo',strtointdef(Grid.Cells[Grid.getcolumn('codcopa'),linha],0));
      ...
      Sistema.SetField('move_operacao',FGeral.GetOperacao);
      Sistema.SetField('move_numerodoc',xnumero);
      Sistema.SetField('move_status','N');
      Sistema.SetField('move_tipomov',TipoMovimento);
      Sistema.SetField('move_unid_codigo',Unidade);
      Sistema.SetField('move_tipo_codigo',Cliente);
      if rtipocad='F' then begin
        Sistema.SetField('move_tipocad','F');
      end else begin
        Sistema.SetField('move_tipocad','C');
        Sistema.SetField('move_repr_codigo',QCliente.FieldByName('clie_repr_codigo').AsString);
      end;
      Sistema.SetField('move_qtde',qtde);
      Sistema.SetField('move_datalcto',Sistema.Hoje);
...
///      Sistema.SetField('move_cst',Grid.Cells[grid.getcolumn('move_cst'),linha]);
      ...
      Sistema.SetField('move_usua_codigo',Global.Usuario.codigo);
      Sistema.SetField('move_perdesco',0);
      Sistema.SetField('move_vendabru',TEstoqueQtde.fieldbyname('esqt_vendavis').ascurrency);
//      Sistema.SetField('move_remessas',remessas);
      Sistema.SetField('move_clie_codigo',Cliente);
      Sistema.SetField('move_aliipi',0);
      Sistema.SetField('move_pecas',pecas);
      Sistema.SetField('move_vendamin',TEstoqueQtde.fieldbyname('esqt_vendamin').ascurrency);
...
}
{
    ...
    Sistema.SetField('moes_operacao',GetOperacao);
    Sistema.SetField('moes_status','N');
    Sistema.SetField('moes_numerodoc',xNumero);
//    Sistema.SetField('moes_romaneio',Romaneio);
    Sistema.SetField('moes_tipomov',TipoMovimento);
    Sistema.SetField('moes_comv_codigo',0);
    Sistema.SetField('moes_unid_codigo',Unidade);
    Sistema.SetField('moes_tipo_codigo',Cliente);
    if pos(Tipomovimento,global.coddevolucaocompra+';'+global.coddevolucaocompraSemestoque+';'+Global.CodRemessaConserto+';'
      +Global.CodRemessaDemo+';'+Global.CodDevolucaoSaida+';'+Global.CodRemessaInd+';'+TiposFornec+';'
      +';'+Global.CodNfeComplementoIPI)>0 then begin
      Sistema.SetField('moes_estado',QCliente.fieldbyname('forn_uf').AsString);
      Sistema.SetField('moes_tipocad','F');
      Sistema.SetField('moes_cida_codigo',QCliente.fieldbyname('forn_cida_codigo').AsInteger);
      rtipocad:='F';
    end else begin
      rtipocad:='C';
      Sistema.SetField('moes_estado',QCliente.fieldbyname('clie_uf').AsString);
      Sistema.SetField('moes_tipocad','C');
      Sistema.SetField('moes_repr_codigo',QCliente.fieldbyname('clie_repr_codigo').AsString);
      Sistema.SetField('moes_cida_codigo',QCliente.fieldbyname('clie_cida_codigo_com').AsInteger);
    end;
    Sistema.SetField('moes_datalcto',Sistema.Hoje);
    campo:=Sistema.GetDicionario('movesto','moes_datasaida');
    if Campo.Tipo<>'' then begin
...
begin
/////////////////////////////////////////////
  if trim(condicao)='' then exit ;   // para permitir a "não geração" de pendência financeira
  ListaPrazos:=TStringList.Create;
  n:=FCondPagto.GetPrazos(Condicao,ListaPrazos);
// ver se for a vista jogar na conta caixa ( configuração na unidade )
  nparcelas:=FCondPagto.GetNumeroParcelas(Condicao);
...
// 22.02.16
  ...
  if campoconta.tipo<>'' then begin
     ...
     if contaavista=0 then contaavista:=Arq.TUnidades.fieldbyname('unid_contacontabil').asinteger;
                                                 ...
                                                 // 06.02.19
  if ( FCondPagto.GetPrimeiroPrazo(Condicao)=0 ) and ( status <> 'H' ) then begin
    ...
    if pos(TipoMovimento,Global.CodDevolucaoVenda+';'+Global.CodDevolucaoRoman+';'+Global.CodDevolucaoIgualVenda+';'+Global.CodDevolucaoVendaConsig )>0then begin
      ...
      codhist:=0;  //  ver de onde pegar hist. de devolução de venda
      complehist:='Devolução de Venda '+inttostr(numero)+' '+FGeral.GetNomeRazaoSocialEntidade(clifor.asinteger,tipocad,'R');
...
//    end else if TipoMovimento=Global.CodDevolucaoCompra then begin  // 14.09.05
      ...
      codhist:=0;  //  ver de onde pegar hist. de devolução de compra
      complehist:='Devolução de Compra '+inttostr(numero)+' '+FGeral.GetNomeRazaoSocialEntidade(clifor.asinteger,tipocad,'R');
//      contarecdes:=Global.CodContaDevVenda;
...
// 03.02.16 - Coorlaf pitanga...lançava pendencia a vista e caia na conta de despesa de  receitas...
...
///////////////////////////////////////////
        ...
        else if (Global.Topicos[1520]) and ( EntSai='E' ) then
          ...
          FGeral.GravaMovFin(Transacao,UNidade,EntSai,complehist,texttodate(FGeral.tirabarra(Gridparcelas.cells[Gridparcelas.getcolumn('pend_datavcto'),1])),Movimento,
            texttodate(FGeral.tirabarra(Gridparcelas.cells[Gridparcelas.getcolumn('pend_datavcto'),1])),Numero,codhist,0,contaavista,valorparcela,contarecdes,
            tipomovimento,0,0,'',status,'','',0,Gridparcelas.cells[Gridparcelas.getcolumn('portador'),1])
...
{  // usando o percentual de entrada definido na condição de pagamento
  if FCondPagto.GetPerEntrada(Condicao)>0 then begin
...
// 11.05.20
        if ( Global.Topicos[1513] ) and ( GridParcelas <> nil  ) and  ( PR = 'P' ) then

           if Gridparcelas.getcolumn('pend_codbarra') <> -1 then

              Sistema.SetField('Pend_codbarra',Gridparcelas.cells[Gridparcelas.getcolumn('pend_codbarra'),p]);
...
// 20.11.17 - nao gravava pend_datacont...
          Sistema.SetField('Pend_DataCont',Movimento);
  ...
  //      Sistema.SetField('Pend_UsuAutPgto','');
        ...
        Sistema.SetField('Pend_LoteCNAB',0);  // no. lote para exportação bancária (CNAB )
        if status='D' then
          Sistema.Setfield('Pend_Opantecipa',opantecipa);   // nro da antecipaçao usada na baixa do titulo
  // 08.09.05
...
function TFGeral.FormataData(data: TDatetime ; ano:boolean=false): string;
//////////////////////////////////////////////////////////////////////////////
begin
...
// 22.10.19
    ...
    begin
      ...
      achou:=false;
      for xp := 0 to ListaIguais.Count-1 do begin

          PCodigosIguais:=ListaIguais[xp];
          if PCodigosIguais.codigo = Grid.Cells[Grid.getcolumn('move_esto_codigo'),linha] then begin
             achou:=true;
             break;
          end;
      ...
      if achou then begin
         ...
         PCodigosIguais.qtde:=PCodigosIguais.qtde + qtde;
         PCodigosIguais.qtdeultima:=qtde;

      end else begin

         New( PcodigosIguais );
         PCodigosIguais.codigo :=Grid.Cells[Grid.getcolumn('move_esto_codigo'),linha];
         PCodigosIguais.qtde:=qtde;
         PCodigosIguais.qtdeultima:=qtde;
         ListaIguais.add( PCodigosIguais );
...
//      Sistema.SetField('move_tipocad','C');
      ...
      if pos(TipoMovimento,Global.CodDevolucaoRoman+';'+Global.CodDevolucaoVenda+';'+Global.CodDevolucaoVendaConsig+';'+
             Global.CodCompraProdutor+';'+Global.CodCompraProdutorReclassifica+';'+
             Global.CodEntradaProdutor+';'+Global.CodEntradaNFPe)>0 then begin
...
//        Sistema.SetField('move_tipocad','F');
        if total=0 then begin
          Sistema.SetField('move_qtde',texttovalor(Grid.Cells[grid.getcolumn('move_qtde'),linha]));
// 22.07.2022 - ver pra gravar o custo 'da ambev' já pronto e com ST..
          if Global.Topicos[1483] then begin

             if texttovalor(Grid.Cells[grid.getcolumn('custocomst'),linha])>0 then
                Sistema.SetField('move_venda',texttovalor(Grid.Cells[grid.getcolumn('custocomst'),linha]));
...
// 09.10.13 - Patoterra
      Sistema.SetField('move_remessas',Grid.Cells[Grid.getcolumn('move_operacao'),linha]);
// 04.07.19 - Seip Brasil - imposto de importacao
      if TipoMovimento=Global.CodDrawBackEnt then
         Sistema.SetField('move_aliii',Grid.Cells[Grid.getcolumn('voutro'),linha]);
// 01.08.19
    if ( trim(cequi_codigo)<>'' ) and ( global.topicos[1367] ) then
       Sistema.SetField('move_equi_codigo',cequi_codigo);
...
//////////
// 02.04.08 - Atualizacao do custo da mao de obra do produto  'ES' - retorno da industria cobrando os serviços
/////////////////////////////////////////////////////////////////////////////////////////
...
// 04.09.06 - coddevolucaoind=RN é compra de materia prima portanto nao precisa explodir
...
//
// 08.10.07 - 02.11.07 - retirado pois esta parte será feita na opção 'desossa'
...
// 14.02.14 - retirado apra atualiza o custo e preço de venda quando for o caso
//      Sistema.Edit('estoqueqtde');
//      Sistema.Setfield('esqt_dtultcompra',Emissao);
//      Sistema.Post('esqt_status=''N'' and esqt_unid_codigo='+stringtosql(Unidade)+' and esqt_esto_codigo='+
//                     stringtosql(Grid.Cells[grid.GetColumn('move_esto_codigo'),linha]) );
...
// 18.11.04
      ...
      base:=totalitem;    // para guardar a base sem a redução
// 23.10.07
      ycfop:=xcfop;
...
// 11.07.13 - SM
        if pos(Grid.Cells[grid.getcolumn('move_natf_codigo'),linha],'1933/2933')>0 then
          FGeral.GeraListaCstPerc(Grid.Cells[grid.getcolumn('move_cst'),linha],pericms,
                       valorcontabil,base,reducao,isentas,outras,basesubs,ListaCstPerc,'S',ycfop,valoricms)
        else
          FGeral.GeraListaCstPerc(Grid.Cells[grid.getcolumn('move_cst'),linha],pericms,
                       valorcontabil,base,reducao,isentas,outras,basesubs,ListaCstPerc,'I',ycfop,valoricms);
      end;
/////// 18.10.10
...
///        Sistema.SetField('Movb_Codigosfis',     'Simb',  talvez nao precise pois ja gravo o % icms
        ...
        Sistema.SetField('Movb_TpImposto',Pcstperc.tpimposto );
        if AnsiPOs(Tipomovimento,Global.CodNfeComplementoIcmsE)>0 then begin
          Sistema.SetField('Movb_BaseCalculo',0);
          Sistema.SetField('Movb_Imposto',pcstperc.vicms);
// 18.03.19
        end else if AnsiPOs(Tipomovimento,Global.CodNfeComplementoIcmsECliente)>0 then begin
          Sistema.SetField('Movb_BaseCalculo',pcstperc.base);
          Sistema.SetField('Movb_Imposto',pcstperc.vicms);
        end else begin
          Sistema.SetField('Movb_BaseCalculo',Pcstperc.base);
...
// para nao lançar as compras a vista
            debito:=0;
            credito:=0;
            cdebito:='';
            ccredito:='';
...
// só hj ela finalmente disse q tinha q criar uma conta de dev. para cada fornec.
        if Global.Topicos[1253] then begin
...
// 06.06.12 - Benato -> SM - lançamento dos cupons fiscais
...
//            entao foi retirado isto pq se tá setado o 1253 é pra avisar q nao tem...
...
// 05.11.10 - para lançar as vendas a vista
          if Global.Topicos[1018] then begin
            debito:=QUnid_codigo.fieldbyname('unid_caixa').asinteger;
            credito:=QUnid_codigo.fieldbyname('Unid_vendaavista').asinteger;
          end else begin
// 04.05.05 - para nao lançar as vendas/compras a vista
            debito:=0;
            credito:=0;
          end;
          cdebito:=inttostr(debito);
...
// para nao lançar as vendas/compras a vista
                debito:=0;
                credito:=0;
              end;
             ccredito:=inttostr(credito);
...
// 22.08.08
        ...
        if Vistaprazo='V' then begin
// 05.11.10 - para lançar as compras a vista
              if Global.Topicos[1018] then begin
                debito:=QUnid_codigo.fieldbyname('unid_comprasavista').asinteger;
                credito:=QUnid_codigo.fieldbyname('unid_caixa').asinteger;
              end else begin
...
// 03.04.07 - clessi
    Sistema.SetField('moes_pesoliq',pesoliq);
// 20.11.18 - Seip
    Sistema.SetField('moes_pesobru',xpesobru);
    Sistema.SetField('Moes_qtdevolume',xQTdevol);
    Sistema.SetField('Moes_especievolume',xEspecievol);
// 12.12.07
...
// 27.02.18
           xmlstring:=ACBrNFe1.NotasFiscais.Items[0].XML;
...
// 17.02.15 - para tentar eviar Casa Nova q grava xml de entrada mas fica sem chave
          ...
          else if trim(ACBrNFe1.NotasFiscais.Items[0].NFe.infNFe.ID)<>'' then
            Sistema.setfield('moes_chavenfe',copy(ACBrNFe1.NotasFiscais.Items[0].NFe.infNfe.id,4,44))
          else
            Sistema.setfield('moes_chavenfe',copy(ACBrNFe1.NotasFiscais.Items[0].XML,pos('NFe',ACBrNFe1.NotasFiscais.Items[0].XML)+3,44) );

        end;

      end else if (xchavecte<>'') and ( copy(Tipodoc,1,3)='CTE' ) then begin  // 12.03.15 - CTE

         Sistema.setfield('moes_chavenfe',xchavecte);
// 10.09.18   - Vida Nova - Dani
         xXMLCTE:=FGeral.TiraBarra(xXMLCTE,chr(39),'"');
// 22.11.17
         Sistema.setfield('moes_xmlnfet',xXMLCTE) ;

      end else if (xchavecte<>'') and ( Tipodoc='NFE' ) then   // 14.08.15 - nferef das devolucoes
...
//////////////////
// 20.08.20 - divisao por conta de despesa
    if contagerencial=9999 then begin
      xsetores:='';
      for i:=1 to xGrid.rowcount do begin
         if trim(xGrid.cells[xGrid.getcolumn('codigo'),i])<>'' then xsetores:=xsetores+xGrid.cells[xGrid.getcolumn('codigo'),i]+';'+xGrid.cells[xGrid.getcolumn('valor'),i]+'|';
      end;
      if length(trim(xsetores))>5 then Sistema.Setfield('moes_devolucoes',xsetores);
    end;
//////////////////
...
// 13.09.16
    ...
    Sistema.SetField('moes_valoriss',xvlriss);
// 20.06.19 - Vida Nova
    if ( Ansipos(Tipomovimento,global.Codcompraprodutor)>0 ) and  ( global.topicos[1462] ) then
       Sistema.SetField('moes_insumos',xinsumos);
// 01.08.19
    if ( trim(cequi_codigo)<>'' ) and ( global.topicos[1367] ) then
       Sistema.SetField('moes_equi_codigo',cequi_codigo);
...
// 13.09.16
      ...
      //////////////////////////////////////////////////////////////////////
// inicio do multiplo em caso de entrada de prestação de serviços
        vlrretencoes:=xvlrpis+xvlrcofins+xvlrir+xvlrinss+xvlrcsll+xvlriss;
        valores:=Stringtosql(transacaocontax)+','+Stringtosql(transacaocontax+'1')+','+Stringtosql('N')+','+Stringtosql(Unidade)+','+Stringtosql(inttostr(debito))+','+Datetosql(Entrada)+','+Datetosql(Sistema.Hoje)+','+
                 Stringtosql('D')+','+Valortosql(valortotal)+','+Stringtosql('2')+','+Stringtosql(' ')+','+Stringtosql('NF '+inttostr(numero)+' '+clifor)+','+
                 Stringtosql(inttostr(NUmero));
        ...
        if (credito>0) and (xvlrir>0) then
          SistemaContax.ExecuteDirect(' Insert into movcon ('+campos+') values ('+valores+')' );
{    - ver inss é conta propria pra inss retido q só ocorre se tiver nf de mao de obra de construcao civil
// 22.09.16 - ver se é necessario criar
        credito:=QUNid_codigo.fieldbyname('unid_containss').asinteger;
       ...
       FGeral.fechaquery(qunid_codigo);
// 16.01.17
        credito:=QUNid_codigo.fieldbyname('unid_contaissret').asinteger;
        historico:=strspace('ISS Retido NF '+inttostr(numero)+' '+clifor,maximocomple);
        valores:=Stringtosql(transacaocontax)+','+Stringtosql(transacaocontax+'7')+','+Stringtosql('N')+','+Stringtosql(Unidade)+','+Stringtosql(inttostr(credito))+','+Datetosql(Entrada)+','+Datetosql(Sistema.Hoje)+','+
                 Stringtosql('C')+','+Valortosql(xvlriss)+','+Stringtosql('2')+','+Stringtosql(' ')+','+Stringtosql(historico)+','+
                 Stringtosql(inttostr(NUmero));
        if (credito>0) and (xvlriss>0) then
          SistemaContax.ExecuteDirect(' Insert into movcon ('+campos+') values ('+valores+')' );
       FGeral.fechaquery(qunid_codigo);
...
{  18.08.05
    ...
    if Arq.TRepresentantes.locate('repr_codigo',repr.text,[]) then
      ...
      Avisoerro('Não encontrado representante '+repr.text+' no cadastro');
}
    if perc>0 then
   ...
   if ( trim(FGeral.GetConfig1AsString('DtInicioMvto'))<>'' ) and
     ...
     if ( datadig < FGeral.GetConfig1AsDate('DtInicioMvto') ) or
        ( datadig > FGeral.GetConfig1AsDate('DtTerminoMvto') ) then
         Avisoerro('Período não pode ser alterado')
     else
        result:=true;
   end else
     result:=true;
...
// 02.06.17
// calcular saldos de contas de despesa/receita
function TFGeral.SaldoAnteriorRecDes(conta: integer; unidade, campo: string; DataInicial: TDatetime): currency;
/////////////////////////////////////////////////////////////////////////////////////////////////////
var QBusca,QBuscaInicio:TSqlquery;
    saldo:currency;
    mesano:string;
...
begin
  ...
  Sistema.beginprocess('Calculando saldo anterior');
    ...
    QBusca:=sqltoquery('select * from movfin where movf_status=''N'' and movf_datamvto<'+DAtetosql(datainicial)+
                       ' and movf_plan_contard='+stringtosql(inttostr(conta))+
                       ' and '+FGeral.getin('movf_unid_codigo',unidade,'C')+
                       ' order by movf_datamvto' );
// 05.09.19 - para prever as baixas parciais q ficam com datamvto em data diferente da 'original' do titulo
//            dai calcula juros a menor...
//                       ' order by movf_datacont' );

    ultimadata:=Sistema.MenorData;
    ...
    while not QBusca.Eof do begin
      ...
      ultimadata:=QBusca.FieldByName('movf_datamvto').AsDateTime;
// 05.09.19 - para prever as baixas parciais q ficam com datamvto em data diferente da 'original' do titulo
//      ultimadata:=QBusca.FieldByName('movf_datacont').AsDateTime;
      QBusca.next;
  ...
  Sistema.endprocess('');
// 04.07.17 - isonel
  if trunc( DataInicial-ultimadata ) > 31 then
    saldo:=0;
...
// 19.12.19 - Fama - Janina - em alguns itens grava preço de venda 'nada a ver' na filial
        Sistema.Setfield('esqt_vendavis',Texttovalor(Grid.Cells[grid.getcolumn('precovenda'),linha]));
...
// - 17.11.05 - para evitar q fique tributação errada quando transfere novos produtos da matriz para filiais
...
///// 19.12.19 - estava 'duas vezes' no esqt_vendavis....
        Sistema.Setfield('esqt_usua_codigo',Global.Usuario.codigo);
        Sistema.Post('');
////////        Sistema.Commit;
      end else begin  // 25.01.06 fazer o custo medio caso na filial já existir o produto...rever...
      end;
...
//      margemlucro:=FCodigosFiscais.GetPercBaseSubs(FEstoque.GetCodigoFiscal(Grid.Cells[grid.GetColumn('move_esto_codigo'),linha],Unidade,UnidDestino.resultfind.fieldbyname('unid_uf').asstring));
      margemlucro:=0 ; // transf. não tem substituição trib.
      if Margemlucro>0 then
        basesubs:=base*(1+(margemlucro/100))
      else
        basesubs:=0;
...
// 13.02.06 - +';'+Global.CodVendaSerie4 - ver, se permitir terá q gravar o nro do romaneio
...
// 05.11.19
function TFGeral.GetReduzidooutraUnidadeContax(pconta: integer;xunidade:string): integer;
////////////////////////////////////////////////////////////////////////////
var Q        : TSqlquery;
    xconta   : integer;
    xclass,
...
begin
   ...
   Q:=SqlToQuerycontax('SELECT pcon_classificacao FROM PlanoCon WHERE pcon_conta = '+IntTostr(pconta) );
   result:=0;
   if not Q.Eof then begin

      xclass := Q.FieldByName('pcon_classificacao').AsString;
      xempresa:=copy(xunidade,2,2);
      Q.Close;
      Q:=SqlToQuerycontax('SELECT pcon_conta FROM PlanoCon WHERE pcon_classificacao = '+Stringtosql(xclass)+
                          ' and pcon_empr_codigo = '+Stringtosql(xempresa) );
      if not Q.Eof then result:=Q.FieldByName('pcon_conta').AsInteger;
...
// se for o caso gerar nf venda com a diferenca - neste caso sempre será o caso, a partir do retorno do romaneio
////////////////////////////////////////////////
      if not Arq.TEstoque.Active then Arq.TEstoque.Open;
      if not Arq.TEstoqueQtde.Active then Arq.TEstoqueQtde.Open;
      if not Arq.TConfMovimento.Active then Arq.TConfMovimento.Open;
...
// verificar se é a vista com apenas uma parcela OU se somente a primeira parcela é a vista ( ou parte do total )
    ...
    //      Sistema.SetField('Pend_CNPJCPF',EdCliente.Resultfind('clie_cnpjcpf);
          ...
          Sistema.SetField('Pend_RP','R');
          if p=nparcelas then
            valorparcela:=valorvenda-acumulado  // para deixar na ultima parcelas "as dízimas"
          else
...
// 02.05.05 - encontrava as DC somente quando não fechava o acerto
...
// 24.05.06 - quase certo q o anta estava com a base no notebook e o pgadmin no servidor..tsc, tsc
  ...
  if QBusca.Eof then begin  // 29.04.05 - é aqui q ijui diz q nao encontra no ret. de consig.
    Avisoerro(Tipomov+' numero '+inttostr(Numero)+' não encontrado');
    exit;
  end;
  if not Confirma('Confirma impressão do retorno/devolução ?') then exit;

// 11.09.17
  if Global.Topicos[1011] then begin
    FImpressao.ImprimeRemessa(QBusca.fieldbyname('moes_numerodoc').AsInteger,QBusca.fieldbyname('moes_datamvto').AsDatetime,QBusca.fieldbyname('moes_unid_codigo').Asstring,Global.CodDevolucaoTroca);
    Sistema.EndProcess('');
    QBusca.Close;
    Freeandnil(QBusca);
    exit;
  ...
  if tipomov=Global.CodDevolucaoConsig then
    tit:='parcial de Consignação'
  else if (TipoMov=Global.CodDevolucaoTroca) then
    tit:='de troca de Consignação'
  else
    tit:='de Pronta Entrega';
  FTextRel.AddTitulo(FGeral.Centra('Devolução '+tit,largura),true,False,false);
...
// 15.02.05
  ...
  FTextRel.AddLinha('Razão Social : '+strspace(QClientes.Fieldbyname('clie_razaosocial').Asstring,43)+
                    'Tel.:'+FGeral.Formatatelefone(QClientes.Fieldbyname('clie_foneres').Asstring)
                    ,false,False,false);
  FTextRel.AddLinha('Endereço.....: '+strspace(QClientes.Fieldbyname('clie_endres').Asstring,40)+space(03)+
  ...
  if Global.Usuario.OutrosAcessos[3309] then  // 26.08.10 - impressora laser/usb
    FTextRel.AddLinha(space(04)+'Codigo'+space(11)+'Quantidade'+space(5)+'Devolução'+space(2)+'Descrição'+space(22)+'Peças Vendidas'+
                    space(01)+'Vlr. Unitário'
                    ,false,False,true)
  else
    FTextRel.AddLinha(space(04)+'Codigo'+space(12)+'Quantidade'+space(5)+'Devolução'+space(2)+'Descrição'+space(36)+'Peças Vendidas'+
                    space(01)+'Vlr. Unitário'
                    ,false,False,true);
  FTextRel.AddLinha(Replicate('-',largura-FTextRel.MargemEsquerda),false,False,false);
  tqtde:=0;titem:=0;
  ...
  while not QBusca.Eof do begin
    ...
    if Global.Usuario.OutrosAcessos[3309] then  // 26.08.10 - impressora laser/usb
      FTextRel.AddLinha(space(04)+strspace(produto,13)+space(03)+
                    ...
                    space(02)+replicate('_',11)+FGeral.Formatavalor(venda,'#,###,##0.00')
                    ,false,False,true)
    else
      FTextRel.AddLinha(space(04)+strspace(produto,13)+space(03)+
                    FGeral.Formatavalor(pqtde,'###,##0.000')+space(02)+
                    replicate('_',12)+space(02)+strspace(FEstoque.GetDescricao(produto),45)+
                    space(02)+replicate('_',11)+FGeral.Formatavalor(venda,f_cr)
                    ,false,False,true);
    inc(titem);
  end;
...
//  end;
//  if tdescacre>0 then
//    FTextRel.AddLinha(space(39)+'Total líquido........: R$ '+FGeral.Formatavalor(liquido,'###,##0.00'),false,False,false);
...
//        Global.Usuario.ContasAutPgto:=';'+FPlanoGer.GetContasSubordinadas(Trim(Arq.TUsuarios.FieldByName('Usua_ContasAutPgto').AsString),'CR,CP');
    ...
    Avisoerro('Usuário '+inttostr(usuariopadrao)+' não encontrado');
...
begin
  ...
  if not Q.eof then begin
    Avisoerro('ATENÇÃO !!! Cliente com remessas/devoluções pendentes na unidade '+Q.fieldbyname('move_unid_codigo').asstring);
  end;
  Q.close;
  Freeandnil(Q);
...
begin
   ...
   Global.CstTransferencia:='051';  // 06.07.05  // 20.09.19
// 27.10.20 - Novicarnes - Ketlen + Simone
//   Global.CstTransferencia:='041';
// 29.10.20 - mudado pra usar o q tiver configurado no produto até .... ?
...
//  Arq.TClientes.Locate('clie_codigo',QBusca.Fieldbyname('Moes_tipo_codigo').AsINteger,[]);
  ...
  FTextRel.AddLinha('Cliente......: '+QClientes.fieldbyname('clie_nome').AsString,false,False,false);
  FTextRel.AddLinha('Codigo.......: '+QClientes.Fieldbyname('clie_codigo').Asstring,false,False,false);
  FTextRel.AddLinha('Razão Social : '+strspace(QClientes.Fieldbyname('clie_razaosocial').Asstring,37)+
                    'Tel.:'+FGeral.Formatatelefone(QClientes.Fieldbyname('clie_foneres').Asstring)
                    ,false,False,false);
...
begin
  ...
  if codigo>0 then begin
    if tipo='R' then
      ...
      result:=' - Veículo : '+inttostr(codigo)+' - '+FTransp.Getnome( strzero(codigo,3) )
    else
...
begin
  result:=false;
{ - 05.05.06 - retirado pois desde 15/04/06 o extrato recalcula todo o saldo desde o primeiro lançamento
  mesano:=strzero(Datetoano(Datalan,true),4)+strzero(Datetomes(DAtalan),2);
...
//  tipomov:='PV';
  tipomov:=Global.CodPedVenda+';'+Global.CodPedVendaMostruario+';'+Global.CodPedVendaMostruarioConsig+';'+
           Global.CodPedVendaPE+';'+Global.CodOrdemdeServico+';'+Global.CodOrcamento;
  sqlsituacao:='';
...
//  if (simples='S') and (cst<>'000') then begin
  if (simples='S') and ( pos(cst,cstsimples)=0 ) then begin
    Avisoerro('Produto com codigo de situação tributária diferente de '+cstsimples+' para empresa do Simples');
    result:=false;
  end;
...
// 06.03.2023
function TFGeral.Checaetraznumfalta(numero: integer; serie,especie,unidade: string;datareferencia: TDatetime): integer;
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
var Qx:TSqlquery;
    Data:TDatetime;
    numnota,
    ...
    achou:boolean;
begin
    Data:=datareferencia-03;
    Qx:=sqltoquery('select moes_numerodoc,moes_especie,moes_serie,moes_dataemissao,moes_datamvto'+
                  ' from movesto where moes_numerodoc <= '+inttostr(numero)+
                  ' and moes_numerodoc > 0 '+
                  ...
                  ' order by moes_unid_codigo,moes_especie,moes_serie,moes_numerodoc');
    ...
    if (Numeroret<NUmero) then begin
      FGeral.Gravalog(43,'Unidade '+Global.CodigoUnidade+'Numero '+inttostr(numeroret)+' Série '+serie+' usado em '+FGeral.formatadata(Datareferencia),true);
...
// numerodoc pra permitir alterar no mesmo pedido
    if (not Q.eof) and (Q.fieldbyname('mpdd_numerodoc').asinteger<>Numerodoc) then begin
      Avisoerro('Encontrado pedido de mostruário '+Q.fieldbyname('mpdd_tipomov').asstring+' no pedido '+Q.fieldbyname('mpdd_numerodoc').asstring+
     ...
     if trim(Grid.cells[Grid.Getcolumn('pend_valor'),p])<>'' then begin
       ...
       if vencimento>0 then begin
         if vencimento>=data then begin
           result:=false;
           Avisoerro('Vencimentos fora de ordem cronológica '+FGeral.FormataData(vencimento,true)+' e '+FGeral.FormataData(data,true));
           break;
         end;
       end else
         vencimento:=data;
...
begin
  Ed.Items.clear;
  Ed.Items.Add('MA - Macho' );
  Ed.Items.Add('FE - Fêmea' );
{
  Ed.Items.Add(Global.CatSofisticado+' - Sofisticado' );
  Ed.Items.Add(Global.CatBasico+' - Básico' );
...
begin
  ...
  for p:=1 to colunas do begin
   if (p=coluna1) and (p>0) then
     ...
     FRel.Addcel(dados6)
   else if (p=coluna7) and (p>0) then
     FRel.Addcel(dados7)
   else if (p=coluna8) and (p>0) then
     FRel.Addcel(dados8)
   else
     FRel.Addcel('');
  end;
...
// 04.07.06 - kelli - calculo de comissão diferenciada
...
// 08.06.11
    ...
    Q:=sqltoquery('select moes_numerodoc from movesto where moes_numerodoc='+inttostr(numero)+
                  ...
                  ' and moes_unid_codigo='+stringtosql(Global.CodigoUnidade) );
    if (Q.eof) then begin
      FGeral.Gravalog(11,'Unidade '+Global.CodigoUnidade+'Nota nro '+inttostr(numero)+' Série '+serie+' detectado em '+FGeral.formatadata(Datareferencia),true);
    end;
    FGeral.Fechaquery(Q);
end;
...
///////////////////
function TFGeral.GetValorJuros(capital: currency; vencimento, baixa: TDatetime  ; RecPag:string ; Emissao:TDatetime=0;xTipomov:string=''): currency;
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
var diasatraso,dias:integer;
    valor,multa:currency;
begin
   valor:=0;multa:=0;
   if RecPag='R' then begin  // por enquanto somente pendencias a receber

     if xTipomov=Global.CodCedulaProdutoRural then begin

       if vencimento>=Sistema.hoje then begin
          dias:=trunc(Sistema.hoje-emissao);
          diasatraso:=0;
       end else begin
          dias:=trunc(Vencimento-emissao);
          diasatraso:=trunc(Sistema.hoje-vencimento);
       end;

         if dias>0 then valor:=( capital*(( 1.5/30 )/100) )*dias;
         if diasatraso>0 then valor:=( capital*(( 2/30 )/100) )*diasatraso;
...
//  end;
  Q:=sqltoquery('select * from estgrades where esgr_status=''N'' and esgr_esto_codigo='+stringtosql(produto)+
                sqlcor+sqltamanho+sqlcopa );
  if Q.Eof then begin
    Avisoerro('Grade não encontrada com esta cor/tamanho');
    result:=false;
  end;
  FGeral.Fechaquery(Q);
...
begin
    ...
    mensagem:='Funcionário : '+inttostr(codfun)+' - '+nome+lf;
   s := 'mailto:'+destino+
      ...
      if not QUsuario.eof then begin
        if copy(QUsuario.FieldByName('Usua_OutrosAcessos').AsString,302,1)<>'S' then begin
           if copy(QUsuario.FieldByName('Usua_OutrosAcessos').AsString,315,1)<>'S' then
             Avisoerro('Usuário sem permissão para faturamento acima limite de crédito.  Solicitar autorização depois prosseguir!');
           QUsuario:=sqltoquery('select Usua_OutrosAcessos from usuarios where usua_codigo='+inttostr(usuario));
           if not QUsuario.eof then begin
             if copy(QUsuario.FieldByName('Usua_OutrosAcessos').AsString,315,1)='S' then begin
               ...
               result:=false;
               Avisoerro('Usuário Ainda sem permissão para faturamento acima limite de crédito');
             end;
           end else
             result:=false;
      ...
      if not QUsuario.eof then begin
        if copy(QUsuario.FieldByName('Usua_OutrosAcessos').AsString,706,1)<>'S' then begin
           if copy(QUsuario.FieldByName('Usua_OutrosAcessos').AsString,712,1)<>'S' then
             Avisoerro('Usuário sem permissão para faturamento com restrição de crédito.  Solicitar autorização depois prosseguir!');
           QUsuario:=sqltoquery('select Usua_OutrosAcessos from usuarios where usua_codigo='+inttostr(usuario));
           if not QUsuario.eof then begin
             if copy(QUsuario.FieldByName('Usua_OutrosAcessos').AsString,712,1)='S' then begin
               ...
               result:=false;
               Avisoerro('Usuário Ainda sem permissão para faturamento com restrição de crédito');
             end;
           end else
             result:=false;
        end else
...
begin
  ...
  try
     ...
     Avisoerro('Não foi possível gravar no cadastro de usuários');
  end;
  FGeral.FechaQuery(QUsuario);
...
begin
  ...
  if FGeral.GetConfig1AsFloat('Permaxcomi')>0 then begin
    if percomissao>FGeral.GetConfig1AsFloat('Permaxcomi') then begin
      Avisoerro('Percentual de '+floattostr(percomissao)+' é maior que o permitido');
      result:=false;
    end;
  end;
end;
...
//     sqlqualidade:=' and '+FGeral.Getin('move_certificado',TipoFSC,'C');
   ...
   if fatorconversao<=0 then begin
     Avisoerro('Fator de conversão ainda não informado nas configurações do sistema');
   end else begin
...
// 25.07.20
procedure TFGeral.ConfiguraACBrGNRE(AcbrGNREx: TAcbrGNRE ; xUnidade:string );
//////////////////////////////////////////////////////////////////////////////
begin

  AcbrGNREx.Configuracoes.WebServices.ProxyHost:='';
  AcbrGNREx.Configuracoes.WebServices.ProxyUser:='';
  AcbrGNREx.Configuracoes.WebServices.ProxyPass:='';
  AcbrGNREx.Configuracoes.WebServices.ProxyPort:='';
  if Global.Topicos[1013] then begin
    AcbrGNREx.Configuracoes.WebServices.ProxyHost:=FGeral.GetConfig1AsString('ipproxy');
    AcbrGNREx.Configuracoes.WebServices.ProxyUser:=FGeral.GetConfig1AsString('usuarioproxy');
    AcbrGNREx.Configuracoes.WebServices.ProxyPass:=FGeral.GetConfig1AsString('senhaproxy');
    AcbrGNREx.Configuracoes.WebServices.ProxyPort:=FGeral.GetConfig1AsString('portaproxy');
  end;
  acbrGNREx.Configuracoes.Certificados.NumeroSerie:=FGeral.GetNroSerieCertificado(xUnidade);

  AcbrGNREx.Configuracoes.WebServices.Visualizar:=false;
//  if trim( FGeral.GetConfig1AsString('Pastaschemas') ) <> '' then
//    AcbrGNREx.Configuracoes.Arquivos.PathSchemas:=FGeral.GetConfig1AsString('Pastaschemas')
//  else
    AcbrGNREx.Configuracoes.Arquivos.PathSchemas:=ExtractFilePath( Application.ExeName )+'SchemasGNRE';
    AcbrGNREx.Configuracoes.Arquivos.PathArqTXT :=ExtractFilePath( Application.ExeName )+'ArqTXT' ;
    if not DirectoryExists( AcbrGNREx.Configuracoes.Arquivos.PathArqTXT ) then
      ForceDirectories( AcbrGNREx.Configuracoes.Arquivos.PathArqTXT );

  AcbrGNREx.Configuracoes.Arquivos.Salvar:=False;
//  AcbrGNREx.Configuracoes.Geral.VersaoDF:=gve400;
//  AcbrGNREx.Configuracoes.Geral.ModeloDF:= monfe;
  AcbrGNREx.Configuracoes.WebServices.TimeOut:=1000;
  if FGeral.GetConfig1AsString('AmbienteNFe') = '1' then
     acbrgnrex.Configuracoes.WebServices.Ambiente:=taProducao
  else
    acbrgnrex.Configuracoes.WebServices.Ambiente:=taHomologacao;

end;

procedure TFGeral.ConfiguraACBrNFe(AcbrNfex: TAcbrNfe;qnf:string='NFe');
/////////////////////////////////////////////////////////////////////////
begin

  Acbrnfex.Configuracoes.WebServices.ProxyHost:='';
  Acbrnfex.Configuracoes.WebServices.ProxyUser:='';
  Acbrnfex.Configuracoes.WebServices.ProxyPass:='';
  Acbrnfex.Configuracoes.WebServices.ProxyPort:='';
  FGeral.ACBrIBPTax1.ProxyHost:='';
  FGeral.ACBrIBPTax1.ProxyUser:='';
  FGeral.ACBrIBPTax1.ProxyPass:='';
  FGeral.ACBrIBPTax1.ProxyPort:='';
  if Global.Topicos[1013] then begin
    Acbrnfex.Configuracoes.WebServices.ProxyHost:=FGeral.GetConfig1AsString('ipproxy');
    Acbrnfex.Configuracoes.WebServices.ProxyUser:=FGeral.GetConfig1AsString('usuarioproxy');
    Acbrnfex.Configuracoes.WebServices.ProxyPass:=FGeral.GetConfig1AsString('senhaproxy');
    Acbrnfex.Configuracoes.WebServices.ProxyPort:=FGeral.GetConfig1AsString('portaproxy');
  ...
  Acbrnfex.Configuracoes.WebServices.Visualizar:=false;
  if trim( FGeral.GetConfig1AsString('Pastaschemas') ) <> '' then
    Acbrnfex.Configuracoes.Arquivos.PathSchemas:=FGeral.GetConfig1AsString('Pastaschemas')
  else
    Acbrnfex.Configuracoes.Arquivos.PathSchemas:=ExtractFilePath( Application.ExeName )+'Schemas20';

  Acbrnfex.Configuracoes.Arquivos.Salvar:=False;

  Acbrnfex.Configuracoes.Geral.VersaoDF:=ve400;

  Acbrnfex.Configuracoes.Geral.ModeloDF:= monfe;
  FGeral.SetaTabelaIBPT;

  if qnf='NFCe' then begin
    Acbrnfex.Configuracoes.Geral.ModeloDF:= monfce;
  end;
  Acbrnfex.Configuracoes.Geral.IdCSC:='1';
  Acbrnfex.Configuracoes.Geral.CSC:=FGeral.getconfig1asstring('idtoken');
  if Global.topicos[1390] then
     Acbrnfex.Configuracoes.WebServices.TimeOut:=1000;
{
...
// 17.06.20
procedure TFGeral.ConfiguraCriptografiaAcbrCTe(AcbrCTex: TAcbrCte);
/////////////////////////////////////////////////////////////////////
begin

   AcbrCTex.Configuracoes.Geral.SSLLib        := LibWincrypt;
   AcbrCTex.Configuracoes.Geral.SSLCryptLib   := cryWinCrypt;
   AcbrCTex.Configuracoes.Geral.SSLHttpLib    := httpWinHttp;
   AcbrCTex.Configuracoes.Geral.SSLXmlSignLib := xsLibXml2;

end;

// 25.07.20
procedure TFGeral.ConfiguraCriptografiaAcbrGNRE(AcbrGNREx: TAcbrGNRE;  cUF: string);
////////////////////////////////////////////////////////////////////
begin

     AcbrGNREx.Configuracoes.Geral.SSLLib        := LibWincrypt;
     AcbrGNREx.Configuracoes.Geral.SSLCryptLib   := cryWinCrypt;
     AcbrGNREx.Configuracoes.Geral.SSLHttpLib    := httpWinHttp;
     AcbrGNREx.Configuracoes.Geral.SSLXmlSignLib := xsLibXml2;
//     AcbrGNREx.Configuracoes.WebServices.SSLType := LT_all;
     AcbrGNREx.Configuracoes.WebServices.SSLType := LT_TLSv1_2;  // 29.04.2022

end;

// 23.05.19
procedure TFGeral.ConfiguraCriptografiaAcbrMdfe(AcbrMdfex: TAcbrMdfe);
////////////////////////////////////////////////////////////////////
begin

   AcbrMdfex.Configuracoes.Geral.SSLLib        := LibWincrypt;
   AcbrMdfex.Configuracoes.Geral.SSLCryptLib   := cryWinCrypt;
   AcbrMdfex.Configuracoes.Geral.SSLHttpLib    := httpWinHttp;
   AcbrMdfex.Configuracoes.Geral.SSLXmlSignLib := xsLibXml2;
   AcbrMdfex.Configuracoes.WebServices.SSLType := LT_TLSv1_2;  // 29.04.2022
//   AcbrMdfex.Configuracoes.WebServices.SSLType := LT_all;

end;

// 04.08.18
procedure TFGeral.ConfiguraCriptografiaAcbrNfe(AcbrNfex: TAcbrNfe; cUF:string='PR');
////////////////////////////////////////////////////////////////////////////////////////
begin
{
   if AnsiPos(cUF , 'SC/RS' ) > 0 then begin

     Acbrnfex.Configuracoes.Geral.SSLLib        := LibWincrypt;
     Acbrnfex.Configuracoes.Geral.SSLCryptLib   := cryWinCrypt;
     Acbrnfex.Configuracoes.Geral.SSLHttpLib    := httpWinHttp;
     Acbrnfex.Configuracoes.Geral.SSLXmlSignLib := xsLibXml2;
     Acbrnfex.Configuracoes.WebServices.SSLType := LT_TLSv1_2;

   end else begin
}

// 09.06.20 - testado com fama e não deu certo..deixado o padrao

     Acbrnfex.Configuracoes.Geral.SSLLib        := LibWincrypt;
     Acbrnfex.Configuracoes.Geral.SSLCryptLib   := cryWinCrypt;
     Acbrnfex.Configuracoes.Geral.SSLHttpLib    := httpWinHttp;
     Acbrnfex.Configuracoes.Geral.SSLXmlSignLib := xsLibXml2;
//     Acbrnfex.Configuracoes.WebServices.SSLType := LT_all;
// 12.04.2022 - Mudança no RS a partir de 11.04.2022
     Acbrnfex.Configuracoes.WebServices.SSLType := LT_TLSv1_2;
...
///////////////
                  ...
                  ' and moes_datacont > '+DateToSql(Global.DataMenorBanco));
        if not Q.eof then begin
          Avisoerro('Encontrado documento '+Q.fieldbyname('numero').asstring+' nos últimos 15 dias. Checar numeração.');
          result:=false;
          if Input('Autorização','Senha',senha,7,false) then begin
...
begin
  ...
  if Categoria='F' then begin
     ...
     if not Q.IsEmpty then begin
        ...
        if (cemails='S') and ( trim(Q.FieldByName('Forn_Email1').AsString)<>'' ) then
...
//        TSqled( Form.Components[i] ).Height:=TSqled( Form.Components[i] ).Height+(size-8);
        if tamanho=8 then
          TSqled( Form.Components[i] ).Height:=21  // padrão
        else
...
// 15.02.17
     SistemaContax.Params.Add('DriverName=DevartPostgreSQL');
//     SistemaContax.Params.Add('Database='+Sistema.SQLServer+'/'+LowerCase('saccarli'));
     ...
     SistemaContax.Params.Add('Password='+LowerCase('erp'+IntToStr(94587361)));
...
// 15.02.17
     ...
     try
       ...
       Avisoerro('Não foi possível conectar o banco '+SistemaContax.Params[1]);
     end;
   end else
     result:=true;
end;
...
////     smtp.MailFrom(Sistema.EmailSMTP,100) ;
...
// 28.11.12 - seto aqui para nao mudar na pasta Synalist o smmtpsend.pas....
    if FGeral.GetConfig1AsString('PORTASMTP')<>'' then
      ...
      SMTP.TargetPort:='25';
//////////////
// 20.11.19 - hotmail da simar nao manda mais...
     if AnsiPos( SMTP.TargetPort,'993/995' ) > 0 then begin
        ...
        SMTP.FullSSL := False  ;
// 21.12.20
//        SMTP.FullSSL := True  ;   // falha no login....
        SMTP.AutoTLS := True
         ...
         repeat
           t := GetEmailAddr(Trim(FetchEx(s, ',', '"')));
             ...
             if not sendOK then begin
               Avisoerro('Erro SMTP Mailto '+t+'|'+smtp.targethost+'|'+smtp.password+'|'+smtp.username+'.  Confirmar usuário e senha');
               Break;
             end;
         until s = '';
//         }
...
// 17.03.11
// envio de email usando cliente de email padrão
...
begin
    ...
    Sistema.Beginprocess('Usando cliente de email padrão');
//    Sistema.Beginprocess('Enviando email');
{
    mensagem:='Funcionário : '+inttostr(codfun)+' - '+nome+lf;
    mensagem:=mensagem+' '+lf;
    if tipo='A' then
      mensagem:=mensagem+'Funcionário ADMITIDO.  Configuração necessária:'+lf
    else if tipo='D' then
      mensagem:=mensagem+'Funcionário DEMITIDO.  Retirar configuração existente:'+lf
    else if tipo='T' then
      mensagem:=mensagem+'Funcionário TRANSFERIDO.  Checar configuração:'+lf;
...
//caminho da planilha a ser alterada
   FileName:= 'E:\Documents and Settings\André\Meus documentos\Clientes\tarefas2010.xls';
// 26.06.20
...
//abrindo uma pasta do excel que você pretende manipular
      ...
      //salva as aalterações e fecha o excel.
...
//      end;
        try
           ...
           Avisoerro('Não foi possível abrir Cupom.  Estado Atual é '+Est) ;
           result:=false;
        end;
//      end;
    end;
...
// ou da venda já define se é DUPLICATAS, CARTAO ,etc
...
// ativar , abrir...  ;;;
    ...
    if not AtivaECF then begin
      FGeral.FechaQuery(Q);
      Sistema.endprocess('Não foi possível ativar ECF');
      exit;
    end;
...
var xsql:string;
begin
// infelizmente da pau na hora de gravar..terá que criar novo campo no estoque
// que contenha as unidades onde ele 'está cadastrado'
...
// 02.03.17
procedure TFGeral.GravaNotaCancelada(xarq, xunidade: string ; aviso:boolean);
/////////////////////////////////////////////////////////////////
var Q:TSqlquery;
    xstatus,xchave,transacao:string;
    Emissao:TDateTime;
    nnumeronf:integer;
    xmlstring:string;
begin
  if Ansipos('110111',xarq)>0 then begin
    xstatus:='X';  // cancelada
    xchave:=FExpNfetxt.GetTag('chnfe',xarq);
    emissao:=TextToDate(copy(FExpNfetxt.GetTag('dhevento',xarq),9,02)+
                        copy(FExpNfetxt.GetTag('dhevento',xarq),6,02)+
                        copy(FExpNfetxt.GetTag('dhevento',xarq),1,04)
                         );
    nnumeronf:=strtoint(copy(xchave,26,09));

  end else begin

    xstatus:='I';
//    xchave:=copy(xarq, AnsiPos('infInut Id',xarq)+14,44);
// 08.03.2022 - numero inutilizado não tem chave.. entao pega o protocolo só pra gravar no campo
//              da chave
//    xchave:=copy(xarq, AnsiPos('nProt',xarq)+07,15);
    xchave:=FExpNfetxt.GetTag('nProt',xarq);
    emissao:=TextToDate(copy(FExpNfetxt.GetTag('dhRecbto',xarq),9,02)+
                      copy(FExpNfetxt.GetTag('dhRecbto',xarq),6,02)+
                      copy(FExpNfetxt.GetTag('dhRecbto',xarq),1,04)
                       );
     nnumeronf:=strtoint(FExpNfetxt.GetTag('nNFIni',xarq));

  end;
///////////////////////////////////////////////
{
  Q:=Sqltoquery('select moes_numerodoc,moes_transacao from movesto where moes_numerodoc='+Inttostr(Nnumeronf)+
                ' and moes_dataemissao='+Datetosql(Emissao)+
                ' and moes_unid_codigo='+Stringtosql(xUnidade)+
                ' and moes_status='+Stringtosql(xstatus));
  if not Q.Eof then begin
    transacao:=GetTransacao;
    Sistema.Insert('movesto');
    Sistema.SetField('moes_status',xstatus);
    Sistema.SetField('moes_transacao',transacao);
    Sistema.SetField('moes_operacao',GetOPeracao);
    ...
    Sistema.SetField('moes_datamvto',emissao);
    Sistema.SetField('moes_datalcto',emissao);
    Sistema.SetField('moes_dataemissao',emissao);
    Sistema.SetField('moes_retornonfe','Cancelada');
    Sistema.SetField('moes_natf_codigo','5102');
    Sistema.SetField('moes_tipomov',Global.CodVendaDireta);
    Sistema.SetField('moes_datacont',emissao);
    Sistema.SetField('moes_numerodoc',nnumeronf);
    Sistema.post;

    Sistema.Insert('movbase');
    Sistema.SetField('movb_status',xstatus);
    Sistema.SetField('movb_transacao',transacao);
    Sistema.SetField('movb_operacao',GetOPeracao);
    Sistema.SetField('movb_unid_codigo',xunidade);
    Sistema.SetField('movb_cst','000');
    Sistema.SetField('movb_tpimposto','I');
    Sistema.SetField('movb_numerodoc',nnumeronf);
    Sistema.SetField('movb_tipomov',Global.CodVendaDireta);
    Sistema.post;
  end;
  FGeral.FechaQuery(Q);
  }
///////////////////////////////////////////////
  ...
  Q:=Sqltoquery('select moes_numerodoc,moes_transacao from movesto where moes_numerodoc='+Inttostr(Nnumeronf)+
                  ' and extract( year from moes_dataemissao ) = '+ inttostr(DAtetoano(Emissao,true))+
                  ' and extract( month from moes_dataemissao ) = '+ inttostr(DAtetomes(Emissao))+
                  ' and moes_unid_codigo='+Stringtosql(xUnidade)+
    ...
    if not Q.Eof then begin
      ...
      if xstatus='X' then
        ...
        Sistema.SetField('moes_retornonfe','Numero Inutilizado');
      Sistema.Setfield('moes_valortotal',0);
      Sistema.Setfield('moes_vlrtotal',0);
      Sistema.Setfield('moes_baseicms',0);
      Sistema.Setfield('moes_valoricms',0);
...
////////  26.11.12 -gravação de nota de saida a partir do xml
// 01.03.16 - mais ajustes para prever nfe de entrada emitida pelo proprio emitente como devolucoes de venda
procedure TFGeral.GravaNotacomxml(NotaFiscal: TACBrNFe;xarquivoxml,xunidade:string;ListaMuni:TList=nil;Aviso:boolean=false;comita:boolean=true);
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
var Transacao:string;
    ...
    moes_tipocad,moes_estado,TiposFornec,cnpjcpfemitente,cnpjcpf:string;
    xmlstring       :WideString;
    pdesc           :currency;
...
// 20.08.15 - SM - FAma digital - importar xmls de entrada e saida
      if cnpjcpfemitente=NotaFiscal.NotasFiscais.Items[0].NFe.Emit.CNPJCPF then begin
        ...
        else if pos( trim(xcfop),'1101/2101/1102/2102/3101/3102/1405/1404/1401/1403' ) > 0 then
...
// 09.09.2021 - 'parelho' pra clessi importar os atrasados...depois rever
        if ( Global.Topicos[1480] ) then begin

           if copy(trim(xcfop),1,1)='5' then result:='1556'
        ...
        else if trim(xcfop)='5102' then begin

             if ( Global.Topicos[1480] ) then
        ...
        end else if trim(xcfop)='6102' then begin
        ...
        end else if trim(xcfop)='5405' then result:='1403'
// 10.07.17 - mecanica zanela
        else if trim(xcfop)='5655' then result:='1652'
        else if trim(xcfop)='6655' then result:='2652'
...
//        Q:=sqltoquery('select * from cidades where cida_codigo='+inttostr(PMUnicipios.codigo));
        achou := false;
        for p := 0 to ListaMuni.count-1 do begin

            PMuni := ListaMUni[p];
            if PMunicipios.codigo = PMuni.codigo then begin
...
// 13.06.15
          xy:=AnsiPOs( #39,Dest.EnderDest.xMun );
          if xy > 0  then
            Sistema.SetField('clie_cidade',copy(Dest.EnderDest.xMun,1,xy-1)+copy(Dest.EnderDest.xMun,xy+2,20) )
          else
...
// 02.03.17
          if length(Dest.EnderDest.fone)>11 then
            Sistema.SetField('clie_foneres',copy(Dest.EnderDest.fone,2,11))
          else if copy(Dest.EnderDest.fone,1,1)='4' then
...
//          Sistema.SetField('clie_email',Emit.EnderEmit....);
          ...
          if length(Dest.EnderDest.fone)>11 then
            Sistema.SetField('clie_fonecom',copy(Dest.EnderDest.fone,2,11))
          else if copy(Dest.EnderDest.fone,1,1)='4' then
...
// 11.06.15
    ...
    begin
        ...
        with NotaFiscal.NotasFiscais.Items[0].NFe do begin
          ...
          if Dest.idEstrangeiro<>'' then begin
            Sistema.SetField('forn_nome',copy(SpecialCase(Dest.xNome),1,40));
            Sistema.SetField('forn_razaosocial',copy(SpecialCase(Dest.xNome),1,40));
  //          Sistema.SetField('forn_cnpjcpf',Emit.cnpjcpf);
  // 05.06.17
            Sistema.SetField('forn_cnpjcpf',cnpjcpf);
            Sistema.SetField('Forn_inscricaoestadual',Dest.IE);
            Sistema.SetField('forn_uf',Dest.Enderdest.UF);
            Sistema.SetField('forn_endereco',copy(SpecialCase(Dest.Enderdest.xLgr)+', '+Emit.EnderEmit.nro,1,40));
    //        Sistema.SetField('clie_endrescompl',PClifor.forn_endereco);
            Sistema.SetField('forn_bairro',SpecialCase(Emit.EnderEmit.xBairro));
            Sistema.SetField('forn_cida_codigo',GetCodigoMunicipio(Dest.EnderDest.xMun,Dest.EnderDest.UF));
            Sistema.SetField('forn_cep',inttostr(Dest.EnderDest.CEP));
  // 02.03.17
            if length(emit.Enderemit.fone)>11 then
              Sistema.SetField('forn_foneres',copy(Dest.EnderDest.fone,2,11))
            else if copy(Emit.EnderEmit.fone,1,1)='4' then
              Sistema.SetField('forn_fone','0'+copy(Dest.EnderDest.fone,1,10))
            else
              Sistema.SetField('forn_fone',copy(Dest.EnderDest.fone,1,11));
            Sistema.SetField('Forn_enderecoind',copy(SpecialCase(Dest.EnderDest.xLgr)+', '+Dest.EnderDest.nro,1,50));
            Sistema.SetField('Forn_cidaind_codigo',GetCodigoMunicipio(Dest.EnderDest.xMun,Dest.Enderdest.UF));
            if length(emit.Enderemit.fone)>11 then
              Sistema.SetField('forn_foneindustria',copy(Dest.EnderDest.fone,2,11))
            else if copy(Emit.EnderEmit.fone,1,1)='4' then
              Sistema.SetField('Forn_foneindustria','0'+copy(Dest.EnderDest.fone,1,10))
            else
              Sistema.SetField('Forn_foneindustria',copy(Dest.EnderDest.fone,1,11));
  ...
  // 02.03.17
            if length(emit.Enderemit.fone)>11 then
              Sistema.SetField('forn_foneres',copy(Emit.EnderEmit.fone,2,11))
            else if copy(Emit.EnderEmit.fone,1,1)='4' then
            ...
            if length(emit.Enderemit.fone)>11 then
              Sistema.SetField('forn_foneindustria',copy(Emit.EnderEmit.fone,2,11))
            else if copy(Emit.EnderEmit.fone,1,1)='4' then
...
{  // retirado em 11.06.15
  if pos( copy( NotaFiscal.NotasFiscais.Items[0].NFe.Det.Items[0].Prod.cfop,1,1 ) , '123' ) > 0 then begin
    Avisoerro('XML de nota de entrada.  Não importado.');
    exit;
  end;
}
...
//    Avisoerro('Nota já encontrada na transação '+Q.fieldbyname('moes_transacao').asstring+'.  Não importado.');
     if aviso then
       FImportaNfe.Texto.Lines.Add('Nota '+Inttostr(NotaFiscal.NotasFiscais.Items[0].NFe.Ide.nNF)+' já encontrada na transação '+Q.fieldbyname('moes_transacao').asstring+'.  Não importado.');
    exit;
  end;
  FGeral.FechaQuery(Q);
//  Sistema.Beginprocess('Lendo municípios');
// 05.12.2021 retirado lemunicipios
  if ListaMuni<>nil then begin

     ListaMunicipios := TList.create;
     ListaMunicipios.Assign( Listamuni );
...
//  end;
  if ( trim( NotaFiscal.NotasFiscais.Items[0].NFe.Dest.CNPJCPF )='' )
     and ( NotaFiscal.NotasFiscais.Items[0].NFe.Ide.modelo=55 )
// 05.06.17
     and ( NotaFiscal.NotasFiscais.Items[0].NFe.Dest.idEstrangeiro='' ) then begin
    Avisoerro('Não encontrado a tag referente o CNPJ/CPF na nota '+inttostr(NotaFiscal.NotasFiscais.Items[0].NFe.Ide.nNF) );
    exit;
  end;
...
// 05.06.17 - notas de importacao da fama/netcom
  cnpjcpf:= trim( NotaFiscal.NotasFiscais.Items[0].NFe.Emit.CNPJCPF );
{
  if ( trim( NotaFiscal.NotasFiscais.Items[0].NFe.Dest.CNPJCPF ) <> '' ) then
    cnpjcpf:= trim( NotaFiscal.NotasFiscais.Items[0].NFe.Dest.CNPJCPF )
  else
    cnpjcpf:= trim( NotaFiscal.NotasFiscais.Items[0].NFe.Dest.idEstrangeiro );
    rever como tratar notas de importacao da fama pra fama
}

// 15.06.17
//  if (CNPJCPF<>cnpjcpfemitente) and (cnpjcpf<>'') and ( trim( NotaFiscal.NotasFiscais.Items[0].NFe.Dest.idEstrangeiro )<>'' ) then begin
  if (CNPJCPF<>cnpjcpfemitente) and (cnpjcpf<>'')  then begin

    Q:=Sqltoquery('select * from fornecedores where forn_cnpjcpf='+Stringtosql(CNPJCPF));
    if Q.Eof then begin
      IncluiFornec;
      Q:=Sqltoquery('select * from fornecedores where Forn_cnpjcpf='+Stringtosql(CNPJCPF));
    end;
    ...
    if (FGeral.GetConfig1AsInteger('clieconsumidor')>0) and (Notafiscal.NotasFiscais.Items[0].Nfe.Ide.modelo=65) then
      Q:=Sqltoquery('select * from clientes where Clie_codigo='+inttostr(FGeral.GetConfig1AsInteger('clieconsumidor')))
    else begin
...
// 08.09.2021 - 'específico' para clessi importar as notas de mat. de uso e consumo não
//              digitadas e q o fiscal de SC exigiu...
    if not ( Global.Topicos[1480] ) then begin
...
// 04.02.17
    if NotaFiscal.NotasFiscais.Items[0].NFe.Ide.modelo=65 then
      Sistema.SetField('moes_especie','NFCE')
    else
...
// 09.09.2021 vDesc é valor e não percentual...
    if ( NotaFiscal.NotasFiscais.Items[0].NFe.Total.ICMSTot.vDesc > 0 )
       and
       ( NotaFiscal.NotasFiscais.Items[0].NFe.Total.ICMSTot.vprod > 0 )
       then begin

       pdesc := NotaFiscal.NotasFiscais.Items[0].NFe.Total.ICMSTot.vDesc/NotaFiscal.NotasFiscais.Items[0].NFe.Total.ICMSTot.vprod;
       pdesc := pdesc * 100;
    ...
    Sistema.SetField('Moes_Perdesco',pdesc);
    Sistema.SetField('Moes_Peracres',0);
//    Sistema.SetField('moes_remessas',remessas);
...
// 08.03.2022
    Sistema.SetField('moes_mensagem',Copy(trim(NotaFiscal.NotasFiscais.Items[0].NFe.InfAdic.infAdFisco)+' '+
                                     trim(NotaFiscal.NotasFiscais.Items[0].NFe.InfAdic.infCpl),1,1000) );
//    Sistema.SetField('moes_pedido',pedido);
...
// 09.09.2021 - vdesc é em valor e nao me percentual
      Sistema.SetField('move_perdesco',pdesc);
...
// 08.09.2021 sem icms
      ...
      end else if NotaFiscal.NotasFiscais.Items[0].NFe.Emit.CRT=crtRegimeNormal then
...
//    localarquivo:='IT-GQ.02.01-00 Preenchimento de Relatorio de Nao Conformidades.PDF';
  ...
  else if xlocal='RequisiçãoAlmox' then
...
// 26.06.20
    ...
    if not FileExists( qtabela ) then begin
// 04.08.18
      versaotab:='18.1.b';
      qtabela:=ExtractFilePath( Application.ExeName )+'IBPT\'+'TabelaIBPTax'+uppercase(Global.UFUnidade)+versaotab+'.csv';
...
// 29.10.14
function TFGeral.GetAliquotaFixaIcmsEstado(xUF: string;xcodigo:integer=0;xtipocad:string='C'): currency;
///////////////////////////////////////////////////////////////////////////////////////
var Qc:TSqlquery;

begin
//  if pos(xUF,'SC')>0 then result:=17
// 20.09.18 - nf venda da a2z para SC
//  if pos(xUF,'SC')>0 then result:=12
// 16.11.18 - calculo do difal para SC
  if pos(xUF,'SC')>0 then result:=17
...
// 01.02.19 - depois ver se coloca os estados dentro de if
  if xcodigo>0 then begin

     if xtipocad='C' then begin

        QC:=sqltoquery('select clie_tipo from clientes where clie_codigo = '+inttostr(xcodigo));
        if not Qc.Eof then begin
           if (Qc.FieldByName('clie_tipo').AsString='F')
               and
               ( Ansipos( xUF,'MT/MS/GO/RO/ES' ) >0 ) then
              result:=17

// 24.11.20 - Seip brasil venda PR -> PR usado 12 na origem e 18 no destino quando é revenda e PJ
           else if ( Ansipos( xUF,'PR' ) >0 ) then
              result:=18
...
// 25.03.2021
function TFGeral.GetAliquotaIcmsEstadoEntradas(xUF: string): currency;
///////////////////////////////////////////////////////////////////////
begin

  if pos(xUF,'SC')>0 then result:=12
  else if pos(xUF,'PR/SP/RS')>0 then result:=18
  else if pos(xUF,'MG')>0 then result:=18
  else result:=7;
...
begin
   if Global.topicos[1112] then begin
     result:=(xSituacao<>'P');
     if xsituacao='P' then Aviso('Pré-cadastro não pode ser usado para vendas')
   end else
     result:=true;
end;
...
procedure TFGeral.EnviaEMailcomAnexo(xemail: string; xListaAnexos:TStringList ; xCorpoEmailTexto: TStrings;xtipo:string='NF-e';xassunto:string='');
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
Var
  xAnexo : Integer;
...
// 10.03.17
email.from:=Sistema.EmailFrom;
email.fromname:=Sistema.EmailFrom;
email.settls:=true;
// 25.05.20
Lista   := TStringList.Create;
xemail2 := '';
if AnsiPos(';',xemail) > 0 then begin

   strtolista(Lista,xemail,';',true);
   if Lista.Count>1 then begin
      xemail  := Lista[0];
      xemail2 := Lista[1];
   end;
   email.AddAddress(xemail);
   if trim(xemail2)<>'' then email.AddCC(xemail2);
...
else if xtipo='Canc' then
   email.Subject:='XML de NF-e cancelada'
else
   email.Subject:='XMLS dos conhecimentos';

// 15.06.18 - corpo do email 'nao funga mais'...
if xassunto<>'' then email.Subject:=xassunto;

email.Body.Text:=xCorpoEmailTexto.Text;
// 15.06.18
//email.Body.Assign( xcorpoemailtexto );
//email.AltBody
try
  email.Send;
  Sistema.endprocess('Email enviado');
except on W:Exception  do
  Sistema.endprocess('Erro tipo '+W.ClassName +' no envio para email '+xemail+' '+W.Message);
end;
...
// 11.02.23
procedure TFGeral.GetTributacaoNCM(QTrib:TSqlquery;xcfop:string;xUnid_codigo: string; var aliicmsncm: currency;
          var cstncm, cfopncm: string; var achou: boolean);
////////////////////////////////////////////////////////////////////////////////////////
begin
// por enquanto somente da 'unid1' na tabela codigos ipi
          if QTrib.fieldbyname('cipi_unid1_codigo').asstring=xUnid_codigo then begin
             ...
             achou:=true;
             if copy(xcfop,1,1)='5' then begin
...
// 21.06.16
// Para uso no lançamento 'online' sem usar a exportacao das notas a prazo
    ...
    if tipocad='C' then begin
      ...
      if not Q.eof then begin
        if Q.FieldByName('clie_contacontabil').AsInteger=0 then begin
          Avisoerro('Falta conta contábil neste cliente');
          result:=false;
        end;
      end;
      Q.close;
      ...
      if not Q.eof then begin
        if ( (Q.FieldByName('forn_contaexp').AsInteger>0) and ( Q.FieldByName('Forn_unidexporta01').AsString=xunidade ) )
          ...
          if modulo<>'FIN' then
            Avisoerro('Falta conta contábil ou qual unidade neste fornecedor para unidade '+xunidade);
          result:=false;
        end;
      end;
      Q.close;
...
begin
  if not Global.Topicos[1396] then begin
     ...
     exit;
      ...
      Lista:=TStringList.create;
      if not Q.eof then begin
         Lista.Add('Notas a autorizar OU números a inutilizar');
         result:=false;
      end;
      while not Q.eof do begin
