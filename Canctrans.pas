...
//////////////////////////////////////////////////////
  ...
  if confirma('Confirma o cancelamento da transação '+EdTransacao.Text+' ?') then begin
...
// 15.02.06
    if copy(EdTransacao.text,1,1)='I' then begin
      Avisoerro('Transação de Importação não pode ser cancelada');
      exit;
    end;
    Sistema.BeginTransaction('Cancelando transação');
...
//      EdTransacao.invalid('Transação inexistente ou já está cancelada');
...
// 06.08.10 - novi - vava - feito 'direito' em 05.10.10
             if Confirma('NFe com retorno '+QMovestoque.fieldbyname('moes_retornonfe').asstring+' :Informe <S> se NÃO foi DENEGADA ou <N> caso FOI DENEGADA pela Sefa') then begin
...
// 20.06.16
      ...
      if (QMovestoque.fieldbyname('move_status').asstring='D') then begin
        ...
        if podecancelar<>'S' then begin
          EdTransacao.invalid('Transação está marcada como consignação encerrada ( status=D )');
          Sistema.EndTransaction('');
          exit;
        end else
          QMovestoque.first;
  ...
  //    and (QMovestoque.fieldbyname('move_tipomov').asstring<>Global.CodVendaProntaEntrega )
        EdTransacao.invalid('Transação está marcada como encerrada ( status=E )');
        Sistema.EndTransaction('');
        exit;
      end;
...
////////////
/////////// - 12.06.06 - 'desbaixa' o pedido de compra usado na nf de compra  - mov é usado para retornar a qtde no est.
...
//
/////////// - 18.09.09- 'desbaixa' o pedido de venda usado na nf de venda  - mov é usado para retornar a qtde no est.   // 19.03.20
...
////////////
// 11.10.07 - saida do estoque a entrada data pela 'decomposição' da nf de produtor
...
// 26.07.04 - voltou a dar o erro podre de "erro na transação'...vamos ver o q dá com este commit aqui...
...
// 06.09.13 - Metalforte - gera lançamento de saida do caixa/bancos quando exclui uma transacao
...
// 06.10.10 - apaga o lançamento do pagamento de comissao se ainda estiver em aberto
...
// deixa as remessas de consigação novamente disponíveis
...
// retorna as devoluçoes de remessa para ficarem disponíveis
{
...
// 21.01.05
       ...
       Sistema.beginprocess('Retornando as devoluções');
...
//      Sistema.post(FGeral.GetIN('moes_numerodoc',Remessas,'N')+' and moes_status=''D''');
      Executesql('update movestoque set move_status=''N'' where '+FGeral.GetIN('move_numerodoc',Remessas,'N')+
       ...
       ListaRemessas:=tstringlist.create;
       strtolista(Listaremessas,Remessas,';',true);
       Sistema.beginprocess('Retornando as devoluções do regime especial');
...
////////////////
// 02.04.08 - cobrança da industrializacao
...
// 22.07.20 - apropriacoes - entradas
    ...
    if statuscancfiscal = 'Y' then
       ...
       Sistema.Endtransaction('Transação '+EdTransacao.text+' DENEGADA')

    else

       Sistema.Endtransaction('Transação '+EdTransacao.text+' cancelada');
...
/////////////////////////////////////////////
   Texto.clear;
   Sistema.beginprocess('Pesquisando transação');
...
// 30.04.18
      if Global.Usuario.OutrosAcessos[0346] then begin
         if  (
            ...
            then begin
              EdTransacao.invalid('Usuário sem permissão de cancelar ESTA NOTA FISCAL');
              Sistema.EndProcess('');
              exit;
            end;
          ...
          if not Global.Usuario.OutrosAcessos[0303] then begin
            EdTransacao.invalid('Usuário sem permissão de cancelar nota fiscal');
            Sistema.EndProcess('');
            exit;
          end;
...
// 07.07.09
     ...
     if Q.fieldbyname('moes_status').asstring='C' then begin
       ...
       EdTransacao.invalid('Transação já cancelada(excluída)');
       Sistema.endprocess('');
       exit;
...
// 22.07.20 - Novicarnes
     if AnsiPos( Q.fieldbyname('moes_status').asstring,'I/X/Y' ) > 0 then begin
       ...
       EdTransacao.invalid('Transação já inutilizada/cancelada/denegada');
       Sistema.endprocess('');
       exit;
     ...
     if (Global.UsaNfe='S') then begin
       ...
       if ( DAtetoano(Q.fieldbyname('moes_dtnfeauto').asdatetime,true)>=1900 )
         ...
         if (DAtetoano(Q.fieldbyname('moes_dtnfecanc').asdatetime,true)<=1900)
           then begin
           Avisoerro('NFe '+Q.fieldbyname('moes_numerodoc').asstring+' ainda não foi cancelada na Sefa');
           EdTransacao.invalid('');
           Sistema.endprocess('');
           exit;
         end;
...
// 13.05.20 - FAma - dai nao deixar 'só apagar' a transacao de uma remessa RC...colocado especie
       ...
       // 09.09.19
           ...
           Avisoerro('NFe '+Q.fieldbyname('moes_numerodoc').asstring+' já denegada no sistema');
           EdTransacao.invalid('');
           Sistema.endprocess('');
           exit;
...
// 07.11.13 - Vivan
           ...
           Aviso('CF '+Q.fieldbyname('moes_numerodoc').asstring+' emitido não pode ser cancelado');
           if not Input('Autorização','Senha',csenha,15,false) then begin
             EdTransacao.invalid('Senha não informada');
             Sistema.endprocess('');
             exit;
           end;
...
//     EdTransacao.Invalid('Transação não encontrada !!!');
//     Sistema.endprocess('');
//     exit;
   end;
     ...
     if not Q.eof then begin
       ...
       if Q.fieldbyname('pend_status').asstring='C' then
         EdTransacao.invalid('Transação já cancelada')
       else if Q.fieldbyname('pend_status').asstring='B' then
         EdTransacao.invalid('Transação baixada.  Cancelar transação da baixa primeiro')
       else begin
         Q.close;Freeandnil(Q);
...
//           EdTransacao.Invalid('Transação  não encontrada no caixa bancos');
           EdTransacao.Invalid('Transação  não encontrada.');
       end;
       Q.close;Freeandnil(Q);
     end;
