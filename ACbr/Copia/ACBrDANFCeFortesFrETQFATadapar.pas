...
// 17.07.18
    ...
    procedure rllabelprodutoBeforePrint(Sender: TObject; var Text: string;
      var PrintIt: Boolean);
    procedure rlnroadaparBeforePrint(Sender: TObject; var AText: string;
      var PrintIt: Boolean);
  private
    FNumItem: Integer;
    FNumPag: Integer;
...
begin
  if self.FACBrNFeDANFCeFortesA4.FpNFe.Det.Items[0].Prod.arma.Count>=1 then
    Text:='Porção de '+self.FACBrNFeDANFCeFortesA4.FpNFe.Det.Items[0].Prod.arma.Items[0].nCano+' gr.'
  else
...
// 29.01.19
   especie:='BOVINO';
   if AnSiPos( 'SUINO',uppercase( self.FACBrNFeDANFCeFortesA4.FpNFe.Det[self.FNumItem].Prod.xProd) ) >0   then
      especie := 'SUINO'
...
begin
   ...
   PintarQRCode( self.FACBrNFeDANFCeFortesA4.FpNFe.infNFeSupl.qrCode, RLImageQRCode.Picture, qrUTF8NoBOM );
...
//     Text:=strspace('Valor Energético',28)+self.FACBrNFeDANFCeFortesA4.FpNFe.Det.Items[0].Prod.arma.Items[1].nCano;
    Text:=strspace('Vlr Energético',15)+FormatFloat('##0.0',self.FACBrNFeDANFCeFortesA4.FpNFe.Det.Items[0].Prod.vUnTrib)+' Kcal';
end;
