...
// minhas 'cagadas'
    function  ImprimeCupomFiscal(xtransacao,TipoMovimento:string;xvalortroco:currency):boolean;
    procedure ReducaoZ;
    procedure LeituraX;
...
uses ACBrUtil, ACBrECFBematech, VendeItem, EfetuaPagamento,
     Relatorio,
     // Sobre,
     TypInfo, Math, ActiveX, MSHTML, IniFiles,
  ConfiguraSerial, ACBrPAFClass, RelatorioGerencialFormatado, uDAV, uDAVOS,
  // uVendaFrenetica,
  Geral, SqlExpr, Sqlfun, fornece, Unidades, cadcli, SqlSis ;
...
begin
  ...
  begin
     if ACBrECF1.RelatoriosGerenciais[A].Descricao <> '' then
        ...
        + ACBrECF1.RelatoriosGerenciais[A].Descricao + ' CER:'
...
function TForm1.ImprimeCupomFiscal(xtransacao, TipoMovimento: string;xvalortroco:currency):boolean;
//////////////////////////////////////////////////////////////////////////////////////
var Q,QP:TSqlquery;
...
/////////////////////////////////////////////////
         ...
         if Estados[ ACBrECF1.Estado ] = 'Não Inicializada' then begin
           if trim( FGeral.GetConfig1AsString( 'PORTAECF1' ) ) <>'' then
              ACBrECF1.Porta := FGeral.GetConfig1AsString( 'PORTAECF1' ) ;
           ACBrECF1.Ativar ;
//////////////////
...
//         ACBrECF1.SubtotalizaCupom( StrToFloat(Desc), Obs );
// 02.10.119
//         ACBrECF1.SubtotalizaCupom( vlrdesco, Obs );
         ACBrECF1.SubtotalizaCupom( percdesco, Obs );
...
// 09.01.12 - pedir forma somente se houver mais que uma além de 'Dinheiro'
// 28.04.15 -               // nao pedir e deixar fixo 'dinheiro'..ou a primeira
     if (Lista.count>1) and ( not Global.topicos[1035] )  then
...
// 20.05.18
      if Global.Usuario.Codigo=100 then Aviso('Pagamento com valor '+floattostr(valor)+
                                              ' Codigo '+codigo+'|'+
                                              ' Vinculado '+BoolToStr( FPG.PermiteVinculado ) );

      ACBrECF1.EfetuaPagamento( Codigo, Valor , '' ,FPG.PermiteVinculado,strtoint(codigo) );
...
//      aviso('Sem alíquotas configuradas');
...
//    totalnf:=Q.fieldbyname('moes_totprod').Ascurrency;
...
//    aviso('efetuar pagamento');
    Sistema.beginprocess( 'Efetuando pagamento' );
...
begin
  ...
  if y then begin
    try
      Sistema.beginprocess('Gravando a Redução Z');
      ACBrECF1.DadosReducaoZ;
      GravaLeitura('Z','' );
// ver questao de mandar 'com ou sem horas'..
      Sistema.beginprocess('Executando Redução Z');
      ACBrECF1.ReducaoZ;
      Sistema.endprocess('');
    except
      Avisoerro('Problemas Reduzão Z.  Estado ECF '+Estados[Acbrecf1.estado]);
    end;
  end;
end;

////////////////////////////////
procedure TForm1.LeituraX;
////////////////////////////////
var y:boolean;
begin
  y:=true;
...
////////////////////////////////////////////////////////////
procedure TForm1.GravaLeitura(xtipo, leitura: string);
////////////////////////////////////////////////////////////
...
//    Sistema.setfield('Mecf_NumeroCOOf',ACBrECF1.NumCOO);
  ...
  try
    ...
    Avisoerro('Não foi possível gravar na tabela de leitura de ECF');
  end;
end;

////////////////////////////////////////
function TForm1.AtivarECF: boolean;
////////////////////////////////////////
begin
      try
         if Estados[ ACBrECF1.Estado ] = 'Não Inicializada' then begin
