...
//              dai não encontra o documento ficando sem a unidade no grid gerando erro no post...
    xunidade := trim(Grid.cells[Grid.getcolumn('pend_unid_codigo'),p]);

    if (trim(xunidade)='') and (trim(seunumero)<>'') then begin

       Avisoerro('Documento '+seunumero+' pode estar com vencimento divergente.  NÃO SERÁ BAIXADO');
       seunumero:='';
       cnossonumero:='';
