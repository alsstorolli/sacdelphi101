...
// 21.03.18 - para alterar PA - entrada de serviços
...
//     ( trim(Grid.cells[Grid.GetColumn('move_esto_codigo'),1])<>'' )
   ...
   then begin
    ...
    Avisoerro('Não permitido itens em Conhecimento de transporte OU tipos EI');
...
// 11.03.20 - polli mudou a planilha com mais colunas e nomes diferentes...
  ...
  if ListaCampos.Count <> ListaColunas.Count then begin
     Avisoerro('Não encontrado TODAS as colunas na linha 01.  '+ListaCampos.Text);
     Excel.quit;
     exit;
  end;
...
//     else Aviso('Célula ref. NCM com tipo não tratado');
     ...
     if Valorc>0 then begin
        ...
        else Aviso('Célula  coordernadas '+inttostr(p)+','+ListaColunas[1]+' com tipo não tratado');
...
// 18.01.19
         if length(trim(xcodigo)) <> FGeral.GetConfig1AsInteger('TAMESTOQUE') then begin
            Sistema.EndProcess('Codigo '+xcodigo+' com numero errado de dígitos');
            Excel.quit;
            exit;
         end;
         if length(trim(xcodigo)) =0 then begin
            Sistema.EndProcess('LInha '+inttostr(p)+' sem codigo do produto do Sac porém com valor');
            Excel.quit;
            exit;
         end;
        ...
        end;
{
        ...
        if trim(celula)<>'' then begin
            ...
            Aviso('LInha '+inttostr(p)+' sem codigo do produto do Sac porém com valores');
//            Sistema.EndProcess('LInha '+inttostr(p)+' sem codigo do produto do Sac porém com valores');
//            Excel.quit;
//            exit;
...
// 20.11.20 - ver quando é 150 ou 051... situação de exceção quando vem de aviao e por SP...
{
...
//    end;
// 23.06.06 - manoel e 35 OU paulo e 34 cfop é final 925
  ...
  if (AcbrNFe1.NotasFiscais.Count >0) and ( not EdArquivoxml.isempty ) then begin
    ...
    if (AcbrNFe1.NotasFiscais.Items[0].NFe.Dest.CNPJCPF <> '')
       ...
       (AcbrNFe1.NotasFiscais.Items[0].NFe.Ide.modelo = 55 )
        ...
        EdUNid_codigo.invalid('Cnpj do destinatário da nota diferente do cnpj da unidade informada');
...
// 09.05.06 - notas do paulo(dastyle) q são tipo1 mas geram financeiro tipo2...Leila+Grazi
...
/////////////////
//    valortotal:=EdDigtotalnf.AsCurrency-valoravista;
// 15.08.05 - para poder lançar Dv sem produtos ( o famoso credito de peças )
...
// 09.11.05
      if ((GridParcelas2.visible) and (EdVAlortotal.ascurrency>0)) or (valortotal2>0) then begin
        ...
        if (p=nparcelas) and (valoravista2=0) then
          valorparcela2:=valortotal2-acumulado2  // para deixar na ultima parcelas "as dízimas"
        else begin
          if (valoravista2>0) then begin
            ...
            if (p=nparcelas) then
              valorparcela2:=valortotal2+valoravista2-acumulado2  // para deixar na ultima parcelas "as dízimas" - 01.06.05
          end else
...
// 31.08.06 - Marcia Lizot
  ...
  if QEstoque.eof then begin
      EdProduto.Invalid('Codigo ainda não cadastrado nesta unidade');
      exit;
  end;
...
//      EdProduto.INvalid('Produto já digitado.  Excluir e incluir');
//  end;
...
// 28.05.15 - nao estava vendo as vendas do cliente...jamantaaaa..
    ...
    if QDev.eof then begin
      EdProduto.invalid('Não encontrado vendas deste produto no prazo permitido para devoluções.  Verificar');
      exit;
    end;
  end;
...
//         EdSt.enabled:=false;
//         EdSt.text:='000'; // se for optante do simples é tributado mas não paga na nota e sim pela tabela
//      end;
//    end;
// 06.07.05
    EdSt.enabled:=false;
...
// 09.06.06 - por enquanto assim até q tenha a grade com custo tbem por tamanho , cor e copa
{ - retirado pois quando exclui item durante a inclusao elimina da listacustos
     ...
     ////////////////////////////////
     begin
// 17.10.05 - para nao pedir confirmacao em devolução de venda
...
//    EditstoGrid;
    if (QEstoque.eof) and (EdComv_codigo.ResultFind.fieldbyname('comv_tipomovto').asstring<>Global.CodTransfEntrada) then begin
      Avisoerro('Não encontrado o custo deste produto.  NAO MOVIMENTADO');
      exit;
    end;
//      exit;
...
//      SetaEditsValores;
  // ver para mudar na função para q quando tiver codigo de grade enviar tamanho e cor para atualizar também no
  // estgrades, além do estoqueqtde - ver se faz outra funcao para movimentar ...
    pervariacao:=0;
...
// 10.01.08  - em ordem de operacao para, no caso de NP, achar primeiro NP e nao a ME q é gerada junto...
...
// 21.03.18 - alteracao de entrada de serviços
        CampostoeditsII(Qt);
// 16.12.19 - Novicarnes
...
//////////////////////////////////////////////
// 25.09.19  - Empresas fazem a nota já descontando o gta...
...
// 15.04.08
      if trim(Grid.Cells[Grid.GetColumn('codigosittrib'),p])='' then begin
        codigosit:=0;
        Avisoerro('Codigo da ST não encontrado');
      end else
// 28.08.04 - fazer o acrescimo ou desconto sobre o total dos itens e não em cada um..daí recalcular icms
...
// 18.10.18 - Seip - soma Imposto de Importação da base do IPI
...
// 15.04.08 - retirado em 08.11.21 - recolocado em 01.02.23 - Mirvane..puxa valor 210 e nos items dá 205...
...
// 01.02.2023 - nota mirvane q 'itens não bate com totais'....
...
// 18.02.16 - ver se nao é desconto equivocado
// 22.12.15 - novicarnes - começou a nao autorizar
     ...
     if Global.Topicos[1354] then begin
       if pos(EdUnid_codigo.ResultFind.Fieldbyname('unid_simples').asstring,'S;2')>0 then begin
         if (EdDigbicms.ascurrency+EdDigvicms.ascurrency)>0 then begin
           Avisoerro('Base Icms e Valor icms não pode ser informado em unidade do Simples');
           result:=false;
           exit;
         end;
       end;
...
// 19.12.19
     if OP = 'I' then begin
        ...
        if not Qx.eof then begin
           ...
           Avisoerro('Nota encontrada na transação '+Qx.FieldByName('moes_transacao').AsString);
           if Input('Autorização','Senha',xsenha,10,false) then begin
     ...
     if campoufentidade = 'forn_uf' then begin
        ...
        if EdFornec.ResultFind.FieldByName('forn_contribuinte').AsString = 'R' then begin
           ...
           if ( EdVlrinss.AsCurrency+EdVlrCofins.AsCurrency+EdVlrir.AsCurrency+
              ...
              result:=false;
              Avisoerro('Fornecedor com RETENÇÃO mas sem valores informados');
              exit;
...
// aqui em 24.05.11 para validar 'fora do if abaixo'
     if (EdDigtotalnf.AsCurrency=0) and
        ( Ansipos( EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring,Global.CodNfeComplementoIcmsE+';'+
                   ...
                   then begin
       Avisoerro('Total da nota não informado');
       result:=false;
       exit;

// 10.07.18 - aqui para nao deixar lançar entrada de serviços sem o item serviços
// 17.12.18 - adicionado compras na checagem
       ...
       Avisoerro('Total de produtos não informado');
       result:=false;
       exit;

// 22.08.11
     end else if Global.Topicos[1351] then begin
// nao dá pra checar nf de produtor pois imprime total da nota menor que o de produtos
...
// 20.06.13 - Notas de entrada com icms ref. subst. tributaria informado e frete embutido jack novi
           if (EdDigtotpro.AsCurrency-EdVlrDesco.ascurrency+EdValoripi.ascurrency+EdFrete.ascurrency+EdValorsubs.ascurrency) <> EdDigtotalnf.AsCurrency then begin
              ...
              if EdValorsubs.ascurrency=0 then begin
                FGeral.Getvalor(valorsubs,'Icms Subs.Tributária');
...
//              Avisoerro('Total da Nota e de produtos não confere com valor do desconto OU ipi informado OU frete OU Subst.Tributária');
...
// 12.09.16
              ...
              then begin
//              Avisoerro('Total da Nota e de produtos não confere com valor do desconto OU ipi informado OU Icms Subst.Tributária');
...
// 03.04.12 - Notas de entrada com icms ref. subst. tributaria informado
              ...
              if (EdDigtotpro.AsCurrency-EdVlrDesco.ascurrency+EdValoripi.ascurrency+EdValorsubs.ascurrency+EdVlracre.ascurrency+EdFrete.ascurrency) <> EdDigtotalnf.AsCurrency then begin
                Avisoerro('Total da Nota e de produtos AINDA não confere com valor do desconto OU ipi informado OU Icms Subst.Tributária');
                result:=false;
              end else
                result:=true;
              exit;
...
//          Avisoerro('Obrigatório digitar o valor dos serviços');
//          result:=false;
//        end;
...
// 05.09.19
        ...
        then begin
       Aviso('Base de icms não confere. Calculada '+floattostr(EdBaseicms.ascurrency)+' Informada '+floattostr(EdDigbicms.ascurrency)+' Tolerancia '+floattostr(vlrtolera) );
//       result:=false;
...
// 05.09.19
        and ( EdValoricms.Enabled )
              then begin
       Aviso('Valor do icms não confere. Calculado '+floattostr(Edvaloricms.ascurrency)+' Informada '+floattostr(EdDigvicms.ascurrency)+' Tolerancia '+floattostr(vlrtolera) );
//       result:=false;
       ...
       Aviso('Total dos produtos não confere. Calculado '+floattostr(Edtotalprodutos.ascurrency)+' Informada '+floattostr(EdDigtotpro.ascurrency)+' Tolerancia '+floattostr(vlrtolera) );
//       result:=false;
     end else if (EdTotal.ascurrency<>EdValortotal.ascurrency) and (EdValortotal.ascurrency>0) then
       Aviso('Valor Total da nota não confere.  Calculado '+floattostr(Edtotal.ascurrency)+' Informada '+floattostr(Edvalortotal.ascurrency)+' Tolerancia '+floattostr(vlrtolera) )
     else if (EdTotalNota.ascurrency<>EdDigtotalnf.ascurrency) and (EdBaseicms.ascurrency>0) then
       Aviso('Total da nota não confere   Calculada '+floattostr(EdTotalNota.ascurrency)+' Informada '+floattostr(EdDigtotalnf.ascurrency)+' Tolerancia '+floattostr(vlrtolera) )
// 01.09.10
//       result:=false;
//     end else if not EdFpgt_codigo.Valid then begin
//       EdFpgt_codigo.INvalid('Obrigatório preenchimento da condição de pagamento');
//       EdPort_codigo.SetFocus;
//       result:=false;
     ...
     totalnfprev:=totalnfprev + EdDigvicms.ascurrency  - vlrfreteliquido + EdValoripi.ascurrency ;
{
   ...
   /////
     ...
     for p:=0 to ListaCustos.count-1 do begin
       ...
       Sistema.Post('esqt_esto_codigo='+stringtosql(PCustos.produto)+' and esqt_unid_codigo='+EdUNid_codigo.Assql+
                     ' and esqt_status=''N'''  );
// 21.08.06 - atualização do custo na grade - rever como fazer
////////////////////////////////////////////////////////
// 22.10.09 - 'ativado'
//{
////////////////////////////////////////////////////////
...
// 31.05.17 - nao é mais usado o gridparcelas2
//      end else begin
...
// codigo da carcaça - colocar direto o peso da carcaca
// 06.01.10 - codigos na config. geral pra diferenciar tratamento de carcaça e do couro
...
// 08.01.10 - codigos na config. geral pra diferenciar tratamento de carcaça e do couro
               if (Q.fieldbyname('cust_perqtde').ascurrency=0) and (pos(PRecla.produto,FGeral.GetConfig1AsString('codigocarcaca'))>0 ) then
...
//     if not EdFpgt_codigo.valid then begin - 15.05.12
     if length( trim(EdFpgt_codigo.text) ) <> EdFpgt_codigo.MaxLength then begin
       Avisoerro('Checar campo Pagto (codigo da condição de pagamento)');
...
//////////// - 30.10.09
  ...
  if ( pos( EdComv_codigo.ResultFind.fieldbyname('comv_tipomovto').asstring,EntranoCusto ) > 0 )
    ...
    if EdFpgt_codigo.isempty then begin
       Avisoerro('Falta condição de pagamento');
       exit;
    end;
// 27.02.14 - retirado pois em lançamentos sem movimento 'desalterava o vencimento...clari+raquel
...
// 06.11.14 - vivan
      if pos( EdFornec.ResultFind.FieldByName('clie_situacao').AsString,'B/R' )>0 then begin
        Avisoerro('Cliente com situação '+EdFornec.ResultFind.FieldByName('clie_situacao').AsString);
        exit;
      end;
// 11.11.14 - vivan  - lindacir
      if Global.Topicos[1413] then begin
          if (trim (EdFornec.ResultFind.fieldbyname('Clie_portadores').asstring)<>'') then begin
            ...
            Avisoerro('Ainda não definido portador(es) neste cliente');
            exit
          end;
      end;
   end;
...
// 19.06.20 - novicarnes nota da guelen compra pessoa juridica com gta sempre os rolo deu diferença de 2 centavos...
...
// 08.05.18 - Clessi - 'gorpe' no grid não gera financeiro
...
// 16.04.2021 - Devereda
       if (valornota<>Totalparcela) and ( pos('F',EdSerie.text)=0)
         ...
         if (valornota > Totalparcela) then
           ...
           Aviso('ATENÇÃO !!! Total da nota :'+formatfloat(f_cr,valornota)+' MAIOR que o total de parcelas :'+formatfloat(f_cr,Totalparcela))
...
// 06.08.12
   if (EdMensagem.IsEmpty) and ( pos(EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring,Global.CodEstornoNFeSai)>0 ) then begin
     Avisoerro('Obrigatório informar a justificativa de estorno de NF-e');
     exit;
   end;
//////////////
...
//      if strtointdef( copy( Grid.Cells[grid.GetColumn('move_operacao'),1],14,4 ),0 ) = 0 then begin
      ...
      if strtointdef( copy( Grid.Cells[grid.GetColumn('move_operacao'),1],posicao+1,4 ),0 ) = 0 then begin
        Avisoerro('Não informado corretamente o codigo do equipamento');
        exit;
     end;
...
// 09.09.08 = retirado devido a nota de produtor que é cliente
        ...
        if (not QVePendencia.eof) and (xcondicao<>FGeral.getconfig1asstring('Fpgtoavista') ) then begin
             Avisoerro('Nota com pendência financeira já baixada.  Alteração não permitida.');
             exit;
        end;
        qVePendencia.close;
...
// 12.10.09
   if (EdComv_codigo.ResultFind.fieldbyname('comv_tipomovto').asstring=Global.CodCompraProdutor)
       ...
       Avisoerro('Nota de Produtor com nota de reclassificação.  Alteração não permitida.');
       exit;
   end;
// 19.10.10
...
// 22.10.10
  if Global.Topicos[1347] then begin
    if not Edcontacredito.Valid then begin
      Avisoerro('Checar conta de crédito');
      exit;
    end;
  end;
// 18.03.10
  if (EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring=Global.CodCompraProdutor) and ( FGeral.getconfig1asinteger('ConfMovRecla')>0 )
     ...
     if not TodostemComposicao(Grid,Grid.GetColumn('move_esto_codigo')) then begin
        Avisoerro('Atenção.  Nota com produtos sem composição para Nota de Reclassificação');
        exit;
     end;
  end;
// 21.06.16
...
// 08.07.20
        ...
        if EdNumerodoc.Asinteger <> numerocte then begin
          ...
          Avisoerro('Número no XML = '+inttostr(numerocte)+'.  Numero '+EdNumerodoc.Text);
          exit;
         ...
         Avisoerro('Não encontrado chave do CTE neste arquivo XML.  Verificar');
         exit;
...
// 12.11.2021 - Alutech
        ...
        if (Edchavenfeacom.Text='') or ( length(trim(Edchavenfeacom.Text))<>44 ) then begin
          ...
          Avisoerro('Chave do Cte não informada ou com tamanho incorreto');
          exit;

        end;
        if copy(Edchavenfeacom.Text,21,02) <> '57' then begin

          Avisoerro('Chave do Cte não com modelo difernte de 57');
          exit;
...
// 08.07.20
        if strtointdef( copy(Edchavenfeacom.Text,26,09),0 ) <> EdNumerodoc.asinteger then begin
          ...
          Avisoerro('Número do Cte informado diferente do que está na chave '+copy(Edchavenfeacom.Text,26,09));
          exit;
     ...
     for x := 1 to GridParcelas.RowCount do begin
         ...
         if TextToValor(GridParcelas.Cells[GridParcelas.GetColumn('pend_valor'),x]) > 0 then begin
            ...
            if trim( Gridcodbarra.Cells[Gridcodbarra.GetColumn('pend_codbarra'),x] ) = '' then
               Avisoerro('Atenção - Parcela sem CODIGO DE BARRA do boleto');
...
// 09.07.20 - tentar evitar digitar numeração errada mesmo usando xml
    ...
    then begin
      ...
      if Ansipos('Id="NFe',AcbrNFe1.NotasFiscais.Items[0].XML) > 0 then begin
        ...
        if EdNumerodoc.Asinteger <> AcbrNFe1.NotasFiscais.Items[0].NFe.Ide.nNF then begin
          Avisoerro('Número no XML = '+inttostr(AcbrNFe1.NotasFiscais.Items[0].NFe.Ide.nNF)+'.  Numero '+EdNumerodoc.Text);
          exit;
        end;
...
// 20.08.2021 - nfpe
   if ( EdComv_codigo.resultfind.fieldbyname('comv_tipomovto').asstring = Global.CodEntradaNFPe )
      ...
      then begin
          ...
          Avisoerro('Obrigatório usar arquivo XML em NFP-e');
          exit;
...
// 24.11.20 - para evitar q pule a numeracao 1 a mais na nf de importacao
        {
// 21.06.06 - exportaçao q agora importa...
...
/////////////////////////////////////////
// 27.03.06 - gravação ref. servicos - dados fiscais - DI
///////////////////////////////////////////////////
{
...
// 23.09.16 - caso precisar alterar o esquecer de ter lançado em qual os...
...
// 06.06.20
           ...
           if confirma('Imprime devolução agora ?') then
...
// 06.06.16                         // 04.08.16 - Vida nova - nf com data 'pra tras
           ...
           if confirma('Imprime transferência agora ?') then
...
// 28.08.04 - por enquanto não calcula
end;
...
begin
  ...
  if OP<>'A' then begin   // 13.01.06
    ...
    if EdDtentrada.Asdate<EdDtEmissao.asdate then
      EDDtEntrada.INvalid('Data de entrada deve ser maior que a data de emissão')
      ...
      if not Global.Usuario.OutrosAcessos[0310] then
        ...
        Aviso('Atenção !  Data de entrada fora do mes/ano atual');

    end else if  (EdDtentrada.Asdate-EdDtEmissao.asdate>30) and (EdDtentrada.Asdate-EdDtEmissao.asdate<=60)then
      Avisoerro('Atenção !  Data de entrada com mais de 30 dias após emissão')

    else if  EdDtentrada.Asdate-EdDtEmissao.asdate>60 then
//      EDDtEntrada.INvalid('Data de entrada deve ser no máximo 60 dias após emissão');
// 23.10.07 - algumas notas ainda chegam atrasadas...
      Avisoerro('Data de entrada deve ser no máximo 60 dias após emissão');
...
// Patoterra - recolocado em 23.09.13
  if not FGeral.ValidaMvto(EdDtentrada) then
      ...
      EdDtentrada.Invalid('Data de entrada deve ser no máximo igual a data atual');
...
//      EdDtemissao.Invalid('Data de emissão deve ser no máximo 60 dias antes da data atual')
// 29.03.10 - Abra
...
//    EdPort_codigo.clearall(FNotaCompra,99);
// 16.04.2021 - retirado para não tirar descontos/acrescimos puxados do xml
...
///////////
  if pos(Edcomv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring,Global.TiposEntrada)=0 then begin
    EdComv_codigo.invalid('Movimento inválido para entradas');
    exit;
  end;
...
//         EdArquivoxml.Next;
         ...
         EdArquivoxml.text:='Não encontrado';
...
// 30.04.05 - colocado conf. movimento dev. romaneio (DA) retorno do represent. pois ainda pedia fornec. e nao cliente
  if pos( EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring,tiposmuda) >0 then begin
    ...
    if trim(FGeral.Getconfig1asstring('Fpgtoavista'))='' then
      EdComv_codigo.invalid('Falta configurar a forma de pagamento a vista nas configurações')
    else
...
// 05.02.13 - Novicarnes...usa DV informando numero...vivan terá q usar DX...
      if pos( EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring,Global.CodDevolucaoVenda+';'+Global.CodRetornoMostruario) >0 then
        EdNumerodoc.enabled:=true
      else
        EdNumerodoc.enabled:=Global.Topicos[1301];
...
//     ' and moes_datamvto='+EdDtEntrada.AsSql );
    if not Q.Eof then begin
      if (Q.fieldbyname('moes_natf_codigo').asstring=EdNatf_codigo.Text) and (Q.fieldbyname('moes_datamvto').AsDatetime=EdDtEntrada.Asdate) then  // 22.06.09
        EdNumerodoc.invalid('Numero já encontrado na transação '+Q.fieldbyname('moes_transacao').asstring+' de '+Q.fieldbyname('moes_datamvto').asstring+' com mesmo CFOP')
// 23.06.09 - ajustado em 29.07.09 para pode desmembrar notas..
        ...
        EdNumerodoc.invalid('Numero já encontrado na transação '+Q.fieldbyname('moes_transacao').asstring+' de '+Q.fieldbyname('moes_datamvto').asstring)
      else if Q.fieldbyname('moes_datamvto').AsDatetime<>EdDtEntrada.Asdate then  // 23.06.09
        Avisoerro('Checar.  Numero já encontrado na transação '+Q.fieldbyname('moes_transacao').asstring+' de '+Q.fieldbyname('moes_datamvto').asstring)
      else
        Avisoerro('Checar.  Numero já encontrado na transação '+Q.fieldbyname('moes_transacao').asstring+' de '+Q.fieldbyname('moes_datamvto').asstring+' CFOP '+Q.fieldbyname('moes_natf_codigo').asstring)
    end else begin
...
// 31.08.09
         ...
         if (not QPendencia.eof) and (OP<>'F') then begin
              EdNumerodoc.invalid('Nota com pendência financeira já baixada.  Alteração não permitida.');
              exit;
         end;
// 07.08.07
       if QBusca.fieldbyname('moes_tipomov').asstring=Global.CodCompraProdutor then begin
              EdNumerodoc.invalid('Alteração não permitida em Nota de compra de Produtor');
              exit;
         end;
       qPendencia.close;
       QBusca.close;
...
// 13.12.11
  ...
  if EdFrete.ascurrency>0 then begin
    Freteembutido:=Confirma('Frete está somado no total desta nota ?');
...
begin
  ...
  if not Arq.TTransp.locate('tran_codigo',EdTran_codigo.Text,[]) then begin
    EdTran_codigo.invalid('Codigo não encontrado');
    exit;
  end;
  IF EdFornec.AsInteger=0 then begin
...
begin
//  EdValoruni.setvalue(EdUnitario.ascurrency);
// 15.09.06 - por causa das lokuragens da importaçao...
// 05.11.09 - Abra - Barras pra metros
  if ( pos( EdComv_codigo.ResultFind.fieldbyname('comv_tipomovto').asstring,EntranoCusto ) > 0 )  and
     ( Global.Topicos[1338] ) and (EdCodTamanho.AsInteger>0) and (EdCodCor.AsInteger>0) then begin
...
// 26.06.13 - vivan - Lindacir - criado topico geral pra ativar
  if Global.Topicos[1363] then begin
    if ( EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring=Global.CodDevolucaoSerie5 ) or
      ...
      if trim(FGeral.Getconfig1asstring('Fpgtoavista'))='' then
        EdPerDesco.invalid('Falta configurar a forma de pagamento a vista nas configurações')
      else begin
        EdFpgt_codigo.text:=FGeral.Getconfig1asstring('Fpgtoavista');
        EdFpgt_codigo.Valid;
...
begin
  if EdVlrdesco.ascurrency>0 then begin
    ...
    if totalitens>0 then
      ...
      Avisoerro('Total de itens está zerado.  Cálculo não feito');
      perdesc:=0;
    end;
      ...
      if ( EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring=Global.CodCompraProdutor )
        ...
        if ( QFornec.fieldbyname('clie_depojudi').asstring='S' ) then begin
           if ( trim(QFornec.fieldbyname('clie_contadepojudi').asstring)='' ) then
             EdFornec.Invalid('Produtor com depósito judicial sem conta informada')
           else if QFornec.fieldbyname('clie_aliinssdepjud').ascurrency=0 then
             EdFornec.Invalid('Produtor com depósito judicial sem % para cálculo do depósito');
        end;
      end;
// 20.06.11
      if ( EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring=Global.CodCompraProdutor ) then begin
           if ( trim(QFornec.fieldbyname('Clie_rgie').asstring)='' )  and (QFornec.fieldbyname('Clie_tipo').asstring='J') then
             Avisoerro('Para nota de Produtor é preciso ter inscrição estadual informada')
      end;
// 03.07.20
      if ( EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring=Global.CodCompraProdutor )
...
// 11.11.14 = vivan
      if (Global.Topicos[1413]) and (EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring=Global.CodDevolucaoVenda)  then begin
        if (trim (EdFornec.ResultFind.fieldbyname('Clie_portadores').asstring)<>'') then begin
          ...
          EdFornec.Invalid('Ainda não definido portador(es) neste cliente');
          exit
        end;
      end;
...
// 02.03.11  // Clessi pra agilizar lançamento - FSC Puro
...
// 17.02.11 - Abra - robson+andre retiraram pois nao é este tipo de 'triangulacao'
//        tipomovimento:=Global.CodCompraIndustria;
// 04.03.11 - Abra - robson pediu para recolocar RI pois tem um tipo de 'quadrigulação'...
//        tipomovimento:=Global.CodCompraIndustria+';'+Global.CodRemessaInd;
...
// 06.02.2012 - devolucoes 'parciais' da abra..só pra 'quebrar as regras'...Amanda
...
//                          ' and moes_unid_codigo='+EdUnid_codigo.Assql+
// 05.11.2012 - Abra triangulacao com outra unidade..chapecó...Andressa+Juliana queriam usar o pedido
...
// 04.11.2021  - 04.11.2021
     if Ansipos( EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring,Global.CodRetornoInd+';'+
        ...
        if EdTotalservicos.ascurrency<=0 then begin
          ...
          EdTotalServicos.INvalid('Obrigatório digitar os valor dos serviços');
...
// criar esta funcao
//  if FGeral.ProcuraGridGrade(0,EdProduto.text,Grid) > 0 then
      Edcodcopa.INvalid('Produto já digitado.  Excluir e incluir');
  end;
...
/////////////////////////
   if( not EdPedido.isempty ) and (OP='I') then begin
      ...
      if QPedido.eof then
        EdPedido.invalid('Pedido não encontrado')
...
begin
  if Edcfis_codigo.resultfind<>nil then begin
    if (Edcfis_codigo.resultfind.FieldByName('cfis_redubase').ascurrency>0 ) and
      ...
      Edcfis_codigo.Invalid('Tipo de movimento '+Global.CodCompraProdutor+' não pode ter redução de base de cálculo')
// 20.04.12
    else if Global.Topicos[1354] then begin
       if pos(EdUnid_codigo.ResultFind.Fieldbyname('unid_simples').asstring,'S;2')>0 then begin
         if (Edcfis_codigo.resultfind.FieldByName('cfis_aliquota').ascurrency<>0 ) then
           Edcfis_codigo.Invalid('Alíquota de Icms não pode ser informada em unidade do Simples');
       end;
    end;
  end;
end;
...
// 18.03.10
      if (EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring=Global.CodCompraProdutor) and ( FGeral.getconfig1asinteger('ConfMovRecla')>0 )
         ...
         if EdRomaneio.isempty then begin
            EdRomaneio.invalid('Está Ativo nota de reclassificação.  Obrigatório informar romaneio');
         end;
      end;
...
begin
   ...
   if trim(copy(EdNfprodutor.text,9,3))<>'' then begin
      ...
      and ( pos('.',Ednfprodutor.text)=0 ) then
        EdNfProdutor.invalid('Notas tem que ser separadas por vírgula, ponto e vírgula, barra ou ponto')
      else if ( pos(',',Ednfprodutor.text)>0 ) and ( pos(';',Ednfprodutor.text)>0 ) then
        EdNfProdutor.invalid('Permitido SOMENTE UM separador : vírgula, ponto e vírgula, barra OU ponto')
      else if ( pos('/',Ednfprodutor.text)>0 ) and ( pos('.',Ednfprodutor.text)>0 ) then
        EdNfProdutor.invalid('Permitido SOMENTE UM separador : vírgula, ponto e vírgula, barra OU ponto')
      else if ( pos(',',Ednfprodutor.text)>0 ) and ( pos('/',Ednfprodutor.text)>0 ) then
        EdNfProdutor.invalid('Permitido SOMENTE UM separador : vírgula, ponto e vírgula, barra OU ponto')
...
begin
   if EdValidadeform.asdate<EdDtemissao.asdate then
     EdValidadeform.invalid('Validade do formulário deve ser maior que a emissão da nota');
end;
      ...
      for i:=1 to Gridservicos.rowcount do begin
        if trim(GridServicos.Cells[Gridservicos.getcolumn('move_numerodoc'),i])<>'' then begin
           if trim(GridServicos.Cells[Gridservicos.getcolumn('move_venda'),i])='' then begin
             Avisoerro('Nota '+GridServicos.Cells[Gridservicos.getcolumn('move_numerodoc'),i]+' sem serviço digitado');
             result:=false;
             break;
           end;
        end;
...
// 21.01.10 - viagem...somar os metros mesmo que é a unidade de controle do estoque de perfis
...
//////////////////////////////////////////////////////////////////////
  ...
  if ValidaGridServicos then begin
      Grid.Clear;
// 21.01.10 - rateio 'mais correto' do peso*metros do serviços sobre os perfis...
      totalpesometros:=GetPesoMetro;

// esta 'viagem de peso metros' só 'phode' as notas..somado em metros pra ratear certo
...
// 15.03.2023 - Devereda - Alterar cfop de itens q vem junto em bonificação
...
//    EdQtdenf.invalid('Quantidade inválida');
    Avisoerro('Checar se a quantidade é válida');
...
begin
  if (EdDtmovimento.asdate<EdDtemissao.asdate) and (not EdDtmovimento.IsEmpty) then
    EdDtmovimento.Invalid('Movimento tem que ser maior que emissão');
// 20.06.11
  if (EdDtmovimento.isempty) and ( pos(EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring,Global.CodCompraProdutor+';'+Global.CodEntradaProdutor)>0 ) then begin
    if ( trim(QFornec.fieldbyname('Clie_rgie').asstring)='' ) then
             EdDtmovimento.invalid('Para nota de Produtor é preciso ter inscrição estadual informada')
  end;
...
// regrava os itens
  ...
  Sistema.BeginProcess('gravando novo valor unitário nos itens');
  for p:=1 to Grid.rowcount do begin
    produto:=Grid.Cells[Grid.getcolumn('move_esto_codigo'),p];
    if trim(produto)<>'' then begin
      if pos(Edcomv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring,Global.CodCompraProdutor+';'+Global.CodEntradaProdutor)=0 then begin
  ...
  try
    Sistema.Commit;
    Sistema.EndProcess('Gravação Terminada');
  except
    Sistema.EndProcess('Problemas na gravação. Tente mais  tarde');
  end;
end;
...
// 20.10.11 -Novicarnes - Jake       
        ...
        then
        EdSt.Invalid('Codigo de situação tributária não permitido para entradas');
    end;
  end;
  EdNatf_codigoitem.text:=FSittributaria.GetCfop(EdSt.AsInteger,EdNatf_codigo.text,EdComv_codigo.ResultFind.fieldbyname('comv_tipomovto').asstring)
end;
  ...
  //        Sistema.SetField('forn_uf',Pclifor.forn_uf);
    ...
    begin
       Aviso('Nome Fantasia:'+NotaFiscal.NotasFiscais.Items[0].NFe.Emit.xFant+';'+
             'Razão Social :'+NotaFiscal.NotasFiscais.Items[0].NFe.Emit.xNome+';'+
             'Nota         :'+inttostr(NotaFiscal.NotasFiscais.Items[0].NFe.Ide.nNF)+';'+
             'Emissão      :'+FGeral.Formatadata(NotaFiscal.NotasFiscais.Items[0].NFe.Ide.dEmi )+';'+
...
// q tá na config. de movimento ou se usa o 'tipos muda' ...
...
// 31.12.15
  if NotaEstorno<>'EstornoNFe' then begin
    ...
    if (NotaFiscal.NotasFiscais.Items[0].NFe.Emit.CNPJCPF='')
      ...
      Avisoerro('Não foi possível identificar o EMITENTE desta nota.  Checar XML');
      EdArquivoxml.Text:='';
      exit;
  ...
  Sistema.Beginprocess('Lendo informações do arquivo XML');
//  if NotaFiscal.NotasFiscais.Items[0].Alertas='EstornoNFe' then begin
// 31.12.15
  if NotaEstorno='EstornoNFe' then begin
// 10.12.13
...
// cfe estava posicionado o clientdataset ora achava ora não achava a placa
...
// ver se será util esta lista de valores por cfop
  ListaBaseCfop.Clear;
  if NotaEstorno<>'EstornoNFe' then begin
// 18.04.12
...
// 29.05.20  - retidao devido novicarnes e simar ( simone e julho )
{
  if EdFpgt_codigo.IsEmpty then begin
     ...
     Avisoerro('Obrigatório cadastrar nova condição de pagamento para lançar estar nota');
     exit;
...
// 30.05.12 - Asatec
  ...
  // 12.03.12 - pra poder checar se é do simples a unidade
  EdUnid_codigo.ValidFind;
  ItensXmltoGrid(Grid);
  SetaEditsValores;
...
//            Grid.Cells[Grid.Getcolumn('esto_descricao'),p]:=FEstoque.GetDescricao(produtoaux)
            ...
            Grid.Cells[Grid.Getcolumn('esto_descricao'),p]:='Não encontrado - NFe'
        end else begin
...
// 06.07.12 - SM - NFe com seviços e produtos...
...
//          qualunitario:=( Det[r].Prod.vProd/qualqtde);
          ...
          if comprimento>0 then
            ...
            Avisoerro('Comprimento zerado ou não encontrado na grade');
          qualunitario:= Det[r].Prod.vProd/qualqtde;
...
//          else begin
            aviso('Ver Cadastro de Sit. Tributária.  Não encontrado cadastro referente Entrada para o CST '+Grid.Cells[Grid.Getcolumn('move_cst'),p]);
            avisoerro('Importação de itens interrompida');
            break;
//          end;
        end;
// 20.10.11 - Silvano - contabilidade
...
// 08.09.14 - acess violation em algums xmls
        ...
        if strtointdef(codigofis,0)=0 then begin
          if Det[r].Imposto.ICMS.pRedBC>0 then
            aviso('Ver cadastro de Codigos de Alíquotas.  Não encontrado cadastro adequado para a alíquota de icms '+Grid.Cells[Grid.Getcolumn('move_aliicms'),p]+
                    ' e redução '+Valortosql(100-Det[r].Imposto.ICMS.pRedBC))
          else
            aviso('Ver cadastro de Codigos de Alíquotas.  Não encontrado cadastro adequado para a alíquota de icms '+Grid.Cells[Grid.Getcolumn('move_aliicms'),p] );
          avisoerro('Importação de itens interrompida');
          break;
...
// 20.08.2021
  if (EdArquivoxml.IsEmpty) and ( EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring=Global.CodEntradaNFPe ) then begin
     ...
     EdArquivoxml.invalid('Obrigatório uso de XML para dar entrada em NPF-e')
...
// 07.12.16
  ...
  if ( not FileExists(EdArquivoxml.text) ) and ( EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring<>Global.CodEstornoNFeSai ) then
    ...
    EdArquivoxml.invalid('Arquivo '+EdArquivoxml.text+' não encontrado')
...
// 28.06.2021
      Acbrnfe1.NotasFiscais.Clear; // pra evitar lançar Cte com chave+xml de nfe

// 20.07.20
      if AnsiPos('<nfeProc',auxarqxml.text ) >0 then begin

         EdArquivoxml.invalid('Arquivo '+EdArquivoxml.text+' não se refere a CT-e');
         exit;
...
// 20.07.20
        if AnsiPos('<cteProc',auxarqxml.text ) >0 then begin
           ...
           EdArquivoxml.invalid('Arquivo '+EdArquivoxml.text+' não se refere a NF-e');
           exit;
...
// 21.05.21
      ...
      if ( not NotaAutorizada(Acbrnfe1.NotasFiscais.Items[0].XML) )  and ( not Global.Usuario.OutrosAcessos[0330] ) then
        ...
        EdArquivoxml.invalid('Arquivo '+EdArquivoxml.text+' não autorizado pela Sefa')

      else if ( not NotaAutorizada(Acbrnfe1.NotasFiscais.Items[0].XML) )  and ( Global.Usuario.OutrosAcessos[0330]) then begin

        Aviso('Arquivo '+EdArquivoxml.text+' não autorizado pela Sefa.  Contactar fornecedor');
        ImportaDadosXML(Acbrnfe1);
        EdFornec.Next;
...
//           filtrar somente do mes da data do sistema e anterior...
        ...
        if ListaNFe.count>0 then begin
           ...
           ListaNFe.sort;
           cchave := SelecionaItems(ListaNFe,'Número de Notas dos XMLs','',false);
...
///////////////
  ...
  if QBusca.Eof then
    EdCodigoProduto.Invalid('Codigo não encontrado no estoque')
  else  if QEsto.eof then
      EdCodigoProduto.Invalid('Codigo ainda não cadastrado na unidade '+EdUnid_codigo.text)
//      exit
  else if JatemnoGrid(EdCodigoproduto.text) then
      Aviso('Atenção.  Codigo de produto já usado nesta nota');
...
// 03.07.13 - Metalforte - Rosangela
    if pos(EdCodigoProduto.text, EdCodigoProduto.Items.Text)=0 then
      Aviso('Atenção.  Codigo NCM da nota fiscal pode ser diferente do NCM no cadastro do produto');
...
// pesquisa SEMPRE pelo codigo de barra ( se vier no xml )
  if (  Global.Topicos[1059] ) and ( trim(xcodbarra)<>'' ) and ( xcodbarra<>'SEM GTIN' ) then begin
    ...
    // 12.03.14 - pesquisa pelo codigo de barra somente quando não encontra codigo já digitado
...
begin
   if Edcontacredito.ResultFind<>nil then begin
     if Edcontacredito.ResultFind.FieldByName('plan_tipo').asstring<>'M' then
       ...
       Edcontacredito.invalid('Conta crédito tem que ser diferente da conta')
   end;
...
begin
    ...
    if pos(copy(EdNatf_codigoitem.text,1,1),'1;2;3;')=0 then
      EdNatf_codigoitem.Invalid('Cfop inválido para nota de entrada')
...
//      Sistema.Commit;
//    except
//      Avisoerro('Não foi possível incluir o NCM '+xNCM)
//    end;
...
// 11.07.2022
    try
      ...
      Avisoerro('Não foi possível incluir/atualizar o NCM '+xNCM)
    end;
  FGeral.FechaQuery(Q);
end;
...
var xport:string;
     ...
     begin
       ...
       if QBanco.eof then begin
          Avisoerro('Não encontrado conta com banco '+xbanco+' configurado nas contas gerenciais');
          result :='';
          ...
          if QPOrt.Eof then begin
            ...
            Avisoerro('Não encontrado portador com  a conta '+inttostr( QBanco.FieldByName('plan_conta').AsInteger));
            result :='';
...
begin
  if (EdMensagem.IsEmpty) and ( pos(EdComv_codigo.ResultFind.FieldByName('comv_tipomovto').asstring,Global.CodEstornoNFeSai)>0 ) then
     EdMensagem.Invalid('Obrigatório informar a justificativa de estorno de NF-e')
...
//////////////////////////////////////
{
    ...
    if not Qm.Eof then begin
      ...
      EdManutencao.Items.Add('OPeração   Documento '+space(01)+'Produto/Serviço');
    ...
    if trim(chavenfeacom)='' then begin
      ...
      if trim(chavenfeacom)='' then begin
        if not Global.Topicos[1470] then
          ...
          Aviso('Atenção - Informar a chave em CT-e')

        else

          EdComv_codigo.invalid('Obrigatório informar a chave em CT-e');
...
begin
   if ( Edqtdeos.ascurrency > Texttovalor( Grid.cells[Grid.getcolumn('move_qtde'),grid.row] ) )
      ...
      then
     EdQtdeos.invalid('Quantidade não pode ser maior que a da nota');
end;
...
begin
   if (Edvlrinss.ascurrency+Edvlrpis.ascurrency+Edvlrcofins.ascurrency+Edvlrcsll.ascurrency+EdVlrir.ascurrency+EdVlrIss.AsCurrency) > EdDigtotalnf.ascurrency then
      Edvlrcsll.invalid('Valor das retenções não pode ser maior que o total da nota');
end;
// 16.11.16
  ...
  //        Sistema.SetField('forn_uf',Pclifor.forn_uf);
    ...
    begin
       Aviso('Nome Fantasia:'+NotaFiscal.Conhecimentos.Items[0].Cte.Emit.xFant+';'+
             'Razão Social :'+NotaFiscal.Conhecimentos.Items[0].CTe.Emit.xNome+';'+
             'Conhecimento :'+inttostr(NotaFiscal.Conhecimentos.Items[0].CTe.Ide.nCT)+';'+
             'Emissão      :'+FGeral.Formatadata(NotaFiscal.Conhecimentos.Items[0].CTe.Ide.dhEmi )+';'+
...
begin
   ...
   Aviso('NCMs sem MVA  ou não encontrado:'+ncmmvazero.text );
