...
uses Geral,SqlFun, Arquiv, equipamentos, Estoque;
...
//   else
   ...
   if OP='A' then begin
     ...
     Caption:='ALTERAÇÃO de lançamentos na Ficha Técnica do Equipamento';
     transacaonota:='';
// 01.10.2021
...
// 23.09.2021 Leila A2z
  ...
  if OP='A' then begin
    ...
    if not Q.eof then PreencheCampos else begin
       ...
       Limpacampos;
       EdNumerodoc.Invalid('Não encontrado');

    end;

  end else
    if Q.eof then Limpacampos else EdNumerodoc.Invalid('Numero já digitado');
end;
...
// 06.10.2021
  EdObs1.Text:=Q.FieldByName('moes_devolucoes').AsString;
    ...
    if OP='I' then begin
      ...
      Transacao:=FGeral.GetTransacao;
        if EdNumerodoc.asinteger=0 then begin
          Avisoerro('Numero zerado.    Não gravado.  Tente novamente !');
          exit;
        end;

        FGeral.GravaMestreConsignacao(EdDtEmissao.AsDate,EdEqui_codigo,1,EdUnid_codigo.text,
             Global.CodManutencaoEquipamento,Transacao,EdNumerodoc.asinteger,
             EdTotalRemessa.AsCurrency,0,op,'N',transacaonota,EdObs.Text,EdObs1.text,EdPreCor.text);
      ...
      try
        ...
        Avisoerro('Problemas na gravação.  Conferir se foi gravado.');
      end;
...
// 22.10.2021
    ...
    begin
       ...
       kmparatroca := EdEqui_codigo.resultfind.fieldbyname('Equi_ProxTroca').asinteger;
...
// 22.10.2021 - A2z
        if (kmhora+kmparatroca > TSqled( Components[p] ).AsCurrency) and ( TSqled( Components[p] ).AsCurrency>0 ) and
