...
uses
  ...
  Vcl.StdCtrls, Vcl.Buttons, SQLBtn, alabel, SqlEd, Vcl.Mask,zEnv, SqlDtg;
...
begin
  ...
  EdGeral.top:=Grid.Columns[Grid.Col].InternalCol ;
  EdGeral.left:=Grid.LeftCol;
  EdGeral.tablefield:=Grid.Columns[Grid.Col].Field;
  EdGEral.Width     :=Grid.Columns[Grid.Col].Width;
  EdGEral.Text      :=Grid.Cells[Grid.Col,Grid.Row];
  EdGeral.Visible:=true;
  EdGeral.Enabled:=true;
  EdGeral.BringToFront;
  EdGeral.setfocus;

//   Grid.Edit;  só se fosse sqled
...
begin
  ...
  EdEsto_dtultvenda.Setdate( FMain.SistemaZeos.Hoje);
  EdEsto_dtultcompra.Setdate(FMain.SistemaZeos.Hoje);
...
//   showmessage( 'entrou no onnewline');
       ...
       Pprecovenda.Caption:=FGeral.Formatavalor(QEstoque.fieldbyname('esqt_vendavis').ascurrency,f_cr);
       Pprecovenda.Update;
       PEmEstoque.Caption:=FGeral.Formatavalor(QEstoque.fieldbyname('esqt_qtde').ascurrency,f_cr);
       PEmEstoque.Update;
       QEstoque.Close;
end;
