...
begin
  ...
  Confbotoes;
// 03.03.20 - Lei 'LGPD' - proteção aos dados
...
// não funga direito ; não pega o codigo do cliente e referencia direito - 19.01.05
//  if Tipo='I' then begin
...
begin
  ...
  if EdMuniRes_Nome.Text='' then EdClie_Muni_Codigo_Res.Invalid('Cidade não cadastrada')
...
begin
  ...
  if EdMuniCom_Nome.Text='' then EdClie_Muni_Codigo_Com.Invalid('Cidade não cadastrada');
end;
...
begin
  ...
  FRel.ReportFromSQL('SELECT * FROM CLIENTES '+w,'RelCadClientes','Relatório do Cadastro de Clientes');
end;
...
begin
  ...
  Sistema.BeginProcess('Imprimindo relatório');
    ...
    FRel.AddTit('Relatório do Ocorrências de Clientes - Período : '+formatdatetime('dd/mm/yy',Sistema.DAtai)+' a '+formatdatetime('dd/mm/yy',Sistema.DAtaf) );

    FRel.AddCol( 60,3,'N','' ,''              ,'Codigo'          ,''         ,'',false);
    FRel.AddCol(220,0,'C','' ,''              ,'Nome'           ,''         ,'',false);
    FRel.AddCol( 60,1,'N','' ,''              ,'Data'        ,''         ,'',false);
    FRel.AddCol(400,1,'C','' ,''              ,'Ocorrência'        ,''         ,'',false);
    FRel.AddCol(120,1,'C','' ,''              ,'Usuário'          ,''         ,'',false);
...
// 21.02.18
// verifica se o cliente é do simples nacional
...
// 17.03.10
    if not Found then Found:=FoundTabela('Pendencias','Pend_tipo_Codigo','Pend_TipoCad','Pendência Financeira');
...
begin
  if Edclie_limcredito.ascurrency>0 then begin
    if not FGeral.Validalimitecredito(EdClie_limcredito.ascurrency) then
       EdClie_limcredito.invalid('Limite máximo é '+floattostr(FGeral.GetConfig1AsFloat('Limitelimcredito')));
  end;
end;
  ...
  if not Edclie_codigofinan.IsEmpty then begin
    ...
    if not Q.eof then begin
      ...
      if (Q.fieldbyname('clie_codigofinan').AsInteger>0) and (Q.fieldbyname('clie_codigofinan').AsInteger<>EdClie_codigofinan.AsInteger) then
        EdClie_codigofinan.invalid('Este cliente já está vinculado ao codigo '+Q.fieldbyname('clie_codigofinan').AsString)
    end else
      SetEdclifinanceiro.Text:='';
    FGEral.fechaquery(Q);
  end else
...
begin
  if Edit.Items.Count=0 then begin
     ...
     Lista.Add('02 - Não Associado');
     Lista.Add('03 - Não contribui');
     Lista.Add('04 - Outras');
     Edit.Items.Assign(Lista);
     Lista.Free;
  end;
...
begin
  ...
  else if tipoinv='04' then result:='Xirú'
  else result:='';
...
begin
   ...
   else if tipomen='02' then result:='Não Associado'
   else if tipomen='03' then result:='Não contribui'
   else result:='Outras';
...
begin
   if (EdClie_consfinal.text='R') and ( not Global.topicos[1107] ) then
     Edclie_consfinal.invalid('Não liberado uso da opção R - Revenda');
end;
...
begin
   ...
   if (length(trim(cnpjcpf))=11) and (tipo<>'F') then begin
     Avisoerro('em CPF é obrigatório ser pessoa física');
     result:=false;
   end else if (length(trim(cnpjcpf))=14) and (tipo<>'J') then begin
     Avisoerro('em CNPJ é obrigatório ser pessoa jurídica');
     result:=false;
   end;
end;
...
begin
  qtd:='';
  if not Input('Quantas fichas ?','Cópias:',qtd,3,false) then exit;
...
//      FRel.AddTit(Global.NomeSistema+' '+Global.VersaoSistema+space(78)+'Emissão:'+Datetostr(Sistema.hoje)+' '+timetostr(time)+'  Pg: [NumPg]',false,false,true );
      ...
      FTextRel.AddLinha(space(margem)+replicate('-',largura+3),false,false,false);
        ...
        FTextRel.SaltaLinha(01);
        FTextREl.AddLinha('Razão Social..: '+replicate('_',40),false,false,false);
        FTextRel.SaltaLinha(01);
        FTextREl.AddLinha('Endereço......: '+replicate('_',40),false,false,false);
        FTextRel.SaltaLinha(01);
        ...
        FTextREl.AddLinha('Data cadastro.: '+replicate('_',10)+' Nascimento: '+replicate('_',10)
                         ,false,false,false);
        FTextRel.SaltaLinha(01);
        FTextREl.AddLinha('End. Cobrança.: '+replicate('_',50)
                         ,false,false,false);
        FTextRel.SaltaLinha(01);
...
// 11.10.2022 - recolocado - Centro - Email de cobrança
...
//  clie_ativo character varying(1),
  ...
  try
    ...
    Sistema.EndProcess('Não foi possível gravar no banco de dados');
  end;
  EdFornec.enabled:=false;
  EdFornec.visible:=false;
...
//    foto: TFileStream;
    ...
    arquivo:string;
begin
  if confirma('Confirma gravaçao ?') then begin
