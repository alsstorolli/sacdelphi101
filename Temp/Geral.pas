...
//        end else begin
// 21.03.07 - retirado pois na entrada nao é necessario...
          Sistema.SetField('Movb_BaseCalculo',Pcstperc.base);
// 23.10.07
          if pcstperc.reducao>0 then
// 22.05.07
     ...
     if trim(Grid.cells[Grid.Getcolumn('pend_valor'),p])<>'' then begin
       ...
       if vencimento>0 then begin
         if vencimento>=data then begin
           result:=false;
           Avisoerro('Vencimentos fora de ordem cronológica');
           break;
         end;
       end else
         vencimento:=data;
...
//    FRel.AddTit(FGeral.TituloRelUnidade(EdUnid_codigo.Text));
    ...
    FRel.AddCol(150,1,'C','' ,''              ,'Observação'    ,''         ,'',False);
...
//    Avisoerro('Nota já encontrada na transação '+Q.fieldbyname('moes_transacao').asstring+'.  Não importado.');
     if aviso then
       FImportaNfe.Texto.Lines.Add('Nota já encontrada na transação '+Q.fieldbyname('moes_transacao').asstring+'.  Não importado.');
    exit;
  end;
  FGeral.FechaQuery(Q);
...
//  end;
  if trim( NotaFiscal.NotasFiscais.Items[0].NFe.Dest.CNPJCPF )='' then begin
    Avisoerro('Não encontrado a tag referente o CNPJ/CPF neste xml');
    exit;
  end;
