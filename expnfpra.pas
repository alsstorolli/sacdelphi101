...
var n,empresa,filial,sist:integer;
    ...
    begin
      ...
      if (trim(EdMesano.text)='') and (Q.fieldbyname('pend_datacont').asdatetime<=1) then
          ...
          result:=true;
// 13.11.09 - Abra - nao contabiliza provisao ref. financiamentos pois estes ja são lançados
//    no caixa bancos
...
// 08.07.20  - exportação de notas de prestacao de serviços NFS-e  nas saidas
...
// 21.08.20  - para não fazer o lançamento quando tem q fazer o multiplo do rateio
         ...
         if debito>0 then begin
           ...
           if not Qp.Eof then begin
              if strtointdef(Qp.FieldByName('pcon_empr_codigo').AsString,0) <> strtointdef(EdUnid_codigo.Text,0) then
                Texto.Lines.Add('Conta débito '+inttostr(debito)+' não pertence ao plano da unidade '+EdUnid_codigo.Text+' Transação '+Q.fieldbyname('moes_transacao').asstring )
           end;
           FGeral.FechaQuery(Qp);
         end;
         if credito>0 then begin
           ...
           if not Qp.Eof then begin
              if strtointdef(Qp.FieldByName('pcon_empr_codigo').AsString,0) <> strtointdef(EdUnid_codigo.Text,0) then
                Texto.Lines.Add('Conta crédito '+inttostr(credito)+' não pertence ao plano da unidade '+EdUnid_codigo.Text+' Transação '+Q.fieldbyname('moes_transacao').asstring )
           end;
           FGeral.FechaQuery(Qp);
         end;
      end;
...
// 06.11.19 - retirado entrada doação da exportacao - Novicarnes - Simone
// Global.CodEntradaBrinde+';'+
// retornado as 'DV' em 02.08.07
// 12.05.11 - colocado prestacao de serviços
...
// 05.06.12 - Benato - nao mandar individual..só leitura Z..
...
// para o postgres 8.4.. ver se não baixa bagunçar os da leilinha...
// certeza q 'baixou' ou melhor não deu certo com postgres 12...retirado
...
//                ' and moes_tipocad='+stringtosql(Cv)+
  ...
  if Q.eof then begin
    Avisoerro('Nada encontrado para exportação em notas de entrada e saida');
//    exit;
  end;

//  Sistema.beginprocess('Eliminando lançamentos da última exportação');
//////////////////  dbfexp.close;
...
//    AssignFile(Arquivo, EdPasta.text+'\'+nomearqtexto+EdUnid_codigo.text+'.txt' );
  ...
  Sistema.beginprocess('Exportando lançamentos');
            ...
            if contagerencial = 9999 then begin
               ...
               for y := 0 to ListaDesp.count-1 do begin
                   ...
                   if trim( ListaDesp[y] ) <> '' then begin
                      ...
                      end else Avisoerro('Ainda não configurado rateio para este sistema contábil');
...
// 23.09.16 - ver se sera necessario criar conta propria pra retenção de inss q só acontece em nf de prest. de serviço
//             de construçao civil
                debito:=0;
                credito:=0;
              end;
...
////////////////////////////////////////
              ...
              if ( (credito>0) or (debito>0) ) and ( valor>0 ) then begin
                ...
                inc(n);              // 13.10.15
                 ...
                 Texto.lines.add('GTA/PIS nf produtor/retido Crédito '+inttostr(credito)+' doc. '+Q.fieldbyname('moes_numerodoc').Asstring+
                     ' tipo '+Q.fieldbyname('moes_tipomov').Asstring+' Cli/for '+Q.fieldbyname('moes_tipo_codigo').Asstring);
...
////////////////////////////////////////
              ...
              if ( (credito>0) or (debito>0) ) and ( valor>0 ) then begin
                ...
                inc(n);              // 13.10.15
                 ...
                 Texto.lines.add('IR retido nf Crédito '+inttostr(credito)+' doc. '+Q.fieldbyname('moes_numerodoc').Asstring+
                     ' tipo '+Q.fieldbyname('moes_tipomov').Asstring+' Cli/for '+Q.fieldbyname('moes_tipo_codigo').Asstring);
...
////////////////////////////////////////
              ...
              if ( (credito>0) or (debito>0) ) and ( valor>0 ) then begin
                ...
                inc(n);              // 13.10.15
                 ...
                 Texto.lines.add('CSLL retido nf Crédito '+inttostr(credito)+' doc. '+Q.fieldbyname('moes_numerodoc').Asstring+
                     ' tipo '+Q.fieldbyname('moes_tipomov').Asstring+' Cli/for '+Q.fieldbyname('moes_tipo_codigo').Asstring);
...
////////////////////////////////////////
              ...
              if ( (credito>0) or (debito>0) ) and ( valor>0 ) then begin
                ...
                inc(n);              // 13.10.15
                 ...
                 Texto.lines.add('ISS retido nf Crédito '+inttostr(credito)+' doc. '+Q.fieldbyname('moes_numerodoc').Asstring+
                     ' tipo '+Q.fieldbyname('moes_tipomov').Asstring+' Cli/for '+Q.fieldbyname('moes_tipo_codigo').Asstring);
...
//                         Q.fieldbyname('moes_transacao').Asstring;
                ...
                if ( FCadcli.getecooperado(Q.fieldbyname('moes_tipo_codigo').Asinteger)) and (cotacapital>0) then
                   Texto.lines.add('Cota Capital/Cofins nf produtor/retido Crédito '+inttostr(credito)+' doc. '+Q.fieldbyname('moes_numerodoc').Asstring+
                     ' tipo '+Q.fieldbyname('moes_tipomov').Asstring+' Cli/for '+Q.fieldbyname('moes_tipo_codigo').Asstring);
              end;
//////////////////////////////////
...
// - aqui deve lançar a parte a prazo
// 24.04.06
////////////////
...
// 14.05.19 - notas de compra futura q nao gera financeiro e nem achou o debito e credito
          ...
          then begin
          Texto.lines.add('Débito '+inttostr(debito)+' Crédito '+inttostr(credito)+' doc. '+Q.fieldbyname('moes_numerodoc').Asstring+
              ' tipo '+Q.fieldbyname('moes_tipomov').Asstring+' Cli/for '+Q.fieldbyname('moes_tipo_codigo').Asstring);
// 20.04.10
        end else if (debito>0) and (credito>0) and ( debito=credito ) then begin
          Texto.lines.add('Contas Iguais Débito '+inttostr(debito)+' Crédito '+inttostr(credito)+' doc. '+Q.fieldbyname('moes_numerodoc').Asstring+
              ' tipo '+Q.fieldbyname('moes_tipomov').Asstring+' Cli/for '+Q.fieldbyname('moes_tipo_codigo').Asstring+
...
// 04.02.16 - Usando cnpf ao inves das contas para clientes e fornecedores
       ...
       Texto.lines.add('Débito '+inttostr(debito)+' Crédito '+inttostr(credito)+' histórico '+historico+' valor '+floattostr(vlrdiario));
...
// financeiro - confirmar se é o tipo 'PF'...lançamento das Provisoes a Pagar direto
//              no financeiro
// 07.03.07
// 14.03.07
...
// 08.08.17
        if ( (debito=0) or (credito=0) ) and (trim(EdMesano.text)<>'') then begin
            Texto.lines.add('Débito '+inttostr(debito)+' Crédito '+inttostr(credito)+' documento '+
                             Q.fieldbyname('pend_numerodcto').Asstring+' tipo '+
                             Q.fieldbyname('pend_tipomov').Asstring+
                            ' Transação '+Q.fieldbyname('pend_transacao').Asstring+
...
// 20.04.10 - Abra - Ligiane
            ...
            Texto.lines.add('Débito '+inttostr(debito)+' Crédito '+inttostr(credito)+' documento '+Q.fieldbyname('pend_numerodcto').Asstring+' tipo '+Q.fieldbyname('pend_tipomov').Asstring+
                            ' '+FGeral.FormataData(Q.fieldbyname('pend_datamvto').AsDatetime)+' Transação '+Q.fieldbyname('pend_transacao').Asstring+
                            ' Cli/for.'+Q.fieldbyname('pend_tipo_codigo').Asstring);
          ...
          if EdSistema.text='02' then begin   // viasoft
            linha:=strzero(empresa,3)+strzero(filial,3)+strspace(EdUnid_codigo.resultfind.fieldbyname('unid_reduzido').asstring,6)+
                   ...
                   strzero(debito,5)+space(06)+strzero(credito,5)+space(06)+Formatovalorcomponto(valor,14,2)+
                   space(03)+strspace(historico,160);
// 18.02.10 - Abra - aqui só em 08.03.10 ..esqueci...
            end else if EdSistema.text='04' then begin   // Questor
//                linha:=strzero(empresa,3)+','+strzero(datetodia(datamvto),2)+strzero(datetomes(datamvto),2)+strzero(datetoano(datamvto,true),4)+','+
//                      strzero(debito,7)+','+strzero(credito,7)+','+Formatovalorcomponto(valor,14,2)+','+
//                      '0'+',"'+trim(historico)+'"';
...
// 08.08.07
  ...
  end;   // configurado para lançar documentos lancados diretamente no financeiro

  FGeral.FEchaquery(Q);
  Freeandnil(Forns);
/////////// 22.10.12 - lançamentos de cheques devolvidos em aberto
...
// inicialmente lançado como venda a vista
...
///////////////////
    ...
    end;  // se tá configurado ECF
/////////////////////////////////////////////////////////
///
  // 23.07.20 - lançar apropriacao caso houver no periodo
/////////////////////////////////////////////////////
            if ( campoapropria.tipo<>'' ) then begin
...
// linha somente para exportar para contax por enquanto...
  ...
  Sistema.endprocess('Exportados '+inttostr(n)+' lançamentos');
...
///////////////////////////  dbfexp.SetShowDeleted(false);
  sistema.beginprocess('checando diretório');
  DriveExp:='';   //// 'F:';
...
{
  if not fileexists(nomearq) then begin
    Avisoerro('Não encontrado '+nomearq);
    close;
    exit;
  end;
}
...

{
  ...
  try
    ...
    Avisoerro('Não foi possível abrir o arquivo '+nomearq);
    close;
    exit;
  end;
}
...
//       EdUnid_codigo.invalid('Unidade sem conta de exportação de compra a vista')
     if EdUnid_codigo.resultfind.fieldbyname('unid_caixa').asinteger=0 then
       EdUnid_codigo.invalid('Unidade sem conta de exportação de caixa')
//     else if EdUnid_codigo.resultfind.fieldbyname('unid_compras').asinteger=0 then begin
//      avisoerro('Falta configurar a conta contábil de compras a prazo na configuração da unidade');
//      close;
//      exit;
//     end;
   end;
  ...
  if Edcv.text<>'C' then begin
    if not Global.topicos[1253] then begin
      if EdUnid_codigo.resultfind.fieldbyname('unid_clientes').asinteger=0 then begin
        EdCv.invalid('Falta configurar a conta contábil de clientes na configuração da unidade');
      end;
      if EdUnid_codigo.resultfind.fieldbyname('unid_vendaaprazo').asinteger=0 then begin
        EdCv.invalid('Falta configurar a conta contábil de vendas a prazo na configuração da unidade');
      end;
    end;
  end;
end;
...
begin
  ...
  if EdSistema.Text='02' then begin  // 17.12.09 - Abra - somente pra viasoft
    if conta>99999 then begin
      Texto.lines.add('Conta '+inttostr(conta)+' Transação '+xtransacao);
      result:=0
    end;
  end;
end;
...
begin
    ...
    if EdSistema.Text<>'03' then begin  // Erpesc
       Avisoerro('Relatório disponível apenas para o sistema Contax');
       exit;
    end;
    nomearqtexto:='CTBNFS'+FormatDatetime('mmyyyy',EdTermino.Asdate);
...
//      Lista.LoadFromFile( EdPasta.text+'\'+nomearqtexto+EdUnid_codigo.text+'.txt');
    ...
    if lista.count>0 then begin
      ...
      FRel.AddTit('Relação de Lançamentos Exportados no arquivo '+EdPasta.text+'\'+nomearqtexto+EdUnid_codigo.text+'.txt' );
      FRel.AddCol( 65,1,'D','' ,''              ,'Data'          ,''         ,'',false);
      FRel.AddCol(080,3,'N','+' ,f_cr            ,'Valor'           ,''         ,'',false);
      FRel.AddCol(060,3,'N','' ,''              ,'Débito'         ,''         ,'',false);
      if trazcontas then
         FRel.AddCol(150,1,'C','' ,''              ,'Descrição Débito'         ,''         ,'',false);
      FRel.AddCol(060,3,'N','' ,''              ,'Crédito'         ,''         ,'',false);
      if trazcontas then
         FRel.AddCol(150,1,'C','' ,''              ,'Descrição Crédito'         ,''         ,'',false);
      FRel.AddCol(200,1,'C','' ,''              ,'Histórico'         ,''         ,'',false);
...
begin
   FTextrel.Init();
   FTextRel.AddTitulo(Global.NomeSistema+' '+Global.VersaoSistema+space(46)+'Emissão:'+Datetostr(Sistema.hoje)+' '+timetostr(time)+'  Pg: [NumPg]',false,false,true );
   Ftextrel.AddTitulo('Relatório de Erros na Exportação Contábil Notas a Prazo',false,false,true);
