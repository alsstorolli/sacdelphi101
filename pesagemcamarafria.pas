...
//   SetaItemsEstoque;
   ...
   if OP='D' then
      ...
      Caption:='Etiquetas para Transformação para Congelamento'
...
begin
   ...
   if ( Qpedido.fieldbyname('movd_pesobalanca').ascurrency>= Qpedido.fieldbyname('movd_pesocarcaca').ascurrency )
     ...
     aviso('Etiqueta ainda NÃO PESADA');
     exit;
...
//       strtolista(Lista,ctransacoes,'|',true);
     ...
     if OP = 'D' then begin
        ...
        if pesoorigem<=0  then begin
           Avisoerro('Peso está zerado.  Nada será gravado');
           exit;
        end else
           GeraeGravaPesos;
...
//                     ' and movd_tipomov='+stringtosql('EA')+
    ...
    if Q1.Eof then Avisoerro('Não encontrado transacao gerada ref. '+EdNumerodoc.Text);
