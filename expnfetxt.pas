...
// 07.03.18
    if Q.fieldbyname('moes_status').AsString='N' then begin
      if Q.fieldbyname('moes_chavenfe').AsString='' then
        Grid.Cells[Grid.GetColumn('retorno'),p]:='Nota ainda não autorizada'
      else
...
begin
  ...
  FRel.AddTit('Relação de Endereços para Correção para NFe');
...
//              pra pegar a chave , municipio de carregamento e destino
   ...
   if not Q.eof then begin
     ...
     if trim(Q.fieldbyname('moes_xmlnfet').asstring)='' then begin
          ...
          Avisoerro('Transação '+xtrans+' sem XML gravado');
          exit;
        ...
        then begin
          ...
          Avisoerro('Nota com destinatário dentro do município da unidade emissora da NFe');
          exit;
          ...
          Avisoerro('Nota não se refere a venda de ração configurada no sistema');
          exit;
...
//                  CampoExtra.codigo := 96;
                  ...
                  CampoExtra.valor := Qg.fieldbyname('moes_chavenfe').asstring;
                   ...
                   ufok := false;
                   Avisoerro('UF '+Qg.fieldbyname('moes_estado').asstring+' não prevista no sistema. Ver aplicativo próprio');
...
begin
  ...
  if trim(Qg.fieldbyname('moes_xmlnfet').asstring)='' then begin
     ...
     Avisoerro('Transação '+ct+' se ref. a nfe em contingência. Autorizar primeiro');
     exit;
     ...
     Avisoerro('NFe ref. Transação '+ct+' foi cancelada em '+FGeral.FormataData(Qg.fieldbyname('moes_dtnfecanc').asdatetime));
     exit;
...
// 30.07.15
        if ListaNotas.NotasFiscais.Items[x].NFe.Ide.modelo<>65 then begin
          if trim(ListaNotas.NotasFiscais.Items[x].NFe.Dest.EnderDest.nro)='' then begin
            Avisoerro('Nota '+inttostr(ListaNotas.NotasFiscais.Items[x].NFe.Ide.cNF)+' Endereço '+ListaNotas.NotasFiscais.Items[x].NFe.Dest.EnderDest.xLgr+' sem numero');
            result:=false;
          end else if trim(ListaNotas.NotasFiscais.Items[x].NFe.Dest.EnderDest.xBairro)='' then begin
            Avisoerro('Nota '+inttostr(ListaNotas.NotasFiscais.Items[x].NFe.Ide.cNF)+' Endereço '+ListaNotas.NotasFiscais.Items[x].NFe.Dest.EnderDest.xLgr+' sem bairro');
            result:=false;
          end else if ListaNotas.NotasFiscais.Items[x].NFe.Dest.EnderDest.CEP=0 then begin
            Avisoerro('Nota '+inttostr(ListaNotas.NotasFiscais.Items[x].NFe.Ide.cNF)+' Endereço '+ListaNotas.NotasFiscais.Items[x].NFe.Dest.EnderDest.xLgr+' sem Cep');
            result:=false;
  // 19.05.11
...
//            Avisoerro('Nota '+inttostr(ListaNotas.NotasFiscais.Items[x].NFe.Ide.cNF)+' Veículo sem placa');
//            result:=false;
...
//            Avisoerro('Nota '+inttostr(ListaNotas.NotasFiscais.Items[x].NFe.Ide.cNF)+' Veículo com placa diferente de 7 caracteres');
//            result:=false;
...
//            Avisoerro('Nota '+inttostr(ListaNotas.NotasFiscais.Items[x].NFe.Ide.cNF)+' Destinatário sem telefone completo');
//            result:=false;
...
// 05.01.19 - venda normal dá calculado pela receita 0,01 centavo a mais...
            xvalor:=TextToValor( GetTAg( 'vbc',GetProduto(Det.Items[x].Prod.xProd,Det.Items[x].Prod.cProd,AcbrNfe1.NotasFiscais.Items[i].XML,x+1) ) );
...
// 31.03.20 - notas de venda da novicarnes de ração com numeros 'quebrado' devido a vender
//            em kilos
...
// 02.09.16 - vivan 0 preço com 3 casas devido desconto na tabela dai da 1096.343 no totprod e 1096,34 no vnf...
// 21.03.18 - Novi - nf produtor com preço unitario com dizima...
  ...
  IF OP='E' THEN BEGIN  // 01.03.12
{
    if (Global.Usuario.Codigo=900) and (global.Usuario.Nome='André') then begin
      ...
      //    Avisoerro('Certificado digital numero de série '+ACBrNFe1.Configuracoes.Certificados.NumeroSerie+' vencido em '+Datetostr(ACBrNFe1.Configuracoes.Certificados.DataVenc));
...
/////    }
// 08.08.18  - apenas para nao verificar quando tem q fazer testes e certif. é A3
...
// 08.11.17
     if ( trim( AcbrNfe1.Configuracoes.Geral.CSC ) = '' ) and ( Grid.cells[Grid.getcolumn('moes_especie'),Grid.row]='NFC' ) then begin
        Avisoerro('Falta configuarar o CSC nas configurações gerais do sistema para autorizar NFC-e');
        exit;
     end;
...
//    Rewrite(ArqTransp);
//  except
//    Avisoerro('Drive ou Pasta não encontrado');
//    Q.Close;
//    EdPasta.setfocus;
//    exit;
//  end;
...
// 09.04.19 - Configurao responsável técnico // ainda nao disponivel na homologacao PR
...
// 09.12.2021 - só q ta somando negativo dai diminuindo né jamanta...
//          totalitens := totalitens + (QItens.fieldbyname('move_qtde').ascurrency*QItens.fieldbyname('move_venda').asfloat);
...
// 11.11.2013 - Metalforte - Devolucao Tributada
      ...
      dadosadicionais:='';
// 27.09.17 - ficavam sem devido a mudanças pra arma de fogo e romaneios vida nova
...
// 20.06.19 - vida Nova
        if ( Ansipos(Q.fieldbyname('moes_tipomov').asstring,global.Codcompraprodutor)>0 ) and  ( global.topicos[1462] ) then begin
           ...
           if Q.fieldbyname('moes_insumos').ascurrency>0 then
              dadosadicionais:=dadosadicionais+' Insumos Produção : '+floattostr(Q.fieldbyname('moes_insumos').ascurrency);
...
///  04.09.20 - recolocado somente para nfc-e
      if ( Uppercase( copy( Q.fieldbyname('moes_especie').asstring,1,3 ) ) = 'NFC' )
         ...
         ( Q.FieldByName('moes_vispra').asstring <> 'V' )
          ...
          QPend.First;
// 28.05.2022 - Benato - nome do cliente aqui pois na nfc-e só sai o nome se informar cpf
...
// 07.08.17  - Usuario e transacao da nota
      if ( Global.Topicos[1397] ) then
        dadosadicionais:=dadosadicionais+' Usuário '+inttostr(Global.Usuario.Codigo)+'-'+
                         FUsuarios.GetNome(Global.Usuario.Codigo)+' Transação '+Q.FieldByName('moes_transacao').AsString;
...
// 20.09.17 - iran informa o frete mas é ele que paga
...
////////////////////////
// 15.02.10 - serviços na mesma nota
...
// 28.10.16 - devido as retencoes q comecam a calcuar nas notas de serviço de entrada
...
// 12.08.19 - a partir de 09.2019 receita não aceita mais desta forma
// 20.08.19 - deixado gerar aleatorio pelo acbr porem devido ao 'fator clevis' mudado para codigo
//            do cliente para que não 'mude a chave' caso tentar autorizar / consultar '18 vezes'
...
//23.10.20 - FAma - estorno de devolução de venda...
...
// 27.08.18 -  siccare e zilmar = quando é venda fora do estado nao enviar a IE do substituto tributario
     if ( Ansipos( copy(Q.fieldbyname('moes_natf_codigo').asstring,1,1),'26' ) > 0 )
        or
        ( ide.modelo=65  )   // 07.12.2021 - Nfce não aceita
        then
        Emit.IEST:='';
...
// 11.11.2021 - Guiber - fica com 'duas vírgulas' no danfe
...
//   22.04.22 - Guiber - venda para Senac que mesmo com IE é nao contribuiente )
...
// 08.09.15 diferença de 0,01 centavos...
...
// 06.04.20 - mudado pra cá..era mais abaixo...
...
// 30.01.2022 - guiber -> floripa - se não tem icms não rateia o frete
...
// 23.10.12 - Asatec - 30.01.23 - só se tem icms - caso 'guiber -> floripa'
              if Q1.fieldbyname('move_aliicms').ascurrency > 0 then begin
...
// 23.02.2023 - Devolucao Olstri com redução de base......a outra 'de cima' é pela dif. de valores sem reduzir a base
                                    vBC   := baseicms*(Q1.fieldbyname('move_redubase').ascurrency/100);
                                    pICMS    := Q1.fieldbyname('move_aliicms').ascurrency;
                                    modBC    := dbiValorOperacao;
...
//                         margemlucro:=1;  // só pra gerar as tags de ST..
                         basesubs:=0;
                         icmsitemsubs:=0;
                         margemlucro:=0;
...
// 20.04.18 - STF liberou empresas do simples a recolher o Difal mas há estados q cobram
// 27.06.18 - Mirvane venda interestadual para MS para consumidor final não autorizou
...
// 27.05.19 - Seip - só calcula se 'for venda' , aqui se gerar financeiro
// 11.06.19 - Seip - 'for venda' para eles é 'só fiscal' - VH - venda entrega..rolos
...
// 26.06.19 - ajustado para informar aliquota mesmo não calculando a difal
                             if Q1.FieldByName('move_aliicms').AsCurrency=0 then
...
// 30.12.2022 - Guiber - notas de venda a consumidor final fora do estado no qual o produto não tinha icms
//                       ficava zerado o icms mas a base do item com valor....
//                        if vlricms=0 then baseicms :=0;
//                      só q tem frete a ser rateado e q faz parte da base do icms...
...
// 04.12.18 - só calcular se for saida..
            ...
            if ( FGeral.GetConfig1AsInteger('GRUPOARMAS')>0 ) and
               ...
               with Prod.arma.add do begin
                  ...
                  nCano:=copy(Q1.fieldbyname('esto_referencia').asstring,Ansipos(';',Q1.fieldbyname('esto_referencia').asstring)+1,4);
                     // numero de serie do cano - ver se é necessario
...
// 16.11.18 - Seip
         InfAdic.infCpl:=InfAdic.infCpl+
              ...
              ' Valor Icms de partilha UF destinatário '+FGeral.Formatavalor( totalvICMSUFDest,f_cr  ) +
...
//      ACBrNFe1.NotasFiscais.GerarNFe;
    ...
    IF OP='E' THEN BEGIN  // 01.03.12
      Sistema.beginprocess('Checando as informações da(s) nota(s)');
  // 19.01.10
...
///////////////////////17.02.17
    if ( OP='E' ) and ( Global.Topicos[1028] ) then begin
      ...
      exit;
                                        ...
                                        // 09.06.11 - pra ver se para 'sumiços' de produtos quando dá os 'retornos' da sefa
...
//          ChecaStatusSefa:=ACBrNFe1.WebServices.StatusServico.Executar;
// 01.04.16 0 retirado esta BIROSCA de checagem q 'sempre' dá q o status tá off..principalmente em win server
...
//            cretornoerro:=ACBrNFe1.WebServices.Retorno.xMotivo;
// 05.10.2021 - mudança pra sincrono
//            if ( xsituacaonotas='NFCe'  )
//                       or
           if    ( ACBrNFe1.NotasFiscais.Count = 1 ) then
...
// 02.09.2022 // para ver se não ocorrer mais de ficar 'autorizada no sac e não autorizada na receita'
              // caso da vida nova..
...
//               mandar o xml da dpeC e nao do gerado...
  ...
  //        Avisoerro('Status Serviço :'+ACBrNFe1.WebServices.StatusServico.Msg);
...
//              dai sistemas como o sage não o importa...
//               xmlstring:=ACBrNFe1.NotasFiscais.Items[i].XMLAssinado;
// 02.09.2022 // para evitar de ficar 'autorizada no sac e não autorizada na receita'
               xmlstring:=ACBrNFe1.NotasFiscais.Items[i].XMLOriginal;
...
// 05.04.17 - Mirvane - Jackson contador - Delcio
                 if trim(xemail)<>'' then begin
                    AcBrNfe1.EnviarEmail(xemail,'NF '+inttostr(ACBrNFe1.NotasFiscais.Items[i].NFe.Ide.nNF)+
                                        ' emissão '+FGeral.FormataData(ACBrNFe1.NotasFiscais.Items[i].NFe.Ide.dEmi),
                                        nil,nil,nil,xmlstream,'Xmlnf_'+inttostr(ACBrNFe1.NotasFiscais.Items[i].NFe.Ide.nNF) );
                 end;
             end;
...
// Para enviar uma NFe, temos dois métodos disponíveis. O método ACBrNFe1.Enviar(NumLote);
//  irá gerar o(s) XML da(s) NFe(s), assiná-la(s), validá-la(s), enviá-la(s) e por fim imprimí-la(s) (se forem autorizadas) de forma automática.
...
begin
    if (EdFormaemissao.text='2') and ( not Global.Topicos[1032] ) then
      Edformaemissao.Invalid('Opção somente permitida para quem tem o formulário de segurança');
end;
...
// 16.03.15
   if not Sistema.GetDataMvto('Emissão das notas') then exit;
   demissao:=Sistema.DataMvto;
   if not confirma('Confirma importação das notas de produtor em '+FGeral.formatadata(demissao)+' ? ') then exit;
   SetaCodigosIbge;
   ...
   if not Q.eof then begin
//     if not confirma('ATENÇÃO.  Numeração de notas está em '+inttostr(xnfsaida)+'.  Confirma ? ') then exit;
     Avisoerro('ATENÇÃO.  Já encontrado notas de produtor em '+Datetostr(demissao));
     Sistema.endProcess('');
     exit;
   end;

   if Global.usuario.codigo=100 then pergunta:=Confirma('Elimina importação anterior ?')
   else pergunta:=true;
...
//   contagerencial:=341;  // criado conta com descricao Leite ver se será dif. por unidade...
...
// 23.09.15
     ...
     if xunidade<>Global.CodigoUnidade then begin
       Sistema.endprocess('Importação interrompida.  Arquivo pentence a unidade '+xunidade );
       exit;
     end;
     if trystrtodate( xemissao,Global.Datasistema ) then begin
// 16.03.15
////       demissao:=strtodate(xemissao);
       ...
       Tipocad:='C';
       Sistema.beginprocess('Importação produtor '+inttostr(xcodigosac)+' '+FGeral.GetNomeRazaoSocialEntidade(xcodigosac,'C','N'));
       xprecoliq:=TextTovalor( ListaLinha[10] );
       xvlrbruto:=TextTovalor( ListaLinha[03] );
       xlitros:=trunc(xvlrbruto/xprecoliq);
// 14.03.16 - para ver os 'chu' q usam mais q 2 casas decimais no preço do leite
//       xvlrbruto:=TextTovalor( ListaLinha[03] );
...
// 19.01.16 - mudanças a receita para autorizar
...
// 07.02.18
    ...
    if not NotaEhValida then begin
      Avisoerro( 'Falha na validação dos dados da nota: ' +
...
// 08.11.2022 - com uma nfe 'sozinha' dava msg ... NFE <> NFE...bem loke...
       if (Grid.Cells[Grid.GetColumn('moes_especie'),i]) <> xespecie then begin
          ...
          result:=false;
          Avisoerro('Não é permitido autorizar NOTAS CONSUMIDOR com NOTAS "DANFE"');
          break;
